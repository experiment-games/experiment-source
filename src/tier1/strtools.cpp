...
bool V_RemoveDotSlashes( char *pFilename, char separator, bool bRemoveDoubleSlashes /* = true */ )
{
    ...
    while ( i <= ( len - 2 ) )
    {
        bool bDoubleSlash = pFilename[i] == separator && pFilename[i + 1] == separator;
        if ( bDoubleSlash )
        {
    ...
    while ( i <= ( len - 2 ) )
    {
        ...
        {
            bool bIsUselessDotSlash = pFilename[i - 1] != '.' && pFilename[i] == '.' && pFilename[i + 1] == separator;
...
inline int V_iswspace( int c )
{
    return ( c <= 0xFFFF ) ? iswspace( ( wint_t )c ) : 0;
}
#else
