...
void ConVar::Create_Vtbl( const char *pName, const char *pDefaultValue, int flags /* = 0 */, const char *pHelpString /* = 0 */, bool bMin /* = false */, float fMin /* = 0.0 */, bool bMax /* = false */, float fMax /* = false */, FnChangeCallback_t callback /* = 0 */ )
{
    Create( pName, pDefaultValue, flags, pHelpString, bMin, fMin, bMax, fMax, false, 0.0, false, 0.0, callback );
}
...
//-----------------------------------------------------------------------------
void ConVar::Create( const char *pName, const char *pDefaultValue, int flags /*= 0*/, const char *pHelpString /*= NULL*/, bool bMin /*= false*/, float fMin /*= 0.0*/, bool bMax /*= false*/, float fMax /*= false*/, bool bCompMin /*= false */, float fCompMin /*= 0.0*/, bool bCompMax /*= false*/, float fCompMax /*= 0.0*/, FnChangeCallback_t callback /*= NULL*/ )
{
    m_pParent = this;
...
bool ConVar::SetCompetitiveMode( bool bCompetitive )
{
    ...
    bool bRequiresClamp = ( var->m_bHasCompMin && var->m_fCompMinVal > var->m_fValue ) || ( var->m_bHasCompMax && var->m_fCompMaxVal < var->m_fValue );
    bool bForceToDefault = !var->m_bHasCompMin && !var->m_bHasCompMax && ( fabs( var->m_fValue - ( fDefaultAsFloat = V_atof( var->m_pszDefaultValue ) ) ) > 0.00001f );
    ...
    Assert( !bRequiresClamp || IsFlagSet( FCVAR_MATERIAL_THREAD_MASK ) || ( ( !var->m_bHasCompMin || var->m_fCompMinVal <= var->m_fValue ) && ( !var->m_bHasCompMax || var->m_fCompMaxVal >= var->m_fValue ) ) );
...
UIConVarRef::UIConVarRef( IVEngineClient *pEngine, const char *pName, bool bIgnoreMissing )
    : ConVarRef( pName, bIgnoreMissing ), m_pEngine( pEngine )
{
}
