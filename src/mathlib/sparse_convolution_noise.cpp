...
static float CellNoise( int ix, int iy, int iz, float xfrac, float yfrac, float zfrac, float ( *pNoiseShapeFunction )( float ) )
{
    float ret = 0;
...
float SparseConvolutionNoise( Vector const &pnt, float ( *pNoiseShapeFunction )( float ) )
{
    ...
    for ( int ox = -1; ox <= 1; ox++ )
        for ( int oy = -1; oy <= 1; oy++ )
            ...
            {
                sum_out += CellNoise( ix + ox, iy + oy, iz + oz, xfrac - ox, yfrac - oy, zfrac - oz, pNoiseShapeFunction );
            }
#ifdef MEASURE_RANGE
