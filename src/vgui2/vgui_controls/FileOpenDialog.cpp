...
void FileOpenDialog::Init( const char *title, KeyValues *pContextKeyValues )
{
    ...
    Label *fileName = new Label( this, "FileNameLabel", ( m_DialogType != FOD_SELECT_DIRECTORY ) ? "#FileOpenDialog_File_name" : "#FileOpenDialog_Directory_Name" );
...
const char *GetFileTimetamp( FILETIME ft )
{
    ...
    Q_snprintf( out, sizeof( out ), "%d/%02d/%04d %d:%02d %s", local.wMonth, local.wDay, local.wYear, hour, local.wMinute,
