...
void RayTracingEnvironment::AddTriangle( int32 id, const Vector &v1, const Vector &v2, const Vector &v3, const Vector &color )
{
    AddTriangle( id, v1, v2, v3, color, 0, 0 );
}

void RayTracingEnvironment::AddTriangle( int32 id, const Vector &v1, const Vector &v2, const Vector &v3, const Vector &color, uint16 flags, int32 materialIndex )
{
...
// Experiment; Commented based on fix by ryanmaclean (https://github.com/ValveSoftware/source-sdk-2013/pull/436/files)
...
void RayTracingEnvironment::Trace4Rays( const FourRays &rays, fltx4 TMin, fltx4 TMax, RayTracingResult *rslt_out, int32 skip_id, ITransparentTriangleCallback *pCallback )
{
...
void RayTracingEnvironment::Trace4Rays( const FourRays &rays, fltx4 TMin, fltx4 TMax, int DirectionSignMask, RayTracingResult *rslt_out, int32 skip_id, ITransparentTriangleCallback *pCallback )
{
    rays.Check();
...
void RayTracingEnvironment::CalculateTriangleListBounds( int32 const *tris, int ntris, Vector &minout, Vector &maxout )
{
...
void RayTracingEnvironment::RefineNode( int node_number, int32 const *tri_list, int ntris, Vector MinBound, Vector MaxBound, int depth )
{
    ...
    for ( int axis = 0; axis < 3; axis++ )
    {
        for ( int ts = -1; ts < ntris; ts += tri_skip )
        {
            for ( int tv = 0; tv < 3; tv++ )
            {
                ...
                float trial_cost =
                    CalculateCostsOfSplit( axis, tri_list, ntris, MinBound, MaxBound, trial_splitvalue, trial_nleft, trial_nright, trial_nboth );
...
void RayTracingEnvironment::SetupAccelerationStructure( void )
{
    ...
    CalculateTriangleListBounds( root_triangle_list, OptimizedTriangleList.Count(), m_MinBound, m_MaxBound );
