...
{
    typedef uint32 TargetType;
    TargetType operator()( const char *pszKey ) const
    {
        return MurmurHash3Functor< const char * >()( pszKey );
    }
};
...
class CUtlHashMapLarge
{
   public:
    ...
    {
        return m_memNodes.Element( i ).m_key;
    }
    ...
    {
        return i >= 0 && i < m_nMaxElement && !IsFreeNodeID( m_memNodes[i].m_iNextNode );
    }
    ...
    {
        return InsertInternal( key, insert, eInsert_UpdateExisting );
    }
    IndexType_t Insert( const KeyType_t &key )
    {
        return InsertInternal( key, ElemType_t(), eInsert_UpdateExisting );
    }
    IndexType_t InsertWithDupes( const KeyType_t &key, const ElemType_t &insert )
    {
        return InsertInternal( key, insert, eInsert_CreateDupes );
    }
    IndexType_t FindOrInsert( const KeyType_t &key, const ElemType_t &insert )
    {
        return InsertInternal( key, insert, eInsert_LeaveExisting );
    }
   ...
   private:
    enum EInsertPolicy
    {
        ...
        eInsert_LeaveExisting,
        eInsert_CreateDupes
    };
    IndexType_t InsertInternal( const KeyType_t &key, const ElemType_t &insert, EInsertPolicy ePolicy );

    inline IndexType_t FreeNodeIDToIndex( IndexType_t i ) const
    {
        return ( 0 - i ) - 3;
    }
    inline IndexType_t FreeNodeIndexToID( IndexType_t i ) const
    {
        return ( -3 ) - i;
    }
    inline bool IsFreeNodeID( IndexType_t i ) const
    {
...
inline int CUtlHashMapLarge< K, T, L, H >::InsertInternal( const KeyType_t &key, const ElemType_t &insert, EInsertPolicy ePolicy )
{
    ...
    while ( iBucket >= m_nMinRehashedBucket && !m_bitsMigratedBuckets.Get( iBucket ) )
    {
...
inline void CUtlHashMapLarge< K, T, L, H >::IncrementalRehash()
{
    if ( m_nMinRehashedBucket < m_nMaxRehashedBucket )
    {
        while ( m_nMinRehashedBucket < m_nMaxRehashedBucket )
        {
            ...
            if ( m_vecHashBuckets[m_nMinRehashedBucket].m_iNode != InvalidIndex() && !m_bitsMigratedBuckets.Get( m_nMinRehashedBucket ) )
            {
                // rehash this bucket
