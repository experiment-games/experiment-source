...
void CThreadSafeMultiMemoryPool::Free( void *pvMem )
{
    ...
    {
        AssertMsg3( false, "CThreadSafeMultiMemoryPool::Free: raw allocation %p (original alloc: %p, %d bytes) not found in allocation map", pvMem, pvAllocedMem, cubAllocSize );
        return;
    }
...
void *CThreadSafeMultiMemoryPool::ReAlloc( void *pvMem, uint32 cubAlloc )
{
    ...
    if ( cubOldAlloc > m_nBlockSizeMax )
    {
        ...
        if ( m_MapRawAllocation.InvalidIndex() == iRawAllocation )
        {
            AssertMsg3( false, "CThreadSafeMultiMemoryPool::ReAlloc: raw allocation %p (original alloc: %p, %d bytes) not found in allocation map", pvMem, pvAllocedMem, cubOldAlloc );
            return NULL;
        }
