    ...
    if ( data.m_fFlags & PARTICLE_DISPATCH_FROM_ENTITY )
    {
        if ( data.m_hEntity.Get() )
        {
            ...
            if ( pEnt && !pEnt->IsDormant() )
            {
                ...
                AssertMsg2( pEffect.IsValid() && pEffect->IsValid(), "%s could not create particle effect %s", C_BaseEntity::Instance( data.m_hEntity )->GetDebugName(), pszName );
