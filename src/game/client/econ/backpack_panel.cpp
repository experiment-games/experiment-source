...
static bool HasPaint( const CEconItemView *pEconItemView, const char *, int )
{
    ...
    return pEconItemView->FindAttribute( pAttrDef_PaintRGB ) || pEconItemView->FindAttribute( pAttrDef_PaintRGB2 );
}
...
// Purpose:
//-----------------------------------------------------------------------------
CBackpackPanel::CBackpackPanel( vgui::Panel *parent, const char *panelName )
    : CBaseLoadoutPanel( parent, panelName )
{
    m_nQuickOpenTxn = 0;
    m_pContextMenu = NULL;
    ...
    if ( pInv )
    {
        for ( int i = 0; i < pInv->GetItemCount(); ++i )
        {
            ...
            if ( nPage >= 0 && nPage < m_Pages.Count() )
            {
                ...
                if ( m_mapSeenItems.Find( pItem->GetItemID() ) == m_mapSeenItems.InvalidIndex() && IsUnacknowledged( pItem->GetInventoryPosition() ) == false && !m_bShowBaseItems && !HasNameFilter() )
                {
...
void CBackpackPanel::OpenContextMenu()
{
    ...
    if ( vecSelectedItems.Count() == 1 )
    {
        ...
        uint32 unEscrowTime;
        const bool bToolIsInEscrow = pItem->FindAttribute( pAttrib_ToolEscrowUntil, &unEscrowTime ) && unEscrowTime > CRTime::RTime32TimeCur();
        ...
        // Add equip sub menu
        {
            ...
            for ( int iClass = TF_FIRST_NORMAL_CLASS; iClass < TF_LAST_NORMAL_CLASS; iClass++ )
            {
                ...
                {
                    case TF_CLASS_SCOUT:
                        pszClassName = "#TF_Class_Name_Scout";
                        break;
                    case TF_CLASS_SNIPER:
                        pszClassName = "#TF_Class_Name_Sniper";
                        break;
                    case TF_CLASS_SOLDIER:
                        pszClassName = "#TF_Class_Name_Soldier";
                        break;
                    case TF_CLASS_DEMOMAN:
                        pszClassName = "#TF_Class_Name_Demoman";
                        break;
                    case TF_CLASS_MEDIC:
                        pszClassName = "#TF_Class_Name_Medic";
                        break;
                    case TF_CLASS_HEAVYWEAPONS:
                        pszClassName = "#TF_Class_Name_HWGuy";
                        break;
                    case TF_CLASS_PYRO:
                        pszClassName = "#TF_Class_Name_Pyro";
                        break;
                    case TF_CLASS_SPY:
                        pszClassName = "#TF_Class_Name_Spy";
                        break;
                    case TF_CLASS_ENGINEER:
                        pszClassName = "#TF_Class_Name_Engineer";
                        break;
                }
...
class CRefurbishItemDialog : public CComboBoxBackpackOverlayDialogBase
{
    ...
    {
        return "#TF_Item_RefurbishItemHeader";
    }
};
...
void CBackpackPanel::DoDeliverItem()
{
    ...
    else if ( pWrappedGiftTool && pWrappedGiftTool->BIsGlobalGift() )
    {
        ...
        CTFGenericConfirmDialog *pDialog = ShowConfirmDialog( "#TF_DeliverGiftDialog_Title", "#TF_DeliverGiftDialog_Random_Text", "#TF_DeliverGiftDialog_Confirm", "#TF_DeliverGiftDialog_Cancel", &UseUntargetedGiftConfirm );
...
{
    if ( pItemPanel->HasItem() && ( cl_showbackpackrarities.GetInt() > 0 || m_bForceShowBackpackRarities ) && ( cl_showbackpackrarities.GetInt() < 2 || pItemPanel->GetItem()->IsMarketable() ) )
    {
