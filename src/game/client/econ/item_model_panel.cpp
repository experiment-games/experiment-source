...
// Purpose:
//-----------------------------------------------------------------------------
CEmbeddedItemModelPanel::CEmbeddedItemModelPanel( vgui::Panel *pParent, const char *pName )
    : BaseClass( pParent, pName )
{
...
void CEmbeddedItemModelPanel::Paint( void )
{
    ...
    if ( iTexture != -1 )
    {
        ...
        for ( int i = 0; i < m_pItem->GetInventoryOverlayImageCount(); i++ )
        {
            ...
            if ( ( iTextureIDIdx == m_iOverlayTextureIDs.InvalidIndex() || m_iOverlayTextureIDs[iTextureIDIdx] == -1 ) )
            {
...
// Purpose:
//-----------------------------------------------------------------------------
CItemModelPanel::CItemModelPanel( vgui::Panel *parent, const char *name )
...
bool CItemModelPanel::CheckRecipeMatches()
{
    ...
    if ( !pTool || V_stricmp( m_ItemData.GetStaticData()->GetEconTool()->GetTypeName(), "dynamic_recipe" ) || m_ItemData.GetStaticData()->GetDefaultLoadoutSlot() != INVALID_EQUIPPED_SLOT )
    {
        if ( m_pMatchesLabel )
        {
...
void CItemModelPanel::UpdateDescription( bool bIsToolTip /* = false */ )
{
    ...
    enum
    {
        kAttribBufferSize = 4 * 1024
    };
    wchar_t wszAttribBuffer[kAttribBufferSize] = L"";

    enum
    {
        kCollectionBufferSize = 4 * 1024
    };
...
CItemModelPanelToolTip::CItemModelPanelToolTip( vgui::Panel *parent, const char *text )
    : vgui::BaseTooltip( parent, text ), m_pMouseOverItemPanel( NULL ), m_iPositioningStrategy( IPTTP_BOTTOM_SIDE )
{
