...
class CEmbeddedItemModelPanel : public CBaseModelPanel
{
    ...
    bool IsForcingModelUsage( void )
    {
        return m_bForceUseModel;
    }
    void SetForceModelUsage( bool bUseModel )
    {
        m_bForceUseModel = bUseModel;
    }
    ...
    InventoryImageType_t GetInventoryImageType() const
    {
        return static_cast< InventoryImageType_t >( m_iInventoryImageType );
    }
    void SetInventoryImageType( InventoryImageType_t eNewImageType )
    {
        m_iInventoryImageType = ( int )eNewImageType;
    }
    void LoadInventoryImage();

    void SetGreyedOut( bool bGreyedOut )
    {
        m_bGreyedOut = bGreyedOut;
    }
    void SetModelHidden( bool bModelHidden )
    {
        m_bModelIsHidden = bModelHidden;
    }
    ...
    {
        return m_bUseRenderTargetAsIcon || m_bUseItemRenderTarget;
    }
...
class CItemModelPanel : public vgui::EditablePanel
{
    ...
    {
        return ( m_ItemData.IsValid() ? &m_ItemData : NULL );
    }
    bool ModelIsHidden( void ) const
    {
        return m_bHideModel;
    }
    ...
    CExLabel *GetNameLabel( void )
    {
        return m_pItemNameLabel;
    }
    ...
    void SetAttribOnly( bool bAttribs )
    {
        m_bAttribOnly = bAttribs;
    }
    void SetTextYPos( int iPos )
    {
        m_iTextYPos = iPos;
    }
    bool IsEquipped( void );
    void SetShowEquipped( bool bShow )
    {
        m_bShowEquipped = bShow;
        UpdateEquippedLabel();
    }
    void SetForceShowEquipped( bool bForce )
    {
        m_bForceShowEquipped = bForce;
        UpdateEquippedLabel();
    }
    void SetGreyedOut( const char *pszGreyedOutReason );  // pass in NULL for "not greyed out"
    void SetShowGreyedOutTooltip( bool bShow )
    {
        m_bShowGreyedOutTooltip = bShow;
    }
    bool IsGreyedOut( void ) const
    {
        return m_pszGreyedOutReason != NULL;
    }
    const char *GetGreyedOutReason( void ) const
    {
        return m_pszGreyedOutReason;
    }
    bool ShouldShowTooltip( void )
    {
        return ( !IsGreyedOut() || m_bShowGreyedOutTooltip );
    }
    void SetShowQuantity( bool bShow )
    {
        m_bShowQuantity = bShow;
    }

    void UpdatePanels( void );

    void SendPanelEnterExits( bool bSend )
    {
        m_bShouldSendPanelEnterExits = bSend;
    }

    void SetShouldShowOthersGiftWrappedItems( bool bShow )
    {
        m_bShowOthersGiftWrappedItems = bShow;
    }
    void Dragged( bool bDragging );
    void HideContainedItemPanel();
    void ShowContainedItemPanel( const CEconItemView *pItem );

    bool IsContainedItem()
    {
        return m_bContainedItem;
    }
    void SetContainedItem( bool bVal )
    {
        m_bContainedItem = bVal;
    }

    void SetSkin( int iSkin );
    void SetItemStyle( style_index_t unStyle )
    {
        m_ItemData.SetItemStyleOverride( unStyle );
    }
    void SetNameOnly( bool bNameOnly )
    {
        m_bNameOnly = bNameOnly;
    }
    void SetSpecialAttributesOnly( bool bSpecialOnly )
    {
        m_bSpecialAttributesOnly = bSpecialOnly;
    }

    CEmbeddedItemModelPanel::InventoryImageType_t GetInventoryImageType() /*const*/
    {
        return m_pModelPanel->GetInventoryImageType();
    }
    void SetInventoryImageType( CEmbeddedItemModelPanel::InventoryImageType_t eNewImageType )
    {
        m_pModelPanel->SetInventoryImageType( eNewImageType );
    }
    ...
    int GetAttribWide( int iMaxWide )
    {
        return ( m_iTextWide ? m_iTextWide : ( iMaxWide - ( ATTRIB_LABEL_INDENT * 2 ) ) );
    }
...
class CItemModelPanelToolTip : public vgui::BaseTooltip
{
    ...
    void SetupPanels( vgui::Panel *pParentPanel, CItemModelPanel *pMouseOverItemPanel )
    {
...
class CEquippableItemsForSlotGenerator
{
    ...
    {
        CEquippableResult()
            : m_pEconItemView( NULL ), m_eDisplayType( kSlotDisplay_Invalid )
        {
            //
        }

        CEquippableResult( CEconItemView *pEconItemView, EItemDisplayType eDisplayType = kSlotDisplay_Normal )
            : m_pEconItemView( pEconItemView ), m_eDisplayType( eDisplayType )
        {
            //
        }
    ...
    const EquippableResultsVec_t &GetDisplayItems() const
    {
        return m_vecDisplayItems;
    }
    CEconItemView *GetEquippedItem() const
    {
        return m_pEquippedItemView;
    }
    const DuplicateCountMap_t &GetDuplicateCountMap() const
    {
