...
// Purpose:
//-----------------------------------------------------------------------------
CItemSelectionPanel::CItemSelectionPanel( Panel *parent )
    : CBaseLoadoutPanel( parent, "ItemSelectionPanel" )
{
    m_pCaller = parent;
...
// Purpose:
//-----------------------------------------------------------------------------
CEquipSlotItemSelectionPanel::CEquipSlotItemSelectionPanel( Panel *parent, int iClass, int iSlot )
    : CItemSelectionPanel( parent )
{
    m_iClass = iClass;
    m_iSlot = iSlot;
...
struct RarityEconIdKey
{
    ...
    RarityEconIdKey()
        : m_iQualitySort( -1 ), m_defIndex( INVALID_ITEM_DEF_INDEX ), m_unKillEaterScore( 0 )
    {
        //
    }

    RarityEconIdKey( int iQuality, item_definition_index_t defIndex, uint32 unKillEaterScore )
        : m_iQualitySort( EconQuality_GetRarityScore( ( EEconItemQuality )iQuality ) ), m_defIndex( defIndex ), m_unKillEaterScore( unKillEaterScore )
    {
        //
    }
    ...
    {
        return m_defIndex < rhs.m_defIndex || m_iQualitySort < rhs.m_iQualitySort || m_unKillEaterScore < rhs.m_unKillEaterScore;
    }
};
    ...
    : m_pEquippedItemView( NULL )
{
    ...
    {
        case 0:
            m_vecDisplayItems.Sort( &SortRarityEconIdKeysDate );
            break;
        case 1:
            m_vecDisplayItems.Sort( &SortRarityEconIdKeysAlphabetical );
            break;
    }
}
...
// Purpose:
//-----------------------------------------------------------------------------
CItemCriteriaSelectionPanel::CItemCriteriaSelectionPanel( Panel *parent, const CItemSelectionCriteria *pCriteria, itemid_t pExceptions[], int iNumExceptions )
    : CItemSelectionPanel( parent )
{
...
CAccountSlotItemSelectionPanel::CAccountSlotItemSelectionPanel( Panel *pParent, int iSlot, const char *pszTitleToken )
    : CEquipSlotItemSelectionPanel( pParent, GEconItemSchema().GetAccountIndex(), iSlot ), m_pszTitleToken( pszTitleToken )
{
}
