...
struct SDecalBlendLayer
{
    ...
    bool FromKV( KeyValues *pkvLayerBlock, CUtlString &errMsg )
    {
        ...
        if ( !Q_stricmp( op, "normal" ) )
            eLayerOp = eLayerBlendOp_Normal;
        else if ( !Q_stricmp( op, "multiply" ) )
            eLayerOp = eLayerBlendOp_Multiply;
        else if ( !Q_stricmp( op, "screen" ) )
            eLayerOp = eLayerBlendOp_Screen;
        else if ( !Q_stricmp( op, "overlay" ) )
            eLayerOp = eLayerBlendOp_Overlay;
        else if ( !Q_stricmp( op, "ReplaceAlpha" ) )
            eLayerOp = eLayerBlendOp_ReplaceAlpha;
        else
        {
...
class CustomTextureStencilGradientMapWidget : public vgui::Panel
{
    ...
    {
        return m_nRangeCount;
    }
    int GetNobCount() const
    {
        return m_nRangeCount - 1;
    }
...
class CConfirmCustomizeTextureDialog : public CBaseToolUsageDialog, private ITextureRegenerator
{
    ...
    struct CroppedImagePanel : public CBitmapPanel
    {
        CroppedImagePanel( CConfirmCustomizeTextureDialog *pDlg, vgui::Panel *parent )
            : CBitmapPanel( parent, "PreviewCroppedImage" ), m_pDlg( pDlg )
        {
        }
    ...
    struct FilteredImagePanel : public CBitmapPanel
    {
        FilteredImagePanel( CConfirmCustomizeTextureDialog *pDlg, vgui::Panel *parent )
            : CBitmapPanel( parent, "PreviewFilteredImage" ), m_pDlg( pDlg )
        {
        }
    ...
    inline bool IsSourceImageSquare() const
    {
        ...
        return m_imgSource.Width() * 99 < m_imgSource.Height() * 100 && m_imgSource.Height() * 99 < m_imgSource.Width() * 100;
    }
...
CConfirmCustomizeTextureDialog::CConfirmCustomizeTextureDialog( vgui::Panel *parent, CEconItemView *pTool, CEconItemView *pToolSubject )
    : CBaseToolUsageDialog( parent, "ConfirmCustomizeTextureDialog", pTool, pToolSubject ), m_hImportImageDialog( NULL ), m_bFilteredImageDirty( true ), m_bStencilShapeReducedImageDirty( true ), m_bSquareImageDirty( true ), m_bCropToSquare( false ), m_bUseAvatar( true ), m_pCurrentPreviewedTexture( NULL ), m_pFilterCombo( NULL ), m_pSquarizeCombo( NULL ), m_pStencilModeCombo( NULL ), m_pUseAvatarRadioButton( NULL ), m_pUseAnyImageRadioButton( NULL ), m_pStencilGradientWidget( NULL ), m_nSelectedStencilPalette( -1 )
{
...
static void ListenToControlsRecursive( vgui::Panel *pPanel, vgui::Panel *pListener )
{
    ...
    if (
        dynamic_cast< vgui::Button * >( pPanel ) || dynamic_cast< vgui::Slider * >( pPanel ) || dynamic_cast< vgui::ComboBox * >( pPanel ) || dynamic_cast< CustomTextureStencilGradientMapWidget * >( pPanel ) )
    {
