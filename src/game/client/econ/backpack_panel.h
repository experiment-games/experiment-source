...
class CBackpackPanel : public CBaseLoadoutPanel
{
   ...
   public:
    ...
    {
        return "Resource/UI/econ/BackpackPanel.res";
    }
    ...
    {
        return BACKPACK_COLUMNS;
    }
    virtual int GetNumRows( void ) OVERRIDE
    {
        return BACKPACK_ROWS;
    }
    ...
    CEconItemView *GetComboBoxOverlayUISeletionItem()
    {
        return &m_ComboBoxOverlaySelectionItem;
    }
    void SetComboBoxOverlaySelectionItem( const CEconItemView *pEconItemView )
    {
        m_ComboBoxOverlaySelectionItem = *pEconItemView;
    }
   ...
   protected:
    ...
    virtual int GetBackpackPosForPanelIndex( int iPanelIndex )
    {
        return iPanelIndex + 1 + ( GetCurrentPage() * GetNumSlotsPerPage() );
    }
    virtual bool NeedsDerivedTickSignal( void )
    {
        return false;
    }
    ...
    virtual bool IsIgnoringItemPanelEnters( void )
    {
        return m_bDragging;
    }
    ...
    {
        return ( !m_bItemsOnly && !InToolSelectionMode() );
    }

    bool InToolSelectionMode() const
    {
        return m_eSelectionMode != StandardSelection;
    }
    ...
    {
        return m_wNameFilter.Count() > 0;
    }
    const wchar_t *GetNameFilter() const
    {
        return HasNameFilter() ? m_wNameFilter.Base() : NULL;
    }
    void UpdateFilteringItems();

    int GetItemQualityForBorder( CItemModelPanel *pItemPanel ) const;

    int GetNumMaxPages() const
    {
        return BACKPACK_MAX_PAGES;
    }
