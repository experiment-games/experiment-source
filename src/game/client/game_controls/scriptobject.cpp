    ...
    *pBuffer = engine->ParseFile( *pBuffer, token, sizeof( token ) );
    ...
    switch ( newType )
    {
        ...
        case O_LIST:
            ...
            break;
        case O_AUTO_LIST:  // Auto list is a list that is filled by the game. Contains no entries except default value and which filler is used
            // Parse the filler
            *pBuffer = engine->ParseFile( *pBuffer, token, sizeof( token ) );
            if ( strlen( token ) <= 0 )
                return false;
            ...
            if ( isNewObject )
            {
                Q_strncpy( filler, token, sizeof( filler ) );
            }
