...
class CNavButton : public CExImageButton
{
    ...
    virtual void ApplySettings( KeyValues *pInResourceData )
    {
        ...
        if ( m_iUserData < 0 )
        {
            m_iUserData = pInResourceData->GetInt( "userdata", -1 );
            AssertMsg( m_iUserData != -1, "Any messages sent for this button will have invalid user data!" );
        }
    }
...
void CNavigationPanel::ApplySettings( KeyValues *pInResourceData )
{
    ...
    */
    ...
    KeyValues *pKVButtonSettings = pInResourceData->FindKey( "ButtonSettings" );
    AssertMsg( pKVButtonSettings, "This is required" );
...
void CNavigationPanel::OnCommand( const char *pCommand )
{
    if ( !V_strnicmp( pCommand, "select_", 7 ) )
    {
        ...
        AssertMsg( m_vecButtons.IsValidIndex( iButton ), "Button index out of range!" );
