...
class CEconNotification_AbuseReportReady : public CEconNotification
{
   public:
    ...
    {
        return dynamic_cast< CEconNotification_AbuseReportReady * >( pNotification ) != NULL;
    }
...
void CAbuseReportManager::FireGameEvent( IGameEvent *event )
{
    ...
    if (
        !Q_strcmp( "teamplay_round_win", eventname ) || !Q_strcmp( "tf_game_over", eventname ) )
    {
...
bool CAbuseReportManager::PopulateIncident()
{
    ...
    if (
        m_adrCurrentServer.IsValid() && !m_adrCurrentServer.IsLocalhost() && m_steamIDCurrentServer.IsValid() && ( !m_adrCurrentServer.IsReservedAdr() || m_steamIDCurrentServer.GetEUniverse() != k_EUniversePublic ) )
    {
    ...
    for ( int i = 1; i <= gpGlobals->maxClients; ++i )
    {
        ...
        if ( p->m_bRenderBoundsValid )
        {
            ...
            p->m_bRenderBoundsValid =
                p->m_screenBoundsMin.x + .01f < p->m_screenBoundsMax.x && p->m_screenBoundsMin.y + .01f < p->m_screenBoundsMax.y;
        }
