...
class ClientModeShared : public IClientMode, public CGameEventListener
{
    ...
    bool IsAnyPanelVisibleExceptScores()
    {
        return m_pViewport->IsAnyPanelVisibleExceptScores();
    }
    bool IsPanelVisible( const char *panel )
    {
        return m_pViewport->IsPanelVisible( panel );
    }
