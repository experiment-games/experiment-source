...
void CFPSPanel::Paint()
{
    ...
    if ( nShowPosMode > 0 )
    {
        ...
        g_pMatSystemSurface->DrawColoredText( m_hFont, x, 2 + i * ( vgui::surface()->GetFontTall( m_hFont ) + 2 ), 255, 255, 255, 255, "pos:  %.02f %.02f %.02f", vecOrigin.x, vecOrigin.y, vecOrigin.z );
        i++;
        ...
        g_pMatSystemSurface->DrawColoredText( m_hFont, x, 2 + i * ( vgui::surface()->GetFontTall( m_hFont ) + 2 ), 255, 255, 255, 255, "vel:  %.2f", vel.Length() );
    }
    ...
    if ( cl_showbattery.GetInt() > 0 )
    {
        ...
        if ( m_BatteryPercent > 0 )
        {
            ...
            {
                g_pMatSystemSurface->DrawColoredText( m_hFont, x, 2 + i * ( vgui::surface()->GetFontTall( m_hFont ) + 2 ), 255, 255, 255, 255, "battery: On AC" );
            }
            else
            {
                g_pMatSystemSurface->DrawColoredText( m_hFont, x, 2 + i * ( vgui::surface()->GetFontTall( m_hFont ) + 2 ), 255, 255, 255, 255, "battery:  %d%%", m_BatteryPercent );
            }
        }
    }
}
...
void CBlockingFileIOPanel::DrawIOTime( int x, int y, int w, int h, int slot, char const *label, const Color &clr )
{
    ...
    g_pMatSystemSurface->DrawColoredText( m_hFont, x + 2, y + 1, clr[0], clr[1], clr[2], clr[3], "%s", label );
         ...
         ( gpGlobals->realtime < latchedtime + 10.0f ) )
    {
        ...
        g_pMatSystemSurface->DrawColoredText( m_hFont, x, y + h + 1, 255, 255, 255, 200, "%s", m_History[slot].m_LastFile.String() );
    }
