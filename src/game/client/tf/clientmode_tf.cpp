...
void ClientModeTFNormal::AskFavoriteOrBlacklist() const
{
    ...
    if ( sessionDuration > 0 )
    {
        ...
        if ( !cl_ask_favorite_opt_out.GetBool() && sessionDuration > cl_ask_favorite_min_session_duration.GetFloat() )
        {
            ...
            ShowConfirmOptOutDialog( "#TF_Serverlist_Ask_Favorite_Title", "#TF_Serverlist_Ask_Favorite_Text", "#TF_Serverlist_Ask_Yes", "#TF_Serverlist_Ask_No", "#TF_ServerList_Ask_Favorite_Opt_Out", "cl_ask_favorite_opt_out", OnAskFavoriteDialogButtonPressed, NULL );
        }
        ...
        if ( !cl_ask_blacklist_opt_out.GetBool() && sessionDuration < cl_ask_blacklist_max_session_duration.GetFloat() )
        {
            ...
            if ( ( !blackList.CanServerBeBlacklisted( lastServer.GetIPHostByteOrder(), lastServer.GetPort(), GetLastConnectedServerName() ) || GTFGCClientSystem()->BIsIPRecentMatchServer( lastServer ) ) && !cl_ask_blacklist_for_any_server.GetBool() )
            {
            ...
            ShowConfirmOptOutDialog( "#TF_Serverlist_Ask_Blacklist_Title", "#TF_Serverlist_Ask_Blacklist_Text", "#TF_Serverlist_Ask_Yes", "#TF_Serverlist_Ask_No", "#TF_ServerList_Ask_Blacklist_Opt_Out", "cl_ask_blacklist_opt_out", OnAskBlacklistDialogButtonPressed, NULL );
        }
    }
}
...
//----------------------------------------------------------------------------
bool ClientModeTFNormal::BuildRichPresenceStatusDirect( wchar_t *pwzOutStatus, size_t uOutSizeBytes, const char *pszState, const char *pszMatchGroupLocTokenSuffix, const char *pszPrettyMapName )
{
    tmZone( TELEMETRY_LEVEL0, TMZF_NONE, "%s", __FUNCTION__ );
