...
extern CBaseEntity *BreakModelCreateSingle( CBaseEntity *pOwner, breakmodel_t *pModel, const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, int nSkin, const breakablepropparams_t &params );
...
{
    killstreak_params_t( int iShaderIndex, const char *pTexture, bool bHasTeamColor, int sheen_r, int sheen_g, int sheen_b, int sheen_a, int color1_r, int color1_g, int color1_b, int color1_a, int color2_r, int color2_g, int color2_b, int color2_a )
    {
...
C_TFPlayerPreviewEffect::C_TFPlayerPreviewEffect()
    : m_nPreviewEffect( PREVIEW_EFFECT_NONE ), m_nTeam( -1 )
{
}
...
void C_TFRagdoll::CreateTFGibs( bool bDestroyRagdoll, bool bCurrentPosition )
{
    ...
    if ( pPlayer && pPlayer->HasBombinomiconEffectOnDeath() )
    {
        ...
        DispatchParticleEffect( TFGameRules()->IsHolidayActive( kHoliday_Halloween ) ? "bombinomicon_burningdebris_halloween" : "bombinomicon_burningdebris",
                                bCurrentPosition ? GetAbsOrigin() : m_vecRagdollOrigin,
                                GetAbsAngles() );
...
struct TextureVarSetter
{
    TextureVarSetter( IMaterialVar *pDestVar, ITexture *pDefaultTexture )
        : m_pDestVar( pDestVar ), m_pTexture( pDefaultTexture )
    {
...
void C_TFPlayer::TauntCamInterpolation()
{
    ...
    if ( pLocalPlayer && m_bTauntInterpolating )
    {
        ...
        trace_t trace;
        UTIL_TraceHull( vecOrigin, vecOrigin - ( vecForward * m_flTauntCamCurrentDist ) + ( vecUp * m_flTauntCamCurrentDistUp ), Vector( -9.f, -9.f, -9.f ), Vector( 9.f, 9.f, 9.f ), MASK_SOLID_BRUSHONLY, pLocalPlayer, COLLISION_GROUP_DEBRIS, &trace );
...
//-----------------------------------------------------------------------------
void C_TFPlayer::AddDecal( const Vector &rayStart, const Vector &rayEnd, const Vector &decalCenter, int hitbox, int decalIndex, bool doTrace, trace_t &tr, int maxLODToDecal )
{
    ...
    if ( event == 7001 )
    {
        ...
        if ( t_pSurface && !this->m_Shared.IsStealthed() && !this->m_Shared.InCond( TF_COND_DISGUISED ) && ( ( vel.x < -150 || vel.x > 150 ) || ( vel.y < -150 || vel.y > 150 ) ) )
        {
            ...
            if ( t_pSurface->game.material == SURFACE_SNOW )
            {
                ...
                UTIL_TraceLine( hipOrigin, hipOrigin + Vector( 0, 0, -COORD_EXTENT * 1.74 ), MASK_SOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &tr );
...
void C_TFPlayer::GetTargetIDDataString( bool bIsDisguised, OUT_Z_BYTECAP( iMaxLenInBytes ) wchar_t *sDataString, int iMaxLenInBytes, bool &bIsAmmoData, bool &bIsKillStreakData )
{
    ...
    if ( bIsDisguised )
    {
        if ( !IsEnemyPlayer() )
        {
            ...
            g_pVGuiLocalize->ConstructString( sDataString, iMaxLenInBytes, g_pVGuiLocalize->Find( "#TF_playerid_friendlyspy_disguise" ), 2, wszAlignment, wszClassName );
        }
...
{
    return iCustomDamageType == TF_DMG_CUSTOM_DECAPITATION || iCustomDamageType == TF_DMG_CUSTOM_TAUNTATK_BARBARIAN_SWING || iCustomDamageType == TF_DMG_CUSTOM_DECAPITATION_BOSS || iCustomDamageType == TF_DMG_CUSTOM_MERASMUS_DECAPITATION;
}
...
void C_TFPlayer::FireGameEvent( IGameEvent *event )
{
    ...
    else if ( FStrEq( event->GetName(), "rocket_jump" ) || FStrEq( event->GetName(), "sticky_jump" ) || FStrEq( event->GetName(), "rocketpack_launch" ) )
    {
    ...
    else if ( FStrEq( event->GetName(), "rocket_jump_landed" ) || FStrEq( event->GetName(), "sticky_jump_landed" ) || FStrEq( event->GetName(), "rocketpack_landed" ) )
    {
        const int iUserID = event->GetInt( "userid" );
