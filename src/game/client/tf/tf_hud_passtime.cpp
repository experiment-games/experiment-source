...
static Color GetTeamColor( int iTeam, byte alpha = 255 )
{
    ...
    {
        case TF_TEAM_RED:
            return Color( 159, 55, 34, alpha );
        case TF_TEAM_BLUE:
            return Color( 76, 109, 128, alpha );
        default:
            return Color( 245, 231, 222, alpha );
    }
}
...
static const char *GetProgressBallImageForTeam( int iTeam )
{
    ...
    {
        case TF_TEAM_RED:
            return "../passtime/hud/passtime_ballcontrol_red";
        case TF_TEAM_BLUE:
            return "../passtime/hud/passtime_ballcontrol_blue";
        default:
            return "../passtime/hud/passtime_ballcontrol_none";
    };
}
...
CTFHudTeamScore::CTFHudTeamScore( vgui::Panel *pParent )
    : CTFHudPasstimePanel( pParent, "HudTeamScore" ), m_pPlayingToCluster( 0 )
{
}
...
void CTFHudPasstimeEventText::Enqueue( C_TFPlayer *pSource, C_TFPlayer *pSubject, const char *pTitle, const char *pDetail, const char *pBonus )
{
    ...
    auto bShowBonus = ( pSubject == pLocalPlayer ) || ( pLocalPlayer->IsObserver() && pLocalPlayer->GetObserverTarget() == pLocalPlayer );
...
bool CTFHudPasstimeBallStatus::BShouldDraw() const
{
    ...
    return ( ( roundstate == GR_STATE_RND_RUNNING ) || ( roundstate == GR_STATE_STALEMATE ) ) && !TFGameRules()->InSetup();
}
...
CTFHudPasstime::CTFHudPasstime( Panel *pParent )
    : CTFHudPasstimePanel( pParent, "HudPasstime" ), m_pBallStatus( new CTFHudPasstimeBallStatus( this ) ), m_pTeamScore( new CTFHudTeamScore( this ) ), m_pBallOffscreenArrow( new CTFHudPasstimeBallOffscreenArrow( this ) ), m_pPassNotify( new CTFHudPasstimePassNotify( this ) )
{
    for ( int i = 0; i < MAX_PLAYERS; ++i )
    {
