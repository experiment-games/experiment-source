...
class CTFGCClientSystem : public CGCClientSystem, public GCSDK::ISharedObjectListener, public CGameEventListener
{
    ...
    CMsgGCDataCenterPing_Update GetPingData()
    {
        return m_msgCachedPingUpdate;
    }
    ...
    uint64_t GetLiveMatchID() const
    {
        return 0u;
    }
    PlayerGroupID_t GetLiveMatchLobbyID() const
    {
        return 0u;
    }

    // Whether the local player has a chat suspension in their assigned match.
    bool BHaveChatSuspensionInCurrentMatch() const
    {
        return m_bAssignedMatchChatSuspension;
    }
    ...
    // World status
    const CMsgTFWorldStatus& WorldStatus() const
    {
        return m_WorldStatus;
    }
    ...
    uint32 GetMostSearchedCount() const
    {
        return m_nMostSearchedMapCount;
    }
    ...
    const CMsgGCMatchMakerStatsResponse& GetMatchMakerStats()
    {
        return m_MatchMakerStats;
    }
    const CUtlDict< float >& GetDataCenterPopulationRatioDict( ETFMatchGroup eMatchGroup )
    {
        return m_dictDataCenterPopulationRatio[eMatchGroup];
    }
    ...
    const CMsgGCSurveyRequest& GetSurveyRequest() const
    {
        return m_msgSurveyRequest;
    }
    ...
    bool BIsIPRecentMatchServer( netadr_t ip )
    {
        return m_vecMatchServerHistory.Find( ip ) != m_vecMatchServerHistory.InvalidIndex();
    }
    ...
    // BConnectedtoGC()
    bool BHealthyGCConnection() const
    {
        return BConnectedtoGC() && !BStalledReliableMessages();
    }

    // Have we received positive signal from the GC that our client version is out of date?
    bool BClientOutOfDate() const
    {
        return m_bClientOutOfDate;
    }
