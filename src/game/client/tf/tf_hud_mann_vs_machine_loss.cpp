...
CMvMWaveLossPanel::CMvMWaveLossPanel( Panel *parent, const char *pName )
...
void CMvMWaveLossPanel::ShowPanel()
{
    ...
    if ( TFObjectiveResource() )
    {
        ...
        if ( GetItemSchema()->GetMvmMissions().IsValidIndex( iChallengeIndex ) )
        {
            ...
            g_pVGuiLocalize->ConstructString_safe( wszLocalizedSummary, g_pVGuiLocalize->Find( "#TF_PVE_MissionSummaryScheme" ), 2, g_pVGuiLocalize->Find( mission.m_sDisplayName.Get() ), g_pVGuiLocalize->Find( mission.m_sMode.Get() ) );
        }
        else
        {
            wchar_t wszName[256];
            g_pVGuiLocalize->ConvertANSIToUnicode( GetMapDisplayName( szTempName ), wszName, sizeof( wszName ) );
            g_pVGuiLocalize->ConstructString_safe( wszLocalizedSummary, g_pVGuiLocalize->Find( "#TF_PVE_MissionSummaryDefault" ), 1, wszName );
        }
