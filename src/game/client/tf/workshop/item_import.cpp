...
// Purpose:
//-----------------------------------------------------------------------------
CTFFileImportTextEditDialog::CTFFileImportTextEditDialog( vgui::Panel *parent, const char *pszTitle, const char *pszCommand )
    : Frame( parent, "ImportFileTextEditDialog" ), m_bShowCancelButton( true ), m_sTitle( pszTitle ), m_sText(), m_sCommand( pszCommand ), m_pTextEntry( NULL )
{
...
class CTFImportMaterialEditDialog : public vgui::Frame
{
   ...
   public:
    ...
    const char *GetBaseTextureFile( int nSkinIndex ) const
    {
        return m_strBaseTextureFile[nSkinIndex].String();
    }
    const char *GetNormalTextureFile() const
    {
        return m_strNormalTextureFile.String();
    }
    const char *GetPhongExponentTextureFile() const
    {
        return m_strPhongExponentTextureFile.String();
    }
    const char *GetSelfIllumTextureFile() const
    {
...
// Purpose:
//-----------------------------------------------------------------------------
CTFImportMaterialEditDialog::CTFImportMaterialEditDialog( vgui::Panel *parent, int nSkinIndex, int nMaterialIndex, KeyValues *pItemValues )
    : Frame( parent, "ImportMaterialEditDialog" ), m_sCommand( CFmtStr( "EditMaterialDone%d,%d", nSkinIndex, nMaterialIndex ) ), m_VMTKeyValues( "VMT" ), m_nSkinIndex( nSkinIndex ), m_nMaterialIndex( nMaterialIndex ), m_pItemValues( pItemValues )
{
    vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx( enginevgui->GetPanel( PANEL_CLIENTDLL ), "resource/ClientScheme.res", "ClientScheme" );
    SetScheme( scheme );
    SetProportional( true );
...
CImportPreviewItemPanel::CImportPreviewItemPanel( vgui::Panel *parent, KeyValues *pItemValues, int nSelectedClass )
    : CTFStorePreviewItemPanel2( parent, "resource/ui/ImportPreviewItemPanel.res", "importpreviewitem", NULL ), m_pItemValues( pItemValues ), m_nCurrentLOD( 0 ), m_nCurrentLoadout( LOADOUT_POSITION_PRIMARY ), m_sCurrentPose( kModelPoses[kDefaultModelPoseIndex] ), m_bIsVCDFileNameOnly( true ), m_flGestureEndTime( 0.0f ), m_nSelectedClass( nSelectedClass )
{
    ResetHandles();
...
CTFFileImportDialog::BUILD_RESULT CTFFileImportDialog::ValidateMaterialValues( KeyValues *pKV, int nMaterialIndex )
{
    ...
    static struct MaterialCheck
    {
        ...
        { BUILD_FAILED_MATERIALMISSINGCLOAK, 0, { "$cloakPassEnabled", "Proxies/invis" } },
        { BUILD_FAILED_MATERIALMISSINGBURNING, 0, { "$detail", "$detailscale", "$detailblendfactor", "$detailblendmode", "Proxies/AnimatedTexture", "Proxies/BurnLevel" } },
        { BUILD_FAILED_MATERIALMISSINGJARATE, CONDITION_NOTPAINTABLE, { "$yellow", "Proxies/YellowLevel", "Proxies/Equals" } },
        { BUILD_FAILED_MATERIALMISSINGJARATE, CONDITION_PAINTABLE, { "$yellow", "Proxies/YellowLevel", "Proxies/Multiply" } },
        { BUILD_FAILED_MATERIALMISSINGPAINTABLE, CONDITION_PAINTABLE, { "$blendtintbybasealpha", "$blendtintcoloroverbase", "$colortint_base", "$color2", "$colortint_tmp", "Proxies/ItemTintColor", "Proxies/SelectFirstIfNonZero" } },
    };
