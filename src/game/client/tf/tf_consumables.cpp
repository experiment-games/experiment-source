...
void CEconTool_WeddingRing::OnClientUseConsumable( CEconItemView *pItem, vgui::Panel *pParent ) const
{
    ...
    // the proposal.
    CTFGenericConfirmDialog *pDialog = ShowConfirmDialog( "#TF_UseWeddingRing_Title", "#TF_UseWeddingRing_Text", "#TF_WeddingRing_AcceptProposal", "#TF_WeddingRing_RejectProposal", &UseItemConfirm );
...
void CEconTool_BackpackExpander::OnClientUseConsumable( CEconItemView *pItem, vgui::Panel *pParent ) const
{
    ...
    CTFGenericConfirmDialog *pDialog = ShowConfirmDialog( "#TF_UseBackpackExpander_Title", "#TF_UseBackpackExpander_Text", "#GameUI_OK", "#Cancel", &UseItemConfirm );
    pDialog->AddStringToken( "item_name", pItem->GetItemName() );
...
void CEconTool_AccountUpgradeToPremium::OnClientUseConsumable( CEconItemView *pItem, vgui::Panel *pParent ) const
{
    ...
    CTFGenericConfirmDialog *pDialog = ShowConfirmDialog( "#TF_UseAccountUpgradeToPremium_Title", "#TF_UseAccountUpgradeToPremium_Text", "#GameUI_OK", "#Cancel", &UseItemConfirm );
    pDialog->AddStringToken( "item_name", pItem->GetItemName() );
    wchar_t wszUsesLeft[32];
    _snwprintf( wszUsesLeft, ARRAYSIZE( wszUsesLeft ), L"%d", pItem->GetItemQuantity() );
    pDialog->AddStringToken( "uses_left", wszUsesLeft );
...
void CEconTool_ClaimCode::OnClientUseConsumable( CEconItemView *pItem, vgui::Panel *pParent ) const
{
    ...
    CTFGenericConfirmDialog *pDialog = ShowConfirmDialog( "#TF_UseClaimCode_Title", "#TF_UseClaimCode_Text", "#GameUI_OK", "#Cancel", &UseItemConfirm );
    pDialog->AddStringToken( "item_name", pItem->GetItemName() );
...
static void ClientConsumableTool_CustomCallback( CEconItemView *pItem, vgui::Panel *pParent, GenericConfirmDialogCallback callback )
{
    ...
    CTFGenericConfirmDialog *pDialog = ShowConfirmDialog( "#TF_UseItem_Title", "#TF_UseItem_Text", "#GameUI_OK", "#Cancel", callback );
    pDialog->AddStringToken( "item_name", pItem->GetItemName() );
    wchar_t wszUsesLeft[32];
    _snwprintf( wszUsesLeft, ARRAYSIZE( wszUsesLeft ), L"%d", pItem->GetItemQuantity() );
    pDialog->AddStringToken( "uses_left", wszUsesLeft );
...
void CEconTool_GrantOperationPass::OnClientUseConsumable( CEconItemView *pItem, vgui::Panel *pParent ) const
{
    ...
    // show a confirmation dialog to make sure they want to consume the charge
    CTFGenericConfirmDialog *pDialog = ShowConfirmDialog( pszTitle, pszBody, "#GameUI_OK", "#Cancel", &OpenPass );
    pDialog->AddStringToken( "item_name", pItem->GetItemName() );
    wchar_t wszUsesLeft[32];
    _snwprintf( wszUsesLeft, ARRAYSIZE( wszUsesLeft ), L"%d", pItem->GetItemQuantity() );
    pDialog->AddStringToken( "uses_left", wszUsesLeft );
...
class CTFPainkitConsumeDialog : public EditablePanel, public CLocalSteamSharedObjectListener
{
    ...
    virtual void OnCommand( const char *pszCommand ) OVERRIDE
    {
        ...
        {
            CTFGenericConfirmDialog *pDialog = ShowConfirmDialog( "#TF_UsePaintkit_Title", "#TF_UsePaintkit_Text", "#GameUI_OK", "#Cancel", PaintkitConfirmCallback );
...
void UseGift( CEconItemView *pItem, CSteamID targetID )
{
    ...
    C_CTF_GameStats.Event_Trading( IE_TRADING_ITEM_GIFTED, pItem, true, steamID.ConvertToUint64(), targetID.ConvertToUint64() );
}
...
class CTFGiftNotification : public CEconNotification
{
        ...
        : CEconNotification()
    {
        ...
        m_bRandomPerson = msg.Body().has_was_random_person() && msg.Body().was_random_person();
...
struct duel_minigame_local_data_t
{
    duel_minigame_local_data_t()
        : m_pEventListener( NULL ), m_steamIDOpponent(), m_unMyScore( 0 ), m_unOpponentScore( 0 ), m_iRequiredPlayerClass( TF_CLASS_UNDEFINED )
    {
    }
    uint32 m_unMyScore;
    ...
    static void ConfirmDuel( bool bConfirmed, void *pContext )
    {
        ...
        if ( bConfirmed && pLocalPlayer && pNotification->m_iRequiredPlayerClass != TF_CLASS_UNDEFINED )
        {
            ...
            if ( pNotification->m_iRequiredPlayerClass != iClass )
            {
                ...
                {
                    case TF_CLASS_SCOUT:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_Scout" ) );
                        break;
                    case TF_CLASS_SNIPER:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_Sniper" ) );
                        break;
                    case TF_CLASS_SOLDIER:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_Soldier" ) );
                        break;
                    case TF_CLASS_DEMOMAN:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_Demoman" ) );
                        break;
                    case TF_CLASS_MEDIC:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_Medic" ) );
                        break;
                    case TF_CLASS_HEAVYWEAPONS:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_HWGuy" ) );
                        break;
                    case TF_CLASS_PYRO:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_Pyro" ) );
                        break;
                    case TF_CLASS_SPY:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_Spy" ) );
                        break;
                    case TF_CLASS_ENGINEER:
                        pKeyValues->SetWString( "player_class", g_pVGuiLocalize->Find( "#TF_Class_Name_Engineer" ) );
                        break;
                }
...
CSelectPlayerForDuelDialog::CSelectPlayerForDuelDialog( uint64 iItemID )
    : CSelectPlayerDialog( NULL ), m_iItemID( iItemID ), m_iPlayerClass( TF_CLASS_UNDEFINED )
{
...
void CSelectPlayerForDuelDialog::SetSelectedClass( int iClass )
{
    ...
    {
        case TF_CLASS_SCOUT:
            pClassName = "#TF_Class_Name_Scout";
            break;
        case TF_CLASS_SNIPER:
            pClassName = "#TF_Class_Name_Sniper";
            break;
        case TF_CLASS_SOLDIER:
            pClassName = "#TF_Class_Name_Soldier";
            break;
        case TF_CLASS_DEMOMAN:
            pClassName = "#TF_Class_Name_Demoman";
            break;
        case TF_CLASS_MEDIC:
            pClassName = "#TF_Class_Name_Medic";
            break;
        case TF_CLASS_HEAVYWEAPONS:
            pClassName = "#TF_Class_Name_HWGuy";
            break;
        case TF_CLASS_PYRO:
            pClassName = "#TF_Class_Name_Pyro";
            break;
        case TF_CLASS_SPY:
            pClassName = "#TF_Class_Name_Spy";
            break;
        case TF_CLASS_ENGINEER:
            pClassName = "#TF_Class_Name_Engineer";
            break;
    }
