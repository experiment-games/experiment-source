...
// Purpose:
//-----------------------------------------------------------------------------
CHudNotificationPanel::CHudNotificationPanel( const char *pElementName )
    : CHudElement( pElementName ), BaseClass( NULL, "NotificationPanel" )
{
...
void CHudNotificationPanel::MsgFunc_HudNotify( bf_read &msg )
{
    ...
    if ( m_mapShowCounts.IsValidIndex( idx ) )
    {
        ...
        if ( showCount.m_pConVar->GetInt() >= showCount.m_nMaxShowCount || showCount.m_flNextAllowedTime > Plat_FloatTime() )
        {
            return;
        }
