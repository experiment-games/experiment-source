...
bool C_TFPasstimeLogic::GetBallReticleTarget( C_BaseEntity **ppEnt, bool *bHomingActive ) const
{
    ...
    if ( !pEnt || ( pEnt == pLocalPlayer ) || ( pEnt->GetEffects() & EF_NODRAW ) || ( ( pEnt->GetTeamNumber() != TEAM_UNASSIGNED ) && ( pEnt->GetTeamNumber() != pLocalPlayer->GetTeamNumber() ) ) || ( pLocalPlayer->IsObserver() && ( GetSpectatorMode() != OBS_MODE_ROAMING ) && ( GetSpectatorTarget() == pEnt->index ) ) )
    {
        return false;
    }
