...
// Purpose:
//-----------------------------------------------------------------------------
CHudMainMenuOverride::CHudMainMenuOverride( IViewPort *pViewPort )
    : BaseClass( NULL, PANEL_MAINMENUOVERRIDE )
{
...
CExplanationPopup *CHudMainMenuOverride::StartHighlightAnimation( mm_highlight_anims iAnim )
{
    ...
    {
        case MMHA_TUTORIAL:
            return ShowDashboardExplanation( "TutorialHighlight" );
        case MMHA_PRACTICE:
            return ShowDashboardExplanation( "PracticeHighlight" );
        case MMHA_NEWUSERFORUM:
            return ShowDashboardExplanation( "NewUserForumHighlight" );
        case MMHA_OPTIONS:
            return ShowDashboardExplanation( "OptionsHighlightPanel" );
        case MMHA_LOADOUT:
            return ShowDashboardExplanation( "LoadoutHighlightPanel" );
        case MMHA_STORE:
            return ShowDashboardExplanation( "StoreHighlightPanel" );
    }
...
void CHudMainMenuOverride::OnCommand( const char *command )
{
        ...
        if ( steamapicontext && steamapicontext->SteamFriends() && steamapicontext->SteamUtils() )
        {
            ...
            switch ( GetUniverse() )
            {
                case k_EUniversePublic:
                    steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage( CFmtStr1024( "http://steamcommunity.com/profiles/%llu/promocodes/tf2", steamID.ConvertToUint64() ) );
                    break;
                case k_EUniverseBeta:
                    steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage( CFmtStr1024( "http://beta.steamcommunity.com/profiles/%llu/promocodes/tf2", steamID.ConvertToUint64() ) );
                    break;
                case k_EUniverseDev:
                    steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage( CFmtStr1024( "http://localhost/community/profiles/%llu/promocodes/tf2", steamID.ConvertToUint64() ) );
                    break;
            }
        }
    }
    ...
    else if ( FStrEq( "view_update_page", command ) )
    {
        ...
        if ( steamapicontext && steamapicontext->SteamFriends() && steamapicontext->SteamUtils() && steamapicontext->SteamUtils()->IsOverlayEnabled() )
        {
            ...
            {
                case k_EUniversePublic:
                    steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage( "http://www.teamfortress.com/meetyourmatch" );
                    break;
                case k_EUniverseBeta:  // Fall through
                case k_EUniverseDev:
                    steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage( "http://csham.valvesoftware.com/tf.com/meetyourmatch" );
                    break;
            }
        }
        else
    ...
    else if ( FStrEq( "view_update_comic", command ) )
    {
        if ( steamapicontext && steamapicontext->SteamFriends() && steamapicontext->SteamUtils() && steamapicontext->SteamUtils()->IsOverlayEnabled() )
        {
            ...
            {
                case k_EUniversePublic:
                    steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage( "http://www.teamfortress.com/gargoyles_and_gravel" );
                    break;
                case k_EUniverseBeta:  // Fall through
                case k_EUniverseDev:
                    steamapicontext->SteamFriends()->ActivateGameOverlayToWebPage( "http://www.teamfortress.com/gargoyles_and_gravel" );
...
void CHudMainMenuOverride::UpdateRankPanelType()
{
    ...
    auto pMatchGroup = GetMatchGroupDescription( eMatchGroup );
    if ( GetProgressionDesc( k_eProgression_Glicko ) == pMatchGroup->m_pProgressionDesc && GTFGCClientSystem()->BConnectedtoGC() )
    {
