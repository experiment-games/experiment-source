...
CRecipeComponentItemModelPanel::CRecipeComponentItemModelPanel( vgui::Panel* parent, const char* name )
    : CItemModelPanel( parent, name ), m_nPageNumber( 0 )
{
    SetItem( NULL );
}
...
{
    return ( iPanelIndex >= ( DYNAMIC_RECIPE_INPUT_COUNT + DYNAMIC_RECIPE_OUTPUT_COUNT ) && !IsInputPanel( iPanelIndex ) && !IsOutputPanel( iPanelIndex ) );
}
...
void CDynamicRecipePanel::SetBorderForItem( CItemModelPanel* pItemPanel, bool bMouseOver )
{
    ...
    if ( IsInputPanel( iIndex ) )
    {
        ...
        if ( pInputPanel->GetRecipeItem( pInputPanel->GetPageNumber() ) == NULL )
        {
            ...
            bool bValidHighlightSourcePanel = pPanel == m_pMouseDragItemPanel || m_vecBackpackModelPanels.Find( static_cast< CRecipeComponentItemModelPanel* >( pPanel ) ) != m_vecBackpackModelPanels.InvalidIndex();
...
bool CDynamicRecipePanel::CanDragTo( CItemModelPanel* pItemPanel, int iPanelIndex )
{
    ...
    if ( IsInputPanel( iDraggedFromPos ) && IsInputPanel( iPanelIndex ) && InputPanelCanAcceptItem( pItemPanel, itemID ) )
        return true;
