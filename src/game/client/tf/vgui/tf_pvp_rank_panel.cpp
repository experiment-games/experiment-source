...
CPvPRankPanel::RatingState_t::RatingState_t( ETFMatchGroup eMatchGroup )
    : m_nStartRating( 0u ), m_nTargetRating( 0u ), m_nActualRating( 0u ), m_bCurrentDeltaViewed( true ), m_eMatchGroup( eMatchGroup ), m_bInitialized( false )
{
...
void CPvPRankPanel::RatingState_t::FireGameEvent( IGameEvent* pEvent )
{
    ...
    else if ( FStrEq( pEvent->GetName(), "server_spawn" ) && GTFGCClientSystem()->BHaveLiveMatch() && GTFGCClientSystem()->GetLiveMatchGroup() == m_eMatchGroup )
    {
        UpdateRating( false );
...
CPvPRankPanel::CPvPRankPanel( Panel* parent, const char* panelName )
    : BaseClass( parent, panelName ), m_eMatchGroup( k_eTFMatchGroup_Invalid ), m_pProgressionDesc( NULL ), m_pContinuousProgressBar( NULL ), m_pModelPanel( NULL ), m_pXPBar( NULL ), m_pBGPanel( NULL ), m_nLastLerpRating( 0 ), m_nLastSeenLevel( 0 ), m_bInitializedBaseState( false )
{
...
void CPvPRankPanel::SOCreated( const CSteamID& steamIDOwner, const CSharedObject* pObject, ESOCacheEvent eEvent )
{
    if ( pObject->GetTypeID() == CTFRatingData::k_nTypeID )
    {
        ...
        {
            UpdateRevealState( ( ETFMatchGroup )eMatchGroup, eEvent == eSOCacheEvent_Subscribed || eEvent == eSOCacheEvent_ListenerAdded );
        }
        m_bRevealingRank = s_rankReavealState[m_eMatchGroup].m_bNeedsRankReveal;
    }
}
