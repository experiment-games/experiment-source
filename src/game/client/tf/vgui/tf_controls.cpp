...
//-----------------------------------------------------------------------------
CTFFooter::CTFFooter( Panel *parent, const char *panelName )
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTFAdvancedOptionsDialog::CTFAdvancedOptionsDialog( vgui::Panel *parent )
    : BaseClass( NULL, "TFAdvancedOptionsDialog" )
{
...
CExScrollingEditablePanel::CExScrollingEditablePanel( Panel *pParent, const char *pszName )
    : EditablePanel( pParent, pszName ), m_nLastScrollValue( 0 ), m_bUseMouseWheelToScroll( true )
{
...
CExpandablePanel::CExpandablePanel( Panel *pParent, const char *pszName )
    : vgui::EditablePanel( pParent, pszName ), m_bExpanded( false ), m_flAnimEndTime( 0.f ), m_flResizeTime( 0.f )
{
}
...
CDraggableScrollingPanel::CDraggableScrollingPanel( Panel *pParent, const char *pszPanelname )
    : EditablePanel( pParent, pszPanelname ), m_iDragStartX( 0 ), m_iDragStartY( 0 ), m_iDragTotalDistance( 0 ), m_bDragging( false ), m_flZoom( 1.f )
{
}
...
void CDraggableScrollingPanel::OnSliderMoved( KeyValues *pParams )
{
    ...
    if ( pSlider )
    {
        ...
        SetZoomAmount( RemapVal( ( float )pSlider->GetValue(), ( float )nMin, ( float )nMax, m_flMinZoom, m_flMaxZoom ), GetParent()->GetWide() / 2.f, GetParent()->GetTall() / 2.f );
    }
}
    ...
    FOR_EACH_VEC( m_vecChildOriginalData, i )
    {
        ...
        if ( child.m_bMoveWithDrag )
        {
            switch ( child.m_ePinPosition )
            {
                case PIN_CENTER:
                {
                    child.m_pChild->SetPos( ( child.m_flX * GetWide() ) - ( child.m_pChild->GetWide() / 2.f ), ( child.m_flY * GetTall() ) - ( child.m_pChild->GetTall() / 2.f ) );
                }
                break;

                case PIN_TOP_LEFT:
                {
                    child.m_pChild->SetPos( child.m_flX * GetWide(), child.m_flY * GetTall() );
                }
                break;

                case PIN_TOP_RIGHT:
                {
                    child.m_pChild->SetPos( ( child.m_flX * GetWide() ) - child.m_pChild->GetWide(), ( child.m_flY * GetTall() ) );
                }
                break;

                case PIN_BOTTOM_LEFT:
                {
                    child.m_pChild->SetPos( ( child.m_flX * GetWide() ), ( child.m_flY * GetTall() ) - child.m_pChild->GetTall() );
                }
                break;

                case PIN_BOTTOM_RIGHT:
                {
                    child.m_pChild->SetPos( ( child.m_flX * GetWide() ) - ( child.m_pChild->GetWide() ), ( child.m_flY * GetTall() ) - ( child.m_pChild->GetTall() ) );
                }
                break;
            }
        }
