...
class CQuestMapPathsPanel : public Panel
{
   public:
    ...
    void SetActiveRegion( EditablePanel* pRegionPanel )
    {
        m_pActiveRegionLinkPanel = pRegionPanel;
    }

    CCircleDrawingHelper& GetCircleDrawer()
    {
...
// Purpose:
//-----------------------------------------------------------------------------
class CQuestMapRegionPanel : public EditablePanel, public CGameEventListener
{
   ...
   public:
    ...
    const CDraggableScrollingPanel* GetZoomPanel() const
    {
        return m_pZoomPanel;
    }
