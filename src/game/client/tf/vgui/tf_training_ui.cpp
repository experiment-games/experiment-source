...
CExButton *SetupButtonActionSignalTarget( Panel *pParent, const char *pButtonName, const char *pCommand = NULL )
{
    ...
    EditablePanel *pTrainingDialog = static_cast< EditablePanel * >( FindAncestorByName( pParent, TRAINING_DIALOG_NAME ) );
    Assert( pTrainingDialog );
...
class CTrainingBasePanel : public EditablePanel
{
    ...
    virtual void ApplySchemeSettings( IScheme *pScheme )
    {
        ...
        EditablePanel *pTrainingDialog = static_cast< EditablePanel * >( FindAncestorByName( this, TRAINING_DIALOG_NAME ) );
        Assert( pTrainingDialog );
        if ( pTrainingDialog )
        {
            ...
            {
                KeyValues *pTitleFormatData = GetTitleFormatData();
                AssertMsg( pTitleFormatData, "Should get valid data here." );
                if ( pTitleFormatData )
                {
...
class CBasicTraining_ClassPanel : public EditablePanel
{
    ...
    virtual void ApplySchemeSettings( IScheme *pScheme )
    {
        ...
        m_pImagePanel = dynamic_cast< ImagePanel * >( FindChildByName( "Image" ) );
        Assert( m_pImagePanel );
        m_pSelectButton = SetupButtonActionSignalTarget( this, "SelectButton" );
...
class COfflinePractice_ModeSelectionPanel : public CTrainingBaseCarouselPanel
{
    ...
    virtual void ApplySettings( KeyValues *pInResourceData )
    {
        ...
        for ( int i = 0; i < NUM_PRACTICE_MODES; ++i )
        {
            ...
            m_ModeInfos[i].m_nId = ( GameMode_t )pInResourceData->GetInt( fmtModeId.Access(), MODE_INVALID );
            Assert( m_ModeInfos[i].m_nId != MODE_INVALID );
        }
...
class CTrainingDialog : public EditablePanel
{
    ...
    virtual void ApplySchemeSettings( IScheme *pScheme )
    {
        ...
        m_pModeSelectionPanel = dynamic_cast< CModeSelectionPanel * >( m_pContainer->FindChildByName( "ModeSelectionPanel" ) );
        Assert( m_pModeSelectionPanel );
        m_pBasicTraining_ClassSelectionPanel = dynamic_cast< CBasicTraining_ClassSelectionPanel * >( m_pContainer->FindChildByName( "BasicTraining_ClassSelectionPanel" ) );
        Assert( m_pBasicTraining_ClassSelectionPanel );
        m_pBasicTraining_ClassDetailsPanel = dynamic_cast< CBasicTraining_ClassDetailsPanel * >( m_pContainer->FindChildByName( "BasicTraining_ClassDetailsPanel" ) );
        Assert( m_pBasicTraining_ClassDetailsPanel );
        m_pOfflinePractice_ModeSelectionPanel = dynamic_cast< COfflinePractice_ModeSelectionPanel * >( m_pContainer->FindChildByName( "OfflinePractice_ModeSelectionPanel" ) );
        Assert( m_pOfflinePractice_ModeSelectionPanel );
        m_pOfflinePractice_MapSelectionPanel = dynamic_cast< COfflinePractice_MapSelectionPanel * >( m_pContainer->FindChildByName( "OfflinePractice_MapSelectionPanel" ) );
        Assert( m_pOfflinePractice_MapSelectionPanel );
