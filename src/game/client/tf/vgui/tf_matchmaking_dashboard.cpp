...
class CTooltipEditablePanel : public EditablePanel
{
   public:
    ...
    virtual void ApplySchemeSettings( vgui::IScheme* pScheme ) OVERRIDE
    {
        ...
        {
            case k_eSmallFont:
                pKVCond->AddSubKey( new KeyValues( "if_small" ) );
                break;
            case k_eMediumFont:
                pKVCond->AddSubKey( new KeyValues( "if_medium" ) );
                break;
            case k_eLargeFont:
                pKVCond->AddSubKey( new KeyValues( "if_large" ) );
                break;
        }
...
REGISTER_FUNC_FOR_DASHBOARD_PANEL_TYPE( []() -> Panel*
                                        { return GetDashboardToolTipPanel( k_eSmallFont ); },
                                        k_eToolTipSmallFont );
REGISTER_FUNC_FOR_DASHBOARD_PANEL_TYPE( []() -> Panel*
                                        { return GetDashboardToolTipPanel( k_eMediumFont ); },
                                        k_eToolTipMediumFont );
REGISTER_FUNC_FOR_DASHBOARD_PANEL_TYPE( []() -> Panel*
                                        { return GetDashboardToolTipPanel( k_eLargeFont ); },
                                        k_eToolTipLargeFont );
...
void CTFMatchmakingDashboard::OnTick()
{
    ...
    // Update the Y-Pos of all the "children" of the dashboard
    {
        ...
        EMMDashboadSidePanel arPanelsDanglingOffTopBar[] = { k_eChat, k_eNextMapWinnerPopup, k_eNextMapVotePopup };
    ...
    if ( engine->IsInGame() )
    {
        ...
        bShouldBeVisible |= GTFGCClientSystem()->BConnectedToMatchServer( false ) && bInEndOfMatch && pMatchDesc && pMatchDesc->BUsesDashboard();
    }
    else
    {
...
void CTFMatchmakingDashboard::OnPlayEvent()
{
    ...
    if ( eEventMatchGroup == k_eTFMatchGroup_Event_Placeholder && tf_comp_welcome_hide.GetBool() == false && !GTFPartyClient()->BInQueueForMatchGroup( eEventMatchGroup ) )
    {
        tf_comp_welcome_hide.SetValue( 1 );
