...
CQuestObjectiveTooltip::CQuestObjectiveTooltip( vgui::Panel* parent, const char* text )
    : vgui::BaseTooltip( parent, text ), m_pObjectivePanel( NULL )
{
...
CQuestViewSubPanel::CQuestViewSubPanel( Panel* pParent,
    ...
    : CExpandablePanel( pParent, pszPanelName ), m_pQuestDef( pQuestDef ), m_pQuest( pQuest ), m_pToolTip( NULL ), m_msgNodeData( msgNodeData )
{
...
CQuestNodeViewPanel::CQuestNodeViewPanel( Panel* pParent, const char* pszPanelname )
    : BaseClass( pParent, pszPanelname ), m_pItemToolTip( NULL ), m_pToolTip( NULL ), m_bQuestCreated( false )
{
    ...
    {
        CQuestViewSubPanel* pQuestTrackerPanel = new CQuestViewSubPanel( m_pContentContainer, CFmtStr( "QuestOption%d", i + 1 ), NULL, NULL, m_msgNodeData );
                ...
                if ( pCashImage )
                {
                    switch ( pNodeDef->GetCashRewardType() )
                    {
                        case CASH_REWARD_LARGE:
                            pCashImage->SetImage( "cyoa/cyoa_cash_large" );
                            break;
                        case CASH_REWARD_MEDIUM:
                            pCashImage->SetImage( "cyoa/cyoa_cash_medium" );
                            break;
                        case CASH_REWARD_SMALL:
                            pCashImage->SetImage( "cyoa/cyoa_cash_small" );
                            break;
...
void CQuestNodeViewPanel::QuestClicked( KeyValues* pParams )
{
    ...
    int nIndex = m_vecQuestSubPanels.FindPredicate( [&pszName]( CQuestViewSubPanel* pPanel )
