...
CTFMatchSummary::CTFMatchSummary( const char *pElementName )
    : CHudElement( pElementName ), EditablePanel( NULL, "MatchSummary" ), m_bXPShown( false )
{
...
void CTFMatchSummary::UpdatePlayerList()
{
    ...
    for ( int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++ )
    {
        if ( g_PR->IsConnected( playerIndex ) || g_PR->IsValid( playerIndex ) )
        {
            ...
            {
                int nMedal = pKey->GetInt();
                pPlayerList->SetColorOverrideForCell( 0, itemID, MS_COLUMN_DAMAGE, ( nMedal == ( int )StatMedal_Bronze ) ? m_clrBronzeMedal : ( nMedal == ( int )StatMedal_Silver ) ? m_clrSilverMedal
                                                                                                                                                                                    : m_clrGoldMedal );
            }
        ...
        if ( pPlayerList )
        {
            ...
            for ( int i = 0; i < pPlayerList->GetItemCount(); i++ )
            {
                ...
                if ( steamID.IsValid() )
                {
                    ...
                    if ( ( nPanelXPos != xGrandParent + xParent + x ) || ( nPanelYPos != yGrandParent + yParent + y ) || ( nPanelWide != wide ) || ( nPanelTall != tall ) )
                    {
...
void CTFMatchSummary::OnTick()
{
    ...
    if ( pMatchDesc->BAllowDrawingAtMatchHistory() && m_pDrawingPanel && ( m_flDrawingPanelTime > 0 ) && ( m_flDrawingPanelTime < gpGlobals->curtime ) )
    {
