...
CQuestMapPathsPanel::CQuestMapPathsPanel( Panel* pParent, const char* pszPanelname, float flZoomScale )
    : Panel( pParent, pszPanelname ), m_mapQuestNodes( DefLessFunc( uint32 ) ), m_mapRegionLinkPanels( DefLessFunc( uint32 ) ), m_flZoomScale( Max( 0.1f, flZoomScale ) )
{
...
CQuestMapRegionPanel::CQuestMapRegionPanel( Panel* pParent, const char* pszPanelName, const CMsgProtoDefID& msgIDCurrentRegion, CQuestNodeViewPanel* pNodeViewPanel )
    : BaseClass( pParent, pszPanelName ), m_mapNodePanels( DefLessFunc( uint32 ) ), m_mapRegionLinkPanels( DefLessFunc( uint32 ) ), m_msgIDCurrentRegion( msgIDCurrentRegion ), m_pQuestMapNodeView( pNodeViewPanel ), m_flLastClickTime( 0.f ), m_flClickScale( 1.f ), m_flZoomTime( 0.f ), m_flZoomStartTime( 0.f )
{
...
void CQuestMapRegionPanel::CreateActivationCircle( KeyValues* pParams )
{
    ...
    m_pPathsPanel->GetCircleDrawer().AddCircle( { Plat_FloatTime(), Plat_FloatTime() + 1.f, pNodePanel->GetXPos() + pNodePanel->GetWide() * 0.5f, pNodePanel->GetYPos() + pNodePanel->GetTall() * 0.5f, 0.f, 200.f, colorStart, colorEnd, true } );
}

void CQuestMapRegionPanel::CreateCircle( KeyValues* pParams )
{
    m_pPathsPanel->GetCircleDrawer().AddCircle( { pParams->GetFloat( "start_time", Plat_FloatTime() ), pParams->GetFloat( "end_time", Plat_FloatTime() + 1.f ), pParams->GetFloat( "xpos", 0.f ), pParams->GetFloat( "ypos", 0.f ), pParams->GetFloat( "start_radius", 0.f ), pParams->GetFloat( "end_radius", 200.f ), pParams->GetColor( "start_color" ), pParams->GetColor( "end_color" ), pParams->GetBool( "filled", true ) } );
}
...
void CQuestMapRegionPanel::NodeCursorEntered( KeyValues* pParams )
{
    ...
    m_pPathsPanel->GetCircleDrawer().AddCircle( { Plat_FloatTime(), Plat_FloatTime() + 0.4, ( float )pNodePanel->GetXPos() + ( pNodePanel->GetWide() * 0.5f ) + float( YRES( 0.5f ) ), ( float )pNodePanel->GetYPos() + ( pNodePanel->GetTall() * 0.5f ) + float( YRES( 0.5f ) ), ( float )YRES( node_medium_radius ) + ( float )YRES( 5 ), ( float )YRES( node_large_radius ), colorStart, colorEnd, false } );

    m_pPathsPanel->GetCircleDrawer().AddCircle( { Plat_FloatTime(), Plat_FloatTime() + 0.4, ( float )pNodePanel->GetXPos() + ( pNodePanel->GetWide() * 0.5f ) + float( YRES( 0.5f ) ), ( float )pNodePanel->GetYPos() + ( pNodePanel->GetTall() * 0.5f ) + float( YRES( 0.5f ) ), ( float )YRES( node_medium_radius ), ( float )YRES( node_large_radius ), colorStart, colorEnd, true } );
}
...
void CQuestMapRegionPanel::CreateClickCircle()
{
    ...
    m_pPathsPanel->GetCircleDrawer().AddCircle( { Plat_FloatTime(), Plat_FloatTime() + ( 0.3f * m_flClickScale ), ( float )nMouseX, ( float )nMouseY, 5.f, 30.f * m_flClickScale, colorStart, colorEnd, false } );
