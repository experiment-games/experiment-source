...
CTFItemInspectionPanel::CTFItemInspectionPanel( Panel *pPanel, const char *pszName )
    : BaseClass( pPanel, pszName ), m_pItemViewData( NULL ), m_pSOEconItemData( NULL ), m_nValidItemsPaintkitDefindex( INVALID_ITEM_DEF_INDEX ), m_nValidPaintkitsItemDefindex( 0xFFFF )
{
...
{
    const int iTeam = pData->GetInt( "userdata", -1 );
    AssertMsg( iTeam >= 0, "Bad filter" );
    if ( iTeam < 0 )
        return;
