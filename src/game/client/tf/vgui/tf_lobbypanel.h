...
class CBaseLobbyPanel : public vgui::PropertySheet, public CGameEventListener
{
   ...
   public:
    ...
    int NumPlayersInParty( void )
    {
        return m_vecPlayers.Count();
    }

    void ToggleJoinLateCheckButton( void )
    {
        m_pJoinLateCheckButton->SetSelected( !m_pJoinLateCheckButton->IsSelected() );
    }

    bool IsPartyActiveGroupBoxVisible()
    {
        return m_pPartyActiveGroupBox != NULL && m_pPartyActiveGroupBox->IsVisible();
    }
