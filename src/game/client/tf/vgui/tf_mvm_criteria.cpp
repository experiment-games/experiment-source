...
static ETFSyncedMMMenuStep GetMenuStep()
{
    ...
    if ( GTFPartyClient()->BIsPartyLeader() || GTFPartyClient()->GetLocalUIState().match_group() == GTFPartyClient()->GetLeaderUIState().match_group() )
    {
...
void CMVMCriteriaPanel::WriteTourList()
{
    ...
    if ( idxSelectedTour >= 0 )
    {
        ...
        if ( tour.m_eDifficulty >= k_EMvMChallengeDifficulty_Expert )
        {
            ...
            if ( !GTFGCClientSystem()->BGetLocalPlayerBadgeInfoForTour( idxSelectedTour, &unBadgeLevel, &unCompletedChallengeMask ) || unBadgeLevel == 0 )
            {
...
void CMVMCriteriaPanel::ApplySchemeSettings( vgui::IScheme *pScheme )
{
    ...
    m_pSquadSurplusCheckButton = dynamic_cast< vgui::CheckButton * >( FindChildByName( "SquadSurplusCheckButton", true ) );
    Assert( m_pSquadSurplusCheckButton );
    m_pMannUpNowButton = dynamic_cast< vgui::Button * >( FindChildByName( "MannUpNowButton", true ) );
    Assert( m_pMannUpNowButton );
