...
CQuestMapItemAdPanel::CQuestMapItemAdPanel( Panel* pParent, const char* pszName, const CQuestMapStoreItem* pStoreItemDef )
    : CItemAdPanel( pParent, pszName, INVALID_ITEM_DEF_INDEX ), m_pStoreItemDef( pStoreItemDef )
{
...
CQuestMapPanel::CQuestMapPanel( Panel* pParent, const char* pszPanelName )
    : BaseClass( pParent, pszPanelName ), m_mapRegions( DefLessFunc( uint32 ) ), m_bMapLoaded( false ), m_bViewingTutorial( false ), m_eIntroState( STATE_0 ), m_pKVRewardItemPanels( NULL ), m_flNextWobbleTime( 0.f ), m_pAdPanel( NULL )
{
...
void CQuestMapPanel::PerformLayout()
{
    ...
    {
        case 0:
            pScreen->SetImage( "cyoa/cyoa_pda_gravel" );
            break;
        case 1:
            pScreen->SetImage( "cyoa/cyoa_pda_bronze" );
            break;
        case 2:
            pScreen->SetImage( "cyoa/cyoa_pda_silver" );
            break;
        case 3:
            pScreen->SetImage( "cyoa/cyoa_pda_gold" );
            break;
    }
