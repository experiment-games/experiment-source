...
class CTFPartyClient : private CAutoGameSystemPerFrame, private CLocalSteamSharedObjectListener
{
    ...
    bool BHaveActiveParty() const
    {
        return !!m_pActiveParty;
    }
    ...
    const CTFParty *GetActiveParty() const
    {
        return m_pActiveParty;
    }

    // Helper to return the active party's group ID or 0 if we are a solo party-of-one
    GCSDK::PlayerGroupID_t GetActivePartyID() const
    {
        return m_unActivePartyID;
    }
    ...
    bool BIsPartyLeader() const
    {
        return !BHaveActiveParty() || m_bIsActivePartyLeader;
    }
    ...
    bool BControllingPartyActions() const
    {
    ...
    const TFSyncedMMUIState &GetLocalUIState() const
    {
    ...
    bool BInAnyMatchQueue() const
    {
        return GetNumQueuedMatchGroups() > 0;
    };
    ...
    const CTFGroupMatchCriteria &GetLocalGroupCriteria() const
    {
        return m_localGroupCriteria;
    }
    const CTFPerPlayerMatchCriteria &GetLocalPlayerCriteria() const
    {
    ...
    struct PartyInvite_t
    {
        CTFPartyInvite *pInvite;
        CSteamID steamIDInviter;
    };
