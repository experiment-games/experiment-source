...
CQuestObjectiveTextPanel::CQuestObjectiveTextPanel( Panel* pParent, const char* pElementName, const QuestObjectiveInstance_t& objective, const char* pszResFileName )
    : EditablePanel( pParent, pElementName ), m_strResFileName( pszResFileName ), m_objective( objective )
{
...
CQuestProgressTrackerPanel::PointsView_t::PointsView_t()
    : m_flCurrentProgress( 0 ), m_flTargetProgress( 0 ), m_nMaxPoints( 0 )
{
}
...
CQuestProgressTrackerPanel::CQuestProgressTrackerPanel( Panel* pParent,
    ...
    : EditablePanel( pParent, pElementName ), m_pQuest( NULL ), m_flLastThink( 0.f ), m_pszSoundToPlay( NULL ), m_nContentTall( 0 ), m_bMapView( false ), m_pQuestDef( pQuestDef ), m_strResFile( pszResFile )
{
...
void CQuestProgressTrackerPanel::FireGameEvent( IGameEvent* pEvent )
{
    ...
    else if ( FStrEq( pEvent->GetName(), "player_spawn" ) || FStrEq( pEvent->GetName(), "inventory_updated" ) || FStrEq( pEvent->GetName(), "localplayer_changeclass" ) )
    {
        InvalidateLayout();
    }
...
CHudItemAttributeTracker::CHudItemAttributeTracker( const char* pElementName )
    : CHudElement( pElementName ), EditablePanel( NULL, "ItemAttributeTracker" ), m_mapTrackers( DefLessFunc( itemid_t ) )
{
...
{
    if ( FStrEq( pEvent->GetName(), "player_spawn" ) || FStrEq( pEvent->GetName(), "inventory_updated" ) || FStrEq( pEvent->GetName(), "client_disconnect" ) || FStrEq( pEvent->GetName(), "localplayer_changeclass" ) || FStrEq( pEvent->GetName(), "quest_objective_completed" ) )
    {
        InvalidateLayout();
    }
}
...
bool CHudItemAttributeTracker::FindTrackerForItem( const CQuest* pItem, CQuestProgressTrackerPanel** ppTracker, bool bCreateIfNotFound )
{
    ...
    {
        ( *ppTracker ) = new CQuestProgressTrackerPanel( this, "ItemTrackerPanel", pItem, pItem->GetDefinition(), "resource/UI/quests/QuestItemTrackerPanel_InGame_Base.res" );
