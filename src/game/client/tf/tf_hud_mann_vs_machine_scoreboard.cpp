...
//-----------------------------------------------------------------------------
CMvMScoreboardEnemyInfo::CMvMScoreboardEnemyInfo( Panel *parent, const char *pName )
...
CTFHudMannVsMachineScoreboard::CTFHudMannVsMachineScoreboard( Panel *parent, const char *pName )
    : vgui::EditablePanel( parent, pName )
{
...
void CTFHudMannVsMachineScoreboard::OnTick()
{
    ...
    if ( !m_bInitialized || !TFGameRules() || !TFGameRules()->IsMannVsMachineMode() || !TFObjectiveResource() )
    {
        return;
    }
    ...
    if ( TFObjectiveResource() )
    {
        if ( Q_strcmp( m_popfile, TFObjectiveResource()->GetMvMPopFileName() ) != 0 )
        {
            ...
            if ( GetItemSchema()->GetMvmMissions().IsValidIndex( iChallengeIndex ) )
            {
                ...
                g_pVGuiLocalize->ConstructString_safe( wszChallengeName, L"%s1 (%s2)", 2, g_pVGuiLocalize->Find( mission.m_sDisplayName.Get() ), g_pVGuiLocalize->Find( mission.m_sMode.Get() ) );
