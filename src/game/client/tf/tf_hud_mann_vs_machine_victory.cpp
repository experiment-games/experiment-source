...
class CShowMannUpLootNotification : public CEconNotification
{
   public:
    ...
    {
        return dynamic_cast< CShowMannUpLootNotification * >( pNotification ) != NULL;
    }
...
CVictoryPanel::CVictoryPanel( Panel *parent, const char *pName )
    : vgui::EditablePanel( parent, pName ), m_pHeaderContainer( NULL ), m_pCreditContainerPanel( NULL ), m_pTotalGameCreditSpendPanel( NULL ), m_pTeamStatsContainerPanel( NULL ), m_pYourStatsContainerPanel( NULL ), m_pRatingContainerPanel( NULL ), m_pDoneButton( NULL )
{
...
void CVictoryPanel::SetMapAndPopFile()
{
    ...
    if ( GetItemSchema()->GetMvmMissions().IsValidIndex( iMissionIndex ) )
    {
        ...
        g_pVGuiLocalize->ConstructString_safe( wszLocalizedSummary, L"%s1 : %s2", 2, wszMapName, g_pVGuiLocalize->Find( mission.m_sDisplayName.Get() ) );
    }
    else
    {
        // Popfile
        ...
        g_pVGuiLocalize->ConstructString_safe( wszLocalizedSummary, L"%s1 : %s2", 2, wszMapName, wszPopFileName );
    }
...
CMvMVictoryMannUpLoot::CMvMVictoryMannUpLoot( Panel *parent, const char *pName )
...
CMvMVictoryMannUpEntry::CMvMVictoryMannUpEntry( Panel *parent, const char *pName )
    : vgui::EditablePanel( parent, pName ), m_LootLables( DefLessFunc( int ) ), m_pPlayerModelPanel( NULL ), m_bHasData( false )
{
...
void CMvMVictoryMannUpEntry::UpdatePlayerData()
{
    ...
    int nCategories[] = { CMsgMvMVictoryInfo_GrantReason_MANN_UP, CMsgMvMVictoryInfo_GrantReason_SQUAD_SURPLUS, CMsgMvMVictoryInfo_GrantReason_BADGE_LEVELED };
...
CMvMVictoryMannUpPlayerTab::CMvMVictoryMannUpPlayerTab( Panel *parent, const char *pName )
    : BaseClass( parent, pName ), m_pMouseoverHighlightPanel( NULL ), m_pActiveTab( NULL ), m_bIsActive( false ), m_pAvatarImage( NULL )
{
...
CMvMVictoryMannUpPanel::CMvMVictoryMannUpPanel( Panel *parent, const char *pName )
    : vgui::EditablePanel( parent, pName ), m_pNoItemServerContainer( NULL )
{
    SetMouseInputEnabled( true );
    m_hasData = false;
