...
static Color GetTeamColor( int iTeam )
{
    ...
    {
        case TF_TEAM_RED:
            return Color( 0xFF, 0x51, 0x51 );
        case TF_TEAM_BLUE:
            return Color( 0xA5, 0xDE, 0xFF );
        default:
            return Color( 0xF5, 0xE7, 0xDE );
    };
}
...
bool C_PasstimeGoalReticle::Update()
{
    ...
    if ( !g_pPasstimeLogic || !g_pPasstimeLogic->GetBall() || IsLocalPlayerSpectator() || !pGoal || pGoal->BGoalTriggerDisabled() || ( pGoal->GetTeamNumber() != pEnt->GetTeamNumber() ) )
    {
        return false;
    }
...
bool C_PasstimePlayerReticle::Update()
{
    ...
    if ( !pLocalPlayer || pLocalPlayer->IsPlayerDead() || !pPlayer || pPlayer->IsPlayerDead() )
    {
        return false;
    }
    ...
    if ( ( bIsFriend && ( iFriendsDetail <= 0 ) ) || ( !bIsFriend && ( iEnemiesDetail <= 0 ) ) )
    {
        // don't show because disabled
        return false;
    }

    if ( !pLocalPlayer->m_Shared.HasPasstimeBall() && ( ( bIsFriend && ( iFriendsDetail == 1 ) ) || ( !bIsFriend && ( iEnemiesDetail == 1 ) ) ) )
    {
