...
#define GCMatchmakingDebugSpew( lvl, ... )                                           \
    do                                                                               \
    {                                                                                \
        if ( lvl <= GCMATCHMAKING_DEBUG_LEVEL || lvl <= tf_mm_debug_level.GetInt() ) \
        {                                                                            \
            Msg( __VA_ARGS__ );                                                      \
        }                                                                            \
    } while ( false )
...
void CTFGCClientSystem::SOCreated( const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject, GCSDK::ESOCacheEvent eEvent )
{
    SOChanged( pObject, SOChanged_Create, eEvent );
}
void CTFGCClientSystem::SOUpdated( const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject, GCSDK::ESOCacheEvent eEvent )
{
    SOChanged( pObject, SOChanged_Update, eEvent );
}
void CTFGCClientSystem::SODestroyed( const CSteamID &steamIDOwner, const GCSDK::CSharedObject *pObject, GCSDK::ESOCacheEvent eEvent )
{
