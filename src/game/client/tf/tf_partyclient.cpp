...
ConVar tf_mm_party_send_criteria_delay( "tf_mm_party_send_criteria_delay", "1.0", FCVAR_DEVELOPMENTONLY, "", true, 0.f, false, 0.f );
...
ConVar tf_mm_party_send_criteria_delay_minimum( "tf_mm_party_send_criteria_delay_minimum", "0.5", FCVAR_DEVELOPMENTONLY, "", true, 0.f, false, 0.f );
...
static bool BPartyDbg()
{
    return tf_mm_partyclient_debug.GetBool();
}
...
#define PartyMsg( ... )                      \
    do                                       \
    {                                        \
        Msg( "[PartyClient] " __VA_ARGS__ ); \
    } while ( false );

#define PartyWarn( ... )                         \
    do                                           \
    {                                            \
        Warning( "[PartyClient] " __VA_ARGS__ ); \
    } while ( false );
...
ConVar tf_mm_custom_ping_enabled( "tf_mm_custom_ping_enabled", "0", FCVAR_ARCHIVE, "Whether to use a custom ping tolerance when matchmaking", false, 0.f, false, 0.f, OnPartyClientPrefConVarChanged );
...
#define RELIABLE_PARTY_MSG( name, ... )                                                             \
    ...
    {                                                                                               \
       public:                                                                                      \
        ...
        {                                                                                           \
            GTFPartyClient()->On##name##Reply( Msg(), msgReply );                                   \
        }                                                                                           \
...
//-----------------------------------------------------------------------------
RELIABLE_PARTY_MSG( ClearPendingPlayer, "Party ID %llu, Pending Player %s", ( unsigned long long )msg.party_id(), CSteamID( msg.pending_player_id() ).Render() )
...
bool CTFPartyClient::BInvitePlayerToParty( CSteamID steamID, bool bExpectingExistingRequestToJoin )
{
    ...
    {
        steamapicontext->SteamFriends()->InviteUserToGame( steamID, CFmtStr( "+tf_party_request_join_user %llu 1", steamapicontext->SteamUser()->GetSteamID().ConvertToUint64() ) );
    }
    return true;
}
...
void CTFPartyClient::CancelOutgoingJoinRequestOrIncomingInvite( CSteamID steamIDTarget )
{
    ...
    {
        PartyWarn(
            "CTFPartyClient::CancelOutgoingJoinRequestOrIncomingInvite for %s -- "
...
void CTFPartyClient::SendPartyChat( const char *pszMessage )
{
    ...
    if ( !bSent )
    {
        ...
        PostChatGameEvent( steamapicontext->SteamUser()->GetSteamID(), k_eTFPartyChatType_Synthetic_SendFailed, pszMessage );
        return;
    }
    ...
    if ( m_pSOCache )
    {
        ...
        if ( pTypeCache && pTypeCache->GetCount() > 0 )
        {
            AssertMsg1( pTypeCache->GetCount() == 1,
                        "Client has %d party objects in his cache!  He should only have 1.",
                        pTypeCache->GetCount() );
...
void CTFPartyClient::CheckResetSentOptions()
{
    ...
    else
    {
        ...
        {
            PartyWarn(
                "Re-joining party %llu we previously had queued messages for.  "
...
class CGCClientJobPartyMMError : public GCSDK::CGCClientJob
{
   public:
    ...
    virtual bool BYieldingRunGCJob( GCSDK::IMsgNetPacket *pNetPacket )
    {
        ...
        PartyWarn( "Received MM Error %s\n", ( CMsgPartyMMError_Type_IsValid( eType ) ? CMsgPartyMMError_Type_Name( eType ).c_str() : CFmtStr( "<unknown (%d)>", eType ).Get() ) );
