...
class CDetailModel : public IClientUnknown, public IClientRenderable
{
   ...
   public:
    ...
    bool Init( int index, const Vector &org, const QAngle &angles, model_t *pModel, ColorRGBExp32 lighting, int lightstyle, unsigned char lightstylecount, int orientation );

    bool InitSprite( int index, bool bFlipped, const Vector &org, const QAngle &angles, unsigned short nSpriteIndex, ColorRGBExp32 lighting, int lightstyle, unsigned char lightstylecount, int orientation, float flScale, unsigned char type, unsigned char shapeAngle, unsigned char shapeSize, unsigned char swayAmount );
   ...
   public:
    ...
    void DrawSwayingQuad( CMeshBuilder &meshBuilder, Vector vecOrigin, Vector vecSway, Vector2D texul, Vector2D texlr, unsigned char *color, Vector width, Vector height );
#endif
...
class CDetailObjectSystem : public IDetailObjectSystem, public ISpatialLeafEnumerator
{
   ...
   private:
    ...
    // Unserialization
    void ScanForCounts( CUtlBuffer &buf, int *pNumOldStyleObjects, int *pNumFastSpritesToAllocate, int *nMaxOldInLeaf, int *nMaxFastInLeaf ) const;
...
// Initialization
//-----------------------------------------------------------------------------
bool CDetailModel::Init( int index, const Vector &org, const QAngle &angles, model_t *pModel, ColorRGBExp32 lighting, int lightstyle, unsigned char lightstylecount, int orientation )
{
    m_Color = lighting;
...
bool CDetailModel::InitSprite( int index, bool bFlipped, const Vector &org, const QAngle &angles, unsigned short nSpriteIndex, ColorRGBExp32 lighting, int lightstyle, unsigned char lightstylecount, int orientation, float flScale, unsigned char type, unsigned char shapeAngle, unsigned char shapeSize, unsigned char swayAmount )
{
    m_Color = lighting;
    if ( lightstylecount > 0 )
    {
...
//-----------------------------------------------------------------------------
#ifdef USE_DETAIL_SHAPES
void CDetailModel::DrawSwayingQuad( CMeshBuilder &meshBuilder, Vector vecOrigin, Vector vecSway, Vector2D texul, Vector2D texlr, unsigned char *color, Vector width, Vector height )
{
    meshBuilder.Position3fv( ( vecOrigin + vecSway ).Base() );
    meshBuilder.TexCoord2fv( 0, texul.Base() );
    meshBuilder.Color4ubv( color );
            ...
            switch ( lump.m_Type )
            {
                case DETAIL_PROP_TYPE_MODEL:
                {
                    ...
                    m_DetailObjects[newObj].Init(
                        newObj, lump.m_Origin, lump.m_Angles, m_DetailObjectDict[lump.m_DetailModel].m_pModel, lump.m_Lighting, lump.m_LightStyles, lump.m_LightStyleCount, lump.m_Orientation );
                    ++detailObjectCount;
                }
                break;
                ...
                case DETAIL_PROP_TYPE_SHAPE_TRI:
                {
                    for ( int i = 0; i < SPRITE_MULTIPLIER; i++ )
                    {
                        ...
                        m_DetailObjects[newObj].InitSprite(
                            newObj, bFlipped, pos, lump.m_Angles, lump.m_DetailModel, lump.m_Lighting, lump.m_LightStyles, lump.m_LightStyleCount, lump.m_Orientation, lump.m_flScale, lump.m_Type, lump.m_ShapeAngle, lump.m_ShapeSize, lump.m_SwayAmount );
                        ++detailObjectCount;
                    }
                }
                break;
...
//-----------------------------------------------------------------------------
int CDetailObjectSystem::CountFastSpritesInLeafList( int nLeafCount, LeafIndex_t const *pLeafList, int *nMaxFoundInLeaf ) const
{
    VPROF_BUDGET( "CDetailObjectSystem::CountSpritesInLeafList", VPROF_BUDGETGROUP_DETAILPROP_RENDERING );
    int nCount = 0;
    int nMax = 0;
...
bool CDetailObjectSystem::EnumerateLeaf( int leaf, intp context )
{
    ...
    ClientLeafSystem()->DrawDetailObjectsInLeaf( leaf, pCtx->m_BuildWorldListNumber, firstDetailObject, detailObjectCount );
