...
//			flags -
//-----------------------------------------------------------------------------
void CTempEnts::BreakModel( const Vector &pos, const QAngle &angles, const Vector &size, const Vector &dir, float randRange, float life, int count, int modelIndex, char flags )
{
    int i, frameCount;
    C_LocalTempEntity *pTemp;
...
void CTempEnts::MuzzleFlash( int type, ClientEntityHandle_t hEntity, int attachmentIndex, bool firstPerson )
{
    ...
    if ( pPlayer != NULL )
    {
        CBaseCombatWeapon *pWeapon =
            dynamic_cast< CExperimentScriptedWeapon * >( pPlayer->GetActiveWeapon() );
