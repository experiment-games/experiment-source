...
//-----------------------------------------------------------------------------
void CClientLeafSystem::SetDetailObjectsInLeaf( int leaf, int firstDetailObject, int detailObjectCount )
{
    if ( !m_Leaf.IsValidIndex( leaf ) )
    {
        Assert( false );
...
//-----------------------------------------------------------------------------
void CClientLeafSystem::GetDetailObjectsInLeaf( int leaf, int &firstDetailObject, int &detailObjectCount )
{
    if ( !m_Leaf.IsValidIndex( leaf ) )
    {
        Assert( false );
    ...
    if ( count < m_RenderablesInLeaf.NumAllocated() )
    {
        ...
        if ( count >= 20000 )
        {
            for ( int j = 0; j < m_RenderablesInLeaf.NumAllocated(); j++ )
            {
                ...
                Msg( " %d: %p group:%d %s %d %d TransCalc:%d renderframe:%d\n", j, info.m_pRenderable, info.m_RenderGroup, pClassName, info.m_LeafList, info.m_RenderLeaf, info.m_TranslucencyCalculated, info.m_RenderFrame );
            }
...
//-----------------------------------------------------------------------------
inline void AddRenderableToRenderList( CClientRenderablesList &renderList, IClientRenderable *pRenderable, int iLeaf, RenderGroup_t group, ClientRenderHandle_t renderHandle, bool bTwoPass = false )
{
#ifdef _DEBUG
...
void CClientLeafSystem::CollateRenderablesInLeaf( int leaf, int worldListLeafIndex, const SetupRenderInfo_t &info )
{
    ...
    for ( ; idx != m_RenderablesInLeaf.InvalidIndex(); idx = m_RenderablesInLeaf.NextElement( idx ) )
    {
        ...
        if ( renderable.m_RenderGroup != RENDER_GROUP_TRANSLUCENT_ENTITY )
        {
            ...
            AddRenderableToRenderList( *info.m_pRenderList, renderable.m_pRenderable, worldListLeafIndex, group, handle );
        }
        else
        {
            ...
            if ( info.m_bDrawTranslucentObjects )
            {
                AddRenderableToRenderList( *info.m_pRenderList, renderable.m_pRenderable, worldListLeafIndex, ( RenderGroup_t )renderable.m_RenderGroup, handle, bTwoPass );
            }

            if ( bTwoPass )  // Also add to opaque list if it's a two-pass model...
            {
                AddRenderableToRenderList( *info.m_pRenderList, renderable.m_pRenderable, worldListLeafIndex, RENDER_GROUP_OPAQUE_ENTITY, handle, bTwoPass );
            }
        }
    }
            ...
            if ( pRenderable )
            {
                if ( pRenderable->IsTransparent() )
                {
                    if ( info.m_bDrawTranslucentObjects )  // Don't draw translucent objects into shadow depth maps
                    {
                        ...
                        {
                            AddRenderableToRenderList( *info.m_pRenderList, pRenderable, worldListLeafIndex, RENDER_GROUP_TRANSLUCENT_ENTITY, DETAIL_PROP_RENDER_HANDLE );
                        }
                    }
                }
                else
                {
                    AddRenderableToRenderList( *info.m_pRenderList, pRenderable, worldListLeafIndex, RENDER_GROUP_OPAQUE_ENTITY, DETAIL_PROP_RENDER_HANDLE );
                }
            }
            ++idx;
        }
