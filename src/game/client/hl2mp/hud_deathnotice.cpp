...
void CHudDeathNotice::Paint()
{
    ...
    for ( int i = 0; i < iCount; i++ )
    {
        ...
        if ( g_PR )
        {
            iKillerTeam = m_DeathNotices[i].Killer.iEntIndex > 0 ? g_PR->GetTeam( m_DeathNotices[i].Killer.iEntIndex ) : 0;
...
void CHudDeathNotice::FireGameEvent( IGameEvent *event )
{
    ...
    const char *killer_name = killer > 0 ? g_PR->GetPlayerName( killer ) : "worldspawn";
