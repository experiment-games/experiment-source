...
// Recording
//-----------------------------------------------------------------------------
static inline void RecordProjectDecal( const Vector &pos, const QAngle &angles, float flDistance, int index )
{
    if ( !ToolsEnabled() )
        return;
...
void TE_ProjectDecal( IRecipientFilter &filter, float delay, const Vector *pos, const QAngle *angles, float distance, int index )
{
    ...
    else
    {
        ...
        ent->AddDecal( *pos, endpos, endpos, hitbox, index, false, tr );
    }
}
