        ...
        for ( i = 0; i < c; i++ )
        {
            ...
            if ( showlist )
            {
                ...
                if ( showlist >= 2 )
                {
                    ...
                    engine->Con_NXPrintf( &np, "%15s %30s (%5i / %5i bytes): %15s", sz, ent->GetClassname(), size, intermediate_size, ent->GetPredictable() ? "predicted" : "client created" );
                ...
                {
                    engine->Con_NXPrintf( &np, "%15s %30s: %15s", sz, ent->GetClassname(), ent->GetPredictable() ? "predicted" : "client created" );
                }
            }
#ifndef _XBOX
            if ( error_check &&
...
//-----------------------------------------------------------------------------
bool CPrediction::PerformPrediction( bool received_new_world_update, C_BasePlayer *localPlayer, int incoming_acknowledged, int outgoing_command )
{
...
//-----------------------------------------------------------------------------
void CPrediction::_Update( bool received_new_world_update, bool validframe, int incoming_acknowledged, int outgoing_command )
{
#if !defined( NO_ENTITY_PREDICTION )
