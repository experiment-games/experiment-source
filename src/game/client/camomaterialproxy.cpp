...
void CCamoMaterialProxy::GetColors( Vector &diffuseColor, Vector &baseColor, int index, const Vector &boxMin, const Vector &boxExtents, const Vector &forward, const Vector &right, const Vector &up, const Vector &entityPosition )
{
...
void CCamoMaterialProxy::GenerateCamoTexture( ITexture *pTexture, IVTFTexture *pVTFTexture )
{
    ...
    {
        GetColors( diffuseColor[i], baseColor, i, mins, maxsMinusMins, forward, right, up, entityPosition );
#if 1
...
#ifndef _XBOX
    if ( !TGALoader::Load( m_pCamoPatternImage, m_pCamoPatternTextureVar->GetStringValue(), m_CamoPatternWidth, m_CamoPatternHeight, IMAGE_FORMAT_I8, dummyGamma, false ) )
    {
