    ...
    virtual void OnTick()
    {
            ...
            if ( YouTube_GetUploadResults( m_uploadHandle, bSuccess, strURLToVideo, strURLToVideoStats ) && bSuccess )
            {
                ...
                if ( steamapicontext && steamapicontext->SteamRemoteStorage() )
                {
                    ...
                    SteamAPICall_t hSteamAPICall = steamapicontext->SteamRemoteStorage()->PublishVideo(
                        k_EWorkshopVideoProviderNone, "", strURLToVideo.Get(), strPreviewFileName.Get(), nConsumerAppId, m_strTitle.Get(), m_strDescription.Get(), eVisibility, &tags );
                    hSteamAPICall;
                }
            }
            else
        ...
        else
        {
            ...
            else
            {
                ...
                g_pVGuiLocalize->ConstructString( wszProgress, sizeof( wszProgress ), g_pVGuiLocalize->Find( "#YouTube_UploadProgress" ), 3, wszPercentage, wszNow, wszTotal );
