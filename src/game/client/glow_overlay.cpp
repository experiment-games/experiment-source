...
void CGlowOverlay::UpdateSkyGlowObstruction( float zFar, bool bCacheFullSceneState )
{
    ...
    // Trace a ray at the object.
    trace_t trace;
    UTIL_TraceLine( CurrentViewOrigin(), CurrentViewOrigin() + ( m_vDirection * MAX_TRACE_LENGTH ), CONTENTS_SOLID, NULL, COLLISION_GROUP_NONE, &trace );
...
void CGlowOverlay::UpdateGlowObstruction( const Vector &vToGlow, bool bCacheFullSceneState )
{
    ...
    if ( m_bInSky )
    {
        // Trace a ray at the object.
        trace_t trace;
        UTIL_TraceLine( CurrentViewOrigin(), CurrentViewOrigin() + ( vToGlow * MAX_TRACE_LENGTH ), CONTENTS_SOLID, NULL, COLLISION_GROUP_NONE, &trace );
