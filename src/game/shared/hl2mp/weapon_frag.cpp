...
class CWeaponFrag : public CBaseHL2MPCombatWeapon {
   ...
   private:
    ...
    // check a throw from vecSrc.  If not valid, move the position back along
    // the line to vecEye
    void CheckThrowPosition(CBasePlayer *pPlayer, const Vector &vecEye,
                            Vector &vecSrc);

    CNetworkVar(bool,
                m_bRedraw);  // Draw the weapon again after throwing a grenade
...
acttable_t CWeaponFrag::m_acttable[] = {
    ...
    {ACT_MP_ATTACK_STAND_PRIMARYFIRE, ACT_HL2MP_GESTURE_RANGE_ATTACK_GRENADE,
     false},
    {ACT_MP_ATTACK_CROUCH_PRIMARYFIRE, ACT_HL2MP_GESTURE_RANGE_ATTACK_GRENADE,
     false},
...
#ifdef CLIENT_DLL
RecvPropBool(RECVINFO(m_bRedraw)), RecvPropBool(RECVINFO(m_fDrawbackFinished)),
    RecvPropInt(RECVINFO(m_AttackPaused)),
#else
SendPropBool(SENDINFO(m_bRedraw)), SendPropBool(SENDINFO(m_fDrawbackFinished)),
...
//			*pOperator -
//-----------------------------------------------------------------------------
void CWeaponFrag::Operator_HandleAnimEvent(animevent_t *pEvent,
                                           CBaseCombatCharacter *pOperator) {
...
#define RETHROW_DELAY 0.5
    if (fThrewGrenade) {
        ...
        m_flTimeWeaponIdle =
            FLT_MAX;  // NOTE: This is set once the animation has finished up!
    }
}
...
void CWeaponFrag::SecondaryAttack(void) {
    ...
    // Note that this is a secondary attack and prepare the grenade attack to
    // pause.
...
void CWeaponFrag::PrimaryAttack(void) {
    ...
    // Note that this is a primary attack and prepare the grenade attack to
    // pause.
            ...
            switch (m_AttackPaused) {
                ...
                case GRENADE_PAUSED_SECONDARY:
                    if (!(pOwner->m_nButtons & IN_ATTACK2)) {
                        ...
                        // Tony; the grenade really should have a secondary
                        // anim. but it doesn't on the player.
                        pOwner->DoAnimationEvent(
                            PLAYERANIMEVENT_ATTACK_PRIMARY);
...
// check a throw from vecSrc.  If not valid, move the position back along the
// line to vecEye
void CWeaponFrag::CheckThrowPosition(CBasePlayer *pPlayer, const Vector &vecEye,
                                     Vector &vecSrc) {
    trace_t tr;
...
void DropPrimedFragGrenade(CHL2MP_Player *pPlayer,
...
void CWeaponFrag::LobGrenade(CBasePlayer *pPlayer) {
    ...
    pPlayer->EyeVectors(&vForward, &vRight, NULL);
    Vector vecSrc =
        vecEye + vForward * 18.0f + vRight * 8.0f + Vector(0, 0, -8);
    CheckThrowPosition(pPlayer, vecEye, vecSrc);
    ...
    CBaseGrenade *pGrenade =
        Fraggrenade_Create(vecSrc, vec3_angle, vecThrow,
                           AngularImpulse(200, random->RandomInt(-600, 600), 0),
                           pPlayer, GRENADE_TIMER, false);
...
void CWeaponFrag::RollGrenade(CBasePlayer *pPlayer) {
    ...
    Vector vecSrc;
    pPlayer->CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 0.0f),
                                                     &vecSrc);
    ...
    trace_t tr;
    UTIL_TraceLine(vecSrc, vecSrc - Vector(0, 0, 16), MASK_PLAYERSOLID, pPlayer,
                   COLLISION_GROUP_NONE, &tr);
    if (tr.fraction != 1.0) {
        // compute forward vec parallel to floor plane and roll grenade along
        // that
        Vector tangent;
    ...
    CBaseGrenade *pGrenade = Fraggrenade_Create(
        vecSrc, orientation, vecThrow, rotSpeed, pPlayer, GRENADE_TIMER, false);
