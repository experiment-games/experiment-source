...
static ConVar tf2_feetyawrunscale(
    "tf2_feetyawrunscale", "2", FCVAR_REPLICATED,
...
CPlayerAnimState::CPlayerAnimState(CHL2MP_Player *outer) : m_pOuter(outer) {
    m_flGaitYaw = 0.0f;
...
void CPlayerAnimState::ComputePlaybackRate() {
    ...
    float maxspeed =
        GetOuter()->GetSequenceGroundSpeed(GetOuter()->GetSequence());
    ...
    if (isMoving && (maxspeed > 0.0f)) {
        ...
        // BUG BUG:
        // This stuff really should be m_flPlaybackRate = speed /
        // m_flGroundSpeed
    } else {
...
#ifndef CLIENT_DLL
    // Adrian: Make the model's angle match the legs so the hitboxes match on
    // both sides.
    GetOuter()->SetLocalAngles(
        QAngle(GetOuter()->GetAnimEyeAngles().x, m_flCurrentFeetYaw, 0));
#endif
}
...
// Output : int
//-----------------------------------------------------------------------------
int CPlayerAnimState::ConvergeAngles(float goal, float maxrate, float dt,
                                     float &current) {
...
void CPlayerAnimState::ComputePoseParam_BodyLookYaw(void) {
    ...
    if (!isMoving) {
        ...
        turning = ConvergeAngles(m_flGoalFeetYaw, turnrate,
        ...
        if (rotated_too_far ||
            (gpGlobals->curtime >
             m_flLastTurnTime + mp_facefronttime.GetFloat())) {
            m_flGoalFeetYaw = GetOuter()->GetAnimEyeAngles().y;
            m_flLastTurnTime = gpGlobals->curtime;
            ...
            /*	float yd = m_flCurrentFeetYaw - m_flGoalFeetYaw;
                    ...
                    turning = ConvergeAngles( m_flGoalFeetYaw, turnrate,
               gpGlobals->frametime, m_flCurrentFeetYaw ); yawdelta =
               GetOuter()->GetAnimEyeAngles().y - m_flCurrentFeetYaw;*/
        }

        // Snap upper body into position since the delta is already smoothed for
        // the feet
    ...
    if (m_nTurningInPlace != TURN_NONE) {
        // If we're close to finishing the turn, then turn off the turning
        // animation
        if (fabs(m_flCurrentFeetYaw - m_flGoalFeetYaw) <
            MIN_TURN_ANGLE_REQUIRING_TURN_ANIMATION) {
            m_nTurningInPlace = TURN_NONE;
        }
    }
    ...
    GetOuter()->SetPoseParameter(upper_body_yaw,
                                 clamp(m_flCurrentTorsoYaw, -60.0f, 60.0f));
