...
DEFINE_FIELD(m_hOwner, FIELD_EHANDLE),
    ...
    // Function Pointers
    DEFINE_FUNCTION(MissileTouch), DEFINE_FUNCTION(AccelerateThink),
    DEFINE_FUNCTION(AugerThink), DEFINE_FUNCTION(IgniteThink),
...
void CMissile::AccelerateThink(void) {
    ...
    // !!!UNDONE - make this work exactly the same as HL1 RPG, lest we have
    // looping sound bugs again!
...
//-----------------------------------------------------------------------------
// Purpose: Causes the missile to spiral to the ground and explode, due to
// damage
//-----------------------------------------------------------------------------
...
void CMissile::Explode(void) {
    ...
    trace_t tr;
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + forward * 16, MASK_SHOT,
...
void CMissile::ComputeActualDotPosition(CLaserDot *pLaserDot,
    ...
    *pHomingSpeed = RPG_HOMING_SPEED;
    ...
    if ((flMissileLength < flLaserLength) || (flTargetLength <= 512.0f)) {
        *pActualDotPosition = UTIL_PointOnLineNearestPoint(
            vLaserStart, pLaserDot->GetChasePosition(), GetAbsOrigin());
    ...
    //	NDebugOverlay::Line( pLaserDot->GetChasePosition(), vLaserStart, 0, 255,
    //0, true, 0.05f ); 	NDebugOverlay::Line( GetAbsOrigin(),
    //*pActualDotPosition, 255, 0, 0, true, 0.05f ); 	NDebugOverlay::Cross3D(
    //*pActualDotPosition, -Vector(4,4,4), Vector(4,4,4), 255, 0, 0, true, 0.05f
    //);
}
...
void CMissile::SeekThink(void) {
    ...
    for (CLaserDot *pEnt = GetLaserDotList(); pEnt != NULL;
         pEnt = pEnt->m_pNext) {
        if (!pEnt->IsOn())
            continue;
    ...
    if (gpGlobals->frametime > 0.0f) {
        if (flSpeed != 0) {
            vNewVelocity =
                (flHomingSpeed * vTargetDir) + ((1 - flHomingSpeed) * vDir);

            // This computation may happen to cancel itself out exactly. If so,
            // slam to targetdir.
    ...
    if (GetAbsVelocity() == vec3_origin) {
        // Strange circumstances have brought this missile to halt. Just blow it
        // up.
        Explode();
        return;
    }
...
// Output : CMissile
//-----------------------------------------------------------------------------
CMissile *CMissile::Create(const Vector &vecOrigin, const QAngle &vecAngles,
                           edict_t *pentOwner = NULL) {
    // CMissile *pMissile = (CMissile *)CreateEntityByName("rpg_missile" );
    CMissile *pMissile = (CMissile *)CBaseEntity::Create(
        "rpg_missile", vecOrigin, vecAngles, CBaseEntity::Instance(pentOwner));
...
class CInfoAPCMissileHint : public CBaseEntity {
   ...
   public:
    ...
    static CBaseEntity *FindAimTarget(CBaseEntity *pMissile,
                                      const char *pTargetName,
                                      const Vector &vecCurrentTargetPos,
                                      const Vector &vecCurrentTargetVel);
...
//-----------------------------------------------------------------------------
//
// This entity is used to create little force boxes that the helicopters should
// avoid.
//
//-----------------------------------------------------------------------------
CUtlVector<CInfoAPCMissileHint::APCMissileHintHandle_t>
...
DEFINE_FIELD(m_hTarget, FIELD_EHANDLE), END_DATADESC()
...
CBaseEntity *CInfoAPCMissileHint::FindAimTarget(
    CBaseEntity *pMissile, const char *pTargetName,
    ...
    for (int i = s_APCMissileHints.Count(); --i >= 0;) {
        ...
        VectorSubtract(pHint->m_hTarget->WorldSpaceCenter(),
                       pMissile->GetAbsOrigin(), vecMissileToHint);
        VectorSubtract(vecCurrentEnemyPos, pMissile->GetAbsOrigin(),
                       vecMissileToEnemy);
        ...
        if (!IntersectRayWithOBB(
                vecCurrentEnemyPos, vecRayDelta,
                pHint->CollisionProp()->CollisionToWorldTransform(),
                pHint->CollisionProp()->OBBMins(),
                pHint->CollisionProp()->OBBMaxs(), 0.0f, &trace)) {
            continue;
        }

        // Determine the amount of time it would take the missile to reach the
        // target If we can reach the target within the time it takes for the
        // enemy to reach the
...
CAPCMissile *FindAPCMissileInCone(const Vector &vecOrigin,
    ...
    for (CAPCMissile *pEnt = GetAPCMissileList(); pEnt != NULL;
         pEnt = pEnt->m_pNext) {
        if (!pEnt->IsSolid())
            continue;
...
DEFINE_FIELD(m_flReachedTargetTime, FIELD_TIME),
    ...
    DEFINE_THINKFUNC(BeginSeekThink), DEFINE_THINKFUNC(AugerStartThink),
...
CAPCMissile *CAPCMissile::Create(const Vector &vecOrigin,
                                 const QAngle &vecAngles,
                                 const Vector &vecVelocity,
                                 CBaseEntity *pOwner) {
    CAPCMissile *pMissile = (CAPCMissile *)CBaseEntity::Create(
        "apc_missile", vecOrigin, vecAngles, pOwner);
...
// Purpose:
//-----------------------------------------------------------------------------
void CAPCMissile::ComputeLeadingPosition(const Vector &vecShootPosition,
                                         CBaseEntity *pTarget,
                                         Vector *pLeadPosition) {
    ...
    // where z = flShooterToPredictedTargetPosition = flShotSpeed * predicted
    // time x = flTargetSpeed * predicted time y = flTargetToShooter solve for
    // predicted time using at^2 + bt + c = 0, t = (-b +/- sqrt( b^2 - 4ac )) /
    // 2a
...
// Purpose:
//-----------------------------------------------------------------------------
void CAPCMissile::ComputeActualDotPosition(CLaserDot *pLaserDot,
                                           Vector *pActualDotPosition,
                                           float *pHomingSpeed) {
    ...
    if ((m_strHint != NULL_STRING) && (!m_hSpecificTarget)) {
        ...
        m_hSpecificTarget = CInfoAPCMissileHint::FindAimTarget(
            this, STRING(m_strHint), vecOrigin, vecVelocity);
    }

    CBaseEntity *pLaserTarget = m_hSpecificTarget
                                    ? m_hSpecificTarget.Get()
                                    : pLaserDot->GetTargetEntity();
    if (!pLaserTarget) {
        BaseClass::ComputeActualDotPosition(pLaserDot, pActualDotPosition,
                                            pHomingSpeed);
        m_flLastHomingSpeed = *pHomingSpeed;
        return;
    }
    ...
    if (pLaserTarget->ClassMatches("npc_bullseye")) {
        if (m_flLastHomingSpeed != RPG_HOMING_SPEED) {
            if (m_flLastHomingSpeed > RPG_HOMING_SPEED) {
                m_flLastHomingSpeed -=
                    HOMING_SPEED_ACCEL * UTIL_GetSimulationInterval();
                if (m_flLastHomingSpeed < RPG_HOMING_SPEED) {
    ...
    *pActualDotPosition = vecLeadPosition;
    ...
    if (flBlendTime > DOWNWARD_BLEND_TIME_START) {
        if (m_flReachedTargetTime != 0.0f) {
            *pHomingSpeed = APC_HOMING_SPEED;
            float flDeltaTime =
                clamp(gpGlobals->curtime - m_flReachedTargetTime, 0.0f,
                      CORRECTION_TIME);
            *pHomingSpeed = SimpleSplineRemapVal(
                flDeltaTime, 0.0f, CORRECTION_TIME, 0.2f, *pHomingSpeed);
            flMinHomingDistance = SimpleSplineRemapVal(
                flDeltaTime, 0.0f, CORRECTION_TIME, MIN_NEAR_HOMING_DISTANCE,
                flMinHomingDistance);
            flMaxHomingDistance = SimpleSplineRemapVal(
                flDeltaTime, 0.0f, CORRECTION_TIME, MAX_NEAR_HOMING_DISTANCE,
                flMaxHomingDistance);
        } else {
            ...
            if (vecDelta.z > MIN_HEIGHT_DIFFERENCE) {
                float flClampedHeight = clamp(vecDelta.z, MIN_HEIGHT_DIFFERENCE,
                                              MAX_HEIGHT_DIFFERENCE);
                float flHeightAdjustFactor =
                    SimpleSplineRemapVal(flClampedHeight, MIN_HEIGHT_DIFFERENCE,
                                         MAX_HEIGHT_DIFFERENCE, 0.0f, 1.0f);
                ...
                if (flDist > flForwardOffset) {
                    ...
                    VectorMA(GetAbsOrigin(), -flForwardOffset, vecDelta,
                             vecNewPosition);
                    vecNewPosition.z = pActualDotPosition->z;

                    VectorLerp(*pActualDotPosition, vecNewPosition,
                               flHeightAdjustFactor, *pActualDotPosition);
                }
            } else {
        ...
        if (flBlendTime > 0.6f) {
            float flTargetLength =
                GetAbsOrigin().DistTo(pLaserTarget->WorldSpaceCenter());
            flTargetLength =
                clamp(flTargetLength, flMinHomingDistance, flMaxHomingDistance);
            *pHomingSpeed =
                SimpleSplineRemapVal(flTargetLength, flMaxHomingDistance,
                                     flMinHomingDistance, *pHomingSpeed, 0.01f);
        }
    }

    float flDot = DotProduct2D(vecTargetToShooter.AsVector2D(),
                               vecTargetToMissile.AsVector2D());
    ...
    //	NDebugOverlay::Line( vecLeadPosition, GetAbsOrigin(), 0, 255, 0, true,
    //0.05f ); 	NDebugOverlay::Line( GetAbsOrigin(), *pActualDotPosition, 255, 0,
    //0, true, 0.05f ); 	NDebugOverlay::Cross3D( *pActualDotPosition,
    //-Vector(4,4,4), Vector(4,4,4), 255, 0, 0, true, 0.05f );
}
...
#ifdef CLIENT_DLL
void RecvProxy_MissileDied(const CRecvProxyData *pData, void *pStruct,
                           void *pOut) {
...
RecvPropBool(RECVINFO(m_bInitialStateUpdate)),
    RecvPropBool(RECVINFO(m_bGuiding)), RecvPropBool(RECVINFO(m_bHideGuiding)),
...
SendPropBool(SENDINFO(m_bInitialStateUpdate)),
    SendPropBool(SENDINFO(m_bGuiding)), SendPropBool(SENDINFO(m_bHideGuiding)),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CWeaponRPG)
            DEFINE_PRED_FIELD(m_bInitialStateUpdate, FIELD_BOOLEAN,
    ...
    END_PREDICTION_DATA()
        ...
        acttable_t CWeaponRPG::m_acttable[] = {
            ...
            {ACT_MP_ATTACK_STAND_PRIMARYFIRE,
             ACT_HL2MP_GESTURE_RANGE_ATTACK_RPG, false},
...
void CWeaponRPG::ItemPostFrame(void) {
    ...
    // If we're pulling the weapon out for the first time, wait to draw the
    // laser
...
Vector CWeaponRPG::GetLaserPosition(void) {
    ...
    // FIXME: The laser dot sprite is not active, this code should not be
    // allowed!
    assert(0);
#endif
    return vec3_origin;
...
void CWeaponRPG::UpdateLaserPosition(Vector vecMuzzlePos, Vector vecEndPos) {
    ...
    UTIL_TraceLine(vecMuzzlePos, vecEndPos, (MASK_SHOT & ~CONTENTS_WINDOW),
                   GetOwner(), COLLISION_GROUP_NONE, &tr);
...
//			This should really be worked into the CBaseCombatWeapon
//class!
//-----------------------------------------------------------------------------
void CWeaponRPG::GetWeaponAttachment(int attachmentId, Vector &outVector,
                                     Vector *dir /*= NULL*/) {
    QAngle angles;
    ...
    if (ShouldDrawUsingViewModel()) {
        ...
        if (pOwner != NULL) {
            pOwner->GetViewModel()->GetAttachment(attachmentId, outVector,
                                                  angles);
...
// Tony; added so when the rpg switches to third person, the beam etc is
// re-created.
...
void CWeaponRPG::InitBeam(void) {
    ...
    beamInfo.m_pszModelName =
        (ShouldDrawUsingViewModel()) ? RPG_BEAM_SPRITE_NOZ : RPG_BEAM_SPRITE;
...
void CWeaponRPG::DrawEffects(void) {
    ...
    int attachmentID = (ShouldDrawUsingViewModel()) ? RPG_GUIDE_ATTACHMENT
                                                    : RPG_GUIDE_ATTACHMENT_3RD;
    ...
    // Get the beam's run
    trace_t tr;
    UTIL_TraceLine(vecAttachment,
                   vecAttachment + (vecDir * RPG_LASER_BEAM_LENGTH), MASK_SHOT,
                   GetOwner(), COLLISION_GROUP_NONE, &tr);
...
DEFINE_FIELD(m_vecSurfaceNormal, FIELD_VECTOR),
    ...
    // DEFINE_FIELD( m_pNext, FIELD_CLASSPTR ),	// don't save - regenerated by
    // constructor
    END_DATADESC()
...
CLaserDot *CLaserDot::Create(const Vector &origin, CBaseEntity *pOwner,
                             bool bVisibleDot) {
#ifndef CLIENT_DLL
    CLaserDot *pLaserDot = (CLaserDot *)CBaseEntity::Create(
        "env_laserdot", origin, QAngle(0, 0, 0));
...
int CLaserDot::DrawModel(int flags) {
    ...
    if (pOwner != NULL && pOwner->IsDormant() == false) {
        ...
        if (pOwner->IsLocalPlayer() &&
            C_BasePlayer::LocalPlayerInFirstPersonView())  // Tony; !!!
        {
        ...
        trace_t tr;
        UTIL_TraceLine(vecAttachment,
                       vecAttachment + (vecDir * MAX_TRACE_LENGTH), MASK_SHOT,
