...
bool PhysCannonAccountableForObject(CBaseCombatWeapon *pPhysCannon,
                                    CBaseEntity *pObject) {
    // BRJ: FIXME! This can't be implemented trivially, so I'm leaving it to
    // Steve or Adrian
    Assert(0);
    return false;
}
...
static void TraceCollideAgainstBBox(const CPhysCollide *pCollide,
                                    const Vector &start, const Vector &end,
                                    const QAngle &angles,
                                    const Vector &boxOrigin, const Vector &mins,
                                    const Vector &maxs, trace_t *ptr) {
    physcollision->TraceBox(boxOrigin, boxOrigin + (start - end), mins, maxs,
                            pCollide, start, angles, ptr);
...
// player can reach down 2ft below his feet (otherwise he'll hold the object
// above the bottom)
...
static void ComputePlayerMatrix(CBasePlayer *pPlayer, matrix3x4_t &out) {
    ...
    if (angles.x < 0) {
        zoffset = RemapVal(angles.x, 0, -PLAYER_LOOK_PITCH_RANGE,
                           PLAYER_HOLD_LEVEL_EYES, PLAYER_HOLD_UP_EYES);
    } else {
        zoffset = RemapVal(angles.x, 0, PLAYER_LOOK_PITCH_RANGE,
                           PLAYER_HOLD_LEVEL_EYES,
                           PLAYER_HOLD_DOWN_FEET + (feet - eyes));
    }
    origin.z += zoffset;
    angles.x = 0;
...
DEFINE_FIELD(targetPosition, FIELD_POSITION_VECTOR),
   ...
   public:
    ...
    void SetTargetPosition(const Vector &target,
                           const QAngle &targetOrientation);
    float ComputeError();
    ...
    QAngle TransformAnglesToPlayerSpace(const QAngle &anglesIn,
                                        CBasePlayer *pPlayer);
    QAngle TransformAnglesFromPlayerSpace(const QAngle &anglesIn,
                                          CBasePlayer *pPlayer);
...
void CGrabController::SetTargetPosition(const Vector &target,
                                        const QAngle &targetOrientation) {
...
void CGrabController::ComputeMaxSpeed(CBaseEntity *pEntity,
...
QAngle CGrabController::TransformAnglesToPlayerSpace(const QAngle &anglesIn,
                                                     CBasePlayer *pPlayer) {
    ...
    return TransformAnglesToLocalSpace(anglesIn,
                                       pPlayer->EntityToWorldTransform());
}

QAngle CGrabController::TransformAnglesFromPlayerSpace(const QAngle &anglesIn,
                                                       CBasePlayer *pPlayer) {
    if (m_bIgnoreRelativePitch) {
        matrix3x4_t test;
        QAngle angleTest = pPlayer->EyeAngles();
    ...
    return TransformAnglesToWorldSpace(anglesIn,
                                       pPlayer->EntityToWorldTransform());
}

void CGrabController::AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity,
                                   IPhysicsObject *pPhys,
                                   bool bIsMegaPhysCannon,
                                   const Vector &vGrabPosition,
                                   bool bUseGrabPosition) {
    // play the impact sound of the object hitting the player
    // used as feedback to let the player know he picked up the object
#ifndef CLIENT_DLL
    PhysicsImpactSound(pPlayer, pPhys, CHAN_STATIC, pPhys->GetMaterialIndex(),
                       pPlayer->VPhysicsGetObject()->GetMaterialIndex(), 1.0,
                       64);
#endif
    Vector position;
    QAngle angles;
...
#ifndef CLIENT_DLL
    Pickup_GetPreferredCarryAngles(pEntity, pPlayer,
                                   pPlayer->EntityToWorldTransform(), angles);
#endif
    ...
    for (int i = 0; i < count; i++) {
        ...
        // reduce the mass to prevent the player from adding crazy amounts of
    ...
    if (m_angleAlignment != 0) {
        m_attachedAnglesPlayerSpace =
            AlignAngles(m_attachedAnglesPlayerSpace, m_angleAlignment);
    }

    VectorITransform(pEntity->WorldSpaceCenter(),
    ...
    if (pProp) {
        m_bHasPreferredCarryAngles = pProp->GetPropDataAngles(
            "preferred_carryangles", m_vecPreferredCarryAngles);
    } else {
        m_bHasPreferredCarryAngles = false;
    }
#else
...
static void ClampPhysicsVelocity(IPhysicsObject *pPhys, float linearLimit,
                                 float angularLimit) {
    Vector vel;
...
#ifndef CLIENT_DLL
                ClampPhysicsVelocity(pPhys, hl2_normspeed.GetFloat() * 1.5f,
                                     2.0f * 360.0f);
#endif
            }
        }
...
IMotionEvent::simresult_e CGrabController::Simulate(
    ...
    shadowParams.maxAngular = m_shadow.maxAngular * m_contactAmount *
                              m_contactAmount * m_contactAmount;
#ifndef CLIENT_DLL
    m_timeToArrive =
        pObject->ComputeShadowControl(shadowParams, m_timeToArrive, deltaTime);
#else
    m_timeToArrive = pObject->ComputeShadowControl(
        shadowParams, (TICK_INTERVAL * 2), deltaTime);
#endif
    ...
    PhysComputeSlideDirection(pObject, velocity, angVel, &velocity, &angVel,
                              GetLoadWeight());
...
void CPlayerPickupController::Init(CBasePlayer *pPlayer, CBaseEntity *pObject) {
    ...
    m_grabController.AttachEntity(pPlayer, pObject, pPhysics, false,
                                  vec3_origin, false);
...
void CPlayerPickupController::Shutdown(bool bThrown) {
    ...
    if (!bThrown && pObject && pObject->VPhysicsGetObject() &&
        pObject->VPhysicsGetObject()->GetContactPoint(NULL, NULL)) {
        bClearVelocity = true;
    }
    ...
    if (pObject != NULL) {
        Pickup_OnPhysGunDrop(pObject, m_pPlayer,
                             bThrown ? THROWN_BY_PLAYER : DROPPED_BY_PLAYER);
    }
    ...
    if (m_pPlayer) {
        ...
        if (m_pPlayer->GetActiveWeapon()) {
            if (!m_pPlayer->GetActiveWeapon()->Deploy()) {
                ...
                // This usually happens when they're holding an empty weapon
                // that doesn't autoswitch away when out of ammo. Switch to next
                // best weapon.
...
void CPlayerPickupController::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                                  USE_TYPE useType, float value) {
    if (ToBasePlayer(pActivator) == m_pPlayer) {
        CBaseEntity *pAttached = m_grabController.GetAttached();

        // UNDONE: Use vphysics stress to decide to drop objects
        // UNDONE: Must fix case of forcing objects into the ground you're
        // standing on (causes stress) before that will work
        if (!pAttached || useType == USE_OFF ||
            (m_pPlayer->m_nButtons & IN_ATTACK2) ||
        ...
        if (m_pPlayer->m_nButtons & IN_ATTACK) {
            ...
            // JAY: Scale this with mass because some small objects really go
            // flying
...
void PlayerPickupObject(CBasePlayer *pPlayer, CBaseEntity *pObject) {
    ...
    CPlayerPickupController *pController =
        (CPlayerPickupController *)CBaseEntity::Create(
            "player_pickup", pObject->GetAbsOrigin(), vec3_angle, pPlayer);
...
class CPhysCannonEffectBeam {
   public:
    ...
    void Init(int startAttachment, int endAttachment, CBaseEntity *pEntity,
              bool firstPerson) {
        if (m_pBeam != NULL)
            return;
        ...
        beamInfo.m_pszModelName =
            (firstPerson) ? PHYSCANNON_BEAM_SPRITE_NOZ : PHYSCANNON_BEAM_SPRITE;
...
class CWeaponPhysCannon : public CBaseHL2MPCombatWeapon {
   public:
    ...
    void ForceDrop(void);
    bool DropIfEntityHeld(
        CBaseEntity *pTarget);  // Drops its held entity if it matches the
                                // entity passed in
    CGrabController &GetGrabController() {
        return m_grabController;
    }
   ...
   protected:
    ...
    CBaseEntity *FindObjectInCone(const Vector &vecOrigin, const Vector &vecDir,
                                  float flCone);
#endif  // !CLIENT_DLL
    ...
    // Punt objects - this is pointing at an object in the world and applying a
    // force to it.
    void PuntNonVPhysics(CBaseEntity *pEntity, const Vector &forward,
                         trace_t &tr);
    ...
    void Physgun_OnPhysGunPickup(CBaseEntity *pEntity, CBasePlayer *pOwner,
    ...
    enum EffectType_t{
        ...
        PHYSCANNON_GLOW2,         PHYSCANNON_GLOW3, PHYSCANNON_GLOW4,
        PHYSCANNON_GLOW5,         PHYSCANNON_GLOW6,
...
#define NUM_GLOW_SPRITES                     \
    ((CWeaponPhysCannon::PHYSCANNON_GLOW6 -  \
      CWeaponPhysCannon::PHYSCANNON_GLOW1) + \
     1)
#define NUM_ENDCAP_SPRITES                     \
    ((CWeaponPhysCannon::PHYSCANNON_ENDCAP3 -  \
      CWeaponPhysCannon::PHYSCANNON_ENDCAP1) + \
     1)
...
#define NUM_PHYSCANNON_BEAMS 3
    ...
    void GetEffectParameters(EffectType_t effectID, color32 &color,
                             float &scale, IMaterial **pMaterial,
    ...
    CInterpolatedValue m_ElementParameter;  // Used to interpolate the position
                                            // of the articulated elements
    CPhysCannonEffect
        m_Parameters[NUM_PHYSCANNON_PARAMETERS];  // Interpolated parameters for
                                                  // the effects
...
#ifdef CLIENT_DLL
RecvPropBool(RECVINFO(m_bActive)), RecvPropEHandle(RECVINFO(m_hAttachedObject)),
    ...
    RecvPropInt(RECVINFO(m_EffectState)), RecvPropBool(RECVINFO(m_bOpen)),
#else
SendPropBool(SENDINFO(m_bActive)), SendPropEHandle(SENDINFO(m_hAttachedObject)),
    ...
    SendPropInt(SENDINFO(m_EffectState)), SendPropBool(SENDINFO(m_bOpen)),
#endif
    END_NETWORK_TABLE()

#ifdef CLIENT_DLL
        BEGIN_PREDICTION_DATA(CWeaponPhysCannon)
            DEFINE_PRED_FIELD(m_EffectState, FIELD_INTEGER,
...
acttable_t CWeaponPhysCannon::m_acttable[] = {
    ...
    {ACT_MP_ATTACK_STAND_PRIMARYFIRE, ACT_HL2MP_GESTURE_RANGE_ATTACK_PHYSGUN,
     false},
    {ACT_MP_ATTACK_CROUCH_PRIMARYFIRE, ACT_HL2MP_GESTURE_RANGE_ATTACK_PHYSGUN,
     false},
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::PuntNonVPhysics(CBaseEntity *pEntity,
                                        const Vector &forward, trace_t &tr) {
    if (m_hLastPuntedObject == pEntity &&
        gpGlobals->curtime < m_flRepuntObjectTime)
        return;
...
// What happens when the physgun picks up something
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::Physgun_OnPhysGunPickup(CBaseEntity *pEntity,
                                                CBasePlayer *pOwner,
                                                PhysGunPickup_t reason) {
    // If the target is debris, convert it to non-debris
...
// Punt vphysics
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::PuntVPhysics(CBaseEntity *pEntity,
                                     const Vector &vecForward, trace_t &tr) {
    CBasePlayer *pOwner = ToBasePlayer(GetOwner());
    ...
    if (Pickup_OnAttemptPhysGunPickup(pEntity, pOwner, PUNTED_BY_CANNON)) {
        ...
        if (forward.z < 0) {
            // reflect, but flatten the trajectory out a bit so it's easier to
            // hit standing targets
            forward.z *= -0.65f;
        }

        // NOTE: Do this first to enable motion (if disabled) - so forces will
        // work Tell the object it's been punted
        Physgun_OnPhysGunPickup(pEntity, pOwner, PUNTED_BY_CANNON);

        // don't push vehicles that are attached to the world via fixed
        // constraints they will just wiggle...
        if ((pList[0]->GetGameFlags() & FVPHYSICS_CONSTRAINT_STATIC) &&
            pEntity->GetServerVehicle()) {
            forward.Init();
        }
        ...
        if (!Pickup_ShouldPuntUseLaunchForces(pEntity, PHYSGUN_FORCE_PUNTED)) {
            ...
            float mass =
                MIN(totalMass, maxMass);  // max 250kg of additional force
            ...
            for (i = 0; i < listCount; i++) {
                ...
                pList[i]->ApplyForceOffset(forward * mass * 600.0f * ratio,
                                           tr.endpos);
            }
        } else {
...
#endif
    // Add recoil
    QAngle recoil = QAngle(random->RandomFloat(1.0f, 2.0f),
                           random->RandomFloat(-1.0f, 1.0f), 0);
...
// Input  : -
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::ApplyVelocityBasedForce(CBaseEntity *pEntity,
                                                const Vector &forward) {
#ifndef CLIENT_DLL
    IPhysicsObject *pPhysicsObject = pEntity->VPhysicsGetObject();
    Assert(
        pPhysicsObject);  // Shouldn't ever get here with a non-vphysics object.
    if (!pPhysicsObject)
        return;
    ...
    AngularImpulse aVel =
        Pickup_PhysGunLaunchAngularImpulse(pEntity, PHYSGUN_FORCE_PUNTED);
...
void CWeaponPhysCannon::PrimaryAttack(void) {
    ...
    if (m_bActive) {
        ...
        if (pHeld != NULL) {
            float heldDist =
                (pHeld->WorldSpaceCenter() - pOwner->WorldSpaceCenter())
                    .Length();
    ...
    trace_t tr;
    UTIL_TraceHull(start, end, -Vector(8, 8, 8), Vector(8, 8, 8),
                   MASK_SHOT | CONTENTS_GRATE, &filter, &tr);
    bool bValid = true;
    CBaseEntity *pEntity = tr.m_pEnt;
    if (tr.fraction == 1 || !tr.m_pEnt ||
        tr.m_pEnt->IsEFlagSet(EFL_NO_PHYSCANNON_INTERACTION)) {
        bValid = false;
    ...
    if (pEntity->GetMoveType() != MOVETYPE_VPHYSICS) {
        ...
        if (GetOwner()->IsPlayer()) {
            // Don't let the player zap any NPC's except regular antlions and
            // headcrabs.
            if (pEntity->IsPlayer()) {
                DryFire();
                return;
...
// Input  : *pObject -
//-----------------------------------------------------------------------------
bool CWeaponPhysCannon::AttachObject(CBaseEntity *pObject,
    ...
    m_bActive = true;
    if (pOwner) {
        // NOTE: This can change the mass; so it must be done before max speed
        // setting
        Physgun_OnPhysGunPickup(pObject, pOwner, PICKED_UP_BY_CANNON);
    }

    // NOTE :This must happen after OnPhysGunPickup because that can change the
    // mass
    m_grabController.AttachEntity(pOwner, pObject, pPhysics, false, vPosition,
                                  false);
    m_hAttachedObject = pObject;
    m_attachedPositionObjectSpace =
        m_grabController.m_attachedPositionObjectSpace;
    ...
    /*	if( pOwner )
            {
                    ...
                    float	maxSpeed = hl2_walkspeed.GetFloat() + ( (
       hl2_normspeed.GetFloat() - hl2_walkspeed.GetFloat() ) * loadWeight );

                    //Msg( "Load perc: %f -- Movement speed: %f/%f\n",
       loadWeight, maxSpeed, hl2_normspeed.GetFloat() ); pOwner->SetMaxSpeed(
       maxSpeed );
            }*/

    // Don't drop again for a slight delay, in case they were pulling objects
    // near them
    ...
    if (GetMotorSound()) {
        (CSoundEnvelopeController::GetController())
            .Play(GetMotorSound(), 0.0f, 50);
        (CSoundEnvelopeController::GetController())
            .SoundChangePitch(GetMotorSound(), 100, 0.5f);
        (CSoundEnvelopeController::GetController())
            .SoundChangeVolume(GetMotorSound(), 0.8f, 0.5f);
    }
...
CWeaponPhysCannon::FindObjectResult_t CWeaponPhysCannon::FindObject(void) {
    ...
    if ((tr.fraction == 1.0) || (tr.m_pEnt == NULL) || (tr.m_pEnt->IsWorld())) {
        UTIL_TraceHull(start, end, -Vector(4, 4, 4), Vector(4, 4, 4),
                       MASK_SHOT | CONTENTS_GRATE, &filter, &tr);
    }
    ...
    if (!bAttach && !bPull) {
        pConeEntity =
            FindObjectInCone(start, forward, physcannon_cone.GetFloat());
    }
    ...
    if (pConeEntity) {
        ...
        if (pEntity->WorldSpaceCenter().DistToSqr(start) <=
    ...
    if (bAttach) {
        return AttachObject(pEntity, tr.endpos) ? OBJECT_FOUND
                                                : OBJECT_NOT_FOUND;
    }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponPhysCannon::FindObjectInCone(const Vector &vecOrigin,
                                                 const Vector &vecDir,
                                                 float flCone) {
    ...
    Vector mins =
        vecOrigin - Vector(flNearestDist, flNearestDist, flNearestDist);
    Vector maxs =
        vecOrigin + Vector(flNearestDist, flNearestDist, flNearestDist);
    ...
    for (int i = 0; i < count; i++) {
        ...
        UTIL_TraceLine(vecOrigin, list[i]->WorldSpaceCenter(),
                       MASK_SHOT | CONTENTS_GRATE, &filter, &tr);
        if (tr.m_pEnt == list[i]) {
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError) {
    ...
    Vector radial = physcollision->CollideGetExtent(
        pPhys->GetCollide(), vec3_origin, pEntity->GetAbsAngles(), -forward);
    ...
    CalcClosestPointOnLine(end, playerLine + Vector(0, 0, playerMins.z),
                           playerLine + Vector(0, 0, playerMaxs.z), nearest,
                           NULL);
    ...
    QAngle angles =
        TransformAnglesFromPlayerSpace(m_attachedAnglesPlayerSpace, pPlayer);

    // Show overlays of radius
    if (g_debug_physcannon.GetBool()) {
#ifdef CLIENT_DLL

        debugoverlay->AddBoxOverlay(end, -Vector(2, 2, 2), Vector(2, 2, 2),
                                    angles, 0, 255, 255, true, 0);

        debugoverlay->AddBoxOverlay(
            GetAttached()->WorldSpaceCenter(), -Vector(radius, radius, radius),
            Vector(radius, radius, radius), angles, 255, 255, 0, true, 0.0f);

#else

        NDebugOverlay::Box(end, -Vector(2, 2, 2), Vector(2, 2, 2), 0, 255, 0,
                           true, 0);

        NDebugOverlay::Box(GetAttached()->WorldSpaceCenter(),
                           -Vector(radius + 5, radius + 5, radius + 5),
                           Vector(radius + 5, radius + 5, radius + 5), 255, 0,
                           0, true, 0.0f);
#endif
    }

#ifndef CLIENT_DLL
    // If it has a preferred orientation, update to ensure we're still oriented
    // correctly.
    Pickup_GetPreferredCarryAngles(pEntity, pPlayer,
                                   pPlayer->EntityToWorldTransform(), angles);
...
void CWeaponPhysCannon::DetachObject(bool playSound, bool wasLaunched) {
    ...
    if (pObject != NULL) {
        Pickup_OnPhysGunDrop(
            pObject, pOwner,
            wasLaunched ? LAUNCHED_BY_CANNON : DROPPED_BY_CANNON);
    }

    // Stop our looping sound
    if (GetMotorSound()) {
        (CSoundEnvelopeController::GetController())
            .SoundChangeVolume(GetMotorSound(), 0.0f, 1.0f);
        (CSoundEnvelopeController::GetController())
            .SoundChangePitch(GetMotorSound(), 50, 1.0f);
    }
...
void CWeaponPhysCannon::ManagePredictedObject(void) {
    ...
    if (m_hAttachedObject) {
        // NOTE :This must happen after OnPhysGunPickup because that can change
        // the mass
        if (pAttachedObject != GetGrabController().GetAttached()) {
            ...
            if (pPhysics == NULL) {
                solid_t tmpSolid;
                PhysModelParseSolid(tmpSolid, m_hAttachedObject,
                                    pAttachedObject->GetModelIndex());

                pAttachedObject->VPhysicsInitNormal(SOLID_VPHYSICS, 0, false,
                                                    &tmpSolid);
            }
            ...
            if (pPhysics) {
                ...
                GetGrabController().AttachEntity(ToBasePlayer(GetOwner()),
                                                 pAttachedObject, pPhysics,
                                                 false, vec3_origin, false);
                GetGrabController().m_attachedPositionObjectSpace =
                    m_attachedPositionObjectSpace;
                GetGrabController().m_attachedAnglesPlayerSpace =
                    m_attachedAnglesPlayerSpace;
            }
        }
    } else {
...
void CWeaponPhysCannon::CheckForTarget(void) {
    ...
    trace_t tr;
    UTIL_TraceHull(startPos, endPos, -Vector(4, 4, 4), Vector(4, 4, 4),
                   MASK_SHOT | CONTENTS_GRATE, pOwner, COLLISION_GROUP_NONE,
                   &tr);

    if ((tr.fraction != 1.0f) && (tr.m_pEnt != NULL)) {
        // FIXME: Try just having the elements always open when pointed at a
        // physics object
        if (CanPickupObject(tr.m_pEnt) ||
            Pickup_ForcePhysGunOpen(tr.m_pEnt, pOwner))
        // if ( ( tr.m_pEnt->VPhysicsGetObject() != NULL ) && (
        // tr.m_pEnt->GetMoveType() == MOVETYPE_VPHYSICS ) )
        {
            m_nChangeState = ELEMENT_STATE_NONE;
            OpenElements();
            return;
...
void CWeaponPhysCannon::DoEffectIdle(void) {
    ...
    for (int i = PHYSCANNON_GLOW1; i < (PHYSCANNON_GLOW1 + NUM_GLOW_SPRITES);
         i++) {
        m_Parameters[i].GetScale().SetAbsolute(
            random->RandomFloat(0.075f, 0.05f) * SPRITE_SCALE);
        m_Parameters[i].GetAlpha().SetAbsolute(random->RandomInt(24, 32));
    }

    // Turn on the glow sprites
    for (int i = PHYSCANNON_ENDCAP1;
         i < (PHYSCANNON_ENDCAP1 + NUM_ENDCAP_SPRITES); i++) {
...
void CWeaponPhysCannon::ItemPostFrame() {
    ...
    // NOTE: Attack2 will be considered to be pressed until the first item is
    // picked up.
...
void CWeaponPhysCannon::LaunchObject(const Vector &vecDir, float flForce) {
    ...
    if (!(m_hLastPuntedObject == pObject &&
          gpGlobals->curtime < m_flRepuntObjectTime)) {
        // FIRE!!!
        if (pObject != NULL) {
...
bool CWeaponPhysCannon::CanPickupObject(CBaseEntity *pTarget) {
    ...
    if (pTarget->GetBaseAnimating() &&
        pTarget->GetBaseAnimating()->IsDissolving())
        return false;
    ...
    return CBasePlayer::CanPickupObject(pTarget, physcannon_maxmass.GetFloat(),
                                        0);
#else
    return false;
#endif
...
CSoundPatch *CWeaponPhysCannon::GetMotorSound(void) {
    if (m_sndMotor == NULL) {
        ...
        m_sndMotor = (CSoundEnvelopeController::GetController())
                         .SoundCreate(filter, entindex(), CHAN_STATIC,
                                      "Weapon_PhysCannon.HoldSound", ATTN_NORM);
    }
...
void CWeaponPhysCannon::StopEffects(bool stopSound) {
    ...
    if (stopSound && GetMotorSound() != NULL) {
        (CSoundEnvelopeController::GetController())
            .SoundFadeOut(GetMotorSound(), 0.1f);
    }
#endif  // !CLIENT_DLL
}
...
void CWeaponPhysCannon::ThirdPersonSwitch(bool bThirdPerson) {
    // Tony; if we switch to first or third person or whatever, destroy and
    // recreate the effects. Note: the sound only ever gets shut off on the
    // server, so it's okay - as this is entirely client side.
    DestroyEffects();
    StartEffects();
}
#endif
...
void CWeaponPhysCannon::StartEffects(void) {
    ...
    if (m_Parameters[PHYSCANNON_CORE].GetMaterial() == NULL) {
        ...
        if (m_Parameters[PHYSCANNON_CORE].SetMaterial(PHYSCANNON_CENTER_GLOW) ==
            false) {
    ...
    if (m_Parameters[PHYSCANNON_BLAST].GetMaterial() == NULL) {
        ...
        if (m_Parameters[PHYSCANNON_BLAST].SetMaterial(
                PHYSCANNON_BLAST_SPRITE) == false) {
            // This means the texture was not found
            Assert(0);
        }
    }
    ...
    const char *attachNamesGlowThirdPerson[NUM_GLOW_SPRITES] = {
        "fork1m", "fork1t", "fork2m", "fork2t", "fork3m", "fork3t",
    };

    const char *attachNamesGlow[NUM_GLOW_SPRITES] = {
        "fork1b", "fork1m", "fork1t", "fork2b", "fork2m", "fork2t"};
    ...
    for (int i = PHYSCANNON_GLOW1; i < (PHYSCANNON_GLOW1 + NUM_GLOW_SPRITES);
        ...
        if (ShouldDrawUsingViewModel()) {
            m_Parameters[i].SetAttachment(
                LookupAttachment(attachNamesGlow[i - PHYSCANNON_GLOW1]));
        } else {
            m_Parameters[i].SetAttachment(LookupAttachment(
                attachNamesGlowThirdPerson[i - PHYSCANNON_GLOW1]));
        }
    ...
    const char *attachNamesEndCap[NUM_ENDCAP_SPRITES] = {"fork1t", "fork2t",
                                                         "fork3t"};
    ...
    for (int i = PHYSCANNON_ENDCAP1;
        ...
        m_Parameters[i].SetAttachment(
            LookupAttachment(attachNamesEndCap[i - PHYSCANNON_ENDCAP1]));
...
void CWeaponPhysCannon::DoEffectHolding(void) {
    ...
    if (ShouldDrawUsingViewModel()) {
        ...
        // Turn on the glow sprites
        for (int i = PHYSCANNON_GLOW1;
             i < (PHYSCANNON_GLOW1 + NUM_GLOW_SPRITES); i++) {
            m_Parameters[i].GetScale().InitFromCurrent(0.5f * SPRITE_SCALE,
                                                       0.2f);
            m_Parameters[i].GetAlpha().InitFromCurrent(64.0f, 0.2f);
            m_Parameters[i].SetVisible();
        }
        ...
        for (int i = PHYSCANNON_ENDCAP1;
             i < (PHYSCANNON_ENDCAP1 + NUM_ENDCAP_SPRITES - 1); i++) {
            m_Parameters[i].SetVisible();
        }
...
void CWeaponPhysCannon::DoEffectLaunch(Vector *pos) {
    ...
    if (pos == NULL) {
        ...
        if (m_hAttachedObject) {
            ...
            trace_t tr;
            UTIL_TraceLine(endPos, endPos + (shotDir * MAX_TRACE_LENGTH),
...
//			effect parameter
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::GetEffectParameters(EffectType_t effectID,
                                            color32 &color, float &scale,
                                            IMaterial **pMaterial,
                                            Vector &vecAttachment) {
    ...
    *pMaterial = (IMaterial *)m_Parameters[effectID].GetMaterial();
    ...
    if (ShouldDrawUsingViewModel()) {
        ...
        if (pOwner != NULL) {
            pOwner->GetViewModel()->GetAttachment(attachment, vecAttachment,
                                                  angles);
...
void PhysCannonForceDrop(CBaseCombatWeapon *pActiveWeapon,
    ...
    CWeaponPhysCannon *pCannon =
        dynamic_cast<CWeaponPhysCannon *>(pActiveWeapon);
    if (pCannon) {
...
bool PlayerPickupControllerIsHoldingEntity(CBaseEntity *pPickupControllerEntity,
                                           CBaseEntity *pHeldEntity) {
    CPlayerPickupController *pController =
        dynamic_cast<CPlayerPickupController *>(pPickupControllerEntity);
...
void CallbackPhyscannonImpact(const CEffectData &data) {
    ...
    if (pWeapon->GetOwner() == CBasePlayer::GetLocalPlayer()) {
        ...
        C_BasePlayer *pPlayer =
            dynamic_cast<C_BasePlayer *>(pWeapon->GetOwner());
