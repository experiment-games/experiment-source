...
extern void DrawHalo(IMaterial *pMaterial, const Vector &source, float scale,
                     float const *color, float flHDRColorScale);
...
#define NUM_BEAM_ATTACHMENTS 9
    ...
    stunstickBeamInfo_t
        m_BeamAttachments[NUM_BEAM_ATTACHMENTS];  // Lookup for arc attachment
                                                  // points on the head of the
                                                  // stick
...
#define FADE_DURATION 0.25f
    ...
    // Tony; third person check thing, this has to be done for the local player
    // if third person switches, so we can re-calc attachment points.
...
SendPropInt(SENDINFO(m_bActive), 1, SPROP_UNSIGNED),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA(CWeaponStunStick) END_PREDICTION_DATA()
...
int CWeaponStunStick::WeaponMeleeAttack1Condition(float flDot, float flDist) {
    ...
    // Project where the enemy will be in a little while, add some randomness so
    ...
    if (pEnemy->IsPlayer()) {
        ...
        Vector vecProjectEnemy =
            pEnemy->GetAbsOrigin() + (pEnemy->GetAbsVelocity() * 0.35);
...
void CWeaponStunStick::Operator_HandleAnimEvent(
    ...
    switch (pEvent->event) {
        case EVENT_WEAPON_MELEE_HIT: {
            ...
            CBaseEntity *pEnemy = pOperator->MyNPCPointer()
                                      ? pOperator->MyNPCPointer()->GetEnemy()
                                      : NULL;
            if (pEnemy) {
                Vector vecDelta;
                VectorSubtract(pEnemy->WorldSpaceCenter(),
                               pOperator->Weapon_ShootPosition(), vecDelta);
                VectorNormalize(vecDelta);

                Vector2D vecDelta2D = vecDelta.AsVector2D();
                Vector2DNormalize(vecDelta2D);
                if (DotProduct2D(vecDelta2D, vecDirection.AsVector2D()) >
                    0.8f) {
            ...
            Vector vecEnd;
            VectorMA(pOperator->Weapon_ShootPosition(), 32, vecDirection,
                     vecEnd);
            // Stretch the swing box down to catch low level physics objects
            CBaseEntity *pHurt = pOperator->CheckTraceHullAttack(
                pOperator->Weapon_ShootPosition(), vecEnd,
                Vector(-16, -16, -40), Vector(16, 16, 16),
                GetDamageForActivity(GetActivity()), DMG_CLUB, 0.5f, false);
...
//-----------------------------------------------------------------------------
bool UTIL_GetWeaponAttachment(C_BaseCombatWeapon *pWeapon, int attachmentID,
                              Vector &absOrigin, QAngle &absAngles) {
    ...
    // Otherwise we need to translate the attachment to the viewmodel's version
    // and reformat it
    CBasePlayer *pOwner = ToBasePlayer(pWeapon->GetOwner());

    if (pOwner != NULL) {
        int ret = pOwner->GetViewModel()->GetAttachment(attachmentID, absOrigin,
                                                        absAngles);
...
void C_WeaponStunStick::SetupAttachmentPoints(void) {
    ...
    if (ShouldDrawUsingViewModel()) {
        const char *szBeamAttachNamesTop[NUM_BEAM_ATTACHMENTS] = {
            "spark1a", "spark2a", "spark3a", "spark4a", "spark5a",
            "spark6a", "spark7a", "spark8a", "spark9a",
        };

        const char *szBeamAttachNamesBottom[NUM_BEAM_ATTACHMENTS] = {
            "spark1b", "spark2b", "spark3b", "spark4b", "spark5b",
            "spark6b", "spark7b", "spark8b", "spark9b",
        };

        // Lookup and store all connections
        for (int i = 0; i < NUM_BEAM_ATTACHMENTS; i++) {
            m_BeamAttachments[i].IDs[0] =
                LookupAttachment(szBeamAttachNamesTop[i]);
            m_BeamAttachments[i].IDs[1] =
                LookupAttachment(szBeamAttachNamesBottom[i]);
        }
...
bool C_WeaponStunStick::InSwing(void) {
    ...
    if (activity == GetPrimaryAttackActivity() ||
        ...
        activity == ACT_VM_MISSCENTER || activity == ACT_VM_MISSCENTER2)
        return true;
...
void C_WeaponStunStick::DrawThirdPersonEffects(void) {
    ...
    IMaterial *pMaterial =
        materials->FindMaterial(STUNSTICK_GLOW_MATERIAL, NULL, false);
    ...
    UTIL_GetWeaponAttachment(this, m_BeamCenterAttachment, vecOrigin,
                             vecAngles);
    DrawHalo(pMaterial, vecOrigin, scale, color);

    if (InSwing()) {
        pMaterial =
            materials->FindMaterial(STUNSTICK_GLOW_MATERIAL2, NULL, false);
        pRenderContext->Bind(pMaterial);
        ...
        if (gpGlobals->frametime != 0.0f && (random->RandomInt(0, 5) == 0)) {
            ...
            beamInfo.m_pStartEnt =
                cl_entitylist->GetEnt(BEAMENT_ENTITY(entindex()));
            beamInfo.m_pEndEnt =
                cl_entitylist->GetEnt(BEAMENT_ENTITY(entindex()));
...
void C_WeaponStunStick::DrawFirstPersonEffects(void) {
    ...
    CMatRenderContextPtr pRenderContext(materials);
    IMaterial *pMaterial =
        materials->FindMaterial(STUNSTICK_GLOW_MATERIAL_NOZ, NULL, false);
    // FIXME: Needs to work with new IMaterial system!
    pRenderContext->Bind(pMaterial);

    // Find where we are in the fade
    float fadeAmount =
        RemapValClamped(gpGlobals->curtime, m_flFadeTime,
                        m_flFadeTime + FADE_DURATION, 1.0f, 0.1f);
    ...
    for (int i = 0; i < (NUM_BEAM_ATTACHMENTS * 2) + 1; i++) {
        if (InSwing()) {
            ...
            color[0] = color[1] = color[2] =
                random->RandomFloat(0.05f, 0.5f) * fadeAmount;
...
void StunstickImpactCallback(const CEffectData &data) {
    ...
    FX_AddQuad(data.m_vOrigin, data.m_vNormal, scale, scale * 2.0f, 1.0f, 1.0f,
               0.0f, 0.0f, random->RandomInt(0, 360), 0,
               Vector(1.0f, 1.0f, 1.0f), 0.1f, "sprites/light_glow02_add", 0);
