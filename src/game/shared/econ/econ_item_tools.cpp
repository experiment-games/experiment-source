...
CEconTool_WrappedGift::CEconTool_WrappedGift( const char *pszTypeName, const char *pszUseString, item_capabilities_t unCapabilities, KeyValues *pUsageKV )
    : IEconTool( pszTypeName, pszUseString, NULL, unCapabilities ), m_pszDeliveredGiftItemDefName( NULL ), m_pDeliveredGiftItemDef( NULL ), m_bIsGlobalGift( false ), m_bIsDirectGift( false )
{
    if ( pUsageKV )
    {
...
CEconTool_GiftWrap::CEconTool_GiftWrap( const char *pszTypeName, const char *pszUseString, item_capabilities_t unCapabilities, KeyValues *pUsageKV )
    : IEconTool( pszTypeName, NULL, NULL, unCapabilities ), m_pszWrappedGiftItemDefName( NULL ), m_pWrappedGiftItemDef( NULL )
{
    if ( pUsageKV )
    {
...
bool CEconTool_GiftWrap::BFinishInitialization()
{
    ...
    return m_pWrappedGiftItemDef != NULL && IEconTool::BFinishInitialization();
}
...
bool CEconTool_StrangePartRestriction::BFinishInitialization()
{
    ...
    return m_eRestrictionType != kStrangeEventRestriction_None && m_unRestrictionValue != ( unsigned int )-1 && IEconTool::BFinishInitialization();
}
...
CEconTool_ItemDynamicRecipe::CBaseRecipeComponent::CBaseRecipeComponent( bool bIsOutput, const CBaseRecipeComponent *pParent )
    : m_bIsOutput( bIsOutput ), m_flChanceOfApplying( 1.f ), m_pParent( pParent ), m_flTotalWeights( 0.f ), m_eQuality( AE_UNDEFINED ), m_attributesMatchingType( ATTRIBUTES_MATCH_NONE )
{
}
...
bool CEconTool_Xifier::ItemDefMatch( const CEconItemDefinition *pTargetItemDef, const CEconItemDefinition *pSubjectItemDef ) const
{
    if ( pTargetItemDef && pSubjectItemDef )
    {
        ...
        if ( pTargetItemDef->GetXifierRemapClass() && *pTargetItemDef->GetXifierRemapClass() && pSubjectItemDef->GetXifierRemapClass() && *pSubjectItemDef->GetXifierRemapClass() )
        {
