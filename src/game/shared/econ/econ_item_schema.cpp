...
#define RETURN_ATTRIBUTE_STRING_F( func_name, attrib_name, default_string ) \
    ...
    {                                                                       \
        RETURN_ATTRIBUTE_STRING( attrib_name, default_string )              \
    }
...
CEconItemQualityDefinition::CEconItemQualityDefinition( void )
    : m_nValue( INT_MAX ), m_bCanSupportSet( false )
{
}
...
bool CEconItemQualityDefinition::BInitFromKV( KeyValues *pKVQuality, CUtlVector< CUtlString > *pVecErrors /* = NULL */ )
{
    ...
    SCHEMA_INIT_CHECK(
        ...
        "Quality definition %s: Invalid value (%d). It is reserved for Any",
        GetName(),
        k_unItemQuality_Any );
...
CEconItemRarityDefinition::CEconItemRarityDefinition( void )
    : m_nValue( INT_MAX ), m_nLootlistWeight( 0 )
{
}
...
bool CEconItemRarityDefinition::BInitFromKV( KeyValues *pKVRarity, KeyValues *pKVRarityWeights, CEconItemSchema &pschema, CUtlVector< CUtlString > *pVecErrors /* = NULL */ )
{
    ...
    SCHEMA_INIT_CHECK(
        ...
        "Rarity definition %s: Missing required field \"value\"",
        pKVRarity->GetName() );

    SCHEMA_INIT_CHECK(
        NULL != pKVRarity->FindKey( "loc_key" ),
        "Rarity definition %s: Missing required field \"loc_key\"",
        pKVRarity->GetName() );
...
CEconItemSetDefinition::CEconItemSetDefinition( void )
    : m_strName( NULL ), m_pszLocalizedName( NULL ), m_iBundleItemDef( INVALID_ITEM_DEF_INDEX ), m_bIsHiddenSet( false )
{
}
...
bool CEconItemSetDefinition::BInitFromKV( KeyValues *pKVItemSet, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    if ( pKVItems )
    {
        FOR_EACH_SUBKEY( pKVItems, pKVItem )
        {
            ...
            SCHEMA_INIT_CHECK(
                ...
                "Item set %s: item definition \"%s\" appears multiple times",
                m_strName.Get(),
                pszName );
            SCHEMA_INIT_CHECK(
                !pDef->GetItemSetDefinition(),
                "Item set %s: item definition \"%s\" specified in multiple item sets",
                m_strName.Get(),
                pszName );
    ...
    if ( pKVAttributes )
    {
        FOR_EACH_SUBKEY( pKVAttributes, pKVAttribute )
        {
            ...
            SCHEMA_INIT_CHECK(
                pAttrDef != NULL,
                "Item set %s: Attribute definition \"%s\" was not found",
                m_strName.Get(),
                pszName );
            SCHEMA_INIT_CHECK(
                pAttrDef->BIsSetBonusAttribute(),
                "Item set %s: Attribute definition \"%s\" is not a set bonus attribute",
                m_strName.Get(),
                pszName );
    ...
    SCHEMA_INIT_CHECK( m_pszLocalizedName != NULL,
                       "Item set %s: Set contains no localized name",
                       m_strName.Get() );
    SCHEMA_INIT_CHECK( m_iItemDefs.Count() > 0,
                       "Item set %s: Set contains no items",
                       m_strName.Get() );
...
CEconItemCollectionDefinition::CEconItemCollectionDefinition( void )
    : m_strName( NULL ), m_pszLocalizedName( NULL ), m_pszLocalizedDesc( NULL ), m_iRarityMin( k_unItemRarity_Any ), m_iRarityMax( k_unItemRarity_Any )
{
}
...
bool CEconItemCollectionDefinition::BInitFromKV( KeyValues *pKVPItemCollection, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    if ( pKVItems )
    {
        ...
        FOR_EACH_TRUE_SUBKEY( pKVItems, pKVRarity )
        {
            ...
            FOR_EACH_VALUE( pKVRarity, pKVItem )
            {
                ...
                SCHEMA_INIT_CHECK(
                    !m_iItemDefs.IsValidIndex( m_iItemDefs.Find( unDefIndex ) ),
                    "Item Collection %s: item definition \"%s\" appears multiple times",
                    m_strName.Get(),
                    pszName );
                ...
                if ( !m_bIsReferenceCollection )
                {
                    SCHEMA_INIT_CHECK(
                        ...
                        "Item Collection %s: item definition \"%s\" specified in multiple item sets",
                        m_strName.Get(),
                        pszName );
    ...
    // Sanity check.
    SCHEMA_INIT_CHECK( m_pszLocalizedName != NULL,
                       "Item Collection %s: Collection contains no localized name",
                       m_strName.Get() );
    SCHEMA_INIT_CHECK( m_pszLocalizedDesc != NULL,
                       "Item Collection %s: Collection contains no localized description",
                       m_strName.Get() );
    SCHEMA_INIT_CHECK( m_iItemDefs.Count() > 0,
                       "Item Collection %s: Collection contains no items",
                       m_strName.Get() );
...
CEconOperationDefinition::CEconOperationDefinition( void )
    : m_pszName( NULL ), m_unRequiredItemDefIndex( INVALID_ITEM_DEF_INDEX ), m_unGatewayItemDefIndex( INVALID_ITEM_DEF_INDEX )
{
}
...
bool BAddLootlistJobFromTemplates( const char *pszContext, CUtlVector< CLootlistJob * > &out_jobs, KeyValues *pLootlistJobKV, const CEconItemSchema *pschema, CUtlVector< CUtlString > *pVecErrors = NULL )
{
    ...
    FOR_EACH_SUBKEY( pLootlistJobKV, pTemplateKV )
    {
        ...
        SCHEMA_INIT_CHECK(
            NULL != pJobTemplate,
            "Context %s: Couldn't find CLootlistJob '%s' from lootlist_job_templates",
            pszContext,
            pszJobName );
    ...
    FOR_EACH_SUBKEY( pKVLootList, pKVListItem )
    {
        ...
        if ( !Q_strcmp( pszName, "loot_list_header_desc" ) )
        {
            ...
            SCHEMA_INIT_CHECK(
                ...
                "Loot list %s: Multiple header descriptions specified",
                m_strName.Get() );
            ...
            SCHEMA_INIT_CHECK(
                ...
                "Loot list %s: Invalid header description specified",
                m_strName.Get() );
        ...
        else if ( !Q_strcmp( pszName, "loot_list_footer_desc" ) )
        {
            ...
            SCHEMA_INIT_CHECK(
                ...
                "Loot list %s: Multiple footer descriptions specified",
                m_strName.Get() );
        ...
        if ( iDef > 0 )
        {
            ...
            if ( FindAttribute( pItemDef, pAttribDef_EndDropDate, &value ) )
            {
                ...
                SCHEMA_INIT_CHECK(
                    ...
                    "Malformed end drop date \"%s\" for item %s.  Must be of the form \"YYYY-MM-DD hh:mm:ss\"",
                    pszEndDate,
                    pItemDef->GetDefinitionName() );
            }
        }
...
bool CEconLootListDefinition::AddLootlistJob( KeyValues *pLootlistJobKV, CEconItemSchema &pschema, CUtlVector< CUtlString > *pVecErrors /*= NULL*/ )
{
    ...
    SCHEMA_INIT_CHECK(
        NULL != pJob,
        "Loot List %s: Failed to create CLootlistJob '%s'",
        m_strName.Get(),
        pszJobName );
    ...
    FOR_EACH_TRUE_SUBKEY( pKVKey, pSubKey )
    {
        ...
        else if ( !V_strcmp( pszName, "additional_drop" ) )
        {
            ...
            SCHEMA_INIT_CHECK(
                ...
                "Malformed start drop date \"%s\" for additional_drop in lootlist %s.  Must be of the form \"YYYY-MM-DD hh:mm:ss\"",
                pszDropPerdiodStartDate,
                m_pszOwnerName );
            ...
            SCHEMA_INIT_CHECK(
                ...
                "Malformed end drop date \"%s\" for additional_drop in lootlist %s.  Must be of the form \"YYYY-MM-DD hh:mm:ss\"",
                pszDropPerdiodEndDate,
                m_pszOwnerName );

            SCHEMA_INIT_CHECK(
                pszLootList && pszLootList[0],
                "Loot list %s: Missing \"additional_drop\" loot list name",
                m_pszOwnerName );

            SCHEMA_INIT_CHECK(
                ( pszRequiredHoliday == NULL ) == ( iRequiredHolidayIndex == kHoliday_None ),
                "Loot list %s: Unknown or missing holiday \"%s\"",
                m_pszOwnerName,
                pszRequiredHoliday ? pszRequiredHoliday : "(null)" );
...
bool CLootlistJob::BPostInit( CUtlVector< CUtlString > *pVecErrors )
{
    ...
    FOR_EACH_VEC( m_vecAdditionalDrops, i )
    {
        ...
        SCHEMA_INIT_CHECK(
            pLootListDef != NULL,
            "Loot list %s: Invalid \"additional_drop\" loot list \"%s\"",
            m_pszOwnerName,
            pszLootList );
    }
...
bool CLootlistJob::AddRandomAtrributes( KeyValues *pRandomAttributesKV, CEconItemSchema &pschema, CUtlVector< CUtlString > *pVecErrors /*= NULL*/ )
{
    ...
    SCHEMA_INIT_CHECK(
        NULL != pRandomAttr,
        "Loot List %s: Failed to create random_attrib_t '%s'",
        m_pszOwnerName,
        pszAttrName );
...
bool CLootlistJob::AddRandomAttributesFromTemplates( KeyValues *pRandomAttributesKV, CEconItemSchema &pschema, CUtlVector< CUtlString > *pVecErrors /*= NULL*/ )
{
    ...
    SCHEMA_INIT_CHECK(
        ...
        "Loot List %s: Couldn't find random_attrib_t '%s' from attribute_templates",
        m_pszOwnerName,
        pszAttrName );
...
bool CEconItemAttributeDefinition::BInitFromKV( KeyValues *pKVAttribute, CUtlVector< CUtlString > *pVecErrors /* = NULL */ )
{
    ...
    SCHEMA_INIT_CHECK(
        ...
        m_pKVAttribute->GetName() );

    SCHEMA_INIT_CHECK(
        NULL != m_pAttrType,
        "Attribute definition %s: Unable to find attribute data type '%s'",
        m_pszDefinitionName,
        pszAttrType ? pszAttrType : "(default)" );

    if ( m_bIsSetBonus )
    {
        SCHEMA_INIT_CHECK(
            m_pAttrType->BSupportsGameplayModificationAndNetworking(),
            "Attribute definition %s: set as set bonus attribute but does not support gameplay modification/networking!",
            m_pszDefinitionName );
    }
...
static void RecursiveInheritKeyValues( KeyValues *out_pValues, KeyValues *pInstance )
{
    ...
    for ( KeyValues *pSubKey = pInstance->GetFirstSubKey(); pSubKey != NULL; pPrevSubKey = pSubKey, pSubKey = pSubKey->GetNextKey() )
    {
        ...
        AssertMsg2( !pPrevSubKey || pPrevSubKey->GetNameSymbol() != pSubKey->GetNameSymbol(),
                    "Item definition \"%s\" has multiple attributes of the same name (%s) can't use prefabs",
                    pInstance->GetName(),
                    pSubKey->GetName() );
        ...
        switch ( eType )
        {
            case KeyValues::TYPE_STRING:
                out_pValues->SetString( pSubKey->GetName(), pSubKey->GetString() );
                break;
            case KeyValues::TYPE_INT:
                out_pValues->SetInt( pSubKey->GetName(), pSubKey->GetInt() );
                break;
            case KeyValues::TYPE_FLOAT:
                out_pValues->SetFloat( pSubKey->GetName(), pSubKey->GetFloat() );
                break;
            case KeyValues::TYPE_WSTRING:
                out_pValues->SetWString( pSubKey->GetName(), pSubKey->GetWString() );
                break;
            case KeyValues::TYPE_COLOR:
                out_pValues->SetColor( pSubKey->GetName(), pSubKey->GetColor() );
                break;
            case KeyValues::TYPE_UINT64:
                out_pValues->SetUint64( pSubKey->GetName(), pSubKey->GetUint64() );
                break;
...
bool CEconItemDefinition::BInitFromKV( KeyValues *pKVItem, CUtlVector< CUtlString > *pVecErrors /* = NULL */ )
{
    ...
    if ( m_pKVItem->FindKey( "item_rarity" ) )
    {
        SCHEMA_INIT_CHECK(
            ...
            "Item definition %s: Undefined item_rarity \"%s\"",
            GetDefinitionName(),
            m_pKVItem->GetString( "item_rarity" ) );
    }
    ...
    if ( m_pKVItem->FindKey( "item_series" ) )
    {
        ...
        SCHEMA_INIT_CHECK(
            ...
            "Item definition %s: Undefined item_series \"%s\"",
            GetDefinitionName(),
            m_pKVItem->GetString( "item_series" ) );
    }
    ...
    if ( pchExpiration && pchExpiration[0] )
    {
        if ( pchExpiration[0] == '!' )
        {
            ...
            SCHEMA_INIT_CHECK(
                ...
                "Unknown/malformed expiration_date string \"%s\" in item %s.",
                pchExpiration,
                m_pszDefinitionName );
        }
        else
...
bool static_attrib_t::BInitFromKV_MultiLine( const char *pszContext, KeyValues *pKVAttribute, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    if ( pAttrDef )
    {
        ...
        SCHEMA_INIT_CHECK(
            ...
            "Context '%s': Attribute \"%s\" could not parse value \"%s\"!",
            pszContext,
            pKVAttribute->GetName(),
        ...
        SCHEMA_INIT_CHECK(
            !pAttrDef->BIsSetBonusAttribute(),
            "Context '%s': Attribute \"%s\" is a set bonus attribute and not supported here",
            pszContext,
            pKVAttribute->GetName() );
    }
...
bool CTimedItemRewardDefinition::BInitFromKV( KeyValues *pKVTimedReward, CUtlVector< CUtlString > *pVecErrors /* = NULL */ )
{
    ...
    SCHEMA_INIT_CHECK(
        ...
        "Time reward %s: Missing required field \"value_min\"",
        pKVTimedReward->GetName() );
    SCHEMA_INIT_CHECK(
        NULL != pKVTimedReward->FindKey( "value_max" ),
        "Time reward %s: Missing required field \"value_max\"",
        pKVTimedReward->GetName() );

    SCHEMA_INIT_CHECK(
        NULL != pKVTimedReward->FindKey( "pctChance" ),
        "Time reward %s: Missing required field \"pctChance\"",
        pKVTimedReward->GetName() );

    SCHEMA_INIT_CHECK(
        NULL == pKVTimedReward->FindKey( "criteria" ),
        "Time reward %s: \"criteria\" is no longer supported. Restructure as \"loot_list\"?",
        pKVTimedReward->GetName() );

    SCHEMA_INIT_CHECK(
        NULL != pKVTimedReward->FindKey( "loot_list" ),
        "Time reward %s: Missing required field \"loot_list\" ",
        pKVTimedReward->GetName() );
    ...
    if ( pszLootList && pszLootList[0] )
    {
        ...
        SCHEMA_INIT_CHECK(
            NULL != m_pLootList,
            "Time Reward %s: loot_list (%s) does not exist",
            pKVTimedReward->GetName(),
            pszLootList );
    }
    ...
    SCHEMA_INIT_CHECK(
        m_flChance >= 0.0f,
        "Time Reward %s: pctChance (%f) must be greater or equal to 0.0",
        pKVTimedReward->GetName(),
        m_flChance );

    SCHEMA_INIT_CHECK(
        m_flChance <= 1.0f,
        "Time Reward %s: pctChance (%f) must be less than or equal to 1.0",
        pKVTimedReward->GetName(),
        m_flChance );

    SCHEMA_INIT_CHECK(
        pKVTimedReward->GetInt( "value_min" ) > 0,
        "Time Reward %s: value_min (%d) must be greater than 0",
        pKVTimedReward->GetName(),
        m_unMinFreq );
    SCHEMA_INIT_CHECK(
        pKVTimedReward->GetInt( "value_max" ) > 0,
        "Time Reward %s: value_max (%d) must be greater than 0",
        pKVTimedReward->GetName(),
        m_unMaxFreq );
    SCHEMA_INIT_CHECK(
        ( m_unMaxFreq >= m_unMinFreq ),
        "Time Reward %s: value_max (%d) must be greater than or equal to value_min (%d)",
        pKVTimedReward->GetName(),
        m_unMaxFreq,
        m_unMinFreq );
...
CEconItemSchema::CEconItemSchema()
    : m_unResetCount( 0 ), m_pKVRawDefinition( NULL ), m_mapItemSeries( DefLessFunc( int ) ), m_mapRarities( DefLessFunc( int ) ), m_mapQualities( DefLessFunc( int ) ), m_mapAttributes( DefLessFunc( int ) ), m_mapRecipes( DefLessFunc( int ) ), m_mapQuestObjectives( DefLessFunc( int ) ), m_mapItemsSorted( DefLessFunc( int ) ), m_mapToolsItems( DefLessFunc( int ) ), m_mapPaintKitTools( DefLessFunc( uint32 ) ), m_mapBaseItems( DefLessFunc( int ) ), m_unVersion( 0 )
#if defined( CLIENT_DLL ) || defined( GAME_DLL )
      ,
...
bool CEconItemSchema::BInitQualities( KeyValues *pKVQualities, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    FOR_EACH_MAP_FAST( m_mapQualities, i )
    {
        ...
        SCHEMA_INIT_CHECK(
            ...
            m_mapQualities[i].GetDBValue(),
            m_mapQualities[i].GetName() );
...
bool CEconItemSchema::BInitAttributes( KeyValues *pKVAttributes, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    FOR_EACH_MAP_FAST( m_mapAttributes, i )
    {
        ...
        SCHEMA_INIT_CHECK(
            ...
            m_mapAttributes.Key( i ),
            m_mapAttributes[i].GetDefinitionName() );
            ...
            else
            {
                ...
                if ( pItemDef->IsTool() )
                {
                    ...
                    if ( pItemDef->GetEconTool() && !V_strcmp( pItemDef->GetEconTool()->GetTypeName(), "paintkit" ) )
                    {
                        ...
                        SCHEMA_INIT_CHECK( iMapIndex == m_mapPaintKitTools.InvalidIndex(), "Duplicate paintkit def index [%d]. Trying to add to item [%d], but item [%d] already has it.", unPaintKitDefIndex, pItemDef->GetDefinitionIndex(), m_mapPaintKitTools[iMapIndex]->GetDefinitionIndex() );
        ...
        if ( item.m_iItemOrLootlistDef >= 0 )
        {
            ...
            if ( pItemDef )
            {
                ...
                SCHEMA_INIT_CHECK( !FindAttribute( pItemDef, pAttribDef_StartDropDate, &value ),
                                   "Lootlist \"%s\" contains lootlist \"%s\", which contains item \"%s\", which has start drop date.",
                                   pRootLootList->GetName(),
                                   pLootList->GetName(),
                                   pItemDef->GetDefinitionName() );
                // Check for end drop date attribute on this item
                SCHEMA_INIT_CHECK( !FindAttribute( pItemDef, pAttribDef_EndDropDate, &value ),
                                   "Lootlist \"%s\" contains lootlist \"%s\", which contains item \"%s\", which has end drop date.",
                                   pRootLootList->GetName(),
                                   pLootList->GetName(),
                                   pItemDef->GetDefinitionName() );
            }
...
bool CEconItemSchema::BInitItemCriteriaTemplates( KeyValues *pKVItemCriteriaTemplates, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    FOR_EACH_TRUE_SUBKEY( pKVItemCriteriaTemplates, pKVItemCriteriaTemplate )
    {
        ...
        SCHEMA_INIT_CHECK(
            NULL != pCriteria,
            "%s: Failed to create CItemSelectionCriteria '%s'",
            __FUNCTION__,
            pszCriteriaName );
...
bool CEconItemSchema::BInitRandomAttributeTemplates( KeyValues *pKVRandomAttributeTemplates, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    FOR_EACH_TRUE_SUBKEY( pKVRandomAttributeTemplates, pKVAttributeTemplate )
    {
        ...
        SCHEMA_INIT_CHECK(
            NULL != pRandomAttr,
            "%s: Failed to create random_attrib_t '%s'",
            __FUNCTION__,
            pszAttrName );
...
bool CEconItemSchema::BInitLootlistJobTemplates( KeyValues *pKVLootlistJobTemplates, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    FOR_EACH_TRUE_SUBKEY( pKVLootlistJobTemplates, pKVJobTemplate )
    {
        ...
        SCHEMA_INIT_CHECK(
            NULL != pJob,
            "%s: Failed to create CreateLootlistJob '%s'",
            __FUNCTION__,
            pszJobName );
...
bool CEconItemSchema::BInitSteamPackageLocalizationToken( KeyValues *pKVSteamPackages, CUtlVector< CUtlString > *pVecErrors )
{
    if ( NULL != pKVSteamPackages )
    {
        FOR_EACH_TRUE_SUBKEY( pKVSteamPackages, pKVEntry )
        {
            ...
            SCHEMA_INIT_CHECK(
                iRawPackageId > 0,
                "Invalid package ID %i for localization",
                iRawPackageId );
    ...
    if ( NULL != pKVItemLevels )
    {
        FOR_EACH_TRUE_SUBKEY( pKVItemLevels, pKVItemLevelBlock )
        {
            ...
            SCHEMA_INIT_CHECK( GetItemLevelingData( pszLevelBlockName ) == NULL,
                               "Duplicate leveling data block named \"%s\".",
                               pszLevelBlockName );
...
bool CEconItemSchema::BInitKillEaterScoreTypes( KeyValues *pKVKillEaterScoreTypes, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    if ( NULL != pKVKillEaterScoreTypes )
    {
        FOR_EACH_TRUE_SUBKEY( pKVKillEaterScoreTypes, pKVScoreType )
        {
            ...
            SCHEMA_INIT_CHECK( GetItemLevelingData( pszLevelBlockName ) != NULL,
                               "Unable to find leveling data block named \"%s\" for kill eater score type %u.",
                               pszLevelBlockName,
                               unIndex );
