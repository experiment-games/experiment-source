...
CStrangeRankLocalizationGenerator::CStrangeRankLocalizationGenerator( const CLocalizationProvider *pLocalizationProvider, const IEconItemInterface *pEconItem, bool bHashContextOff )
    : m_bValid( false ), m_loc_Rank( NULL ), m_loc_SecondaryRank( NULL ), m_unType( kKillEaterEvent_PlayerKill ), m_unScore( 0 ), m_unUsedStrangeSlot( 0 )
{
    Assert( pLocalizationProvider );
    Assert( pEconItem );
...
void CEconItemDescription::Generate_ItemLevelDesc_Default( const CLocalizationProvider *pLocalizationProvider, const IEconItemInterface *pEconItem, const locchar_t *locTypename )
{
    ...
    bool bForceLevelDisplay = FindAttribute_UnsafeBitwiseCast< attrib_value_t >( pEconItem, pAttrDef_ForceLevelDisplay, &fForceLevelDisplayValue ) && fForceLevelDisplayValue > 0.0f;
    ...
    if ( ( locTypename && *locTypename ) || bForceLevelDisplay )
    {
        if ( locTypename )
        {
            ...
            if ( pEconItem->FindAttribute( pAttrDef_OverrideItemLevelDescString, &unOverrideItemLevelDescString ) && unOverrideItemLevelDescString != 0 && unOverrideItemLevelDescString <= ARRAYSIZE( s_pszCustomItemLevelDescLocalizationTokens ) )
            {
       ...
       public:
        ...
        virtual void OnIterate( item_definition_index_t unItemDefIndex ) OVERRIDE
        {
            ...
            if ( pItemDef )
            {
                ...
                m_pThis->AddDescLine(
                    CConstructLocalizedString( LOCCHAR( "%s1%s2" ), pCheckmark, CEconItemLocalizedFullNameGenerator( m_pLocalizationProvider, pItemDef, m_bUseProperName ).GetFullName() ),
                    colorRarity,
                    kDescLineFlag_Misc,
                    NULL,
    ...
    // return before adding any of them.
    {
        ...
        FOR_EACH_VEC( pItemSetDef->m_iItemDefs, i )
        {
            ...
            AddDescLine( CEconItemLocalizedFullNameGenerator(
                             ...
                             .GetFullName(),
                         bItemPresent ? ATTRIB_COL_ITEMSET_EQUIPPED : ATTRIB_COL_ITEMSET_MISSING,
                         kDescLineFlag_Set,
                         &vecPotentialDescLines,
                         usLinkItemDefIndex );
...
void CEconItemDescription::Generate_CollectionDesc( const CLocalizationProvider *pLocalizationProvider, const IEconItemInterface *pEconItem )
{
    ...
    FOR_EACH_VEC( pCollection->m_iItemDefs, index )
    {
        ...
        if ( pTempItemDef )
        {
            ...
            AddDescLine(
                CConstructLocalizedString( LOCCHAR( "%s1%s2" ), pCheckmark, CEconItemLocalizedFullNameGenerator( pLocalizationProvider, pTempItemDef, TF_ANTI_IDLEBOT_VERIFICATION_ONLY_ARG_BOOL_TRUE( m_pHashContext == NULL ) ).GetFullName() ),
                colorRarity,
                eFlag,
                NULL,
    ...
    if ( vecLines.Count() > 0 )
    {
        ...
        if ( loc_AttribFormat_AdditionalNode )
        {
            ...
            FOR_EACH_VEC( vecLines, i )
            {
                ...
                {
                    AddDescLine( pszLocalizationSubKey ? CConstructLocalizedString( pLocalizationProvider->Find( pszLocalizationKey ), pLocalizationProvider->Find( pszLocalizationSubKey ) ) :  // has subtoken, doesn't use additional note format
...
void CEconItemDescription::AddAttributeDescription( const CLocalizationProvider *pLocalizationProvider, const CEconItemAttributeDefinition *pAttribDef, attrib_value_t value, attrib_colors_t eOverrideDisplayColor /* = NUM_ATTRIB_COLORS */, uint32 unAdditionalMetaType /*= 0*/ )
{
    ...
    CEconAttributeDescription AttrDesc( pLocalizationProvider,
                                        pAttribDef,
                                        value,
                                        TF_ANTI_IDLEBOT_VERIFICATION_ONLY_ARG( m_pHashContext ) TF_ANTI_IDLEBOT_VERIFICATION_ONLY_COMMA this );
