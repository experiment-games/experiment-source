...
class CEconStoreCategoryManager
{
    ...
    struct StoreCategory_t
    {
        StoreCategory_t()
        {
            V_memset( this, 0, sizeof( StoreCategory_t ) );
        }

        int GetNumSubcategories() const
        {
            return m_vecSubcategories.Count();
        }
        bool HasSubcategories() const
        {
            return m_vecSubcategories.Count() > 1;
        }
        bool BIsSubcategory() const
        {
            return m_unParentCategoryID != CEconStoreCategoryManager::k_CategoryID_Invalid;
        }
    ...
    const StoreCategoryID_t GetHomeCategoryID() const
    {
        Assert( m_unHomeCategoryID != k_CategoryID_Invalid );
        return m_unHomeCategoryID;
    }

    const StoreCategory_t *GetStoreCategoryFromID( StoreCategoryID_t unID ) const;
    int GetNumCategories( void ) const
    {
        return m_vecCategories.Count();
    }
    const StoreCategory_t *GetCategoryFromIndex( int i ) const
    {
        Assert( i >= 0 && i < m_vecCategories.Count() );
        return &m_vecCategories[i];
    }

    StoreCategory_t *GetFeaturedItems() const
    {
        return NULL;
    }
