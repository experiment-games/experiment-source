...
ENUMSTRINGS_START( ECurrency ){ k_ECurrencyUSD, "USD" },
    { k_ECurrencyGBP, "GBP" },
    { k_ECurrencyEUR, "EUR" },
    { k_ECurrencyRUB, "RUB" },
    { k_ECurrencyBRL, "BRL" },
    ...
    { k_ECurrencyKZT, "KZT" },
    { k_ECurrencyBYN, "BYN" },
    { k_ECurrencyInvalid, "Invalid" } ENUMSTRINGS_REVERSE( ECurrency, k_ECurrencyInvalid )

        ENUMSTRINGS_START( EPurchaseState ){ k_EPurchaseStateInvalid, "Invalid" },
    ...
    ENUMSTRINGS_END( EPurchaseState )
        ...
        ENUMSTRINGS_START( EPurchaseResult ){ k_EPurchaseResultOK, "OK" },
    ...
    ENUMSTRINGS_END( EPurchaseResult )
        ...
        ENUMSTRINGS_START( EGCTransactionAuditReason ){ k_EGCTransactionAudit_GCTransactionCompleted, "Completed" },
...
{
    return GetSalePrice( eCurrency ) > 0 && GetSalePrice( eCurrency ) < GetBasePrice( eCurrency );
}

bool econ_store_entry_t::IsRentable() const
{
    return m_fRentalPriceScale < 100.0f && m_fRentalPriceScale > 0.0f;
}
...
CEconStorePriceSheet::CEconStorePriceSheet()
    : m_pKVRaw( NULL ), m_mapEntries( DefLessFunc( uint16 ) ), m_mapRentalPriceScales( DefLessFunc( const char * ) ), m_RTimeVersionStamp( 0 ), m_pStorePromotionFirstTimePurchaseItem( NULL ), m_pStorePromotionFirstTimeWebPurchaseItem( NULL ), m_unFeaturedItemIndex( 0 ), m_eEconStoreSortType( kEconStoreSortType_Name_AToZ )
{
    Clear();
...
/*static*/ item_price_t econ_store_entry_t::CalculateSalePrice( item_price_t unPreDiscountPrice, ECurrency eCurrency, float fDiscountPercentage, int32 *out_pAdjustedDiscountPercentage /*=NULL*/ )
{
    ...
    */
    ...
    if ( out_pAdjustedDiscountPercentage )
    {
        ...
        const int32 nDiscountPercentageForCurrency = bUseDefaultDiscountPercentage ? RoundFloatToInt( fDiscountPercentage ) : RoundFloatToInt( fActualPercent );
