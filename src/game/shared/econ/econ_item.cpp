...
// --------------------------------------------------------------------------
CEconItem::CEconItem()
    : BaseClass(), m_pCustomData( NULL ), m_ulID( INVALID_ITEM_ID ), m_unStyle( 0 ), m_pszSmallIcon( NULL ), m_pszLargeIcon( NULL )
{
    Init();
}
...
{
    return !m_dirtyBits.m_bInUse && IEconItemInterface::IsTradable();
}
...
{
    return !m_dirtyBits.m_bInUse && IEconItemInterface::IsMarketable();
}
...
{
    return !m_dirtyBits.m_bInUse && IEconItemInterface::IsCommodity();
}
...
{
    return !m_dirtyBits.m_bInUse && IEconItemInterface::IsUsableInCrafting();
}
