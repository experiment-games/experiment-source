...
class CPlayerInventory : public GCSDK::ISharedObjectListener
{
   ...
   public:
    ...
    bool RetrievedInventoryFromSteam( void )
    {
        return m_bGotItemsFromSteam;
    }
    bool IsWaitingForSteam( void )
    {
        return ( m_iPendingRequests > 0 );
    }

    // Inventory access
    CSteamID &GetOwner( void )
    {
        return m_OwnerID;
    }
    int GetItemCount( void ) const
    {
        return m_aInventoryItems.Count();
    }
    ...
    {
        return DEFAULT_NUM_BACKPACK_SLOTS;
    }
    CEconItemView *GetItem( int i )
    {
    ...
    GCSDK::CGCClientSharedObjectCache *GetSOC( void )
    {
        return m_pSOCache;
    }
   ...
   protected:
    ...
    virtual void OnItemChangedPosition( CEconItemView *pItem, uint32 iOldPos )
    {
        return;
    }
    ...
    void ResortInventory( void )
    {
        m_aInventoryItems.RedoSort( true );
    }
...
class CInventoryManager : public CAutoGameSystemPerFrame
{
   ...
   public:
    ...
    virtual CPlayerInventory *GeneratePlayerInventoryObject() const
    {
    ...
    virtual CPlayerInventory *GetLocalInventory( void )
    {
        return NULL;
    }
    ...
    void OnItemDeleted( CPlayerInventory *pInventory )
    {
        if ( pInventory == GetLocalInventory() ) m_iPredictedDiscards--;
    }
    ...
    bool IsInBackpackSort( void )
    {
        return m_bInBackpackSort;
    }

    void PredictedBackpackPosFilled( int iBackpackPos )
    {
        m_PredictedFilledSlots.FindAndRemove( iBackpackPos );
    }
...
class CEconItemViewHandle
{
   public:
    ...
    CEconItemViewHandle( CEconItemView *pItem )
        : m_pItem( pItem ), m_pInv( NULL ), m_bPointerDirty( false )
    {
        SetItem( pItem );
    }
