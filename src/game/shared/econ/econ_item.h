...
struct schema_attribute_stat_bucket_t
{
    ...
    void OnAllocateInlineInstance()
    {
        m_unLiveInlineCount++;
        m_unLifetimeInlineCount++;
    }
    void OnFreeInlineInstance()
    {
        Assert( m_unLiveInlineCount > 0 );
        m_unLiveInlineCount--;
    }
    void OnAllocateHeapInstance()
    {
        m_unLiveHeapCount++;
        m_unLifetimeHeapCount++;
    }
    void OnFreeHeapInstance()
    {
        Assert( m_unLiveHeapCount );
        m_unLiveHeapCount--;
    }
};
...
// trying to do yet.
template < typename T >
uint32 WrapDeprecatedUntypedEconItemAttribute( T tValue )
{
    COMPILE_TIME_ASSERT( sizeof( T ) == sizeof( uint32 ) );
    return *reinterpret_cast< uint32 * >( &tValue );
}
...
class CEconItem : public GCSDK::CSharedObject, public CMaterialOverrideContainer< IEconItemInterface >
{
   public:
    ...
    {
        EquippedInstance_t()
            : m_unEquippedClass( 0 ), m_unEquippedSlot( INVALID_EQUIPPED_SLOT ) {}
        EquippedInstance_t( equipped_class_t unClass, equipped_slot_t unSlot )
            : m_unEquippedClass( unClass ), m_unEquippedSlot( unSlot ) {}
    ...
    {
        return IEconItemInterface::GetTradableAfterDateTime();
    }
   ...
   public:
    ...
    void SetDefinitionIndex( uint32 unDefinitionIndex )
    {
        m_unDefIndex = unDefinitionIndex;
    }
    ...
    void SetItemLevel( uint32 unItemLevel )
    {
        m_unLevel = unItemLevel;
    }
    ...
    void SetQuality( int32 nQuality )
    {
        m_nQuality = nQuality;
    }

    uint32 GetInventoryToken() const
    {
        return m_unInventory;
    }
    void SetInventoryToken( uint32 unToken )
    {
        m_unInventory = unToken;
    }
    ...
    void SetFlags( uint8 unFlags )
    {
        m_unFlags = unFlags;
    }

    void SetFlag( uint8 unFlag )
    {
        m_unFlags |= unFlag;
    }
    void ClearFlag( uint8 unFlag )
    {
        m_unFlags &= ~unFlag;
    }
    bool CheckFlags( uint8 unFlags ) const
    {
        return ( m_unFlags & unFlags ) != 0;
    }

    eEconItemOrigin GetOrigin() const
    {
        return ( eEconItemOrigin )m_unOrigin;
    }
    void SetOrigin( eEconItemOrigin unOrigin )
    {
        m_unOrigin = unOrigin;
        Assert( m_unOrigin == unOrigin );
    }
    bool IsForeign() const
    {
        return m_unOrigin == kEconItemOrigin_Foreign;
    }

    style_index_t GetStyle() const;
    void SetStyle( uint8 unStyle )
    {
        m_unStyle = unStyle;
        DirtyIconURL();
    }
    ...
    void SetDynamicAttributeValue( const CEconItemAttributeDefinition *pAttrDef, const T &value )
    {
        ...
        AssertMsg4( !::FindAttribute( GetItemDefinition(), pAttrDef ),
                    ...
                    GetItemID(),
                    GetItemDefinition()->GetDefinitionName(),
                    pAttrDef->GetDefinitionName(),
    ...
    bool BHasDynamicAttributes() const
    {
   ...
   public:
    ...
    const CEconItem *GetInteriorItem() const
    {
        return const_cast< CEconItem * >( this )->GetInteriorItem();
    }

    const CEconItemCustomData *GetCustomData() const
    {
...
class CCrateLootListWrapper
{
   public:
        ...
        : m_pLootList( NULL ), m_unAuditDetailData( 0 ), m_bIsDynamicallyAllocatedLootList( false )
    {
        ...
        if ( !BAttemptCrateSeriesInitialization( pEconItem ) && !BAttemptLootListStringInitialization( pEconItem ) && !BAttemptLineItemInitialization( pEconItem ) )
        {
