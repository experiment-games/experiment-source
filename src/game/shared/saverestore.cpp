...
/**
 *
 * Logic for pushing a datamap onto the Lua stack.
 *
 */
...
/**
 * Builds a Lua table containing all saveable keys and values for a given object.
 * This function inspects the object's data map (`datamap_t`) and recursively
 * constructs a Lua table that mirrors the object's saveable data structure.
 * The resulting table is left on the top of the Lua stack.
 *
...
/**
 * Pushes the value of a specific field from an object's datamap onto the Lua stack.
 * This function searches through the object's datamap hierarchy to find the requested
 * field and pushes only that field's value to the Lua stack.
 *
 * @param L The Lua state.
 * @param pObject A pointer to the C++ object instance (e.g., a CBaseEntity*).
 * @param pMap A pointer to the data map for the object's class.
 * @param pszFieldName The name of the field to retrieve.
...
///**
// * Recursively sets fields in an object's data map from a Lua table.
// * The Lua table should be at the top of the stack.
// */
//bool SetLuaDataMap( lua_State *L, void *pObject, datamap_t *pMap )
//{
//    if ( !pMap || !lua_istable( L, -1 ) )
//        return false;
//
//    bool success = true;
//
...
//    if ( pMap->baseMap )
//    {
//        if ( !SetLuaDataMap( L, pObject, pMap->baseMap ) )
//            success = false;
//    }
//
//    // Iterate through all fields in the current data map
...
//        typedescription_t *pField = &pMap->dataDesc[i];
//
//        // Only process fields marked for saving
//        if ( !( pField->flags & FTYPEDESC_SAVE ) )
//        {
//            continue;
//        }
//
//        // Check if this field exists in the Lua table
//        lua_pushstring( L, pField->fieldName );
//        lua_gettable( L, -2 );  // Get table[fieldName]
//
//        if ( lua_isnil( L, -1 ) )
//        {
//            lua_pop( L, 1 );  // Pop the nil value
//            continue;         // Skip fields that don't exist in the table
//        }
//
//        // Get a pointer to the actual data for this field
//        void *pFieldData = ( char * )pObject + pField->fieldOffset[TD_OFFSET_NORMAL];
//
//        // Set the field's value from the Lua stack
//        if ( !SetFieldFromLua( L, pFieldData, pField ) )
//        {
//            success = false;
//        }
//
...
/**
 * Sets the value of a specific field in an object's datamap from the value on top of the Lua stack.
 * This function searches through the object's datamap hierarchy to find the requested
 * field and sets its value from the Lua stack.
 *
 * @param L The Lua state. The value to set should be on top of the stack.
 * @param pObject A pointer to the C++ object instance (e.g., a CBaseEntity*).
 * @param pMap A pointer to the data map for the object's class.
