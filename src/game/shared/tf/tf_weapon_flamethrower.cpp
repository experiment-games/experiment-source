...
#if defined( CLIENT_DLL )
    : m_FlameEffects( this ), m_MmmmphEffect( this )
#endif
{
    WeaponReset();
...
void ExtinguishPlayer( CEconEntity *pExtinguisher, CTFPlayer *pOwner, CTFPlayer *pTarget, const char *pExtinguisherName )
{
    ...
    UTIL_LogPrintf( "\"%s<%i><%s><%s>\" triggered \"player_extinguished\" against \"%s<%i><%s><%s>\" with \"%s\" (attacker_position \"%d %d %d\") (victim_position \"%d %d %d\")\n",
                    ...
                    pOwner->GetTeam()->GetName(),
                    pTarget->GetPlayerName(),
                    pTarget->GetUserID(),
                    pTarget->GetNetworkIDString(),
                    pTarget->GetTeam()->GetName(),
                    pExtinguisherName,
                    ( int )pOwner->GetAbsOrigin().x,
                    ( int )pOwner->GetAbsOrigin().y,
                    ( int )pOwner->GetAbsOrigin().z,
                    ( int )pTarget->GetAbsOrigin().x,
                    ( int )pTarget->GetAbsOrigin().y,
                    ( int )pTarget->GetAbsOrigin().z );
}
...
void CTFFlameThrower::ComputeCrayAirBlastForce( CTFPlayer *pTarget, CTFPlayer *pPlayer, Vector &vecForward, Vector &vecOutForce )
{
    ...
    if ( bDebug )
    {
        ...
        NDebugOverlay::HorzArrow( vecDebugMuzzle, vecDebugMuzzle + vecAim * 5.f, flDebugOverlayWidth, 100, 0, 0, 255, true, flDebugOverlayDuration );
    }
    ...
    if ( bDebug )
    {
        ...
        NDebugOverlay::HorzArrow( vecDebugMuzzle, vecDebugMuzzle + vecResult * flDebugOverlayScale, flDebugOverlayWidth + 1.f, 255, 255, 255, 150, true, flDebugOverlayDuration );
...
bool CTFFlameThrower::DeflectEntity( CBaseEntity *pTarget, CTFPlayer *pOwner, Vector &vecForward )
{
    ...
    if ( ( pTarget->GetTeamNumber() == pOwner->GetTeamNumber() ) && !( g_pPasstimeLogic && ( g_pPasstimeLogic->GetBall() == pTarget ) ) )
    {
        return false;
    }
...
const char *CTFFlameThrower::FlameEffectName( bool bIsFirstPersonView )
{
    ...
    {
        case TF_FLAMETHROWER_MODE_PHLOG:
            return "drg_phlo_stream_new_flame";
        case TF_FLAMETHROWER_MODE_GIANT:
            return "flamethrower_giant_mvm_new_flame";
        case TF_FLAMETHROWER_MODE_RAINBOW:
...
const char *CTFFlameThrower::FlameCritEffectName( bool bIsFirstPersonView )
{
    ...
    {
        case TF_FLAMETHROWER_MODE_PHLOG:
            return "drg_phlo_stream_crit_new_flame";
        case TF_FLAMETHROWER_MODE_GIANT:
            return "flamethrower_crit_giant_mvm_new_flame";
        case TF_FLAMETHROWER_MODE_RAINBOW:
            return "flamethrower_rainbow_new_flame";
        default:
        {
...
const char *CTFFlameThrower::FullCritChargedEffectName( void )
{
    ...
    {
        case TF_TEAM_BLUE:
            return "medicgun_invulnstatus_fullcharge_blue";
        case TF_TEAM_RED:
            return "medicgun_invulnstatus_fullcharge_red";
        default:
            return "";
    }
}
