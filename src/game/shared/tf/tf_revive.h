...
class CTFReviveMarker : public CBaseAnimating
{
    ...
    void SetReviver( CTFPlayer *pPlayer )
    {
        m_pReviver = pPlayer;
    }
    CTFPlayer *GetOwner( void ) const
    {
        return m_hOwner;
    }
    CTFPlayer *GetReviver( void ) const
    {
        return m_pReviver;
    }
#ifdef GAME_DLL
    ...
    bool HasOwnerBeenPrompted( void )
    {
        return m_bOwnerPromptedToRevive;
    }
    void SetOwnerHasBeenPrompted( bool bValue )
    {
        m_bOwnerPromptedToRevive = bValue;
    }
#endif  // GAME_DLL
