...
class CPasstimeGun : public CTFWeaponBase, public ITFChargeUpWeapon
{
    ...
    virtual bool HasPrimaryAmmo() OVERRIDE
    {
        return true;
    }
    ...
    virtual bool IsEnergyWeapon() const OVERRIDE
    {
        return true;
    }
    ...
    struct AttackInputState
    {
        AttackInputState( int button )
            : iButton( button ), eButtonState( BUTTONSTATE_UP ), bLatchedUp( false )
        {
        }
        const int iButton;
        ...
        bool Is( EButtonState state ) const
        {
            return eButtonState == state;
        }
        void Disable()
        {
            eButtonState = BUTTONSTATE_DISABLED;
        }
        void Enable()
        {
