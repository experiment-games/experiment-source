...
class CTFLobbyShared : public GCSDK::ILobby
{
    ...
    class MatchPlayers_t : public CUtlIndexRange< int, MatchPlayers_t >
    {
        ...
        {
            return idx.value < m_nMemberCount;
        }
        bool BIsPending( MatchPlayers_t::index idx ) const
        {
            return !BIsMember( idx );
        }
        int Count() const
        {
            return end().value;
    ...
    // Inline helpers
    CSOTFGameServerLobby::State GetState() const
    {
        return GSObj().state();
    }
    const char *GetMissionName() const
    {
        return GSObj().mission_name().c_str();
    }
    ETFMatchGroup GetMatchGroup() const
    {
        return GSObj().has_match_group() ? ( ETFMatchGroup )GSObj().match_group() : k_eTFMatchGroup_Invalid;
    }
    uint64 GetMatchID( void ) const
    {
        return GSObj().match_id();
    }
    ...
    {
        return GSObj().map_name().c_str();
    }
    virtual GCSDK::PlayerGroupID_t GetGroupID() const OVERRIDE
    {
        return GSObj().lobby_id();
    }
    bool GetLateJoinEligible() const
    {
        return GSObj().late_join_eligible();
    }
    CSteamID GetServerID() const
    {
        return GSObj().server_id();
    }
    bool HasConnect() const
    {
        return GSObj().has_connect();
    }
    const char *GetConnect() const
    {
        return GSObj().connect().c_str();
    }
    uint32_t GetLobbyMMVersion() const
    {
        return GSObj().lobby_mm_version();
    }

#ifdef USE_MVM_TOUR
    // Returns name of tour that we are playing for.  Returns NULL if we are not playing for bragging rights!
    bool BHasMannUpTourName() const
    {
        return GSObj().has_mannup_tour_name();
    }
    ...
    virtual CSteamID GetPendingPlayerInviter( int ) const OVERRIDE
    {
        return k_steamIDNil;
    }
...
class ITFLobbyPlayer
{
    ...
    {
        return CSteamID( Proto().id() );
    }
    RTime32 GetLastConnectTime() const
    {
        return RTime32( Proto().last_connect_time() );
    }
    TF_GC_TEAM GetTeam() const
    {
        return Proto().team();
    }
    bool GetSquadSurplus() const
    {
        return Proto().squad_surplus();
    }
    uint32_t GetBadgeLevel() const
    {
        return Proto().badge_level();
    }
    CTFLobbyPlayerProto_ConnectState GetConnectState() const
    {
        return Proto().connect_state();
    };
    double GetNormalizedRating() const
    {
        return Proto().normalized_rating();
    };
    int GetRank() const
    {
        return Proto().rank();
    }
    bool GetChatSuspension() const
    {
        return Proto().chat_suspension();
    }

    bool BMatchPlayer() const
    {
        return Proto().type() == CTFLobbyPlayerProto_Type_MATCH_PLAYER;
    }
    ...
    GCSDK::PlayerGroupID_t GetOriginalPartyID() const
    {
        return Proto().original_party_id();
    };
...
class ConstTFLobbyPlayer : public ITFLobbyPlayer
{
    ...
    virtual const CTFLobbyPlayerProto &Proto() const final OVERRIDE
    {
        return m_proto;
    }
                        ...
                        public GCSDK::IPlayerGroupInvite
{
    ...
    virtual CSteamID GetInviter() const OVERRIDE
    {
        return k_steamIDNil;
    }  // Not supported/used in lobby invites
    virtual GCSDK::CSharedObject *GetSharedObject() OVERRIDE
    {
        return this;
    }
};
