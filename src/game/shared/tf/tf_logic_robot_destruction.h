...
class CTFRobotDestruction_RobotSpawn : public CBaseEntity
{
    ...
    CTFRobotDestruction_Robot* GetRobot() const
    {
        return m_hRobot.Get();
    }
    void OnRobotKilled();
    void ClearRobot();
    void SpawnRobot();
    void SetGroup( class CTFRobotDestruction_RobotGroup* pGroup )
    {
        m_hGroup.Set( pGroup );
    }
    // Inputs
...
class CTFRobotDestruction_RobotGroup : public CBaseEntity, public IRobotDestructionGroupAutoList
{
    ...
    float GetTeamRespawnScale() const
    {
        return m_flTeamRespawnReductionScale;
    }
    ...
    virtual int GetTeamNumber( void ) const OVERRIDE
    {
        return m_iTeamNum;
    }
    ...
    float GetRespawnStartTime() const
    {
        return m_flRespawnStartTime;
    }
    float GetRespawnEndTime() const
    {
        return m_flRespawnEndTime;
    }
    float GetLastAttackedTime() const
    {
...
#endif
{
    ...
    {
        return m_nMaxPoints.Get();
    }
    float GetFinaleWinTime( int nTeam ) const;
    float GetFinaleLength() const
    {
        return m_flFinaleLength;
    }
    ...
    RDScoreMethod_t GetLastScoreMethod( int nTeam ) const
    {
        return ( RDScoreMethod_t )m_eWinningMethod[nTeam];
    }
    ...
    const char* GetResFile() const
    {
        return STRING( m_szResFile );
    }
    ...
    float GetScoringInterval() const
    {
        return m_flRobotScoreInterval;
    }
    ...
    void DBG_SetMaxPoints( int nNewMax )
    {
        m_nMaxPoints.Set( nNewMax );
    }
    void InputRoundActivate( inputdata_t &inputdata );
    virtual int GetHealDistance( void )
    {
        return 64;
    }
#endif

    virtual void SetCountdownEndTime( float flTime )
    {
        m_flCountdownEndTime = flTime;
    }
    virtual float GetCountdownEndTime()
    {
        return m_flCountdownEndTime;
    }
    virtual CTFPlayer* GetTeamLeader( int iTeam ) const
    {
        return NULL;
    }
    virtual string_t GetCountdownImage( void )
    {
        return NULL_STRING;
    }
    virtual bool IsUsingCustomCountdownImage( void )
    {
        return false;
    }
