    ...
    if ( TFGameRules()->IsInTraining() )
    {
        ...
        switch ( GetTFWpnData().m_iWeaponType )
        {
            case TF_WPN_TYPE_PRIMARY:
                bVisible = training_can_select_weapon_primary.GetBool();
                break;
            case TF_WPN_TYPE_SECONDARY:
                bVisible = training_can_select_weapon_secondary.GetBool();
                break;
            case TF_WPN_TYPE_MELEE:
                bVisible = training_can_select_weapon_melee.GetBool();
                break;
            case TF_WPN_TYPE_BUILDING:
                bVisible = training_can_select_weapon_building.GetBool();
                break;
            case TF_WPN_TYPE_PDA:
                bVisible = training_can_select_weapon_pda.GetBool();
                break;
            case TF_WPN_TYPE_ITEM1:
                bVisible = training_can_select_weapon_item1.GetBool();
                break;
            case TF_WPN_TYPE_ITEM2:
                bVisible = training_can_select_weapon_item2.GetBool();
                break;
        }  // switch
        return bVisible;
    }
...
const char *CTFWeaponBase::GetTracerType( void )
{
    ...
    if ( tf_useparticletracers.GetBool() )
    {
        ...
        if ( pszTracerEffect && pszTracerEffect[0] )
        {
            if ( !m_szTracerName[0] )
            {
                Q_snprintf( m_szTracerName, MAX_TRACER_NAME, "%s_%s", pszTracerEffect, ( GetOwner() && GetOwner()->GetTeamNumber() == TF_TEAM_RED ) ? "red" : "blue" );
            }
