...
class CQuestMapRegion : public CTypedProtoBufScriptObjectDefinition< CMsgQuestMapRegionDef, DEF_TYPE_QUEST_MAP_REGION >
{
    ...
    const char* GetRegionNameLocToken() const
    {
        return m_msgData.name().c_str();
    }
    const char* GetResFile() const
    {
        return m_msgData.resfile().c_str();
    }
    int GetNumLinks() const
    {
        return m_msgData.links_size();
    }
    const CMsgQuestMapRegionDef_RegionLink& GetLink( int i ) const
    {
        return m_msgData.links( i );
    }
    virtual bool BPostDataLoaded( CUtlVector< CUtlString >* pVecErrors ) OVERRIDE;
    float GetRadioFreq() const
    {
        return m_msgData.radio_freq();
    }
    float GetZoomScale() const
    {
        return m_msgData.zoom_scale();
    }
    const CUtlVector< const CQuestMapNodeDefinition* >& GetNodesInRegion() const
    {
        return m_vecNodesWithin;
    }
    const CQuestMapStarType* GetStarType() const;

    const CQuestMapRegion* GetParent() const
    {
        return m_pParent;
    }
