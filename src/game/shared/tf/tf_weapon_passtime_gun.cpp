...
CPasstimeGun::CPasstimeGun()
    : m_flTargetResetTime( 0 ), m_attack( IN_ATTACK ), m_attack2( IN_ATTACK2 )
{
...
void CPasstimeGun::ItemPostFrame()
{
    ...
    if ( pOwner->m_Shared.HasPasstimeBall() )
    {
        ...
        // autopass
        if ( tf_passtime_experiment_autopass.GetBool() && m_attack2.Is( EButtonState::BUTTONSTATE_DOWN ) && pOwner->m_Shared.GetPasstimePassTarget() )
        {
    ...
    //
    if ( !pOwner->m_Shared.HasPasstimeBall() && ( ( m_eThrowState != THROWSTATE_THROWN ) || ( m_flNextPrimaryAttack <= gpGlobals->curtime ) ) )
    {
...
void CPasstimeGun::UpdateThrowArch()
{
    ...
    for ( int iPoint = 0; iPoint < iMaxTraces; ++iPoint )
    {
        ...
        UTIL_TraceHull( vecStart, vecEnd, -launchParams.traceHullSize, launchParams.traceHullSize, MASK_PLAYERSOLID, &traceFilter, &tr );
...
CPasstimeGun::LaunchParams CPasstimeGun::CalcLaunch( CTFPlayer *pPlayer, bool bHoming )
{
    ...
    auto fwdspeed = tf_passtime_throwspeed_velocity_scale.GetFloat() * params.viewFwd.Dot( pPlayer->GetAbsVelocity() );
