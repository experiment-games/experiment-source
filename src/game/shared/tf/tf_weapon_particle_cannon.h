...
class CTFParticleCannon : public CTFRocketLauncher
{
    ...
    {
        return TF_WEAPON_PARTICLE_CANNON;
    }
    ...
    virtual bool ShouldPlayFireAnim( void )
    {
        return !m_bChargedShot;
    }
    ...
    {
        return TF_PARTICLE_MAX_CHARGE_TIME;
    }
    ...
    bool CanChargeFire( void )
    {
        if ( Energy_FullyCharged() && ( m_flChargeBeginTime <= 0 ) )
            return true;
        else
            return false;
    }
