...
class CTFWeaponBaseGrenadeProj : public CBaseGrenade
{
    ...
    {
        return TF_BASE_PROJECTILE_GRENADE;
    }
#endif
    ...
    virtual void SetLauncher( CBaseEntity *pLauncher ) OVERRIDE
    {
        m_hLauncher = pLauncher;
        BaseClass::SetLauncher( pLauncher );
    }
    CBaseEntity *GetLauncher( void )
    {
        return m_hLauncher;
    }
    ...
    void ResetDeflected( void )
    {
        m_iDeflected = 0;
    }
    int GetDeflected( void )
    {
        return m_iDeflected;
    }
    void SetDeflectOwner( CBaseEntity *pPlayer )
    {
        m_hDeflectOwner = pPlayer;
    }
    CBaseEntity *GetDeflectOwner( void )
    {
        return m_hDeflectOwner;
    }
    ...
    {
        m_flDetonateTime = time;
    }
    float GetDetonateTime( void )
    {
    ...
    bool UseImpactNormal()
    {
        return m_bUseImpactNormal;
    }
    const Vector &GetImpactNormal( void ) const
    {
        return m_vecImpactNormal;
    }
    ...
    virtual bool IsDestroyable( bool bOrbAttack = false ) OVERRIDE
    {
        return ( !bOrbAttack ? ( gpGlobals->curtime > m_flDestroyableTime ) : true );
    }
