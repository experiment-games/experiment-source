...
bool CTFPlayer::IsAllowedToTaunt( void )
{
    ...
    if ( pActiveWeapon )
    {
        ...
        if ( pActiveWeapon->GetWeaponID() == TF_WEAPON_PDA_ENGINEER_BUILD || pActiveWeapon->GetWeaponID() == TF_WEAPON_PDA_ENGINEER_DESTROY )
            return false;
    }
                    ...
                    if ( m_aHealers[i].pHealScorer.IsValid() && m_aHealers[i].pHealer.IsValid() )
                    {
                        ...
                        if ( pHealer && IsAlly( pHealer ) )
                        {
                            ...
                            if ( pHealScorer )
                            {
                                ...
                                if ( ( m_flHealedPerSecondTimer <= gpGlobals->curtime || m_pOuter->GetHealth() >= m_pOuter->GetMaxHealth() ) && m_aHealers[i].flHealedLastSecond > 1 )
                                {
...
{
    bool bShouldDisplayCritBoostEffect = IsCritBoosted() || InCond( TF_COND_ENERGY_BUFF )
                                        //|| IsHypeBuffed()
...
void CTFPlayerShared::FadeInvis( float fAdditionalRateScale )
{
    ...
    float flInvisFadeTime = fAdditionalRateScale * ( tf_spy_invis_unstealth_time.GetFloat() * flDecloakRateScale );
        ...
        if ( m_hDisguiseWeapon )
        {
            ...
            if ( !m_hDisguiseWeapon->IsMeleeWeapon() )
            {
                ...
                if ( pDisguiseTarget )
                {
                    ...
                    {
                        m_iDisguiseAmmo = pWeapon->UsesClipsForAmmo1() ? pWeapon->Clip1() : pDisguiseTarget->GetAmmoCount( pWeapon->GetPrimaryAmmoType() );
                    }
                }
                ...
                if ( !m_iDisguiseAmmo )
                {
                    int nMaxCount = m_hDisguiseWeapon->UsesClipsForAmmo1() ? m_hDisguiseWeapon->GetMaxClip1() : m_pOuter->GetMaxAmmo( m_hDisguiseWeapon->GetPrimaryAmmoType(), m_nDisguiseClass );
...
bool CTFPlayerShared::CanRecieveMedigunChargeEffect( medigun_charge_types eType ) const
{
    ...
    if ( pItem && pItem->GetItemID() == TF_ITEM_CAPTURE_FLAG )
    {
        ...
        if ( ( eType == MEDIGUN_CHARGE_MEGAHEAL ) || ( eType == MEDIGUN_CHARGE_BULLET_RESIST ) || ( eType == MEDIGUN_CHARGE_BLAST_RESIST ) || ( eType == MEDIGUN_CHARGE_FIRE_RESIST ) )
        {
            bCanRecieve = true;
        }
    }
...
void CTFPlayer::GetHorriblyHackedRailgunPosition( const Vector &vStart, Vector *out_pvStartPos )
{
    ...
    *out_pvStartPos = vStart + ( vForward * 60.9f ) + ( vRight * 13.1f ) + ( vUp * -15.1f );
}
...
void CTFPlayer::FireBullet( CTFWeaponBase *pWpn, const FireBulletsInfo_t &info, bool bDoEffects, int nDamageType, int nCustomDamageType /*= TF_DMG_CUSTOM_NONE*/ )
{
    ...
    if ( bPenetratingShot && trace.m_pEnt )
    {
        if ( trace.m_pEnt->IsCombatCharacter() || trace.m_pEnt->IsBaseObject() )
        {
            ...
            Ray_t ray;
            ray.Init( vecStart, vecEnd, Vector( -penetrationHullExtension, -penetrationHullExtension, -penetrationHullExtension ), Vector( penetrationHullExtension, penetrationHullExtension, penetrationHullExtension ) );
...
void CTFPlayer::ImpactWaterTrace( trace_t &trace, const Vector &vecStart )
{
    ...
    trace_t traceWater;
    UTIL_TraceLine( vecStart, trace.endpos, ( MASK_SHOT | CONTENTS_WATER | CONTENTS_SLIME ), this, COLLISION_GROUP_NONE, &traceWater );
...
const char *CTFPlayer::GetOverrideStepSound( const char *pszBaseStepSoundName )
{
    ...
    struct override_sound_entry_t
    {
        int iOverrideIndex;
        const char *pszBaseSoundName;
        const char *pszNewSoundName;
    };
...
void CTFPlayerShared::CreateVehicleMove( float flInputSampleTime, CUserCmd *pCmd )
{
    ...
    else if ( pCmd->buttons & IN_BACK )  // Hitting the brakes
    {
        ...
        else
        {
            ...
            if ( ( pCmd->buttons & IN_BACK ) || m_pOuter->GetCurrentTauntMoveSpeed() < 0 || m_pOuter->GetVehicleReverseTime() < gpGlobals->curtime )
            {
...
//-----------------------------------------------------------------------------
CTFPlayer *CTFPlayerShared::GetPasstimePassTarget() const
{
    return m_hPasstimePassTarget.Get();
}
