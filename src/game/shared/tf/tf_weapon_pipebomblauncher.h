...
#endif
{
    ...
    virtual float GetChargeForceReleaseTime( void )
    {
        return GetChargeMaxTime();
    }
    int GetPipeBombCount( void )
    {
        return m_iPipebombCount;
    }
    ...
    bool CanDestroyStickies( void ) const
    {
        int iMode = 0;
        CALL_ATTRIB_HOOK_INT( iMode, stickies_detonate_stickies );
        return ( iMode == 1 );
    };
    ...
    virtual void SetInternalChargeBeginTime( float flChargeBeginTime )
    {
        m_flChargeBeginTime = flChargeBeginTime;
    }
    float GetInternalChargeBeginTime() const
    {
        return m_flChargeBeginTime;
    }
