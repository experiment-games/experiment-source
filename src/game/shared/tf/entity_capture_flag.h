...
class CCaptureFlag : public CTFItem, public ICaptureFlagAutoList
{
   public:
    ...
    bool IsPoisonous( void )
    {
        return m_flTimeToSetPoisonous > 0 && gpGlobals->curtime > m_flTimeToSetPoisonous;
    }
    float GetPoisonTime( void ) const
    {
        return m_flTimeToSetPoisonous;
    }

    bool IsVisibleWhenDisabled( void )
    {
        return m_bVisibleWhenDisabled;
    }

    CBaseEntity *GetPrevOwner( void )
    {
        return m_hPrevOwner.Get();
    }
...
// Game DLL Functions
    ...
    void ResetFlagNeutralTime( void )
    {
        m_flNeutralTime = 0;
    }
    ...
    float GetMaxResetTime()
    {
        return m_flMaxResetTime;
    }
   ...
   public:
    ...
    ETFFlagType GetType( void ) const
    {
        return ( ETFFlagType )m_nType.Get();
    }
   ...
   private:
    ...
    void SetGlowEnabled( bool bGlowEnabled )
    {
        m_bGlowEnabled = bGlowEnabled;
    }
#endif

    bool IsGlowEnabled( void )
    {
   ...
   private:
    ...
    {
        return m_nType == TF_FLAGTYPE_CTF || m_nType == TF_FLAGTYPE_ROBOT_DESTRUCTION || m_nType == TF_FLAGTYPE_RESOURCE_CONTROL;
    }
