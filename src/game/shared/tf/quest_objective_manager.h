...
class CBaseQuestObjectiveTracker : public CTFQuestEvaluator
{
   public:
    ...
    {
        return m_objectiveInstance.GetObjectiveDef()->GetDefIndex();
    }
    const QuestObjectiveInstance_t& GetObjectiveInstance() const
    {
        return m_objectiveInstance;
    }
    ...
    const CSteamID& GetTrackingPlayer() const
    {
...
#endif
{
   public:
    ...
    inline const CUtlVector< const CBaseQuestObjectiveTracker* >& GetObjectiveTrackers() const
    {
        return m_vecObjectiveTrackers;
    }

    uint32 GetEarnedPoints( uint32 eType ) const;
    const CQuest* GetItem() const
    {
        return m_pQuest;
    }
