...
CTFViewModel::CTFViewModel()
    : m_LagAnglesHistory( "CPredictedViewModel::m_LagAnglesHistory" ), m_bBodygroupsDirty( true )
{
    m_vLagAngles.Init();
...
void CViewModelInvisProxy::OnBind( C_BaseEntity *pEnt )
{
    ...
    if ( bIsViewModel == true )
    {
        ...
        flWeaponInvis = ( flPercentInvisible < 0.01 ) ? 0.0 : RemapVal( flPercentInvisible, 0.0, 1.0, TF_VM_MIN_INVIS, TF_VM_MAX_INVIS );
