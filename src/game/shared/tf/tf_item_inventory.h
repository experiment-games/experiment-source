...
class CTFPlayerInventory : public CPlayerInventory
{
    ...
    bool ClassLoadoutHasChanged( int iClass )
    {
        return m_bLoadoutChanged[iClass];
    }
    ...
    int GetActiveLocalPreset( equipped_class_t unClass )
    {
        return m_ActivePreset[unClass];
    }
...
class CTFInventoryManager : public CInventoryManager
{
    ...
    {
        return m_pBaseLoadoutItems.Count();
    }
    CEconItemView *GetBaseItem( int iIndex )
    {
...
class CEconNotification_HasNewItemsOnKill : public CEconNotification_HasNewItems
{
    ...
    static bool IsNotificationType( CEconNotification *pNotification )
    {
        return dynamic_cast< CEconNotification_HasNewItemsOnKill * >( pNotification ) != NULL;
    }
};
#endif
