...
class CTFWeaponInvis : public CTFWeaponBase
{
   public:
    ...
    int GetInvisType( void )
    {
        int iMode = 0;
        CALL_ATTRIB_HOOK_INT( iMode, set_weapon_mode );
        return iMode;
    };
    virtual bool HasFeignDeath( void )
    {
        return ( GetInvisType() == INVIS_FEIGN_DEATH );
    }
    virtual bool HasMotionCloak( void )
    {
        return ( GetInvisType() == INVIS_MOTION_CLOAK );
    }
