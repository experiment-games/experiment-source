...
class CQuestMapNodeDefinition : public CTypedProtoBufScriptObjectDefinition< CMsgQuestMapNodeDef, DEF_TYPE_QUEST_MAP_NODE >
{
   public:
    ...
    float GetXPos() const
    {
        return m_msgData.x_pos();
    }
    float GetYPos() const
    {
        return m_msgData.y_pos();
    }

    // The region we live in
    uint32 GetRegionDefIndex() const
    {
        return m_msgData.owning_region().defindex();
    }

    // Offered quests.  Nodes have a few potential quests that users can select from.
    uint32 GetNumOfferedQuests() const
    {
        return m_msgData.quest_options_size();
    }
    const CQuestDefinition* GetOfferedQuest( uint32 nIndex ) const;
    const char* GetRewardItem() const
    {
        return m_msgData.has_reward_item_name() ? m_msgData.reward_item_name().c_str() : NULL;
    }
    const char* GetRewardLootListName() const
    {
        return m_msgData.has_reward_lootlist_name() ? m_msgData.reward_lootlist_name().c_str() : NULL;
    }
    int GetRewardCredits() const;
    ENodeCashReward GetCashRewardType() const
    {
        return m_msgData.cash_reward();
    }
    const CEconOperationDefinition* GetAssociatedOperation() const;
    const EdgeVec_t& GetLinkedNodes() const
    {
        return m_vecConnectedNodes;
    }
    uint32 GetStarType() const;
    const CQuestMapStarType* GetStarTypeDef() const;
    uint32 GetNumStarsToUnlock() const
    {
...
class CQuestMapNode : public GCSDK::CProtoBufSharedObject< CSOQuestMapNode, k_EEconTypeQuestMapNode >
{
   public:
    ...
    bool BHasLootBeenClaimed() const
    {
        return Obj().loot_claimed();
    }
    ...
    const CEconOperationDefinition* GetAssociatedOperation() const
    {
        return ( GetNodeDefinition() ? GetNodeDefinition()->GetAssociatedOperation() : NULL );
    }
};
