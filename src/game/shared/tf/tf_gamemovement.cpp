...
ConVar tf_movement_aircurrent_friction_mult( "tf_movement_aircurrent_friction_mult", "0.75", FCVAR_REPLICATED | FCVAR_CHEAT, "Friction multiplier when sliding against surfaces while trapped in an air current" );
ConVar tf_movement_aircurrent_aircontrol_mult( "tf_movement_aircurrent_aircontrol_mult", "0.25", FCVAR_REPLICATED | FCVAR_CHEAT, "Multiplier on air control when player is in an air current (such as airblast)" );
ConVar tf_movement_lost_footing_restick( "tf_movement_lost_footing_restick", "50.0", FCVAR_REPLICATED | FCVAR_CHEAT, "Early escape the lost footing condition if the player is moving slower than this across the ground" );
ConVar tf_movement_lost_footing_friction( "tf_movement_lost_footing_friction", "0.1", FCVAR_REPLICATED | FCVAR_CHEAT, "Ground friction for players who have lost their footing" );
...
{
    // Handle control stun.
    if ( m_pTFPlayer->m_Shared.IsControlStunned() || m_pTFPlayer->m_Shared.IsLoserStateStunned() )
    {
            ...
            if ( traceresult.startsolid )
            {
                if ( m_pTFPlayer->m_playerMovementStuckTimer.HasStarted() && m_pTFPlayer->m_playerMovementStuckTimer.IsElapsed() )
                {
                    ...
                    UTIL_LogPrintf( "\"%s<%i><%s><%s>\" startsolid killed (position \"%3.2f %3.2f %3.2f\")\n",
                                    ...
                                    m_pTFPlayer->GetAbsOrigin().x,
                                    m_pTFPlayer->GetAbsOrigin().y,
                                    m_pTFPlayer->GetAbsOrigin().z );
...
extern void TracePlayerBBoxForGround( const Vector &start, const Vector &end, const Vector &minsSrc, const Vector &maxsSrc, IHandleEntity *player, unsigned int fMask, int collisionGroup, trace_t &pm );
