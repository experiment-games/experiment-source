...
{
    return BIsActive() && BHasPassToUnlock( helper ) && BHasRequiredNodesCompltedToUnlock( helper ) && BHasStarsToUnlock( helper );
}
...
bool CQuestMapNodeDefinition::BConditionsMet_Recursive( const CMsgQuestMapNodeCondition& msgCondition, const MapQuestNodes_t& nodes, CUtlMap< uint32, bool >* pMapOutConnectedNodeCondition ) const
{
...
bool CQuestMapNode::BIsMedalEarned( EQuestPoints eType ) const
{
    switch ( eType )
    {
        case QUEST_POINTS_NOVICE:
            return Obj().star_0_earned() != 0;
        case QUEST_POINTS_ADVANCED:
            return Obj().star_1_earned() != 0;
        case QUEST_POINTS_EXPERT:
            return Obj().star_2_earned() != 0;
        default:
            Assert( false );
            return false;
    }
