...
class CTFGrenadePipebombProjectile : public CTFWeaponBaseGrenadeProj
{
    ...
    void SetChargeTime( float flChargeTime )
    {
        m_flChargeTime = flChargeTime;
    }
    ...
    void SetFullDamage( float flFullDamage )
    {
        m_flFullDamage = flFullDamage;
    }
    ...
    void SetDetonateOnPulse( bool bDet )
    {
        m_bDetonateOnPulse = bDet;
    }
    ...
    {
        if ( m_bPulsed ) m_bHighlight = bHighlight;
    }
    ...
    // Creation.
    static CTFGrenadePipebombProjectile *Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseCombatCharacter *pOwner, const CTFWeaponInfo &weaponInfo, int iPipeBombType, float flMultDmg );
    ...
    void SetCanTakeDamage( bool bCanTakeDamage )
    {
        m_bCanTakeDamage = bCanTakeDamage;
    }
    ...
    virtual bool IsDeflectable( void ) OVERRIDE
    {
        return true;
    }
