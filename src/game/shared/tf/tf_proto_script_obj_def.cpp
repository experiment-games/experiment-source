...
void SpewUniversalFieldID( const CMsgUniversalFieldID& universalFieldID )
{
    ...
    DevMsg( "\"%s\" (%d): ", pDef->GetName(), pDef->GetDefIndex() );
...
{
    SpewOnDestruct( CUtlVector< CUtlString >& vecErrors )
        : m_vecErrors( vecErrors ) {}
    ~SpewOnDestruct()
    {
...
bool CProtoBufScriptObjectDefinitionManager::BInitDefinitions()
{
...
#if defined( TF_DLL ) || defined( TF_CLIENT_DLL )
    {
        ...
        CUtlString sSignatureFilename( g_pszProtoDefFile );
        sSignatureFilename.Append( ".sig" );
        ...
        bool bSignatureValid = CheckValveSignature(
            buffer.Base(), buffer.TellPut(), bufSignatureBinary.Base(), bufSignatureBinary.TellPut() );
        ...
        if ( !bSignatureValid )
        {
            bSignatureValid = CheckValveSignature(
                "", 0, bufSignatureBinary.Base(), bufSignatureBinary.TellPut() );
        }
