...
class CTFParty : public GCSDK::CProtoBufSharedObject< CSOTFParty, k_EProtoObjectTFParty >, public GCSDK::IParty
{
   ...
   public:
    ...
    virtual bool BShouldDeleteByCache() const OVERRIDE
    {
        return false;
    }
    ...
    virtual CSharedObject* GetSharedObjectForMember( const CSteamID& steamID ) OVERRIDE
    {
        return this;
    }
    ...
    {
        return BaseClass::Dump();
    }

    virtual const CSteamID GetLeader() const OVERRIDE;

    uint64 GetAssociatedLobbyID() const
    {
        return Obj().associated_lobby_id();
    }
    ETFMatchGroup GetAssociatedLobbyMatchGroup() const
    {
        return Obj().associated_lobby_match_group();
    }

    virtual int GetNumMembers() const OVERRIDE
    {
        return Obj().member_ids_size();
    }
    ...
    {
        return Obj().pending_members_size();
    }
    ...
    {
        return Obj().matchmaking_queues_size();
    }
    ETFMatchGroup GetQueueEntryMatchGroup( int i ) const
    {
        return Obj().matchmaking_queues( i ).match_group();
    }
    RTime32 GetQueueEntryStartTime( int i ) const
    {
        return Obj().matchmaking_queues( i ).queued_time();
    }
    int GetQueueEntryIdxByMatchGroup( ETFMatchGroup eMatchGroup ) const;
    bool BQueuedForMatchGroup( ETFMatchGroup eMatchGroup ) const
    {
        return GetQueueEntryIdxByMatchGroup( eMatchGroup ) != -1;
    }
