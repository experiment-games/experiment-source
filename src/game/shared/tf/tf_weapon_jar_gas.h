...
class CTFJarGas : public CTFJar
{
   public:
    ...
    virtual float GetDefaultItemChargeMeterValue( void ) const OVERRIDE
    {
        return 0.f;
    }
#else
    ...
    virtual const char *GetEffectLabelText( void ) OVERRIDE
    {
        return "#TF_Gas";
    }
    virtual bool ShouldUpdateMeter() const OVERRIDE;
    virtual float InternalGetEffectBarRechargeTime( void ) OVERRIDE
    {
        return 0.f;
    }
...
class CTFProjectile_JarGas : public CTFProjectile_Jar
{
   public:
...
#ifdef GAME_DLL
    static CTFProjectile_JarGas *Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseCombatCharacter *pOwner, const CTFWeaponInfo &weaponInfo );

    virtual void SetCustomPipebombModel() OVERRIDE;

    virtual const char *GetImpactEffect() OVERRIDE
    {
        return ( GetTeamNumber() == TF_TEAM_BLUE ) ? "gas_can_impact_blue" : "gas_can_impact_red";
    }
    virtual ETFCond GetEffectCondition( void ) OVERRIDE
    {
        return TF_COND_GAS;
    }
    virtual const char *GetExplodeSound() OVERRIDE
    {
        return TF_WEAPON_JARGAS_EXPLODE_SOUND;
    }
    ...
    virtual int GetWeaponID( void ) const OVERRIDE
    {
        return TF_WEAPON_GRENADE_JAR_GAS;
    }
...
class CTFGasManager : public CTFPointManager
{
   ...
   protected:
    ...
    {
        return TF_GAS_POINT_RADIUS;
    }
    virtual int GetMaxPoints() const OVERRIDE
    {
        return 20;
    }
    virtual bool ShouldIgnoreStartSolid( void ) OVERRIDE
    {
        return true;
    }
