...
bool CTFItemDefinition::BInitFromKV( KeyValues *pKVItem, CUtlVector< CUtlString > *pVecErrors )
{
    ...
    if ( *pszLoadoutSlot )
    {
        ...
        SCHEMA_INIT_CHECK(
            ...
            "Item definition %i \"%s\" used unknown loadout slot: %s!",
            GetDefinitionIndex(),
            GetItemBaseName(),
            pszLoadoutSlot );
    }
    ...
    SCHEMA_INIT_CHECK(
        ...
        "Item definition %i \"%s\" used in both wearable and not wearable slots!",
        GetDefinitionIndex(),
        GetItemBaseName() );
    ...
    if ( pTauntKV )
    {
        ...
        SCHEMA_INIT_CHECK(
            ...
            "Item definition %i \"%s\" failed to initialize taunt data!",
            GetDefinitionIndex(),
            GetItemBaseName() );
    }
...
bool CTFItemDefinition::IsContentStreamable() const
{
    ...
    return item_enable_content_streaming.GetBool() && CEconItemDefinition::IsContentStreamable();
}
...
CTFItemSchema::CTFItemSchema()
    : m_mapQuestObjectiveConditions( DefLessFunc( ObjectiveConditionDefIndex_t ) ), m_mapWars( DefLessFunc( WarDefinitionMap_t::KeyType_t ) ), m_mapGameCategories( DefLessFunc( GameCategoryMap_t::KeyType_t ) ), m_mapMMGroups( DefLessFunc( MMGroupMap_t::KeyType_t ) )
{
    ...
    FOR_EACH_TRUE_SUBKEY( pKVMvmMaps, pKVMap )
    {
        ...
        if ( pKVMissions )
        {
            ...
            FOR_EACH_TRUE_SUBKEY( pKVMissions, pKVMission )
            {
                ...
                if ( ( Q_stricmp( mission.m_sPop.Get(), map.m_sMap.Get() ) != 0 ) && ( Q_strnicmp( mission.m_sPop.Get(), map.m_sMap.Get(), nMapNameLen ) != 0 || mission.m_sPop.Get()[nMapNameLen] != '_' ) )
                {
                    pVecErrors->AddToTail( CUtlString( CFmtStr(
...
bool CTFItemSchema::BCanStrangeFilterApplyToStrangeSlotInItem( uint32 /*strange_event_restriction_t*/ unRestrictionType, uint32 unRestrictionValue, const IEconItemInterface *pItem, int iStrangeSlot, uint32 *out_pOptionalScoreType ) const
{
    ...
    if ( unStrangeScoreType == kKillEaterEvent_CosmeticOperationContractsCompleted || unStrangeScoreType == kKillEaterEvent_CosmeticOperationKills || unStrangeScoreType == kKillEaterEvent_CosmeticOperationContractsPoints || unStrangeScoreType == kKillEaterEvent_CosmeticOperationBonusPoints || unStrangeScoreType == kKillEaterEvent_ContractPointsEarned || unStrangeScoreType == kKillEaterEvent_ContractPointsContributedToFriends )
    {
        return false;
    }
