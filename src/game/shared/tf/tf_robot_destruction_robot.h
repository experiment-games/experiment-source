...
{
    RobotSpawnData_t()
        : m_eType( ROBOT_TYPE_FRUSTUM ), m_nRobotHealth( 100 ), m_nPoints( 0 ), m_nNumGibs( 0 ), m_pszPathName( NULL ), m_pszGroupName( NULL )
    {
    }
...
#endif
{
    ...
    virtual float GetHealRate() const OVERRIDE
    {
        return 5.f;
    }

    virtual int DispenseMetal( CTFPlayer * ) OVERRIDE
    {
        return 0;
    }
    virtual bool DispenseAmmo( CTFPlayer * ) OVERRIDE
    {
        return false;
    }
...
#endif
{
    ...
    virtual int GetHealth( void ) const OVERRIDE
    {
        return m_iHealth;
    }
    ...
    virtual bool IsProjectileCollisionTarget( void ) const OVERRIDE
    {
        return true;
    }

    void RepairSelfThink( void );
    bool GetShieldedState( void ) const
    {
        return m_bShielded;
    }
    CPathTrack *GetNextPath( void ) const
    {
        return m_hNextPath;
    }
    void ArriveAtPath( void );
    void SetRobotSpawnData( const RobotSpawnData_t &data )
    {
        m_spawnData = data;
        m_eType = data.m_eType;
    }
    const RobotSpawnData_t &GetRobotSpawnData() const
    {
        return m_spawnData;
    }
    ...
    void SetSpawn( class CTFRobotDestruction_RobotSpawn *pSpawn )
    {
        m_hSpawn.Set( pSpawn );
    }
    ...
    // INextBot
    virtual CRobotIntention *GetIntentionInterface( void ) const
    {
        return m_intention;
    }
    virtual CRobotLocomotion *GetLocomotionInterface( void ) const
    {
        return m_locomotor;
    }
    virtual CHeadlessHatmanBody *GetBodyInterface( void ) const
    {
        return m_body;
    }
    void SetNewActivity( Activity activity );
    void SetIsPanicked( bool bPanicked )
    {
        m_bIsPanicked = bPanicked;
    }
    bool GetIsPanicked( void ) const
    {
        return m_bIsPanicked;
    }
