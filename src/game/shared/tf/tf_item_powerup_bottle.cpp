...
void CEquipMvMCanteenNotification::Accept()
{
    ...
    for ( int i = 0; i < pLocalInv->GetItemCount(); ++i )
    {
        ...
        if ( pItem->GetItemDefinition() == pItemDef_Robo || pItem->GetItemDefinition() == pItemDef_KritzOrTreat || pItem->GetItemDefinition() == pItemDef_Canteen || pItem->GetItemDefinition() == pItemDef_DefaultCanteen )
        {
            pCanteen = pItem;
            break;
        }
