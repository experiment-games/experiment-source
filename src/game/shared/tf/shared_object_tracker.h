...
class CBaseSOTracker
{
    ...
    const CSharedObject* GetSObject() const
    {
        return m_pSObject;
    }
    const CSteamID GetOwnerSteamID() const
    {
...
struct CommitRecord_t
{
    CommitRecord_t( ::google::protobuf::Message* pMessage )
        : m_flLastCommitTime( Plat_FloatTime() ), m_flReportedTime( Plat_FloatTime() ), m_pProtoMsg( pMessage )
    {
    }
...
#endif
{
    ...
    template < class T >
    T GetTypedTracker( SOTrackerMap_t::KeyType_t nKey ) const
    {
        return assert_cast< T >( GetTracker( nKey ) );
    }
