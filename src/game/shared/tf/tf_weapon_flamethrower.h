...
#endif  // GAME_DLL
{
   ...
   public:
    ...
    virtual bool IsFiring( void ) const OVERRIDE
    {
        return m_iWeaponState == FT_STATE_FIRING;
    }
    ...
    int GetFlameThrowerMode( void ) const
    {
        int iMode = 0;
        CALL_ATTRIB_HOOK_INT( iMode, set_weapon_mode );
        return iMode;
    }
    bool IsCritFire( void )
    {
        return m_bCritFire;
    }
...
class CTFFlameEntity : public CBaseEntity, public ITFFlameEntityAutoList
{
   ...
   public:
    ...
    void SetCritFromBehind( bool bState )
    {
        m_bCritFromBehind = bState;
    }
    bool IsEntityAttacker( CBaseEntity *pEnt )
    {
