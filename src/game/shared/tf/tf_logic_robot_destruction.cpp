...
void cc_tf_rd_score_blue_points( const CCommand &args )
{
    ...
    if ( CTFRobotDestructionLogic::GetRobotDestructionLogic() )
        CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints( TF_TEAM_BLUE, nPoints, SCORE_CORES_COLLECTED, NULL );
}
...
void cc_tf_rd_score_red_points( const CCommand &args )
{
    ...
    if ( CTFRobotDestructionLogic::GetRobotDestructionLogic() )
        CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints( TF_TEAM_RED, nPoints, SCORE_CORES_COLLECTED, NULL );
}
...
void CTFRobotDestruction_RobotSpawn::Activate()
{
    ...
    if ( pEnt != pGroup )
    {
        const char *pszMsg = CFmtStr( "%s specified '%s' as its group, but %s is a %s", STRING( GetEntityName() ), m_spawnData.m_pszGroupName, m_spawnData.m_pszGroupName, pEnt->GetClassname() );
    ...
    if ( pPath != pEnt )
    {
        const char *pszMsg = CFmtStr( "%s specified '%s' as its first path, but %s is a %s", STRING( GetEntityName() ), m_spawnData.m_pszPathName, m_spawnData.m_pszPathName, pEnt->GetClassname() );
        AssertMsg( 0, "%s", pszMsg );
        Warning( "%s", pszMsg );
    }
    else if ( pEnt == NULL )
    {
        const char *pszMsg = CFmtStr( "%s specified '%s' as its first path, but %s doesn't exist", STRING( GetEntityName() ), m_spawnData.m_pszPathName, m_spawnData.m_pszPathName );
        AssertMsg( 0, "%s", pszMsg );
        Warning( "%s", pszMsg );
    }
#endif
...
CTFRobotDestruction_RobotGroup::CTFRobotDestruction_RobotGroup()
    : m_flRespawnTime( 0.f ), m_nTeamNumber( 0 )
{
...
void CTFRobotDestructionLogic::AddRobotGroup( CTFRobotDestruction_RobotGroup *pGroup )
{
    ...
    FOR_EACH_VEC( m_vecSpawnGroups, i )
    {
        Assert( m_vecSpawnGroups[i]->GetGroupNumber() != pGroup->GetGroupNumber() || m_vecSpawnGroups[i]->GetTeamNumber() != pGroup->GetTeamNumber() );
    }
...
int CRobotDestructionVaultTrigger::StealPoints( CTFPlayer *pPlayer )
{
    ...
    if ( pFlag && CTFRobotDestructionLogic::GetRobotDestructionLogic() )
    {
        ...
        if ( nEnemyPoints )
        {
            ...
            CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints( nEnemyTeamNumber, -nPointsToSteal, SCORE_REACTOR_STEAL, pPlayer );
