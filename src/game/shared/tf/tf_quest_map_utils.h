...
#endif
{
...
#endif
    CSteamID GetOwnerSteamID() const
    {
        return m_steamID;
    }
    const CQuest* GetActiveQuest() const;

    uint32 GetNumCollectedMedals( EQuestPoints eType ) const
    {
        return m_nCollectedMedals[eType];
    }
    uint32 GetNumTotalMedals( EQuestPoints eType ) const
    {
        return m_nTotalMedals[eType];
    }
#ifndef GAME_DLL
    int GetNumRewardCredits() const;
#endif  // GAME_DLL
    int GetNumCurrentlyUnlockableNodes() const
    {
        return m_nLockedAndRequirementsMetNodes;
    }
    int GetNumUnlockedNodes() const
    {
        return m_nNumUnlockedNodes;
    }
    ...
    const MapQuestNodes_t& GetQuestMapNodes() const
    {
        return m_mapNodes;
    }
    ...
    QUESTHELPER_CONSTNESS CUtlVector< CQuest* >& GetAllQuests() QUESTHELPER_CONSTNESS
    {
        return m_vecQuests;
    }
