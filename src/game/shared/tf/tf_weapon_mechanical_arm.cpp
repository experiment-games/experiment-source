...
void CTFMechanicalArm::SecondaryAttack( void )
{
    ...
    if ( ShockAttack() )
    {
        ...
        Vector vecSrc = pOwner->EyePosition() + ( vecForward * 40.f ) + ( vecRight * 15.f ) + ( vecUp * -10.f );
...
void CTFMechanicalArm::UpdateParticleBeam()
{
    ...
    if ( pFiringPlayer && pFiringPlayer->m_nButtons & IN_ATTACK && pFiringPlayer->GetActiveWeapon() == this && pFiringPlayer->GetWaterLevel() != WL_Eyes && pFiringPlayer->m_flNextAttack < gpGlobals->curtime )
    {
        trace_t tr;
        Vector vecAiming;
