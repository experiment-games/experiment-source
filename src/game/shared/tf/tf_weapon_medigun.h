...
class CWeaponMedigun : public CTFWeaponBaseGun
{
    ...
    virtual bool AppliesModifier( void )
    {
        return true;
    }
    ...
    CBaseEntity *GetHealTarget( void )
    {
        return IsAttachedToBuilding() ? NULL : m_hHealingTarget.Get();
    }
    ...
    CBaseEntity *GetMostRecentHealTarget( void )
    {
        return m_hLastHealingTarget.Get();
    }
    void RecalcEffectOnTarget( CTFPlayer *pPlayer, bool bInstantRemove = false );
#endif

    float GetReleaseStartedAt( void )
    {
        return m_flReleaseStartedAt;
    }
    ...
    void ForceHealingTargetUpdate( void )
    {
