...
{
    V_snprintf( pchName, nBuffSize, "%s_%s", ( ( m_szHudIcon[0] != '\0' ) ? ( m_szHudIcon.Get() ) : ( TF_FLAG_ICON ) ), ( ( nTeam == TF_TEAM_BLUE ) ? ( "blue" ) : ( "red" ) ) );
}
...
{
    V_snprintf( pchName, nBuffSize, "effects/%s_%s.vmt", ( ( m_szTrailEffect[0] != '\0' ) ? ( m_szTrailEffect.Get() ) : ( TF_FLAG_TRAIL ) ), ( ( nTeam == TF_TEAM_RED ) ? ( "red" ) : ( "blu" ) ) );
}
...
int CCaptureFlag::GetReturnTimeShotClockMode( int nStartReturnTime )
{
    ...
    if ( IsFlagShotClockModePossible() )
    {
        if ( m_bUseShotClockMode )
        {
            ...
            float flCreditTime = ( gpGlobals->curtime - m_flLastPickupTime ) * tf_flag_return_time_credit_factor.GetFloat() + m_flLastResetDuration;
...
void CCaptureFlag::Reset( void )
{
    ...
    if ( m_nType == TF_FLAGTYPE_ROBOT_DESTRUCTION && !IsDisabled() )
    {
        if ( m_nPointValue > 0 )
        {
            ...
            {
                CTFRobotDestructionLogic::GetRobotDestructionLogic()->ScorePoints( GetTeamNumber(), m_nPointValue.Get(), SCORE_REACTOR_RETURNED, NULL );
