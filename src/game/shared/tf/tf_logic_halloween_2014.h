                    ...
                    public IMinigameAutoList
{
    ...
    const bool AllowedInRandom() const
    {
        return m_bMinigameAllowedInRamdomPool;
    }
    virtual void UpdateDeadPlayers( int nTeam, COutputEvent &eventWin, COutputEvent &eventAllDead, bool &bCanWin );
    EMinigameType GetMinigameType() const
    {
        return m_eMinigameType;
    }
    void SetAdvantagedTeam( int iAdvantageTeam )
    {
        m_iAdvantagedTeam = iAdvantageTeam;
    }
#else
    const char *GetResFile() const
    {
        return m_pszHudResFile;
    }
    int GetMaxScore( void ) const
    {
        return m_nMaxScoreForMiniGame;
    }
...
class CTFMinigameLogic : public CBaseEntity
{
    ...
    static CTFMinigameLogic *GetMinigameLogic()
    {
        return m_sMinigameLogic;
    }
    CTFMiniGame *GetActiveMinigame() const
    {
        return m_hActiveMinigame;
    }
#ifdef GAME_DLL
    DECLARE_DATADESC();
    virtual int UpdateTransmitState() OVERRIDE
