...
#endif
{
    ...
    virtual float UberChargeAmmoPerShot( void )
    {
        float fAmmo = 0;
        CALL_ATTRIB_HOOK_FLOAT( fAmmo, ubercharge_ammo );
        return fAmmo * 0.01f;
    }

    virtual int Clip1()
    {
        return IsEnergyWeapon() ? Energy_GetEnergy() : m_iClip1;
    }
    virtual int Clip2()
    {
        return m_iClip2;
    ...
    bool IsCurrentAttackACrit() const
    {
        return m_bCurrentAttackIsCrit;
    }
    bool IsCurrentAttackARandomCrit() const
    {
        return m_bCurrentAttackIsCrit && m_bCurrentCritIsRandom;
    }
    bool IsCurrentAttackDuringDemoCharge() const
    {
        return m_bCurrentAttackIsDuringDemoCharge;
    }
    ...
    float GetLastPrimaryAttackTime( void ) const
    {
        return m_flLastPrimaryAttackTime;
    }
    virtual bool CanPerformSecondaryAttack() const OVERRIDE;
    virtual bool IsFiring( void ) const
    {
        return false;
    }
    ...
    virtual bool CheckReloadMisfire( void )
    {
        return false;
    }
    ...
    {
        return GetTFWpnData().m_iWeaponType;
    }
    ...
    virtual float GetDeflectionRadius() const
    {
        return 128.f;
    }
    ...
    virtual int GetCanAttackFlags() const
    {
        return TF_CAN_ATTACK_FLAG_NONE;
    }
    ...
    // Muzzleflashes
    virtual const char *GetMuzzleFlashEffectName_3rd( void )
    {
        return NULL;
    }
    virtual const char *GetMuzzleFlashEffectName_1st( void )
    {
        return NULL;
    }
    ...
    virtual bool IsBlastImpactWeapon( void ) const
    {
        return false;
    }
    float Energy_GetMaxEnergy( void ) const;
    float Energy_GetEnergy( void ) const
    {
        return m_flEnergy;
    }
    void Energy_SetEnergy( float flEnergy )
    {
        m_flEnergy = flEnergy;
    }
    ...
    virtual bool UseServerRandomSeed( void ) const
    {
        return true;
    }
...
// Server specific.
    ...
    virtual float GetLastHitTime( void )
    {
        return m_flLastHitTime;
    }

    virtual int GetDropSkinOverride( void )
    {
        return -1;
    }

    int GetKillStreak() const
    {
    ...
    float GetClipScale() const
    {
        return m_flClipScale;
    }
    void SetClipScale( float flScale )
    {
        m_flClipScale = flScale;
    }

    virtual float GetInitialAfterburnDuration() const
    {
        return 0.f;
    }
    virtual float GetAfterburnRateOnHit() const
    {
        return 0.f;
    }
...
// Client specific.
    ...
    {
        return ( m_iState == WEAPON_IS_ACTIVE ) && !IsBeingRepurposedForTaunt();
    }
    ...
    virtual const char *GetEffectLabelText()
    {
        return "";
    }
    ...
    bool IsUsingOverrideModel() const
    {
        return m_iWorldModelIndex != m_iCachedModelIndex;
    }
    ...
    void SetIsBeingRepurposedForTaunt( bool bCanOverride )
    {
        m_bBeingRepurposedForTaunt = bCanOverride;
    }
    bool IsBeingRepurposedForTaunt() const
    {
        return m_bBeingRepurposedForTaunt;
    }

    int GetKillComboClass( void ) const
    {
        return m_nKillComboClass;
    }
    int GetKillComboCount( void ) const
    {
        return m_nKillComboCount;
    }
    void ClearKillComboCount( void )
    {
        m_nKillComboCount = 0;
    }
    ...
    bool HasEffectBarRegeneration( void )
    {
        return InternalGetEffectBarRechargeTime() > 0;
    }  // Check the base, not modified by attribute, because attrib may have reduced it to 0.
    ...
    void DecrementBarRegenTime( float flTime )
    {
    ...
    virtual bool CanPickupOtherWeapon() const
    {
        return true;
    }
    ...
    {
        return 0;
    }  // Time it takes for this regeneration bar to fully recharge from 0 to full.
    ...
    TFWeaponInspectStage GetInspectStage() const
    {
        return ( TFWeaponInspectStage )m_nInspectStage.Get();
    }
    float GetInspectAnimEndTime() const
    {
        return m_flInspectAnimEndTime;
    }
...
class CTFWeaponAttachmentModel : public CBaseAnimating, public IHasOwner
{
    ...
    void SetWeaponAssociatedWith( CTFWeaponBase *pWeapon )
    {
        m_hWeaponAssociatedWith = pWeapon;
    }
    CBaseEntity *GetWeaponAssociatedWith( void ) const
    {
        return m_hWeaponAssociatedWith.Get();
    }

    bool BIsViewModelAttachment()
    {
