...
BallGet::BallGet( IGameEvent *pEvent )
    : ownerIndex( pEvent->GetInt( s_keyOwnerIndex ) ), team( pEvent->GetInt( s_keyTeam ) )
{
    Assert( IsType< BallGet >( pEvent ) );
}

BallGet::BallGet( int ownerIndex_, int team_ )
    : ownerIndex( ownerIndex_ ), team( team_ )
{
}
...
Score::Score( IGameEvent *pEvent )
    : scorerIndex( pEvent->GetInt( s_keyScorerIndex ) ), assisterIndex( pEvent->GetInt( s_keyAssisterIndex ) ), numPoints( pEvent->GetInt( s_keyNumPoints ) )
{
...
Score::Score( int scorerIndex_, int numPoints_ )
    : scorerIndex( scorerIndex_ ), assisterIndex( -1 ), numPoints( numPoints_ )
{
}
...
BallFree::BallFree()
    : ownerIndex( -1 ), attackerIndex( -1 )
{
}

BallFree::BallFree( int ownerIndex_ )
    : ownerIndex( ownerIndex_ ), attackerIndex( -1 )
{
}

BallFree::BallFree( int ownerIndex_, int attackerIndex_ )
    : ownerIndex( ownerIndex_ ), attackerIndex( attackerIndex_ )
{
}
...
PassCaught::PassCaught( IGameEvent *pEvent )
    : passerIndex( pEvent->GetInt( s_keyPasserIndex ) ), catcherIndex( pEvent->GetInt( s_keyCatcherIndex ) ), dist( pEvent->GetFloat( s_keyDist ) ), duration( pEvent->GetFloat( s_keyDuration ) )
{
    Assert( IsType< PassCaught >( pEvent ) );
}

PassCaught::PassCaught()
    : passerIndex( -1 ), catcherIndex( -1 ), dist( 0 ), duration( 0 )
{
}
...
BallStolen::BallStolen( int victimIndex_, int attackerIndex_ )
    : victimIndex( victimIndex_ ), attackerIndex( attackerIndex_ )
{
}
...
BallBlocked::BallBlocked()
    : ownerIndex( -1 ), blockerIndex( -1 )
{
}

BallBlocked::BallBlocked( int ownerIndex_, int blockerIndex_ )
    : ownerIndex( ownerIndex_ ), blockerIndex( blockerIndex_ )
{
}
