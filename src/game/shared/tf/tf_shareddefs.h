...
inline bool IsValidTFPlayerClass( int iClass )
{
    return iClass >= TF_FIRST_NORMAL_CLASS && iClass < TF_LAST_NORMAL_CLASS;
}
inline bool IsValidTFTeam( int iTeam )
{
    return iTeam == TF_TEAM_RED || iTeam == TF_TEAM_BLUE;
}
...
{
    return AlmostEqual( flCurAttributeValue, flCap ) || ( flAttrDeltaValue > 0 && flCurAttributeValue >= flCap ) || ( flAttrDeltaValue < 0 && flCurAttributeValue <= flCap );
}
