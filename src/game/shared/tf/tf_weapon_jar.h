...
class CTFJar : public CTFWeaponBaseGun
{
   public:
...
#ifdef GAME_DLL
    virtual const AngularImpulse GetAngularImpulse( void )
    {
        return AngularImpulse( 300, 0, 0 );
    }
    virtual Vector GetVelocityVector( const Vector &vecForward, const Vector &vecRight, const Vector &vecUp );

    virtual bool ShouldSpeakWhenFiring( void )
    {
        return true;
    }
    ...
    virtual CTFProjectile_Jar *CreateJarProjectile( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseCombatCharacter *pOwner, const CTFWeaponInfo &weaponInfo );
#endif

    virtual bool CanThrowUnderWater( void )
    {
        return false;
    }
...
class CTFProjectile_Jar : public CTFGrenadePipebombProjectile
{
   public:
    ...
    static CTFProjectile_Jar *Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseCombatCharacter *pOwner, const CTFWeaponInfo &weaponInfo );
    ...
    {
        return TF_WEAPON_GRENADE_JAR;
    }
    virtual float GetDamage()
    {
        return 0.f;
    }
    virtual bool ExplodesOnHit()
    {
        return true;
    }
    ...
    virtual const char *GetImpactEffect()
    {
        return "peejar_impact";
    }
    virtual ETFCond GetEffectCondition( void )
    {
        return TF_COND_URINE;
    }

    virtual const char *GetExplodeSound()
    {
...
class CTFProjectile_JarMilk : public CTFProjectile_Jar
{
   public:
...
#ifdef GAME_DLL
    static CTFProjectile_JarMilk *Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseCombatCharacter *pOwner, const CTFWeaponInfo &weaponInfo );

    virtual void SetCustomPipebombModel() OVERRIDE;

    virtual const char *GetImpactEffect() OVERRIDE
    {
        return "peejar_impact_milk";
    }
    virtual ETFCond GetEffectCondition( void ) OVERRIDE
    {
    ...
    virtual int GetWeaponID( void ) const OVERRIDE
    {
        return TF_WEAPON_GRENADE_JAR_MILK;
    }
...
class CTFProjectile_Cleaver : public CTFProjectile_Jar
{
   public:
...
#ifdef GAME_DLL
    static CTFProjectile_Cleaver *Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseCombatCharacter *pOwner, const CTFWeaponInfo &weaponInfo, int nSkin );
#endif
    ...
    virtual int GetWeaponID( void ) const OVERRIDE
    {
        return TF_WEAPON_GRENADE_CLEAVER;
    }
