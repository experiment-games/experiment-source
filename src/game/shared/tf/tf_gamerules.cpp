...
static bool BIsCvarIndicatingHolidayIsActive( int iCvarValue, /*EHoliday*/ int eHoliday )
{
    ...
    switch ( eHoliday )
    {
        case kHoliday_TFBirthday:
            return iCvarValue == kHoliday_TFBirthday;
        ...
        case kHoliday_Christmas:
            return iCvarValue == kHoliday_Christmas;
        case kHoliday_Valentines:
            return iCvarValue == kHoliday_Valentines || iCvarValue == kHoliday_HalloweenOrFullMoonOrValentines;
        case kHoliday_MeetThePyro:
            return iCvarValue == kHoliday_MeetThePyro;
        ...
        case kHoliday_HalloweenOrFullMoonOrValentines:
            return iCvarValue == kHoliday_Halloween || iCvarValue == kHoliday_FullMoon || iCvarValue == kHoliday_Valentines || iCvarValue == kHoliday_HalloweenOrFullMoon || iCvarValue == kHoliday_HalloweenOrFullMoonOrValentines;
        case kHoliday_AprilFools:
            return iCvarValue == kHoliday_AprilFools;
        case kHoliday_EOTL:
            return iCvarValue == kHoliday_EOTL;
        case kHoliday_CommunityUpdate:
            return iCvarValue == kHoliday_CommunityUpdate;
        case kHoliday_Soldier:
            return iCvarValue == kHoliday_Soldier;
        case kHoliday_Summer:
            return iCvarValue == kHoliday_Summer;
    }
...
ConVar tf_competitive_required_late_join_timeout( "tf_competitive_required_late_join_timeout", "120", FCVAR_DEVELOPMENTONLY, "How long to wait for late joiners in matches requiring full player counts before canceling the match" );
ConVar tf_competitive_required_late_join_confirm_timeout( "tf_competitive_required_late_join_confirm_timeout", "30", FCVAR_DEVELOPMENTONLY, "How long to wait for the GC to confirm we're in the late join pool before canceling the match" );
#endif  // GAME_DLL
...
bool CTFGameRules::IsCompetitiveMode( void ) const
{
    ...
    if ( pMatchDesc )
    {
        return pMatchDesc->GetMatchType() == MATCH_TYPE_COMPETITIVE || pMatchDesc->GetMatchType() == MATCH_TYPE_CASUAL;
    }
...
bool CTFGameRules::ReportMatchResultsToGC( CMsgGC_Match_Result_Status nCode )
{
    ...
    for ( int idxPlayer = 0; idxPlayer < pMatch->GetNumTotalMatchPlayers(); idxPlayer++ )
    {
        ...
        uint32 unLeaveTime = ( pMatchPlayer && ( pMatchPlayer->bDropped || pMatchPlayer->BDropWasAbandon() ) ) ? pMatchPlayer->GetLastActiveEventTime() : 0u;
...
#ifdef GAME_DLL
    : m_mapCoachToStudentMap( DefLessFunc( uint32 ) ), m_flNextStrangeEventProcessTime( g_flStrangeEventBatchProcessInterval ), m_mapTeleportLocations( DefLessFunc( string_t ) ), m_bMapCycleNeedsUpdate( false ), m_flSafeToLeaveTimer( -1.f ), m_flCompModeRespawnPlayersAtMatchStart( -1.f ), m_bMapForcedTruceDuringBossFight( false ), m_flNextHalloweenGiftUpdateTime( -1 )
#else
    ...
    if ( pVictimBaseEntity && pVictimBaseEntity->m_takedamage != DAMAGE_EVENTS_ONLY && pVictim )
    {
        ...
        if ( pAttacker == pVictimBaseEntity )
        {
            ...
            if ( info.GetWeapon() )
            {
                ...
                const bool bIgnoreThisSelfDamage = ( iNoSelfBlastDamage == kSelfBlastResponse_IgnoreProjectilesFromAllWeapons ) || ( ( iNoSelfBlastDamage == kSelfBlastResponse_IgnoreProjectilesFromThisWeapon ) && ( info.GetDamageCustom() == TF_DMG_CUSTOM_PRACTICE_STICKY ) );
...
class CStrangeEventValidator
{
    ...
    CStrangeEventValidator( IEconItemInterface *pEconEntity, CTFPlayer *pOwner, CTFPlayer *pVictim, kill_eater_event_t eEventType )
        : m_unItemID( INVALID_ITEM_ID ), m_eEventType( eEventType )
    {
        m_bIsValid = BInitEventParams( pEconEntity, pOwner, pVictim );
    }

    bool BIsValidEvent() const
    {
        return m_bIsValid;
    }
    CSteamID GetKillerSteamID() const
    {
        Assert( m_bIsValid );
        return m_KillerSteamID;
    }
    CSteamID GetVictimSteamID() const
    {
        Assert( m_bIsValid );
        return m_VictimSteamID;
    }
    ...
    kill_eater_event_t GetEventType() const
    {
        Assert( m_bIsValid );
        return m_eEventType;
    }
    ...
    if ( pTFPlayerScorer && pTFPlayerVictim && pTFPlayerScorer != pTFPlayerVictim )
    {
        ...
        if ( pAttackerEconWeapon )
        {
            if ( !( IsPVEModeActive() && pTFPlayerVictim->GetTeamNumber() == TF_TEAM_PVE_INVADERS ) )
            {
                ...
                enum
                {
                    kFirstKillStreakAnnouncement = 5
                };
...
void CTFGameRules::RequestClientInventory( CSteamID steamID )
{
    ...
    for ( int i = 1; i <= MAX_PLAYERS; i++ )
    {
        ...
        if ( pPotentialPlayer )
        {
            ...
            if ( pPotentialPlayer->GetSteamID( &playerSteamID ) && playerSteamID == steamID )
            {
...
void CTFGameRules::TeleportPlayersToTargetEntities( int iTeam, const char *pszEntTargetName, CUtlVector< CTFPlayer * > *pTeleportedPlayers )
{
    ...
    FOR_EACH_VEC( vecPlayers, i )
    {
        ...
        if ( ( pPlayer->GetTeamNumber() != TF_TEAM_RED && pPlayer->GetTeamNumber() != TF_TEAM_BLUE ) || pPlayer->IsPlayerClass( TF_CLASS_UNDEFINED ) )
            continue;
...
int CTFGameRules::GetClassLimit( int iClass )
{
    if ( IsInTournamentMode() || IsPasstimeMode() )
    {
        ...
        {
            case TF_CLASS_SCOUT:
                return tf_tournament_classlimit_scout.GetInt();
                break;
            case TF_CLASS_SNIPER:
                return tf_tournament_classlimit_sniper.GetInt();
                break;
            case TF_CLASS_SOLDIER:
                return tf_tournament_classlimit_soldier.GetInt();
                break;
            case TF_CLASS_DEMOMAN:
                return tf_tournament_classlimit_demoman.GetInt();
                break;
            case TF_CLASS_MEDIC:
                return tf_tournament_classlimit_medic.GetInt();
                break;
            case TF_CLASS_HEAVYWEAPONS:
                return tf_tournament_classlimit_heavy.GetInt();
                break;
            case TF_CLASS_PYRO:
                return tf_tournament_classlimit_pyro.GetInt();
                break;
            case TF_CLASS_SPY:
                return tf_tournament_classlimit_spy.GetInt();
                break;
            case TF_CLASS_ENGINEER:
                return tf_tournament_classlimit_engineer.GetInt();
                break;
            default:
                break;
        }
...
bool CTFGameRules::CanBotChangeClass( CBasePlayer *pPlayer )
{
    ...
    if ( pTFPlayer && pTFPlayer->GetPlayerClass() && pTFPlayer->GetPlayerClass()->GetClassIndex() != TF_CLASS_UNDEFINED )
    {
        ...
        {
            case TF_TEAM_RED:
                return m_hRedBotRoster ? m_hRedBotRoster->IsClassChangeAllowed() : true;
                break;
            case TF_TEAM_BLUE:
                return m_hBlueBotRoster ? m_hBlueBotRoster->IsClassChangeAllowed() : true;
                break;
        }
    }
    return true;
...
void CTrainingModeLogic::OnPlayerSpawned( CTFPlayer *pPlayer )
{
    ...
    {
        case TF_CLASS_SCOUT:
            m_outputOnPlayerSpawnAsScout.FireOutput( this, this );
            break;
        case TF_CLASS_SNIPER:
            m_outputOnPlayerSpawnAsSniper.FireOutput( this, this );
            break;
        case TF_CLASS_SOLDIER:
            m_outputOnPlayerSpawnAsSoldier.FireOutput( this, this );
            break;
        case TF_CLASS_DEMOMAN:
            m_outputOnPlayerSpawnAsDemoman.FireOutput( this, this );
            break;
        case TF_CLASS_MEDIC:
            m_outputOnPlayerSpawnAsMedic.FireOutput( this, this );
            break;
        case TF_CLASS_HEAVYWEAPONS:
            m_outputOnPlayerSpawnAsHeavy.FireOutput( this, this );
            break;
        case TF_CLASS_PYRO:
            m_outputOnPlayerSpawnAsPyro.FireOutput( this, this );
            break;
        case TF_CLASS_SPY:
            m_outputOnPlayerSpawnAsSpy.FireOutput( this, this );
            break;
        case TF_CLASS_ENGINEER:
            m_outputOnPlayerSpawnAsEngineer.FireOutput( this, this );
            break;
    }
}
...
void CTrainingModeLogic::OnPlayerSwitchedWeapons( CTFPlayer *pPlayer )
{
    ...
    switch ( pWeapon->GetTFWpnData().m_iWeaponType )
    {
        case TF_WPN_TYPE_PRIMARY:
            m_outputOnPlayerSwappedToWeaponSlotPrimary.FireOutput( this, this );
            break;
        case TF_WPN_TYPE_SECONDARY:
            m_outputOnPlayerSwappedToWeaponSlotSecondary.FireOutput( this, this );
            break;
        case TF_WPN_TYPE_MELEE:
            m_outputOnPlayerSwappedToWeaponSlotMelee.FireOutput( this, this );
            break;
        case TF_WPN_TYPE_BUILDING:
            m_outputOnPlayerSwappedToWeaponSlotBuilding.FireOutput( this, this );
            break;
        case TF_WPN_TYPE_PDA:
            m_outputOnPlayerSwappedToWeaponSlotPDA.FireOutput( this, this );
            break;
    }
}
...
#ifdef GAME_DLL
    if ( !Q_strcmp( eventName, "player_turned_to_ghost" ) || !Q_strcmp( eventName, "player_disconnect" ) || !Q_strcmp( eventName, "player_team" ) )
    {
...
{
    public:
    CGCCoaching_CoachJoining( GCSDK::CGCClient *pClient )
...
{
    public:
    CGCCoaching_RemoveCurrentCoach( GCSDK::CGCClient *pClient )
        : GCSDK::CGCClientJob( pClient ) {}
...
{
    public:
    CGCUseServerModificationItemJob( GCSDK::CGCClient *pClient )
        : GCSDK::CGCClientJob( pClient ) {}
...
{
    public:
    CGCUpdateServerModificationItemStateJob( GCSDK::CGCClient *pClient )
        : GCSDK::CGCClientJob( pClient ) {}
...
bool CTFGameRules::CanUpgradeWithAttrib( CTFPlayer *pPlayer, int iWeaponSlot, attrib_definition_index_t iAttribIndex, CMannVsMachineUpgrades *pUpgrade )
{
    ...
    switch ( iAttribIndex )
    {
        ...
        case 31:  // "critboost on kill"
        {
            ...
            return ( pMelee && ( pPlayer->IsPlayerClass( TF_CLASS_DEMOMAN ) ||
        ...
        case 335:  // "clip size bonus upgrade"
        {
            return ( pWeapon && !pWeapon->IsBlastImpactWeapon() && pWeapon->UsesClipsForAmmo1() && pWeapon->GetMaxClip1() > 1 && iWeaponID != TF_WEAPON_FLAREGUN_REVENGE && iWeaponID != TF_WEAPON_SPELLBOOK );
        }
        break;
...
bool ScriptIsHolidayMap( int nHoliday )
{
    return TFGameRules()->IsHolidayMap( nHoliday );
}
...
void ScriptSetMannVsMachineAlarmStatus( bool bEnabled )
{
    return TFGameRules()->SetMannVsMachineAlarmStatus( bEnabled );
}
...
{
    return TFGameRules()->InMatchStartCountdown();
}
bool ScriptMatchmakingShouldUseStopwatchMode()
{
    return TFGameRules()->MatchmakingShouldUseStopwatchMode();
}
...
{
    return TFGameRules()->SetGravityMultiplier( flMultiplier );
}
float ScriptGetGravityMultiplier()
{
    return TFGameRules()->GetGravityMultiplier();
}
void ScriptSetPlayersInHell( bool bInHell )
{
    return TFGameRules()->SetPlayersInHell( bInHell );
}
bool ScriptArePlayersInHell()
{
    return TFGameRules()->ArePlayersInHell();
}
void ScriptSetUsingSpells( bool bUsingSpells )
{
    return TFGameRules()->SetUsingSpells( bUsingSpells );
}
...
void ScriptForceEnableUpgrades( int nState )
{
    TFGameRules()->ForceEnableUpgrades( nState );
}
void ScriptForceEscortPushLogic( int nState )
{
    TFGameRules()->ForceEscortPushLogic( nState );
}
