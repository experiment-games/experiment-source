...
class CMOTDEntryDefinition
{
   public:
    ...
    {
        return m_pKVMOTD ? m_pKVMOTD->GetName() : "Unknown Entry";
    }
    const char *GetTitle( ELanguage eLang );
    const char *GetText( ELanguage eLang );
    const char *GetURL( void )
    {
        return m_pKVMOTD ? m_pKVMOTD->GetString( "url", "" ) : NULL;
    }
    const char *GetImage( void )
    {
        return m_pKVMOTD ? m_pKVMOTD->GetString( "image", "" ) : NULL;
    }
    const char *GetPostTimeStr( void )
    {
        return m_pKVMOTD ? m_pKVMOTD->GetString( "time" ) : NULL;
    }

    const int GetHeaderType( void )
    {
        return m_pKVMOTD ? m_pKVMOTD->GetInt( "header_type" ) : 0;
    }
    const char *GetHeaderTitle( ELanguage eLang );
    const char *GetHeaderIcon( void )
    {
        return m_pKVMOTD ? m_pKVMOTD->GetString( "header_icon", "" ) : NULL;
    }

    // Post time is the time displayed on the client screen for this post
    const RTime32 GetPostTime( void ) const
    {
        return m_PostTime;
    }
    ...
    const RTime32 GetChangedTime( void ) const
    {
        return m_ChangedTime;
    }

    const int GetNameInt( void ) const
    {
...
class CMOTDManager
{
   public:
    ...
    int GetNumMOTDs( void )
    {
        return m_vecMOTDEntries.Count();
    }
