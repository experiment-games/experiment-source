...
#define FORMAT_AND_TRUNCATE( buffer, format, ... )                       \
    char buffer[MAX_MSG_LENGTH + 1];                                     \
    int len = snprintf( buffer, MAX_MSG_LENGTH, format, ##__VA_ARGS__ ); \
    if ( len >= MAX_MSG_LENGTH )                                         \
    {                                                                    \
        strcpy( buffer + MAX_MSG_LENGTH - 3, "..." );                    \
    }

#define SHOW_LUA_ERROR( format, ... )                 \
    FORMAT_AND_TRUNCATE( msg, format, ##__VA_ARGS__ ) \
