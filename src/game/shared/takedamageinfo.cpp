...
void CalculateBulletDamageForce( CTakeDamageInfo *info, int iBulletType, const Vector &vecBulletDir, const Vector &vecForceOrigin, float flScale )
{
    ...
    // Experiment;  We added checks to ensure the force is never set to zero,
    //              since Lua may cause zero force to be set. It can also
    //              happen when no ammo type is specified, or when
    //              the ammo type doesn't have a damage force.
    //              For more information see the comment and assert in
    //              CBaseEntity::VPhysicsTakeDamage.
    //              We setup a minimum amount of force for these reasons.
    float ammoForce = GetAmmoDef()->DamageForce( iBulletType );

    if ( ammoForce != 0 )
        vecForce *= ammoForce;

    float physicsPushScale = phys_pushscale.GetFloat();

    if ( physicsPushScale != 0 )
        vecForce *= physicsPushScale;
