...
CBaseEntity *CInfoAPCMissileHint::FindAimTarget( CBaseEntity *pMissile, const char *pTargetName, const Vector &vecCurrentEnemyPos, const Vector &vecCurrentEnemyVel )
{
    ...
    for ( int i = s_APCMissileHints.Count(); --i >= 0; )
    {
        ...
        if ( !IntersectRayWithOBB( vecCurrentEnemyPos, vecRayDelta, pHint->CollisionProp()->CollisionToWorldTransform(), pHint->CollisionProp()->OBBMins(), pHint->CollisionProp()->OBBMaxs(), 0.0f, &trace ) )
        {
            continue;
        }
...
void CWeaponRPG::PrimaryAttack( void )
{
    ...
    CExperiment_Player *pPlayer = ToExperimentPlayer( GetOwner() );
    ...
    pMissile->SetDamage( GetExperimentWpnData().m_iPlayerDamage );
...
int CLaserDot::DrawModel( int flags )
{
    ...
    C_Experiment_Player *pOwner = ToExperimentPlayer( GetOwnerEntity() );
