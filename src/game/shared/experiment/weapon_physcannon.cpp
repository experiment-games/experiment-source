...
void CPlayerPickupController::Init( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
    ...
    CExperiment_Player *pOwner = ( CExperiment_Player * )ToBasePlayer( pPlayer );
    if ( pOwner )
    {
...
void CPlayerPickupController::Shutdown( bool bThrown )
{
    ...
    if ( m_pPlayer )
    {
        CExperiment_Player *pOwner = ( CExperiment_Player * )ToBasePlayer( m_pPlayer );
        if ( pOwner )
        {
...
bool CWeaponPhysCannon::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
    ...
    CExperiment_Player *pOwner = ( CExperiment_Player * )ToBasePlayer( GetOwner() );
...
#if defined( EXPERIMENT_SOURCE )
        pOwner->SetMaxSpeed( pOwner->GetNormalSpeed() );
#elif defined( HL2MP )
        pOwner->SetMaxSpeed( HL2_NORM_SPEED );
#else
