...
class CHL2MPBot : public NextBotPlayer< CHL2MP_Player >, public CGameEventListener
{
    ...
    {
        return static_cast< CNavArea * >( BaseClass::GetLastKnownArea() );
    ...
    // INextBot
    virtual CHL2MPBotLocomotion *GetLocomotionInterface( void ) const
    {
        return m_locomotor;
    }
    virtual CHL2MPBotBody *GetBodyInterface( void ) const
    {
        return m_body;
    }
    virtual CHL2MPBotVision *GetVisionInterface( void ) const
    {
        return m_vision;
    }
...
#define MY_CURRENT_GUN NULL                                                    // can be passed as weapon to following queries
    ...
    {
        SetActionPoint( ScriptToEntClass< CHL2MPBotActionPoint >( hPoint ) );
    }
...
#define MISSION_DOESNT_RESET_BEHAVIOR_SYSTEM false
    ...
    void MarkPhyscannonPickupTime()
    {
        m_flPhyscannonPickupTime = gpGlobals->curtime;
    }
    float GetPhyscannonPickupTime()
    {
        return m_flPhyscannonPickupTime;
    }
