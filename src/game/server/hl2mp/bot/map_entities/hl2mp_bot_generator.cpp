...
CHL2MPBotGenerator::CHL2MPBotGenerator( void )
    : m_bSuppressFire( false ), m_bDisableDodge( false ), m_bUseTeamSpawnpoint( false ), m_bRetainBuildings( false ), m_bExpended( false ), m_iOnDeathAction( kOnDeath_RemoveSelf ), m_difficulty( CHL2MPBot::UNDEFINED ), m_spawnCountRemaining( 0 ), m_bSpawnOnlyWhenTriggered( false ), m_bEnabled( true )
{
    SetThink( NULL );
}
