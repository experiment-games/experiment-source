...
void CHL2MP_Player::SetAnimation(PLAYER_ANIM playerAnim) {
    ...
    /*	if ( ( m_nButtons & ( IN_FORWARD | IN_BACK | IN_MOVELEFT | IN_MOVERIGHT
            ...
            {
                    if ( speed > 1.0f && speed < hl2_normspeed.GetFloat()
       - 20.0f )
                    {
                            bRunning = false;
                    }
    ...
    // This could stand to be redone. Why is playerAnim abstracted from
    // activity? (sjb)
    if (playerAnim == PLAYER_JUMP) {
        ...
        if (GetActivity() == ACT_HOVER || GetActivity() == ACT_SWIM ||
            GetActivity() == ACT_HOP || GetActivity() == ACT_LEAP ||
        ...
        if (!(GetFlags() & FL_ONGROUND) &&
            GetActivity() == ACT_HL2MP_JUMP)  // Still jumping
        {
            idealActivity = GetActivity();
        }
        /*
    ...
    if (idealActivity == ACT_HL2MP_GESTURE_RANGE_ATTACK) {
        ...
        animDesired =
            SelectWeightedSequence(Weapon_TranslateActivity(idealActivity));
        ...
        for (CEntitySphereQuery sphere(pSpot->GetAbsOrigin(), 128);
            ...
            if (ent->IsPlayer() && !(ent->edict() == player))
                ent->TakeDamage(CTakeDamageInfo(
                    ...
                    GetContainingEntity(INDEXENT(0)), 300, DMG_GENERIC));
        }
        goto ReturnSpot;
    }
