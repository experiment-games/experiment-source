...
bool ScriptSendGlobalGameEvent( const char *szName, HSCRIPT params )
{
    ...
    // Josh: This wasn't as bad as I thought it would be!
    {
        ...
        for ( int i = 0; i < nEntries; i++ )
        {
            ...
            {
                case FIELD_BOOLEAN:
                    event->SetBool( pszKeyName, ( bool )vValue );
                    break;
                case FIELD_FLOAT:
                    event->SetFloat( pszKeyName, ( float )vValue );
                    break;
                case FIELD_INTEGER:
                    event->SetInt( pszKeyName, ( int )vValue );
                    break;
                case FIELD_CSTRING:
                    event->SetString( pszKeyName, ( const char * )vValue );
                    break;
                case FIELD_UINT64:
                    event->SetUint64( pszKeyName, ( uint64 )vValue );
                    break;
                default:
                {
...
#define DECLARE_SCRIPT_CONST_TABLE( x )  \
    ScriptVariant_t vConstantsTable_##x; \
    g_pScriptVM->CreateTable( vConstantsTable_##x );
