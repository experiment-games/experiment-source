        ...
        if ( to->how <= GO_WEST )  // walk along the floor to the next area
        {
            ...
            if ( expectedHeightDrop > mover->GetStepHeight() )
            {
                ...
                for ( pushDist = 0.0f; pushDist <= maxPushDist; pushDist += inc )
                {
                    ...
                    UTIL_TraceHull( pos, lowerPos, Vector( -halfWidth, -halfWidth, stepHeight ), Vector( halfWidth, halfWidth, hullHeight ), bot->GetBodyInterface()->GetSolidMask(), &filter, &result );
