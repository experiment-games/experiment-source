    ...
    if ( g_debug_npc_vehicle_roles.GetBool() )
    {
        ...
        for ( int i = 0; i < m_PassengerRoles.Count(); i++ )
        {
            ...
            for ( int j = 0; j < m_PassengerRoles[i].m_PassengerSeats.Count(); j++ )
            {
                ...
                {
                    Msg( "\t\t\tAnimation:\t%s\t(Priority %d)\n", STRING( m_PassengerRoles[i].m_PassengerSeats[j].m_EntryTransitions[nEntry].m_strAnimationName ), m_PassengerRoles[i].m_PassengerSeats[j].m_EntryTransitions[nEntry].m_nPriority );
                }
