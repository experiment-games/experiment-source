...
void Bot_Think( CExperiment_Player *pBot );
...
CBasePlayer *BotPutInServer( bool bFrozen, int iTeam )
{
    ...
    CExperiment_Player *pPlayer = ( ( CExperiment_Player * )CBaseEntity::Instance( pEdict ) );
    pPlayer->ClearFlags();
...
void Bot_RunAll( void )
{
    ...
    {
        CExperiment_Player *pPlayer = ToExperimentPlayer( UTIL_PlayerByIndex( i ) );
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CExperiment_Player *fakeclient, const QAngle &viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, float frametime )
{
    if ( !fakeclient )
        return;
    ...
    if ( pBot->IsAlive() && ( pBot->GetSolid() == SOLID_BBOX ) )
    {
        ...
        if ( !pBot->IsEFlagSet( EFL_BOT_FROZEN ) && pBot->m_iHealth == 100 )
        {
            ...
            while ( --maxtries >= 0 )
            {
                ...
                UTIL_TraceHull( vecSrc, vecEnd, VEC_HULL_MIN_SCALED( pBot ), VEC_HULL_MAX_SCALED( pBot ), MASK_PLAYERSOLID, pBot, COLLISION_GROUP_NONE, &trace );
