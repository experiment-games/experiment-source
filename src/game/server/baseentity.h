...
class CBaseEntity : public IServerEntity
{
   ...
   public:
    ...
    bool KeyValueFromString( const char *szKeyName, const char *szValue )
    {
        return KeyValue( szKeyName, szValue );
    }
    bool KeyValueFromFloat( const char *szKeyName, float flValue )
    {
        return KeyValue( szKeyName, flValue );
    }
    bool KeyValueFromInt( const char *szKeyName, int nValue )
    {
        return KeyValue( szKeyName, nValue );
    }
    bool KeyValueFromVector( const char *szKeyName, const Vector &vecValue )
    {
...
#ifdef TF_DLL
    virtual bool IsProjectileCollisionTarget( void ) const
    {
        return false;
    }
    virtual bool IsFuncLOD( void ) const
    {
        return false;
    }
    virtual bool IsBaseProjectile( void ) const
    {
        return false;
    }
#endif  // TF_DLL
...
#define ENTITYFUNCPTR_SIZE 4
   ...
   public:
    ...
    {
        static Vector vecUp;
        this->GetVectors( NULL, NULL, &vecUp );
        return vecUp;
    }
    ...
    inline Vector ScriptEyePosition()
    {
        return EyePosition();
    }
    ...
    static void EmitSentenceByIndex( IRecipientFilter &filter, int iEntIndex, int iChannel, int iSentenceIndex, float flVolume, soundlevel_t iSoundlevel, int iFlags = 0, int iPitch = PITCH_NORM, const Vector *pOrigin = NULL, const Vector *pDirection = NULL, bool bUpdatePositions = true, float soundtime = 0.0f );
   ...
   public:
    ...
    virtual bool ShouldForceTransmitsForTeam( int iTeam )
    {
        return false;
    }

    void SetTruceValidForEnt( bool bTruceValidForEnt )
    {
        m_bTruceValidForEnt = bTruceValidForEnt;
    }
    virtual bool IsTruceValidForEnt( void ) const
    {
        return m_bTruceValidForEnt;
    }

#ifdef TF_DLL
    virtual float GetDefaultItemChargeMeterValue( void ) const
    {
        return 100.f;
    }
#endif  // TF_DLL

    virtual bool BCanCallVote()
    {
        return true;
    }
...
//-----------------------------------------------------------------------------
inline void CBaseEntity::FireBullets( int cShots, const Vector &vecSrc, const Vector &vecDirShooting, const Vector &vecSpread, float flDistance, int iAmmoType, int iTracerFreq, int firingEntID, int attachmentID, int iDamage, CBaseEntity *pAttacker, bool bFirstShotAccurate, bool bPrimaryAttack )
{
