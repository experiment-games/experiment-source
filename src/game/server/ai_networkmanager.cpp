        ...
        if ( aiNet->GetNode( node )->GetType() != NODE_DELETED )
        {
                ...
                nNodeType == aiNet->GetNode( node )->GetType() )
            {
                ...
                if ( dot > bestDot )
                {
                    ...
                    trace_t tr;
                    AI_TraceLine( origin, aiNet->GetNode( node )->GetPosition( m_iHullDrawNum ), MASK_BLOCKLOS, NULL, COLLISION_GROUP_NONE, &tr );
                    if ( tr.fraction == 1.0 )
                    {
                        bestDot = dot;
...
void CAI_NetworkBuilder::InitClimbNodePosition( CAI_Network *pNetwork, CAI_Node *pNode )
{
    ...
    AI_TraceHull( posOnLadder, posOnLadder + Vector( 0, 0, -37 ), NAI_Hull::Mins( HULL_SMALL_CENTERED ), NAI_Hull::Maxs( HULL_SMALL_CENTERED ), MASK_NPCSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &trace );
