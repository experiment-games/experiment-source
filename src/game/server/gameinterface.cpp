...
void DrawAllDebugOverlays( void )
{
                ...
                if ( pAILink )
                {
                    ...
                    if ( pAILink->m_LinkInfo & bits_LINK_OFF )
                    {
                        NDebugOverlay::BoxDirection(
                            startPos, Vector( -4, -4, -4 ), Vector( -linkLen, 4, 4 ), linkDir, 0, 255, 0, 40, 0 );
                    }
                    else
                    {
                        NDebugOverlay::BoxDirection(
                            startPos, Vector( -4, -4, -4 ), Vector( -linkLen, 4, 4 ), linkDir, 255, 0, 0, 40, 0 );
                    }
                }
            }
            else
    ...
    if ( sv_massreport.GetInt() )
    {
        ...
        for ( ; pInfo; pInfo = pInfo->m_pNext )
        {
            ...
            char tempstr[512];
            Q_snprintf( tempstr, sizeof( tempstr ), "%s: Mass: %.2f kg / %.2f lb (%s)", STRING( ent->GetModelName() ), ent->VPhysicsGetObject()->GetMass(), kg2lbs( ent->VPhysicsGetObject()->GetMass() ), GetMassEquivalent( ent->VPhysicsGetObject()->GetMass() ) );
    ...
    ( engine->GetClientConVarValue( player->entindex(), v ) )
        ...
        if ( pMinUpdateRate && pMaxUpdateRate )
            player->m_nUpdateRate =
                clamp( player->m_nUpdateRate, ( int )pMinUpdateRate->GetFloat(), ( int )pMaxUpdateRate->GetFloat() );
                                                ...
                                                int pvssize )
{
    ...
    for ( unsigned short i = g_AreaPortals.Head();
          i != g_AreaPortals.InvalidIndex();
          i = g_AreaPortals.Next( i ) )
    {
...
void CServerGameClients::GetBugReportInfo( char *buf, int buflen )
{
    ...
    if ( gpGlobals->maxClients == 1 )
    {
        ...
        if ( ent )
        {
            Q_snprintf( buf, buflen, "Picker %i/%s - ent %s model %s\n", ent->entindex(), ent->GetClassname(), STRING( ent->GetEntityName() ), STRING( ent->GetModelName() ) );
        }
