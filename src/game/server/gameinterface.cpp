...
bool UTIL_GetModDir(char *lpszTextOut, unsigned int nSize) {
  ...
  if (Q_strnchr(lpszTextOut, '/', nSize) ||
    ...
    // Find the difference in string lengths and take that difference from the
    // original string as the mod dir
      ...
      if (g_pAINetworkManager->GetEditOps()->m_bLinkEditMode) {
        ...
        if (pAILink) {
          ...
          if (pAILink->m_LinkInfo & bits_LINK_OFF) {
            NDebugOverlay::BoxDirection(startPos, Vector(-4, -4, -4),
                                        Vector(-linkLen, 4, 4), linkDir, 0, 255,
                                        0, 40, 0);
          } else {
            NDebugOverlay::BoxDirection(startPos, Vector(-4, -4, -4),
                                        Vector(-linkLen, 4, 4), linkDir, 255, 0,
                                        0, 40, 0);
          }
        }
      } else {
        ...
        if (pAINode) {
          Vector vecPos = pAINode->GetPosition(
              g_pAINetworkManager->GetEditOps()->m_iHullDrawNum);
          NDebugOverlay::Box(vecPos, Vector(-8, -8, -8), Vector(8, 8, 8), 255,
                             0, 0, 40, 0);
          ...
          if (pAINode->GetHint()) {
            ...
            if (pEnt->GetEntityName() != NULL_STRING) {
              NDebugOverlay::Text(vecPos + Vector(0, 0, 6),
                                  STRING(pEnt->GetEntityName()), false, 0);
            }
            NDebugOverlay::Text(
                vecPos, GetHintTypeDescription(pAINode->GetHint()), false, 0);
          }
        }
      }
      // ------------------------------------
...
void CServerGameDLL::PostInit() { IGameSystem::PostInitAllSystems(); }
...
void CServerGameDLL::GameShutdown(void) { ResetGlobalState(); }

static bool g_OneWayTransition = false;
void Game_SetOneWayTransition(void) { g_OneWayTransition = true; }
...
//  simulation going on
//-----------------------------------------------------------------------------
bool CServerGameDLL::IsRestoring() { return g_InRestore; }
...
bool CServerGameDLL::LevelInit(const char *pMapName, char const *pMapEntities,
  ...
  if (loadGame) {
    ...
    if (pOldLevel && sv_autosave.GetBool() == true) {
      ...
      CBaseEntity *pAutosave =
          CBaseEntity::Create("logic_autosave", vec3_origin, vec3_angle, NULL);
      if (pAutosave) {
...
#define EndCheckChainedActivate(bCheck)                                        \
  if (0)                                                                       \
    ...
    if (bCheck) {                                                              \
      AssertMsg(g_bReceivedChainedActivate,                                    \
...
void CServerGameDLL::CreateNetworkStringTables(void) {
  ...
  g_pStringTableMaterials =
      networkstringtable->CreateStringTable("Materials", MAX_MATERIAL_STRINGS);
  g_pStringTableInfoPanel =
      networkstringtable->CreateStringTable("InfoPanel", MAX_INFOPANEL_STRINGS);
  g_pStringTableClientSideChoreoScenes = networkstringtable->CreateStringTable(
      "Scenes", MAX_CHOREO_SCENES_STRINGS);
...
#ifdef TF_DLL
  sResult.Format("tf_mm_trusted:%d,tf_mm_servermode:%d", tf_mm_trusted.GetInt(),
                 tf_mm_servermode.GetInt());
...
void CServerGameDLL::PrepareLevelResources(/* in/out */ char *pszMapName,
                                           ...
                                           size_t nMapFileSize) {
#ifdef TF_DLL
  TFMapsWorkshop()->PrepareLevelResources(pszMapName, nMapNameSize, pszMapFile,
                                          nMapFileSize);
#endif // TF_DLL
}
...
CServerGameDLL::CanProvideLevel(/* in/out */ char *pMapName, int nMapNameMax) {
#ifdef TF_DLL
...
void CServerGameDLL::BuildAdjacentMapList(void) {
  ...
  if (pSaveData)
    pSaveData->levelInfo.connectionCount =
        BuildChangeList(pSaveData->levelInfo.levelList, MAX_LEVEL_CONNECTIONS);
}
...
void UpdateRichPresence(void) {
  ...
  if (Q_stristr(modDir, "hl2")) {
    ...
    Warning("UpdateRichPresence failed in GameInterface. Didn't recognize "
            "-game parameter.");
  }
  ...
  if (commentary.GetBool()) {
    ...
    if (!xboxsystem->UserSetContext(XBX_GetPrimaryUserId(), X_CONTEXT_PRESENCE,
    ...
    if (!xboxsystem->UserSetContext(XBX_GetPrimaryUserId(), X_CONTEXT_PRESENCE,
                                    iGamePresenceID, true)) {
      Warning("GameInterface: UserSetContext failed.\n");
    }
  }

  // Set which game the user is playing
  if (!xboxsystem->UserSetContext(XBX_GetPrimaryUserId(), CONTEXT_GAME, iGameID,
                                  true)) {
    Warning("GameInterface: UserSetContext failed.\n");
  }
...
int GetParticleSystemIndex(const char *pParticleSystemName) {
  if (pParticleSystemName) {
    int nIndex =
        g_pStringTableParticleEffectNames->FindStringIndex(pParticleSystemName);
    if (nIndex != INVALID_STRING_INDEX)
      return nIndex;
...
public:
  ...
  virtual void CheckTransmit(CCheckTransmitInfo *pInfo,
                             const unsigned short *pEdictIndices, int nEdicts);
};
...
void CServerGameEnts::CheckTransmit(CCheckTransmitInfo *pInfo,
  ...
  for (int i = 0; i < nEdicts; i++) {
    ...
    // Sidenote: call of AreaNum() ensures that PVS data is up to date for this
    // entity
    ...
    // If the entity is marked "check PVS" but it's in hierarchy, walk up the
    // hierarchy looking for the
    ...
    // BUG BUG:  I think it might be better to build up a list of edict indices
    // which "depend" on other answers and then resolve them in a second pass.
    // Not sure what happens if an entity has two parents who both request PVS
    // check?
    while (check) {
      ...
      int checkFlags =
          checkEdict->m_fStateFlags & (FL_EDICT_DONTSEND | FL_EDICT_ALWAYS |
                                       FL_EDICT_PVSCHECK | FL_EDICT_FULLCHECK);
      ...
      // Continue up chain just in case the parent itself has a parent that's in
      // the PVS...
...
void CServerGameClients::ClientDisconnect(edict_t *pEdict) {
  ...
  if (player) {
    if (!g_fGameOver) {
      ...
      CSound *pSound;
      pSound =
          CSoundEnt::SoundPointerForIndex(CSoundEnt::ClientSoundIndex(pEdict));
      {
...
#define QUICKGETCVARVALUE(v)                                                   \
  (engine->GetClientConVarValue(player->entindex(), v))
    ...
    // Unfortunately, we have to duplicate the code in cdll_bounded_cvars.cpp
    // here because the client doesn't send the virtualized value up (because it
    // has no way to know when the virtualized value changes). Possible todo:
    // put the responsibility on the bounded cvar to notify the engine when its
    // virtualized value has changed.
...
int TestAreaPortalVisibilityThroughPortals(CFuncAreaPortalBase *pAreaPortal,
  ...
  for (int i = 0; i != iPortalCount; ++i) {
    ...
    if (pLocalPortal && pLocalPortal->m_bActivated) {
      ...
      // Make sure this portal's linked portal is in the PVS before we add what
      // it can see
...
void CServerGameClients::ClientSetupVisibility(edict_t *pViewEntity,
  ...
  for (unsigned short i = g_AreaPortals.Head();
    ...
    isOpen[iOutPortal] =
        pCur->UpdateVisibility(org, fovDistanceAdjustFactor, bIsOpenOnClient);
    ...
    // Version 0 portals (ie: shipping Half-Life 2 era) are always treated as
    // open for purposes of the m_chAreaPortalBits array on the client.
...
float CServerGameClients::ProcessUsercmds(edict_t *player, bf_read *buf,
  ...
  // Client not fully connected or server has gone inactive  or is paused, just
  // ignore
  if (ignore || !pPlayer) {
    return 0.0f;
  }
  ...
  if (gpGlobals->maxClients == 1) {
    ...
    if (ent) {
      Q_snprintf(buf, buflen, "Picker %i/%s - ent %s model %s\n",
                 ...
                 STRING(ent->GetEntityName()), STRING(ent->GetModelName()));
    }
...
void EntityMessageBegin(CBaseEntity *entity, bool reliable /*= false*/) {
  ...
  g_pMsgBuffer = engine->EntityMessageBegin(entity->entindex(),
                                            entity->GetServerClass(), reliable);
}
...
public:
  ...
  virtual int Count() { return m_Systems.Count(); }
