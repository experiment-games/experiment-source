...
bool CBaseEntity::Intersects( CBaseEntity *pOther )
{
    ...
    return IsOBBIntersectingOBB(
        pMyProp->GetCollisionOrigin(), pMyProp->GetCollisionAngles(), pMyProp->OBBMins(), pMyProp->OBBMaxs(), pOtherProp->GetCollisionOrigin(), pOtherProp->GetCollisionAngles(), pOtherProp->OBBMins(), pOtherProp->OBBMaxs() );
}
                            ...
                            case AE_CL_PLAYSOUND:
                            {
                                ...
                                else
                                {
                                    Warning( "-- Error --:  empty soundname, .qc error on AE_CL_PLAYSOUND in model %s, sequence %s, animevent # %i\n",
                                             studioHdr.GetRenderHdr()->pszName(),
                                             seq.pszLabel(),
                                             j + 1 );
                                }
                            }
...
int CBaseEntity::PrecacheModel( const char *name, bool bPreload )
{
    ...
    // Experiment; TODO: Remove this once we support model version 49
    // Until we support MDL version 49 (https://github.com/experiment-games/experiment-source/issues/20)
    // we check for versions higher than 48 by peeking the bytes 5-8 of the model file for the version number
    FileHandle_t file = filesystem->Open( name, "rb", "GAME" );

    if ( file )
    {
        uint32_t version;
        filesystem->Seek( file, 4, FILESYSTEM_SEEK_HEAD );
        filesystem->Read( &version, sizeof( version ), file );
        filesystem->Close( file );

        if ( version > 48 )
        {
            Warning( "Cannot precache model because '%s' has unsupported version '%d'! Defaulting to error model to prevent crashes.\n", name, version );
...
//			soundtime -
//-----------------------------------------------------------------------------
void CBaseEntity::EmitSentenceByIndex( IRecipientFilter &filter, int iEntIndex, int iChannel, int iSentenceIndex, float flVolume, soundlevel_t iSoundlevel, int iFlags /*= 0*/, int iPitch /*=PITCH_NORM*/, const Vector *pOrigin /*=NULL*/, const Vector *pDirection /*=NULL*/, bool bUpdatePositions /*=true*/, float soundtime /*=0.0f*/ )
{
    CUtlVector< Vector > dummy;
    enginesound->EmitSentenceByIndex( filter, iEntIndex, iChannel, iSentenceIndex, flVolume, iSoundlevel, iFlags, iPitch, 0, pOrigin, pDirection, &dummy, bUpdatePositions, soundtime );
}
