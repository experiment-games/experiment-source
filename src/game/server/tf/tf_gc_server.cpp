                                    ...
                                    k_EMsgGC_ChangeMatchPlayerTeamsResponse >
{
    ...
    void OnPrepare()
    {
        Msg().Body().set_match_id( ReliableMsgCheckUpdateMatchID( Msg().Body().match_id() ) );
    }

    const char *MsgName()
    {
        return "ChangeMatchPlayerTeams";
    }
    void InitDebugString( CUtlString &dbgStr )
    {
                                    ...
                                    k_EMsgGCMvMVictoryReply >
{
    ...
    void InitDebugString( CUtlString &dbgStr )
    {
        dbgStr.Format( "Lobby %016llx", ( unsigned long long )Msg().Body().lobby_id() );
    }
};
                                    ...
                                    k_EMsgGC_Match_ResultResponse >
{
    ...
    void InitDebugString( CUtlString &dbgStr )
    {
        dbgStr.Format( "Match %016llx", ( unsigned long long )Msg().Body().match_id() );
    }
};
...
CTFGCServerSystem::CTFGCServerSystem()
    : m_flTimeRequestedLateJoin( -1.f ), m_bLateJoinEligible( false ), m_iSavedVisibleMaxPlayers( -1 ), m_bOverridingVisibleMaxPlayers( false ), m_bWaitingForNewMatchID( false ), m_flWaitingForNewMatchTime( 0.f )
{
...
void CTFGCServerSystem::PreClientUpdate()
{
    ...
    // with a new match ID.
    if ( Plat_FloatTime() > m_flWaitingForNewMatchTime && m_flWaitingForNewMatchTime != 0.f )
    {
...
void CTFGCServerSystem::VoteKickPlayerRequestResponse( CSteamID voterSteamID, CSteamID targetSteamID, TFVoteKickReason eReason, bool bAllowed, bool bVoterInhibit, bool bTargetInhibit )
{
    ...
    if ( bAllowed )
    {
        ...
        if ( pVoteController )
        {
            ...
            bCreated = pVoteController->CreateVote( voterentindex, "Kick", UTIL_VarArgs( "%d %s", targetuserid, pszReason ) );
...
void CTFGCServerSystem::SetMatchPlayerDropped( CSteamID steamID, TFMatchLeaveReason eReason )
{
    ...
    switch ( eReason )
    {
        ...
        case TFMatchLeaveReason_IDLE:
            break;
        default:
            AssertMsg( false, "Unhandled TFMatchLeaveReason" );
    }
    ...
    if ( pObject->GetTypeID() == CTFGSLobby::k_nTypeID )
    {
        ...
        if ( m_bMMServerMode && bNewLobby )
        {
            ...
            if ( m_pMatchInfo )
            {
                ...
                MMLog(
                    "!! Received new anticipated lobby while running existing match. "
                    "Old match ID [ %llu ] ended [ %u ] "
                    "New matchID [ %llu ]\n",
                    m_pMatchInfo->m_nMatchID,
                    m_pMatchInfo->m_bMatchEnded,
                    pLobby->GetMatchID() );
                Assert( false );
        ...
        else
        {
            ...
            {
                MMLog(
                    "!! Got assigned a lobby not in server-setup state, or when not accepting lobbies. Rejecting.\n"
                    "Lobby matchID [ %llu ], existing match [ %llu ]\n",
                    pLobby->GetMatchID(),
                    m_pMatchInfo ? m_pMatchInfo->m_nMatchID : 0ull );
        ...
        if ( m_pMatchInfo )
        {
            ...
            if ( nExpectedLobbyMatchID != pLobby->GetMatchID() )
            {
                if ( m_pMatchInfo->m_nPreviousMatchID && m_pMatchInfo->m_nPreviousMatchID == pLobby->GetMatchID() )
                {
                    ...
                    {
                        MMLog(
                            "Bad state -- lobby is pointing at the previous match, but no request to update it "
                            " is queued. Lobby MatchID [ %llu ], Previous MatchID [ %llu ], "
                            "Current MatchID [ %llu ], Next MatchID [ %llu ]\n",
                            pLobby->GetMatchID(),
                            m_pMatchInfo->m_nPreviousMatchID,
                            m_pMatchInfo->m_nMatchID,
                ...
                else if ( m_bWaitingForNewMatchID )
                {
                    ...
                    MMLog(
                        "Received new matchID while waiting for new matchID. "
                ...
                else
                {
                    ...
                    MMLog(
                        "Received new matchID when we weren't expecting one! "
...
void CTFGCServerSystem::UpdateConnectedPlayersAndServerInfo( CMsgGameServerMatchmakingStatus_Event event, bool bForceSendMessages )
{
    ...
    for ( int i = 1; i <= gpGlobals->maxClients; i++ )
    {
        ...
        if ( pLobby )
        {
            ...
            if ( matchPlayers.BValidIdx( idxLobby ) && !matchPlayers.BIsPending( idxLobby ) )
            {
                ...
                if ( dbg_spew_connected_players_level.GetInt() >= 4 )
                {
                    Msg( "     '%s' In lobby with state %s\n", sPlayerInfo.name, CTFLobbyPlayerProto_ConnectState_Name( eLobbyState ).c_str() );
                }
                switch ( eLobbyState )
                {
        ...
        nUnconnectedPlayerReservationRequests + nClients <= gpGlobals->maxClients )
    {
        ...
        FOR_EACH_VEC( vecReservationRequests, idx )
        {
            ...
            AcceptGCReservation( vecReservationRequests[idx].m_steamID, member, !bNewMatch, vecReservationRequests[idx].m_nEntindex, vecReservationRequests[idx].m_bActive );
    ...
    switch ( TFGameRules()->State_Get() )
    {
        ...
        case GR_STATE_STALEMATE:
            if ( TFGameRules()->IsMannVsMachineMode() )
            {
                ...
                if (
                    TFGameRules()->State_Get() == GR_STATE_TEAM_WIN && TFGameRules()->GetWinningTeam() == TF_TEAM_PVE_DEFENDERS )
                {
    ...
    if ( m_bMMServerMode )
    {
        ...
        if ( !m_pMatchInfo && !pLobby )
        {
                ...
                ( nHumans && tf_mm_strict.GetInt() == 1 ) )
            {
                ...
                {
                    MMLog(
                        "!! No match, but no usable slots or unexpected clients, cannot enroll in matchmaking. "
