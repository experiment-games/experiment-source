...
const char *CBaseObject::GetResponseRulesModifier( void )
{
    ...
    {
        case OBJ_DISPENSER:
            return "objtype:dispenser";
            break;
        case OBJ_TELEPORTER:
            return "objtype:teleporter";
            break;
        case OBJ_SENTRYGUN:
            return "objtype:sentrygun";
            break;
        case OBJ_ATTACHMENT_SAPPER:
            return "objtype:sapper";
            break;
        default:
            break;
    }
...
void CBaseObject::SetSolidToPlayers( OBJSOLIDTYPE stp, bool force )
{
    ...
    {
        SetCollisionGroup(
            IsSolidToPlayers() ? TFCOLLISION_GROUP_OBJECT_SOLIDTOPLAYERMOVEMENT : TFCOLLISION_GROUP_OBJECT );
    }
}
...
int CBaseObject::DrawDebugTextOverlays( void )
{
    ...
    if ( m_debugOverlays & OVERLAY_TEXT_BIT )
    {
        ...
        Q_snprintf( tempstr, sizeof( tempstr ), "Built by: (%d) %s", pBuilder ? pBuilder->entindex() : -1, pBuilder ? pBuilder->GetPlayerName() : "invalid builder" );
        EntityText( text_offset, tempstr, 0 );
        text_offset++;
...
{
    const bool bShouldBeEnabled = !m_bHasSapper && !m_bPlasmaDisable && ( !TFGameRules()->RoundHasBeenWon() || TFGameRules()->GetWinningTeam() == GetTeamNumber() );
