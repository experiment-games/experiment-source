...
CTFBotGenerator::CTFBotGenerator( void )
    : m_bBotChoosesClass( false ), m_bSuppressFire( false ), m_bDisableDodge( false ), m_bUseTeamSpawnpoint( false ), m_bRetainBuildings( false ), m_bExpended( false ), m_iOnDeathAction( kOnDeath_RemoveSelf ), m_difficulty( CTFBot::UNDEFINED ), m_spawnCountRemaining( 0 ), m_bSpawnOnlyWhenTriggered( false ), m_bEnabled( true )
{
    SetThink( NULL );
}
...
CTFBotActionPoint::CTFBotActionPoint()
    : m_stayTime( 0.0f ), m_desiredDistance( 1.0f )
