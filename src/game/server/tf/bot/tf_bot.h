...
class CTFBot : public NextBotPlayer< CTFPlayer >, public CGameEventListener
{
    ...
    virtual CTFNavArea *GetLastKnownArea( void ) const
    {
        return static_cast< CTFNavArea * >( BaseClass::GetLastKnownArea() );
    ...
    // INextBot
    virtual CTFBotLocomotion *GetLocomotionInterface( void ) const
    {
        return m_locomotor;
    }
    virtual CTFBotBody *GetBodyInterface( void ) const
    {
        return m_body;
    }
    virtual CTFBotVision *GetVisionInterface( void ) const
    {
        return m_vision;
    }
    ...
    HSCRIPT ScriptGetSpawnArea( void ) const
    {
        return ToHScript( this->GetSpawnArea() );
    }
...
#define LOOK_FOR_ENEMIES true
    ...
    void ScriptSetAttentionFocus( HSCRIPT hFocusOn )
    {
        this->SetAttentionFocus( ToEnt( hFocusOn ) );
    }
    bool ScriptIsAttentionFocusedOn( HSCRIPT hWho ) const
    {
        return this->IsAttentionFocusedOn( ToEnt( hWho ) );
    }
    ...
    void ScriptDelayedThreatNotice( HSCRIPT hWho, float flNoticeDelay )
    {
        this->DelayedThreatNotice( ToEnt( hWho ), flNoticeDelay );
    }
    ...
    HSCRIPT ScriptFindVantagePoint( float maxTravelDistance )
    {
        return ToHScript( this->FindVantagePoint( maxTravelDistance ) );
    }
...
#define MY_CURRENT_GUN NULL                                           // can be passed as weapon to following queries
    ...
    int ScriptGetDifficulty( void ) const
    {
        return this->GetDifficulty();
    }
    void ScriptSetDifficulty( int difficulty )
    {
        this->SetDifficulty( ( DifficultyType )difficulty );
    }
    bool ScriptIsDifficulty( int difficulty ) const
    {
        return this->IsDifficulty( ( DifficultyType )difficulty );
    }
    ...
    HSCRIPT ScriptGetHomeArea( void )
    {
        return ToHScript( this->GetHomeArea() );
    }
    ...
    void ScriptSetActionPoint( HSCRIPT hPoint )
    {
        SetActionPoint( ScriptToEntClass< CTFBotActionPoint >( hPoint ) );
    }
    HSCRIPT ScriptGetActionPoint( void ) const
    {
        return ToHScript( GetActionPoint() );
    }
    ...
    void ScriptDisbandCurrentSquad( void )
    {
        if ( GetSquad() ) GetSquad()->DisbandAndDeleteSquad();
    }
    ...
    void ScriptGenerateAndWearItem( const char *pszItemName )
    {
        if ( pszItemName ) BotGenerateAndWearItem( this, pszItemName );
    }
...
#define MISSION_DOESNT_RESET_BEHAVIOR_SYSTEM false
    ...
    void ScriptSetMissionTarget( HSCRIPT hTarget )
    {
        this->SetMissionTarget( ToEnt( hTarget ) );
    }
    HSCRIPT ScriptGetMissionTarget( void ) const
    {
        return ToHScript( this->GetMissionTarget() );
    }
    ...
    HSCRIPT ScriptGetNearestKnownSappableTarget( void )
    {
        return ToHScript( this->GetNearestKnownSappableTarget() );
    }
    ...
    bool ShouldQuickBuild() const
    {
        return m_bForceQuickBuild;
    }
    void SetShouldQuickBuild( bool bShouldQuickBuild )
    {
        m_bForceQuickBuild = bShouldQuickBuild;
    }

    void SetAutoJump( float flAutoJumpMin, float flAutoJumpMax )
    {
        m_flAutoJumpMin = flAutoJumpMin;
        m_flAutoJumpMax = flAutoJumpMax;
    }
    bool ShouldAutoJump();

    void SetFlagTarget( CCaptureFlag *pFlag );
    CCaptureFlag *GetFlagTarget() const
    {
        return m_hFollowingFlagTarget;
    }
    bool HasFlagTaget() const
    {
        return m_hFollowingFlagTarget != NULL;
    }
