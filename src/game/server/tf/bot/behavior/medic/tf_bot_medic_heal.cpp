...
ActionResult< CTFBot > CTFBotMedicHeal::Update( CTFBot *me, float interval )
{
    ...
    if ( medigun )
    {
        if ( medigun->GetMedigunType() == MEDIGUN_RESIST )
        {
            ...
            while ( ( me->HasAttribute( CTFBot::PREFER_VACCINATOR_BULLETS ) && medigun->GetResistType() != MEDIGUN_BULLET_RESIST ) || ( me->HasAttribute( CTFBot::PREFER_VACCINATOR_BLAST ) && medigun->GetResistType() != MEDIGUN_BLAST_RESIST ) || ( me->HasAttribute( CTFBot::PREFER_VACCINATOR_FIRE ) && medigun->GetResistType() != MEDIGUN_FIRE_RESIST ) )
            {
