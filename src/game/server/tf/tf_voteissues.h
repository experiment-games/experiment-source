...
class CKickIssue : public CBaseTFIssue
{
    ...
    virtual const char *GetVotePassedString( void ) OVERRIDE;
    virtual bool IsTeamRestrictedVote( void ) OVERRIDE
    {
        return true;
    }
    ...
    virtual EVoteAction ProcessResults( const CUtlVector< const char * > &vecOptions, const int arVoteCountByOption[], const CUtlMap< CSteamID, int > &mapVotesBySteamID, int nHighestCountOption, int nTotalVotes, int nPotentialVoters ) OVERRIDE;
...
class CEnableTemporaryHalloweenIssue : public CBaseTFIssue
{
    ...
    {
        return false;
    }  // we'll manually set up this vote when an item gets used
    virtual const char *GetDisplayString( void ) OVERRIDE
    {
        return "#TF_vote_eternaween";
    }
    virtual const char *GetVotePassedString( void ) OVERRIDE
    {
        return "#TF_vote_passed_eternaween";
    }

    virtual bool BRecordVoteFailureEventForEntity( int iVoteCallingEntityIndex ) const OVERRIDE
    {
        return true;
    }
