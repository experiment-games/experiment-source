...
class CTFNavArea : public CNavArea
{
   public:
    ...
    Vector ScriptGetCorner( int corner ) const
    {
        return GetCorner( ( NavCornerType )corner );
    }
    void ScriptMarkAsBlocked( int teamID );
    int ScriptGetAdjacentCount( int dir ) const
    {
    ...
    int ScriptGetPlayerCount( int teamID ) const
    {
        return GetPlayerCount( teamID );
    }
    bool ScriptIsOverlapping( HSCRIPT hArea ) const;
    bool ScriptIsOverlappingOrigin( const Vector &pos, float tolerance ) const
    {
        return IsOverlapping( pos, tolerance );
    }
    bool ScriptIsEdge( int dir ) const
    {
        return IsEdge( ( NavDirType )dir );
    }
    bool ScriptContains( HSCRIPT hArea ) const;
    bool ScriptContainsOrigin( const Vector &pos ) const
    {
        return Contains( pos );
    }
    float ScriptComputeGroundHeightChange( HSCRIPT hArea );
    HSCRIPT ScriptGetParent( void );
    int ScriptGetParentHow( void ) const
    {
        return GetParentHow();
    }
    void ScriptUnblockArea( void );
    bool ScriptIsVisible( const Vector &eye ) const
    {
        return IsVisible( eye );
    }
    float ScriptGetZ( const Vector &pos ) const
    {
        return GetZ( pos );
    }
...
template <>
ScriptClassDesc_t *GetScriptDesc< CTFNavArea >( CTFNavArea * );
