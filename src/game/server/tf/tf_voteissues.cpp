...
const char *CKickIssue::GetDisplayString( void )
{
    ...
    {
        case TFVoteKickReason_Other:
            return "#TF_vote_kick_player_other";
        case TFVoteKickReason_Cheating:
            return "#TF_vote_kick_player_cheating";
        case TFVoteKickReason_Idle:
            return "#TF_vote_kick_player_idle";
        case TFVoteKickReason_Scamming:
                                                    ...
                                                    int nPotentialVoters )
{
    ...
    else if ( bTargetInMatch )
    {
        ...
        EVoteAction eDefault = CBaseIssue::ProcessResults( vecOptions, arVoteCountByOption, mapVotesBySteamID, nHighestCountOption, nTotalVotes, nPotentialVoters );
        bool bWouldPass = ( eDefault == eVoteAction_Pass );

        // Match, but haven't submitted this (first time called, since no other options stall)
        GTFGCClientSystem()->SubmitVoteKickResults( m_steamIDVoteCaller, m_steamIDVoteTarget, m_eKickReason, mapVotesBySteamID, bWouldPass );
    ...
    else
    {
        ...
        // thing
        return CBaseIssue::ProcessResults( vecOptions, arVoteCountByOption, mapVotesBySteamID, nHighestCountOption, nTotalVotes, nPotentialVoters );
    }
}
