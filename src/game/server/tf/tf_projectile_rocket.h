...
class CTFProjectile_Rocket : public CTFBaseRocket, public IScorer
{
   public:
    ...
    void SetDirectHit( bool bDirectHit )
    {
        m_bDirectHit = bDirectHit;
    }
    virtual int GetWeaponID( void ) const
    {
        return ( m_bDirectHit ? TF_WEAPON_ROCKETLAUNCHER_DIRECTHIT : TF_WEAPON_ROCKETLAUNCHER );
    }

    void SetEyeBallRocket( bool state )
    {
        m_bEyeBallRocket = state;
    }
