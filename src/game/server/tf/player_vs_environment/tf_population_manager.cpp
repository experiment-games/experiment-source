...
void CPopulationManager::PlayerDoneViewingLoot( const CTFPlayer *pPlayer )
{
    ...
    if ( m_donePlayers.Find( pPlayer ) == m_donePlayers.InvalidIndex() && playerVector.Find( pPlayer ) != playerVector.InvalidIndex() )
    {
...
void CPopulationManager::AdjustMinPlayerSpawnTime( void )
{
    ...
    {
        flTime = m_bFixedRespawnWaveTime ? m_nRespawnWaveTime : MIN( m_nRespawnWaveTime, float( iWaveNum * kMvMRespawnTimeAddPerWave ) );
    }
    ...
    while ( nBotCash >= 100 && vecAvailableUpgrades.Count() > 0 )
    {
        ...
        FOR_EACH_VEC( m_EndlessActiveBotUpgrades, iUpgrade )
        {
            if ( !V_strcmp( m_EndlessActiveBotUpgrades[iUpgrade].szAttrib, upgrade.szAttrib ) )
            {
                ...
                if ( ( upgrade.flMax > 0 && upgrade.flMax <= m_EndlessActiveBotUpgrades[iUpgrade].flValue ) || ( upgrade.flMax < 0 && upgrade.flMax >= m_EndlessActiveBotUpgrades[iUpgrade].flValue ) )
                {
        ...
        if ( !bUpgradeFound )
        {
            ...
            nBotCash -= upgrade.nCost;
            // remove this upgrade if its been max
            if ( ( upgrade.flMax > 0 && upgrade.flMax <= upgrade.flValue ) || ( upgrade.flMax < 0 && upgrade.flMax >= upgrade.flValue ) )
            {
                vecAvailableUpgrades.FastRemove( index );
            }
        }
