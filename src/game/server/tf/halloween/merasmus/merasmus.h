...
class CMerasmus : public CHalloweenBaseBoss, public CGameEventListener, public IMerasmusAutoList
{
   public:
    ...
    // INextBot
    virtual CMerasmusIntention *GetIntentionInterface( void ) const
    {
        return m_intention;
    }
    virtual ILocomotion *GetLocomotionInterface( void ) const
    {
        if ( m_isFlying ) return m_flyingLocomotor;
        return m_locomotor;
    }
    virtual CMerasmusBody *GetBodyInterface( void ) const
    {
        return m_body;
    }
    ...
    void SetRevealer( CTFPlayer *pPlayer )
    {
        m_hMerasmusRevealer = pPlayer;
    }
    ...
    void StartAOEAttack()
    {
        m_bIsDoingAOEAttack = true;
    }
    void StopAOEAttack()
    {
        m_bIsDoingAOEAttack = false;
    }
    bool IsDoingAOEAttack() const
    {
        return m_bIsDoingAOEAttack;
    }
    ...
    int GetBombHitCount() const
    {
        return m_nBombHitCount;
    }
    void ResetBombHitCount()
    {
        m_nBombHitCount = 0;
    }
    ...
    void OnBeginStun();
    void OnEndStun();
    bool HasStunTimer() const
    {
        return !m_stunTimer.IsElapsed();
    }
    ...
    {
        return HALLOWEEN_BOSS_MERASMUS;
    }
