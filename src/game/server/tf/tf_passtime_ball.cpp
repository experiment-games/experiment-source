...
CTFPlayer *CPasstimeBall::GetCarrier() const
{
    return m_hCarrier;
}
CTFPlayer *CPasstimeBall::GetPrevCarrier() const
{
    return m_hPrevCarrier;
}
...
//-----------------------------------------------------------------------------
int CPasstimeBall::GetCollisionCount() const
{
...
static const char *GetTrailEffectForTeam( int iTeam )
{
    ...
    {
        case TF_TEAM_RED:
            return "passtime/passtime_balltrail_red.vmt";
        case TF_TEAM_BLUE:
            return "passtime/passtime_balltrail_blu.vmt";
        default:
            return "passtime/passtime_balltrail_unassigned.vmt";
    };
}
...
//-----------------------------------------------------------------------------
bool CPasstimeBall::BOutOfPlay() const
{
    return m_eState == STATE_OUT_OF_PLAY;
}
...
{
    if ( !TFGameRules() || ( TFGameRules()->State_Get() != GR_STATE_RND_RUNNING ) || ( m_eState != STATE_FREE ) )
    {
        return false;
    }
...
void CPasstimeBall::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
    ...
    if ( g_pPasstimeLogic && ( g_pPasstimeLogic->GetBall() == this ) && g_pPasstimeLogic->OnBallCollision( this, index, pEvent ) && IsGroundCollision( index, pEvent ) )
    {
        OnCollision();
    }
