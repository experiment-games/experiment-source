...
typedef struct botdata_t
{
    ...
    //------------------------------------------------------
    // Sensing
    void SetIgnoreHumans( bool bIgnore )
    {
        m_bIgnoreHumans = bIgnore;
    }
    void SetFrozen( bool bFrozen )
    {
        if ( bFrozen )
            m_hBot->AddEFlags( EFL_BOT_FROZEN );
        else
            m_hBot->RemoveEFlags( EFL_BOT_FROZEN );
    }
    ...
    CBaseEntity *GetCommandTarget( void )
    {
        return m_Commands.Count() ? m_Commands[0].pTarget : NULL;
    }
    Vector GetCommandPosition( void )
    {
        return m_Commands.Count() ? m_Commands[0].vecTarget : vec3_origin;
    }
...
CBasePlayer *BotPutInServer( bool bTargetDummy, bool bFrozen, int iTeam, int iClass, const char *pszCustomName )
{
    ...
    else if ( bot_randomnames.GetBool() )
    {
        ...
        {
            case 0:
                Q_snprintf( botname, sizeof( botname ), "Bot" );
                break;
            case 1:
                Q_snprintf( botname, sizeof( botname ), "This is a medium Bot" );
                break;
            case 2:
                Q_snprintf( botname, sizeof( botname ), "This is a super long bot name that is too long for the game to allow" );
                break;
            case 3:
                Q_snprintf( botname, sizeof( botname ), "Another bot" );
                break;
            case 4:
                Q_snprintf( botname, sizeof( botname ), "Yet more Bot names, medium sized" );
                break;
            default:
                Q_snprintf( botname, sizeof( botname ), "B" );
                break;
        }
    }
    else
