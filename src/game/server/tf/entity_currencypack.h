...
class CCurrencyPack : public CTFPowerup, public ICurrencyPackAutoList
{
    ...
    virtual bool AffectedByRadiusCollection() const
    {
        return true;
    }
    ...
    {
        m_bClaimed = true;
    }  // Radius collection code "steers" packs toward the player
    bool IsClaimed( void )
    {
        return m_bClaimed;
    }  // So don't allow other players to interfere
    void DistributedBy( CBasePlayer *pMoneyMaker );

    virtual CurrencyRewards_t GetPackSize( void )
    {
        return TF_CURRENCY_PACK_LARGE;
    }
...
class CCurrencyPackMedium : public CCurrencyPack
{
    ...
    virtual CurrencyRewards_t GetPackSize( void )
    {
        return TF_CURRENCY_PACK_MEDIUM;
    }
    virtual const char *GetDefaultPowerupModel( void )
    {
...
class CCurrencyPackCustom : public CCurrencyPack
{
    ...
    virtual CurrencyRewards_t GetPackSize( void )
    {
        return TF_CURRENCY_PACK_CUSTOM;
    }
    virtual const char *GetDefaultPowerupModel( void );
};
