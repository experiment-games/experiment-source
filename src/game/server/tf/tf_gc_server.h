...
class CMatchInfo
{
    ...
    struct PlayerMatchData_t
    {
        ...
        PlayerMatchData_t( CSteamID steamID, ConstTFLobbyPlayer memberData )
            : steamID( steamID ), strOriginalName( memberData.GetName() ), uOriginalPartyID( memberData.GetOriginalPartyID() ), eGCTeam( memberData.GetTeam() ), bDropped( false ), bConnected( false ), rtJoinedMatch( CRTime::RTime32TimeCur() ), bPendingVoteKickRequest( false ), bCannotCallVoteKicks( false ), bCannotBeTargetedByVoteKicks( false ), nVoteKickAttempts( 0 ), nDisconnectedSeconds( 0 ), nScoreMedal( 0 ), nKillsMedal( 0 ), nDamageMedal( 0 ), nHealingMedal( 0 ), nSupportMedal( 0 ), bLateJoin( false ), nScore( 0 ), bPlayed( false ), bEverConnected( false ), bEverActive( false ), bEverDisconnected( false ), flNormalizedMMSkillRating( memberData.GetNormalizedRating() ), nRank( memberData.GetRank() ), unClassesPlayed( 0u ), bChatSuspension( memberData.GetChatSuspension() ), bVoteKickPending( false ), rtLastActiveEvent( CRTime::RTime32TimeCur() ), bAlwaysSafeToLeave( false ), bDropWasAbandon( false ), eDropReason( TFMatchLeaveReason_UNSPECIFIED ), nConnectingButNotActiveIndex( 0 ), m_mapXPAccumulation( DefLessFunc( CMsgTFXPSource::XPSourceType ) )
        {
        ...
        const CMsgTFXPSourceBreakdown &GetXPSources() const
        {
            return m_XPBreakdown;
        }
        ...
        void MarkAlwaysSafeToLeave()
        {
            bAlwaysSafeToLeave = true;
        }

        bool BDropWasAbandon()
        {
            return bDropped && bDropWasAbandon;
        }
        TFMatchLeaveReason GetDropReason()
        {
            return bDropped ? eDropReason : TFMatchLeaveReason_UNSPECIFIED;
        }
        RTime32 GetLastActiveEventTime( void )
        {
            return rtLastActiveEvent;
        }
        ...
        {
            XPBonusPool_t()
                : m_nBonusPoolRemaining( 0 ), m_flMultiplier( 1.f )
            {
            }
    ...
    bool BSentResult() const
    {
        return m_bSentResult;
    }
    ...
    const char *GetMatchMap() const
    {
        return m_strMapName.Length() ? m_strMapName.Get() : NULL;
    }
...
class CTFGCServerSystem : public CGCClientSystem, public GCSDK::ISharedObjectListener, public IServerGCLobby, public CGameEventListener
{
    ...
    const CMatchInfo *GetMatch() const
    {
        return m_pMatchInfo;
    }
    ...
    CMatchInfo *GetLiveMatch()
    {
        return ( m_pMatchInfo && !m_pMatchInfo->m_bMatchEnded ) ? m_pMatchInfo : NULL;
    }
    const CMatchInfo *GetLiveMatch() const
    {
        return const_cast< CTFGCServerSystem * >( this )->GetLiveMatch();
    }
    ...
    void SubmitVoteKickResults( CSteamID steamIDInitiator, CSteamID steamIDTarget, TFVoteKickReason eReason, const CUtlMap< CSteamID, int > &mapVotesBySteamID, bool bDefaultPass );
    ...
    inline bool IsMMServerModeActive() const
    {
        return m_bMMServerMode;
    }
    ...
    float GetTimeLastConnectedToGC( void )
    {
        return m_timeLastConnectedToGC;
    }
    ...
    double GetTimeRequestedLateJoin()
    {
        return m_flTimeRequestedLateJoin;
    }
    ...
    // Reliable Messages
    //
    const CReliableMessageQueue &ReliableMsgQueue() const
    {
        return m_ReliableMsgQueue;
    }
    CReliableMessageQueue &ReliableMsgQueue()
    {
        return m_ReliableMsgQueue;
    }
    bool BPendingReliableMessages() const
    {
        return ReliableMsgQueue().NumPendingMessages() > 0;
    }
    bool BStalledReliableMessages() const
    {
        return ReliableMsgQueue().BStalled();
    }
    ...
    bool BPendingNewMatch() const
    {
        return m_flWaitingForNewMatchTime != 0.f;
    }
