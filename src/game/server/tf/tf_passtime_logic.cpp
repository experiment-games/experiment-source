...
//-----------------------------------------------------------------------------
CPasstimeBall *CTFPasstimeLogic::GetBall() const
{
    return m_hBall;
}
...
static CBaseEntityOutput *FindOutput( CBaseEntity *pEnt, const char *pOutputName )
{
    ...
    while ( dmap )
    {
        ...
        for ( int i = 0; i < fields; i++ )
        {
            ...
            if ( ( dataDesc->fieldType == FIELD_CUSTOM ) && ( dataDesc->flags & FTYPEDESC_OUTPUT ) && !strcmp( dataDesc->externalName, pOutputName ) )
            {
...
// Would belong in BaseEnity, but this this hacky so I'm just hiding it here.
//------------------------------------------------------------------------------
static void HookOutput( const char *pSourceName, string_t pTargetName, const char *pOutputName, const char *pInputName, const char *pParameter = nullptr, int nTimesToFire = EVENT_FIRE_ALWAYS )
{
...
static uint64 CalcPackMemberBits( CTFPlayer *pBallCarrier )
{
    ...
    for ( int i = 1; i <= MAX_PLAYERS; ++i, nMask <<= 1 )
    {
        ...
        if ( !pPlayer || !pPlayer->IsAlive() || ( pPlayer->GetTeamNumber() != iCarrierTeam ) || ( pPlayer->GetAbsOrigin().DistToSqr( vecCarrierPos ) > flPackRangeSqr ) )
        {
            continue;
        }
...
{
    MapEventStat( input.pActivator, GetBall(), &CTF_GameStats.m_passtimeStats.summary.nTotalSpeedBoosts, &CTF_GameStats.m_passtimeStats.summary.nTotalCarrierSpeedBoosts );
}

//-----------------------------------------------------------------------------
void CTFPasstimeLogic::InputJumpPadUsed( inputdata_t &input )
{
    MapEventStat( input.pActivator, GetBall(), &CTF_GameStats.m_passtimeStats.summary.nTotalJumpPads, &CTF_GameStats.m_passtimeStats.summary.nTotalCarrierJumpPads );
}
...
{
    if ( IsGamestatePlayable() && pGoal->BEnablePlayerScore() && pPlayer->m_Shared.HasPasstimeBall() && ( pPlayer->GetTeamNumber() == pGoal->GetTeamNumber() ) )
    {
        Score( pPlayer, pGoal );
    }
}
...
bool CTFPasstimeLogic::BCanPlayerPickUpBall( CTFPlayer *pPlayer, HudNotification_t *pReason ) const
{
    ...
    if ( pPlayer->m_Shared.InCond( TF_COND_INVULNERABLE ) || pPlayer->m_Shared.InCond( TF_COND_PHASE ) || pPlayer->m_Shared.InCond( TF_COND_INVULNERABLE_WEARINGOFF ) )
    {
    ...
    auto bEnemyBall = ( pBall->GetTeamNumber() != TEAM_UNASSIGNED ) && ( pBall->GetTeamNumber() != pPlayer->GetTeamNumber() );
    ...
    if ( pActiveWeapon )
    {
        bool bCanHolster = pActiveWeapon->CanHolster() && !( pActiveWeapon->IsReloading() && pActiveWeapon->ReloadsSingly() && pActiveWeapon->CanOverload() );  // semihack to fix beggars bazooka problems
...
void CTFPasstimeLogic::RespawnBall()
{
    ...
    bool bGameOver = ( TFTeamMgr()->GetFlagCaptures( TF_TEAM_RED ) >= iScoreLimit ) || ( TFTeamMgr()->GetFlagCaptures( TF_TEAM_BLUE ) >= iScoreLimit );
...
//-----------------------------------------------------------------------------
void CTFPasstimeLogic::OnBallCarrierDamaged( CTFPlayer *pPlayer, CTFPlayer *pAttacker, const CTakeDamageInfo &info )
{
    // TODO refactor OnBallCarrierMeleeHit and OnBallCarrierDamaged for less copypasta
...
void CTFPasstimeLogic::Score( CTFPlayer *pPlayer, int iTeam, int iPoints, bool bForceWin )
{
    ...
    //
    {
        ...
        for ( unsigned short i = 0; i < m_ballLastHeldTimes.Count(); i++ )
        {
            ...
            if ( ( pPossibleAssister->GetTeamNumber() == pPlayer->GetTeamNumber() ) && ( pPossibleAssister != pPlayer ) && ( flSecAgo < 10.0f ) && ( flSecAgo < flAssisterTime ) )
            {
...
void CTFPasstimeLogic::OnPlayerTouchBall( CTFPlayer *pCatcher, CPasstimeBall *pBall )
{
    ...
    if ( ( ( iExperiment == EPasstimeExperiment_Telepass::TeleportToCatcherMaintainPossession ) || ( iExperiment == EPasstimeExperiment_Telepass::SwapWithCatcher ) ) && BCanPlayerPickUpBall( pThrower, nullptr ) )
    {
...
struct SetSectionParams
{
    ...
    SetSectionParams()
        : num( -1 ), pSectionStart( 0 ), pSectionEnd( 0 ) {}
};
...
bool CTFPasstimeLogic::ParseSetSection( const char *pStr, SetSectionParams &s ) const
{
    ...
    return ( s.num >= 0 ) && ( s.num < m_iNumSections ) && s.pSectionStart && s.pSectionEnd;
}
...
template < typename T >
variant_t make_variant( T value );
template <>
variant_t make_variant( int value )
{
    variant_t v;
    v.SetInt( value );
    return v;
...
void CTFPasstimeLogic::statica( inputdata_t &input )
{
    ...
    if ( !pActivator || ( info.iTeam != iActivatorTeam ) || ( info.iClass != iActivatorClass ) )
    {
...
void CTFPasstimeLogic::staticc( inputdata_t &input )
{
    ...
    for ( const auto &info : s_SecretRoom_TriggerInfo )
    {
        g_EventQueue.AddEvent( info.pTriggerName, "Enable", variant_t(), 0.0f, this, this );
    }
}
...
void CTFPasstimeLogic::SecretRoom_Solve()
{
    ...
    for ( const auto &info : s_SecretRoom_TriggerInfo )
    {
        g_EventQueue.AddEvent( info.pTriggerName, "Disable", variant_t(), 0.0f, this, this );
    }
