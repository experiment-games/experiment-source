...
CFuncNoBuild::CFuncNoBuild()
    : m_bAllowSentry( false ), m_bAllowDispenser( false ), m_bAllowTeleporters( false )
{
}
...
bool PointInNoBuild( const Vector &vecBuildOrigin, const CBaseObject *pObj )
{
    ...
    for ( int i = 0; i < IFuncNoBuildAutoList::AutoList().Count(); ++i )
    {
        ...
        if ( pNoBuild->GetActive() && pNoBuild->PointIsWithin( vecBuildOrigin ) && pNoBuild->PreventsBuildOf( pObj->GetType() ) )
        {
