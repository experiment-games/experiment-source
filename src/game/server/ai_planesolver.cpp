...
bool CAI_PlaneSolver::MoveLimit( Navigation_t navType, const Vector &target, bool ignoreTransients, bool fCheckStep, int contents, AIMoveTrace_t *pMoveTrace )
{
    ...
    return pProbe->MoveLimit( navType, GetLocalOrigin(), target, contents, m_pNpc->GetNavTargetEntity(), ( fCheckStep ) ? 100 : 0, flags, pMoveTrace );
}
                                                    ...
                                                    int probeOffset )
{
    ...
    {
        suggestion.Set( type, 0, arcCenter, arcSpan, moveTrace.pObstruction );
    ...
    suggestion.Set( type, weight, arcCenter, thirdSpan, moveTrace.pObstruction );

    m_Solver.AddRegulation( suggestion );

    suggestion.Set( type, ( favorRight ) ? favoredWeight : weight, arcCenter - thirdSpan, thirdSpan, moveTrace.pObstruction );

    m_Solver.AddRegulation( suggestion );

    suggestion.Set( type, ( favorLeft ) ? favoredWeight : weight, arcCenter + thirdSpan, thirdSpan, moveTrace.pObstruction );
...
void CAI_PlaneSolver::CalcYawsFromOffset( float yawScanCenter, float spanPerProbe, int probeOffset, float *pYawTest, float *pYawCenter )
{
    if ( probeOffset != 0 )
    {
...
AI_SuggestorResult_t CAI_PlaneSolver::GenerateObstacleSuggestion( const AILocalMoveGoal_t &goal, float yawScanCenter, float probeDist, float spanPerProbe, int probeOffset )
{
...
AI_SuggestorResult_t CAI_PlaneSolver::GenerateObstacleSuggestions( const AILocalMoveGoal_t &goal, const AIMoveTrace_t &directTrace, float distClear, float probeDist, float degreesToProbe, int nProbes )
{
    Assert( nProbes % 2 == 1 );
    ...
    if ( PLANE_SOLVER_THINK_FREQUENCY[AIStrongOpt()] == 0.0 || m_RefreshSamplesTimer.Expired() )
    {
        ...
        for ( i = 1; i <= nSideProbes; i++ )
        {
            if ( !foundClear )
            {
                AI_SuggestorResult_t curSeekResult = GenerateObstacleSuggestion( goal, yawGoalDir, probeDist, spanPerProbe, i );
...
bool CAI_PlaneSolver::RunMoveSolver( const AILocalMoveGoal_t &goal, const AIMoveTrace_t &directTrace, float degreesPositiveArc, bool fDeterOscillation, Vector *pResult )
{
