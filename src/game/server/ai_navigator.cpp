...
bool CAI_Navigator::PrependLocalAvoidance( float distObstacle, const AIMoveTrace_t &directTrace )
{
    ...
    if ( distObstacle < GetHullWidth() * 0.5 )
    {
        ...
        if ( GetMoveProbe()->MoveLimit( GetNavType(), GetLocalOrigin(), vTestBackaway, MASK_NPCSOLID, GetNavTargetEntity(), 100.0, flags, &backawayTrace ) )
        {
    ...
    if ( !GetMoveProbe()->MoveLimit( GetNavType(), GetLocalOrigin(), testPos, MASK_NPCSOLID, GetNavTargetEntity(), 100.0, flags ) )
    {
...
void CAI_Navigator::SimplifyFlyPath( unsigned collisionMask, const CBaseEntity *pTarget, float strictPointTolerance, float blockTolerance, AI_NpcBlockHandling_t blockHandling )
{
    AI_ProgressFlyPathParams_t params( collisionMask, strictPointTolerance, blockTolerance, 0, 0, blockHandling );
...
bool CAI_Navigator::CanFitAtPosition( const Vector &vStartPos, unsigned int collisionMask, bool bIgnoreTransients, bool bAllowPlayerAvoid )
{
    ...
    vEndPos.z += 0.01;
    trace_t tr;
    AI_TraceHull( vStartPos, vEndPos, GetHullMins(), GetHullMaxs(), collisionMask, &traceFilter, &tr );
