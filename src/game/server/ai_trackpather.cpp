...
float CAI_TrackPather::MaxDistanceFromCurrentPath() const
{
    ...
    float t;
    Vector vecTemp;
    CalcClosestPointOnLine( GetAbsOrigin(), pPrevPath->GetAbsOrigin(), m_pCurrentPathTarget->GetAbsOrigin(), vecTemp, &t );
    t = clamp( t, 0.0f, 1.0f );
...
float CAI_TrackPather::ClosestPointToCurrentPath( Vector *pVecPoint ) const
{
    ...
    float t;
    CalcClosestPointOnLine( GetAbsOrigin(), m_vecSegmentStartPoint, m_pCurrentPathTarget->GetAbsOrigin(), *pVecPoint, &t );
    return t;
}
...
void CAI_TrackPather::ComputePathTangent( float t, Vector *pVecTangent ) const
{
    ...
    Catmull_Rom_Spline_Tangent( m_vecSegmentStartSplinePoint, m_vecSegmentStartPoint, m_pCurrentPathTarget->GetAbsOrigin(), pNextTrack->GetAbsOrigin(), t, *pVecTangent );
