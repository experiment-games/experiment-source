...
void CAI_BaseNPC::TryRestoreHull( void )
{
    if ( IsUsingSmallHull() && GetCurSchedule() )
    {
        ...
        AI_TraceHull( GetAbsOrigin(), vUpBit, GetHullMins(), GetHullMaxs(), MASK_SOLID, this, COLLISION_GROUP_NONE, &tr );
...
void CAI_BaseNPC::SetPlayerAvoidState( void )
{
    ...
    if ( m_bPerformAvoidance || ( ShouldPlayerAvoid() && bIsMoving ) )
    {
        ...
        if ( pLocalPlayer )
        {
            bShouldPlayerAvoid = IsBoxIntersectingBox( GetAbsOrigin() + vMins, GetAbsOrigin() + vMaxs, pLocalPlayer->GetAbsOrigin() + pLocalPlayer->WorldAlignMins(), pLocalPlayer->GetAbsOrigin() + pLocalPlayer->WorldAlignMaxs() );
        }
        ...
        if ( IsInSquad() && GetSquad() && !CAI_Squad::IsSilentMember( this ) && ( GetSquad()->IsLeader( this ) || GetSquad()->NumMembers() == 1 ) )
        {
            ...
            for ( CAI_BaseNPC *pSquadMember = pSquad->GetFirstMember( &iter, false ); pSquadMember; pSquadMember = pSquad->GetNextMember( &iter, false ) )
            {
                if ( pSquadMember != this )
                    NDebugOverlay::Line( EyePosition(), pSquadMember->EyePosition(), 0, CAI_Squad::IsSilentMember( pSquadMember ) ? 127 : 255, 0, false, 0.1 );
            }
        }
    }
    ...
    if ( bWeaponRequired )
    {
        ...
        if ( !sUniqueActivities.Find( weaponTranslation ) )
        {
            ...
            DevWarning( "%s missing activity \"%s\" needed by weapon\"%s\"\n",
                        GetClassname(),
                        GetActivityName( weaponTranslation ),
...
void CAI_BaseNPC::MaintainActivity( void )
{
    ...
    if ( ( GetActivity() != m_IdealActivity ) || ( GetSequence() != m_nIdealSequence ) )
    {
        if ( ai_sequence_debug.GetBool() == true && ( m_debugOverlays & OVERLAY_NPC_SELECTED_BIT ) )
        {
            DevMsg( "MaintainActivity %s : %s:%s -> %s:%s\n", GetClassname(), GetActivityName( GetActivity() ), GetSequenceName( GetSequence() ), GetActivityName( m_IdealActivity ), GetSequenceName( m_nIdealSequence ) );
        }
...
void CAI_BaseNPC::StartTargetHandling( CBaseEntity *pTargetEnt )
{
    ...
    AI_NavGoal_t goal( GOALTYPE_PATHCORNER, pTargetEnt->GetAbsOrigin(), bIsFlying ? ACT_FLY : ACT_WALK, AIN_DEF_TOLERANCE, AIN_YAW_TO_DEST );
    ...
    if ( m_debugOverlays & OVERLAY_TEXT_BIT )
    {
        ...
        if ( GetCurSchedule() )
        {
            ...
            if ( m_debugOverlays & OVERLAY_NPC_TASK_BIT )
            {
                ...
                {
                    Q_snprintf( tempstr, sizeof( tempstr ), "%s%s%s%s", ( ( i == 0 ) ? "Task:" : "       " ), ( ( i == GetScheduleCurTaskIndex() ) ? "->" : "   " ), TaskName( GetCurSchedule()->GetTaskList()[i].iTask ), ( ( i == GetScheduleCurTaskIndex() ) ? "<-" : "" ) );
        ...
        {
            char tmp[512];
            Q_snprintf( tmp, sizeof( tmp ), "Vel %.1f %.1f %.1f   Ang: %.1f %.1f %.1f\n", GetAbsVelocity().x, GetAbsVelocity().y, GetAbsVelocity().z, GetLocalAngularVelocity().x, GetLocalAngularVelocity().y, GetLocalAngularVelocity().z );
    ...
    if ( flDistSqr > pInteraction->flDistSqr )
    {
        if ( bDebug )
        {
            if ( m_debugOverlays & OVERLAY_NPC_SELECTED_BIT || pOtherNPC->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT )
            {
                ...
                {
                    Msg( "   %s distsqr: %0.2f (%0.2f %0.2f %0.2f), desired: <%0.2f (%0.2f %0.2f %0.2f)\n", GetDebugName(), flDistSqr, pOtherNPC->GetAbsOrigin().x, pOtherNPC->GetAbsOrigin().y, pOtherNPC->GetAbsOrigin().z, pInteraction->flDistSqr, vecOrigin.x, vecOrigin.y, vecOrigin.z );
                }
            }
        }
        return false;
