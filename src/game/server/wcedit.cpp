...
void NWCEdit::CreateAINode( CBasePlayer *pPlayer )
{
    ...
    if ( bPositionValid )
    {
        ...
        vUpBit.z += 1;
        trace_t tr;
        UTIL_TraceHull( testHull->GetAbsOrigin(), vUpBit, NAI_Hull::Mins( hullType ), NAI_Hull::Maxs( hullType ), MASK_NPCSOLID, testHull, COLLISION_GROUP_NONE, &tr );
        ...
        CON_COMMAND( hammer_update_entity, "Updates the entity's position/angles when in edit mode" )
{
    ...
    if ( args.ArgC() < 2 )
    {
        ...
        Vector forward;
        pPlayer->EyeVectors( &forward );
        UTIL_TraceLine( pPlayer->EyePosition(), pPlayer->EyePosition() + forward * MAX_COORD_RANGE, MASK_SHOT_HULL | CONTENTS_GRATE | CONTENTS_DEBRIS, pPlayer, COLLISION_GROUP_NONE, &tr );
