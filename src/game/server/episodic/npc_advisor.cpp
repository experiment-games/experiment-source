        ...
        case TASK_ADVISOR_STAGE_OBJECTS:
        {
            ...
            if ( m_hvStagedEnts.Count() < m_iStagingNum )
            {
                ...
                if ( pThrowable )
                {
                    ...
                    DispatchParticleEffect( "advisor_object_charge", PATTACH_ABSORIGIN_FOLLOW, pThrowable, 0, false );
                }
            }
...
void CNPC_Advisor::PreHurlClearTheWay( CBaseEntity *pThrowable, const Vector &toPos )
{
    ...
    ray.Init( pThrowable->GetAbsOrigin(), toPos, Vector( -boundingRadius, -boundingRadius, -boundingRadius ), Vector( boundingRadius, boundingRadius, boundingRadius ) );
