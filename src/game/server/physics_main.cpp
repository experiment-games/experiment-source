...
//-----------------------------------------------------------------------------
static void Physics_TraceEntity( CBaseEntity *pBaseEntity, const Vector &vecAbsStart, const Vector &vecAbsEnd, unsigned int mask, trace_t *ptr )
{
...
bool CPhysicsPushedEntities::SpeculativelyCheckPush( PhysicsPushedInfo_t &info, const Vector &vecAbsPush, bool bRotationalPush )
{
    ...
    UTIL_TraceEntity( pBlocker, pBlocker->GetAbsOrigin(), pushDestPosition, pBlocker->PhysicsSolidMaskForEntity(), &pushFilter, &info.m_Trace );
...
class CPushBlockerEnum : public IPartitionEnumerator
{
   ...
   private:
    ...
    bool IntersectsPushers( CBaseEntity *pTest )
    {
        ...
        enginetrace->SweepCollideable( pCollision, pTest->GetAbsOrigin(), pTest->GetAbsOrigin(), pCollision->GetCollisionAngles(), pTest->PhysicsSolidMaskForEntity(), &m_pushersOnly, &tr );
