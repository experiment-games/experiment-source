...
void CAmbientGeneric::RampThink( void )
{
    ...
    if ( flags && fChanged )
    {
        ...
        if ( pSoundSource )
        {
            UTIL_EmitAmbientSound( pSoundSource->GetSoundSourceIndex(), pSoundSource->GetAbsOrigin(), STRING( m_iszSound ), ( vol * 0.01 ), m_iSoundLevel, flags, pitch );
        }
    }
...
void CAmbientGeneric::SendSound( SoundFlags_t flags )
{
    ...
    if ( pSoundSource )
    {
        ...
        {
            UTIL_EmitAmbientSound( pSoundSource->GetSoundSourceIndex(), pSoundSource->GetAbsOrigin(), szSoundFile, ( m_dpv.vol * 0.01 ), m_iSoundLevel, flags, m_dpv.pitch );
        }
    }
    else
    {
