...
void CCombineDropshipContainer::CreateCorpse()
{
    ...
    te->Explosion( filter, 0.0f, &vecAbsPoint, g_sModelIndexFireball, random->RandomInt( 4, 10 ), random->RandomInt( 8, 15 ), TE_EXPLFLAG_NOPARTICLES, 100, 0 );
...
int CCombineDropshipContainer::OnTakeDamage( const CTakeDamageInfo &info )
{
    ...
    if ( nPrevHealth != GetHealth() )
    {
        ...
        {
            ExplosionCreate( dmgInfo.GetDamagePosition(), vec3_angle, this, 1000, 500.0f, SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE, 0 );
