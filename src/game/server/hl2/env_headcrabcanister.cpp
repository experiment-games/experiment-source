...
void CEnvHeadcrabCanister::Spawn( void )
{
    ...
    else if ( !HasSpawnFlags( SF_START_IMPACTED ) )
    {
        ...
        if ( !HasSpawnFlags( SF_LAND_AT_INITIAL_POSITION ) )
        {
            ...
            trace_t trace;
            UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() + vecForward * 10000, MASK_NPCWORLDSTATIC, this, COLLISION_GROUP_NONE, &trace );
...
void CEnvHeadcrabCanister::ComputeWorldEntryPoint( Vector *pStartPosition, QAngle *pStartAngles, Vector *pStartDirection )
{
    ...
    UTIL_TraceLine( GetAbsOrigin() + vecForward * 100, GetAbsOrigin() + vecForward * 10000, CONTENTS_SOLID, &filter, &tr );
...
CSkyCamera *CEnvHeadcrabCanister::PlaceCanisterInWorld()
{
    ...
    if ( m_iszLaunchPositionName != NULL_STRING )
    {
        ...
        else
        {
            ...
            m_Shared.InitInWorld( gpGlobals->curtime, pLaunchPos->GetAbsOrigin(), GetAbsAngles(), vecImpactDirection, m_vecImpactPosition, true );
    ...
    else if ( DetectInSkybox() )
    {
        ...
        m_Shared.InitInSkybox( gpGlobals->curtime, m_vecImpactPosition, GetAbsAngles(), vecForward, m_vecImpactPosition, pCamera->m_skyboxData.origin, pCamera->m_skyboxData.scale );
    ...
    else
    {
        ...
        if ( pCamera )
        {
            m_Shared.InitInSkybox( gpGlobals->curtime, vecStartPosition, vecStartAngles, vecDirection, m_vecImpactPosition, pCamera->m_skyboxData.origin, pCamera->m_skyboxData.scale );
        ...
        {
            m_Shared.InitInWorld( gpGlobals->curtime, vecStartPosition, vecStartAngles, vecDirection, m_vecImpactPosition );
            SetThink( &CEnvHeadcrabCanister::HeadcrabCanisterWorldThink );
            SetNextThink( gpGlobals->curtime );
        }
    }
...
void CEnvHeadcrabCanister::Detonate()
{
    ...
    {
        // Normal explosion
        ExplosionCreate( m_vecImpactPosition, GetAbsAngles(), this, 50.0f, 500.0f, SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSOUND, 1300.0f );
