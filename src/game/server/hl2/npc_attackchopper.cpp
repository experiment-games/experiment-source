        ...
        case BULLRUSH_MODE_SHOOT_GUN:
        {
            ...
            if ( m_bRushForward )
            {
                m_flBullrushAdditionalHeight = ClampSplineRemapVal( flDistanceToGoal,
                                                                    flSwitchToBombDist,
                                                                    flSwitchToBombDist + flDropDownDist,
                                                                    0.0f,
                                                                    flShootingHeight );
...
// Where are how should we avoid?
//-----------------------------------------------------------------------------
void CAvoidSphere::ComputeAvoidanceForces( CBaseEntity *pEntity, float flEntityRadius, float flAvoidTime, Vector *pVecAvoidForce )
{
    pVecAvoidForce->Init();
    ...
    for ( int i = s_AvoidSpheres.Count(); --i >= 0; )
    {
        ...
        float t1, t2;
        if ( !IntersectRayWithSphere( vecEntityCenter, vecEntityDelta, vecAvoidCenter, flTotalRadius, &t1, &t2 ) )
        {
            continue;
        }
...
void CAvoidBox::ComputeAvoidanceForces( CBaseEntity *pEntity, float flEntityRadius, float flAvoidTime, Vector *pVecAvoidForce )
{
    ...
    for ( int i = s_AvoidBoxes.Count(); --i >= 0; )
    {
        ...
        float t1, t2;
        if ( !IntersectInfiniteRayWithSphere( vecEntityCenter, vecEntityDelta, vecAvoidCenter, flTotalRadius, &t1, &t2 ) )
        {
            continue;
        }
