...
int CNPC_Houndeye::RangeAttack1Conditions( float flDot, float flDist )
{
    ...
    trace_t tr;
    AI_TraceHull( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0, 0.1 ), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr );
    if ( tr.startsolid )
    {
...
void CNPC_Houndeye::NPCThink( void )
{
    ...
    else
    {
        ...
        trace_t tr;
        AI_TraceHull( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0, 1 ), GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr );
        if ( !tr.startsolid )
        {
        ...
        case NPC_STATE_COMBAT:
        {
                 ...
                 HasCondition( COND_HEAVY_DAMAGE ) )
            {
                if ( random->RandomFloat( 0, 1 ) <= 0.4 )
                {
                    ...
                    Vector forward;
                    AngleVectors( GetAbsAngles(), &forward );
                    AI_TraceHull( GetAbsOrigin(), GetAbsOrigin() + forward * -128, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr );
