...
void CWindowPane::Die( void )
{
    ...
    te->ShatterSurface( filter, 0.0, &GetAbsOrigin(), &GetAbsAngles(), &GetAbsVelocity(), &GetAbsOrigin(), WINDOW_PANEL_SIZE, WINDOW_PANEL_SIZE, WINDOW_SMALL_SHARD_SIZE, SHATTERSURFACE_GLASS, 255, 255, 255, 255, 255, 255 );
    ...
    else if ( info.GetDamageType() & ( DMG_SONIC | DMG_BLAST ) )
    {
        ...
        else
        {
            ...
            else if ( m_nNumBrokenPanes < 0.1 * ( m_nNumWide * m_nNumHigh ) )
            {
                ...
                CreateShards( m_vCorner, vAngles, vBlastDir, ptr->endpos, m_nNumWide * m_flPanelWidth, m_nNumHigh * m_flPanelHeight, WINDOW_LARGE_SHARD_SIZE );
            }
            // ---------------------------------------------------------------
...
void CBreakableSurface::CreateShards( const Vector &vBreakPos, const QAngle &vAngles, const Vector &vForce, const Vector &vForcePos, float flWidth, float flHeight, int nShardSize )
{
    ...
    te->ShatterSurface( filter, 0.0, &vAdjustedBreakPos, &vAngles, &vAdjustedForce, &vForcePos, flWidth, flHeight, WINDOW_SMALL_SHARD_SIZE, m_nSurfaceType, front_r, front_g, front_b, back_r, back_g, back_b );  // 4);
}
