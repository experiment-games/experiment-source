...
//-----------------------------------------------------------------------------
void ComputeDirectLightingAtPoint( Vector &position, Vector &normal, Vector &outColor, int iThread, int static_prop_id_to_skip = -1, int nLFlags = 0 )
{
    ...
    for ( directlight_t *dl = activelights; dl != NULL; dl = dl->next )
    {
        ...
        GatherSampleLightSSE( sampleOutput, dl, -1, adjusted_pos4, &normal4, 1, iThread, nLFlags | GATHERLFLAGS_FORCE_FAST, static_prop_id_to_skip, flEpsilon );
        ...
        for ( int modelID = 0; modelID < pBodyPart->nummodels; ++modelID )
        {
                        ...
                        else
                        {
                            if ( numbounce >= 1 )
                                ComputeIndirectLightingAtPoint(
                                    samplePosition, sampleNormal, indirectColor, iThread, true, ( prop.m_Flags & STATIC_PROP_IGNORE_NORMALS ) != 0 );
                        }
            ...
            if ( badVerts.Count() && ( prop.m_bLightingOriginValid || badVerts.Count() != numVertexes ) )
            {
                for ( int nBadVertex = 0; nBadVertex < badVerts.Count(); nBadVertex++ )
                {
                    ...
                    ComputeIndirectLightingAtPoint( bestPosition, badVerts[nBadVertex].m_Normal, indirectColor, iThread, true );
