...
void DumpRayToGlView( Ray_t const& ray, float dist, Vector* pColor, const char* pFilename )
{
    ...
    int vert = 0;
    CmdLib_FPrintf( fp, "3\n" );
    CmdLib_FPrintf( fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", start1.x, start1.y, start1.z, pColor->x, pColor->y, pColor->z );
    vert++;
    CmdLib_FPrintf( fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", start2.x, start2.y, start2.z, pColor->x, pColor->y, pColor->z );
    vert++;
    CmdLib_FPrintf( fp, "%6.3f %6.3f %6.3f %.2f %.2f %.2f\n", end.x, end.y, end.z, pColor->x, pColor->y, pColor->z );
    vert++;
...
static void ComputeAmbientFromSurface( dface_t* pFace, directlight_t* pSkylight, Vector& radcolor )
{
...
//-----------------------------------------------------------------------------
void ComputeIndirectLightingAtPoint( Vector& position, Vector& normal, Vector& outColor, int iThread, bool force_fast, bool bIgnoreNormals )
{
    Ray_t ray;
    CLightSurface surfEnum( iThread );
