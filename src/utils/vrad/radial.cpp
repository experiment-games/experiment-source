    ...
    if ( g_FacePatches.Element( rad->facenum ) != g_FacePatches.InvalidIndex() )
    {
        for ( patch = &g_Patches.Element( g_FacePatches.Element( rad->facenum ) ); patch; patch = pNextPatch )
        {
            ...
            if ( ValidDispFace( &g_pFaces[facenum] ) )
            {
                ...
                AddBouncedToRadial( rad, patchOrigin, mins, maxs, patch->totallight.light, needsBumpmap, needsBumpmap );
            }
            else
            {
                AddBouncedToRadial( rad, patch->origin, mins, maxs, patch->totallight.light, needsBumpmap, needsBumpmap );
            }
        }
    }
...
radial_t *BuildLuxelRadial( int facenum, int style )
{
    ...
    for ( int j = 0; j < fn->numneighbors; j++ )
    {
        ...
        for ( int k = 0; k < fl->numsamples; k++ )
        {
            ...
            AddDirectToRadial( rad, fl->sample[k].pos, mins, maxs, light, needsBumpmap, neighborHasBumpmap );
        }
    }
    ...
    for ( k = 0; k < lightstyles; k++ )
    {
        ...
        for ( j = 0; j < fl->numluxels; j++ )
        {
            ...
            for ( bumpSample = 0; bumpSample < bumpSampleCount; bumpSample++ )
            {
                ...
                VectorToColorRGBExp32( Vector( lb[bumpSample].m_vecLighting.x, lb[bumpSample].m_vecLighting.y, lb[bumpSample].m_vecLighting.z ), *( ColorRGBExp32 * )pdata[bumpSample] );
#endif
