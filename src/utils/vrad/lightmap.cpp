...
void GetBumpNormals( const float *sVect, const float *tVect, const Vector &flatNormal, const Vector &phongNormal, Vector bumpNormals[NUM_BUMP_VECTS] )
{
...
void WriteCoordWinding( FILE *out, lightinfo_t *l, winding_t *w, Vector &color )
{
    ...
    for ( i = 0; i < w->numpoints; i++ )
    {
        ...
        fprintf( out, "%5.2f %5.2f %5.2f %5.3f %5.3f %5.3f\n", pos[0], pos[1], pos[2], color[0] / 256, color[1] / 256, color[2] / 256 );
    }
}
...
void DumpFaces( lightinfo_t *pLightInfo, int ndxFace )
{
    ...
    for ( int ndxEdge = 0; ndxEdge < pLightInfo->face->numedges; ndxEdge++ )
    {
        ...
        CmdLib_FPrintf( out, "%f %f %f %f %f %f\n", centroid[0] + pLightInfo->modelorg[0], centroid[1] + pLightInfo->modelorg[1], centroid[2] + pLightInfo->modelorg[2], fn->facenormal[0] * 0.5 + 0.5, fn->facenormal[1] * 0.5 + 0.5, fn->facenormal[2] * 0.5 + 0.5 );
    }
...
int LightForString( char *pLight, Vector &intensity )
{
    ...
    argCnt = sscanf( pLight, "%lf %lf %lf %lf %lf %lf %lf %lf", &r, &g, &b, &scaler, &r_hdr, &g_hdr, &b_hdr, &scaler_hdr );
...
bool CanLeafTraceToSky( int iLeaf )
{
    ...
    for ( int j = 0; j < NUMVERTEXNORMALS; j += 4 )
    {
        ...
        delta.LoadAndSwizzle( g_anorms[j], g_anorms[min( j + 1, NUMVERTEXNORMALS - 1 )], g_anorms[min( j + 2, NUMVERTEXNORMALS - 1 )], g_anorms[min( j + 3, NUMVERTEXNORMALS - 1 )] );
...
void GatherSampleSkyLightSSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum, FourVectors const &pos, FourVectors *pNormals, int normalCount, int iThread, int nLFlags, int static_prop_index_to_ignore, float flEpsilon )
{
...
void GatherSampleAmbientSkySSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum, FourVectors const &pos, FourVectors *pNormals, int normalCount, int iThread, int nLFlags, int static_prop_index_to_ignore, float flEpsilon )
{
    bool bIgnoreNormals = ( nLFlags & GATHERLFLAGS_IGNORE_NORMALS ) != 0;
    bool force_fast = ( nLFlags & GATHERLFLAGS_FORCE_FAST ) != 0;
...
void GatherSampleStandardLightSSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum, FourVectors const &pos, FourVectors *pNormals, int normalCount, int iThread, int nLFlags, int static_prop_index_to_ignore, float flEpsilon )
{
    bool bIgnoreNormals = ( nLFlags & GATHERLFLAGS_IGNORE_NORMALS ) != 0;
...
void GatherSampleLightSSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum, FourVectors const &pos, FourVectors *pNormals, int normalCount, int iThread, int nLFlags, int static_prop_index_to_ignore, float flEpsilon )
{
    ...
    {
        case emit_skylight:
            GatherSampleSkyLightSSE( out, dl, facenum, pos, pNormals, normalCount, iThread, nLFlags, static_prop_index_to_ignore, flEpsilon );
            break;
        case emit_skyambient:
            GatherSampleAmbientSkySSE( out, dl, facenum, pos, pNormals, normalCount, iThread, nLFlags, static_prop_index_to_ignore, flEpsilon );
            break;
        case emit_point:
        case emit_surface:
        case emit_spotlight:
            GatherSampleStandardLightSSE( out, dl, facenum, pos, pNormals, normalCount, iThread, nLFlags, static_prop_index_to_ignore, flEpsilon );
            break;
        default:
...
static void GatherSampleLightAt4Points( SSE_SampleInfo_t &info, int sampleIdx, int numSamples )
{
    ...
    for ( directlight_t *dl = activelights; dl != NULL; dl = dl->next )
    {
        ...
        if ( g_pIncremental && ( dl->light.style == 0 ) )
        {
            ...
            {
                g_pIncremental->AddLightToFace( dl->m_IncrementalID, info.m_FaceNum, sampleIdx + i, info.m_LightmapSize, SubFloat( fxdot[0], i ), info.m_iThread );
            }
        }
...
//-----------------------------------------------------------------------------
static int SupersampleLightAtPoint( lightinfo_t &l, SSE_SampleInfo_t &info, int sampleIndex, int lightStyleIndex, LightingValue_t *pLight, int flags )
{
    ...
    else
    {
        ...
        superSampleOffsets.LoadAndSwizzle( Vector( csshift, csshift, 0 ), Vector( csshift, csshift + cscale, 0 ), Vector( csshift + cscale, csshift, 0 ), Vector( csshift + cscale, csshift + cscale, 0 ) );
        superSampleLightCoord.DuplicateVector( sampleLightOrigin );
...
//-----------------------------------------------------------------------------
static void ComputeLightmapGradients( SSE_SampleInfo_t &info, bool const *pHasProcessedSample, float *pIntensity, float *gradient )
{
...
//-----------------------------------------------------------------------------
static inline void ComputeLuxelIntensity( SSE_SampleInfo_t &info, int sampleIdx, LightingValue_t **ppLightSamples, float *pSampleIntensity )
{
    ...
    if ( l.isflat )
    {
        ...
        if ( info.m_NormalCount > 1 )
        {
            ...
            GetBumpNormals( info.m_pTexInfo->textureVecsTexelsPerWorldUnits[0],
                            info.m_pTexInfo->textureVecsTexelsPerWorldUnits[1],
                            l.facenormal,
                            l.facenormal,
                            bumpVects );  //&info.m_PointNormal[1] );
