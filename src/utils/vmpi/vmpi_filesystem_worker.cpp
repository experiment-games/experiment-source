        ...
        while ( m_nUnfinishedFiles > 0 )
        {
            ...
            */
            ...
            if ( !( pTestFile->m_ChunksReceived[iChunk >> 3] & ( 1 << ( iChunk & 7 ) ) ) )
            {
                ...
                {
                    Plat_FatalError(
                        "ListenFor(): invalid payload length for '%s' (%d should be %d)\n"
                        "pInfo = '%s', pTestFile = '%s'\n"
                ...
                if ( pTestFile->m_nChunksToReceive == 0 )
                {
                    ...
                    char str[512];
                    V_snprintf( str, sizeof( str ), "Got %s (%dk) in %.2fs", printableFilename, ( pTestFile->m_UncompressedData.Count() + 511 ) / 1024, pTestFile->m_Timer.GetDuration().GetSeconds() );
                    Msg( "\r%-79s\n", str );
