...
void CJobSearchDlg::RepopulateJobsList()
{
    ...
    FOR_EACH_LL( jobInfos, j )
    {
        ...
        {
            Q_snprintf( dateStr, sizeof( dateStr ), "%c%c/%c%c %c%c:%c%c:00", pDate[4], pDate[5], pDate[6], pDate[7], pDate[8], pDate[9], pDate[10], pDate[11] );
        }
...
void CJobSearchDlg::OnDblclkWorkerList()
{
    ...
    if ( sel != LB_ERR )
    {
        ...
        Q_snprintf( query, sizeof( query ),
                    "select job_master_start.RunningTimeMS, job_master_start.JobID, job_master_start.BSPFilename, job_master_start.StartTime, job_master_start.MachineName "
