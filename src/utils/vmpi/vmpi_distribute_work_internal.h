...
class CVisibleWindowVectorT : protected CUtlVector< T >
{
    ...
    inline void PostInitElement( uint64 uiRealIdx, T &newElement, T const &x )
    {
        newElement = x;
    }
    ...
    uint64 FirstPossibleIndex() const
    {
        return m_uiBase;
    }

    // Last possible index in this vector
    uint64 PastPossibleIndex() const
    {
        return m_uiTotal;
    }
    // Past visible window index in this vector
    uint64 PastVisibleIndex() const
    {
