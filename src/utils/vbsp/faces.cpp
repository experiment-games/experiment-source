...
static void SubdivideFaceBySubdivSize( face_t *f, float subdivsize )
{
    ...
    for ( yi = 0, y = yStart; y < yEnd; y += ( int )subdivsize, yi++ )
    {
        for ( xi = 0, x = xStart; x < xEnd; x += ( int )subdivsize, xi++ )
        {
            ...
            ClipWindingEpsilon( tempWinding, normal, planeDist, ON_EPSILON, &frontWinding, &backWinding );
            if ( tempWinding )
            {
    ...
    Stripify( triListIndices.Size() / 3, triListIndices.Base(), &numTristripIndices, &pStripIndices );
