...
class CDmSmdSerializer : public IDmSerializer
{
    ...
    CDmSmdSerializer()
        : m_bOptAutoStripPrefix( false ), m_bOptImportSkeleton( true ), m_bOptAnimation( false ), m_flFrameRate( 30.0f )
    {
        SetUpAxis( Z_AXIS );
    }
    ...
    virtual const char *GetImportedFormat() const
    {
        return NULL;
    }
    virtual int GetImportedVersion() const
    {
        return 1;
    }
    ...
    void SetIsAnimation( bool bOptAnimation )
    {
        m_bOptAnimation = bOptAnimation;
    }
    bool IsReadAnimation() const
    {
        return m_bOptAnimation;
    }

    void SetFrameRate( float flFrameRate )
    {
        m_flFrameRate = MAX( 0.1f, flFrameRate );
    }  // Don't allow 0 or negative frame rate
    ...
    struct SmdJoint_t
    {
        ...
        SmdJoint_t()
            : m_nId( -1 ), m_nActualId( -1 ), m_nParentId( -1 ), m_nLineNumber( -1 ), m_pDmeDag( NULL )
        {
        }
    };
