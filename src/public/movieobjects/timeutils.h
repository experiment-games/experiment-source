...
class DmeFramerate_t
{
    ...
    DmeFramerate_t( const DmeFramerate_t& src )
        : m_num( src.m_num ), m_den( src.m_den ) {}
    ...
    bool operator==( DmeFramerate_t f ) const
    {
    ...
    bool operator<( DmeFramerate_t f ) const
    {
        return m_num * ( int )f.m_den < f.m_num * ( int )m_den;
    }
    bool operator>( DmeFramerate_t f ) const
    {
        return m_num * ( int )f.m_den > f.m_num * ( int )m_den;
    ...
    DmeFramerate_t operator*( int i ) const
    {
        return DmeFramerate_t( m_num * i, m_den );
    }
    DmeFramerate_t operator/( int i ) const
    {
        return DmeFramerate_t( m_num, m_den * i );
    }
...
class DmeTime_t
{
    ...
    explicit DmeTime_t( int tms )
        : m_tms( tms ) {}
    explicit DmeTime_t( float sec )
        : m_tms( RoundSecondsToTMS( sec ) ) {}
    explicit DmeTime_t( double sec )
        : m_tms( RoundSecondsToTMS( sec ) ) {}
    DmeTime_t( int frame, DmeFramerate_t framerate );

    // time operators

    bool operator==( DmeTime_t t ) const
    {
        return m_tms == t.m_tms;
    }
    bool operator!=( DmeTime_t t ) const
    {
        return m_tms != t.m_tms;
    ...
    DmeTime_t operator%( DmeTime_t t ) const
    {
        return DmeTime_t( m_tms % t.m_tms );
    }
    DmeTime_t operator+( DmeTime_t t ) const
    {
        return DmeTime_t( m_tms + t.m_tms );
    ...
    DmeTime_t operator-() const
    {
        return DmeTime_t( -m_tms );
    }

    DmeTime_t operator+=( DmeTime_t t )
    {
        m_tms += t.m_tms;
        return *this;
    }
    ...
    friend DmeTime_t operator*( DmeTime_t t, float f )
    {
        t *= f;
        return t;
    }
    friend DmeTime_t operator*( float f, DmeTime_t t )
    {
        t *= f;
        return t;
    }
    ...
    friend float operator/( DmeTime_t n, DmeTime_t d )
    {
        return float( n.m_tms / double( d.m_tms ) );
    }
    ...
    bool IsValid() const
    {
        return m_tms != INT_MIN;
    }

    static DmeTime_t InvalidTime()
    {
        return DmeTime_t( INT_MIN );
    }
    static DmeTime_t MinTime()
    {
        return DmeTime_t( INT_MIN + 1 );
    }
    static DmeTime_t MaxTime()
    {
        return DmeTime_t( INT_MAX );
    }
    static DmeTime_t MinTimeDelta()
    {
        return DmeTime_t( 1 );
    }
    ...
    {
        return DMETIME_TO_SECONDS( m_tms );
    }
    ...
    friend DmeTime_t abs( DmeTime_t t )
    {
        return t.m_tms >= 0 ? t : -t;
    }
