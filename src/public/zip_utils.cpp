...
HANDLE CZipFile::ParseFromDisk( const char *pFilename )
{
    ...
    for ( int i = 0; i < numZipFiles; i++ )
    {
        ...
        if ( zipFileHeader.signature != PKID( 1, 2 ) || ( zipFileHeader.compressionMethod != IZip::eCompressionType_None && zipFileHeader.compressionMethod != IZip::eCompressionType_LZMA ) )
        {
            // bad contents
#ifdef WIN32
            CloseHandle( hFile );
...
class CZip : public IZip
{
   public:
    ...
    virtual void AddBufferToZip( const char *relativename, void *data, int length, bool bTextMode, eCompressionType compressionType ) OVERRIDE;
