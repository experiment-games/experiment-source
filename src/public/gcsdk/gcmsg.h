...
struct GCMsgHdr_t
{
    ...
    const char *PchMsgName() const
    {
        return PchMsgNameFromEMsg( m_eMsg );
    }
};
...
struct ProtoBufMsgHeader_t
{
    ...
    ProtoBufMsgHeader_t()
        : m_EMsgFlagged( 0 ), m_cubProtoBufExtHdr( 0 ) {}
    ProtoBufMsgHeader_t( MsgType_t eMsg, uint32 cubProtoBufExtHdr )
        : m_EMsgFlagged( eMsg | k_EMsgProtoBufFlag ), m_cubProtoBufExtHdr( cubProtoBufExtHdr ) {}
    const char *PchMsgName() const
    {
        return PchMsgNameFromEMsg( GetEMsg() );
    }
    MsgType_t GetEMsg() const
    {
        return ( MsgType_t )( m_EMsgFlagged & ( ~k_EMsgProtoBufFlag ) );
    }
};
#pragma pack( pop )
...
class CStructNetPacket : public IMsgNetPacket
{
    ...
    EMsgFormatType GetEMsgFormatType() const
    {
        return k_EMsgFormatTypeStruct;
    }
    CNetPacket *GetCNetPacket() const
    {
        return m_pNetPacket;
    }
    uint8 *PubData() const
    ...
    MsgType_t GetEMsg() const
    {
        return ( MsgType_t )m_pHeader->m_eMsg;
    }
    JobID_t GetSourceJobID() const
    {
        return m_pHeader->m_JobIDSource;
    }
    JobID_t GetTargetJobID() const
    {
        return m_pHeader->m_JobIDTarget;
    }
    void SetTargetJobID( JobID_t ulJobID )
    {
        m_pHeader->m_JobIDTarget = ulJobID;
    }

    CSteamID GetSteamID() const
    {
        return CSteamID( m_pHeader->m_ulSteamID );
    }
    void SetSteamID( CSteamID steamID )
    {
        m_pHeader->m_ulSteamID = steamID.ConvertToUint64();
    }
    ...
    virtual bool BHasTargetJobName() const
    {
        return false;
    }
    virtual const char *GetTargetJobName() const
    {
        return NULL;
    }
...
class CGCMsg : public CGCMsgBase
{
    ...
    MSG_BODY_TYPE &Body()
    {
        return *( MSG_BODY_TYPE * )( m_pubPkt + m_cubMsgHdr );
    }
    const MSG_BODY_TYPE &Body() const
    {
        return *( MSG_BODY_TYPE * )( m_pubPkt + m_cubMsgHdr );
    }
    GCMsgHdrEx_t *PGCMsgHdr()
    {
        return ( ( GCMsgHdrEx_t * )m_pubPkt );
    }
    MsgType_t GetEMsg() const
    {
        return Hdr().m_eMsg;
    ...
    bool BIsExpectingReply()
    {
        return Hdr().m_JobIDSource != k_GIDNil;
    }
};
