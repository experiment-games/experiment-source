...
class CWebAPIResponse
{
   public:
    ...
    void SetStatusCode( EHTTPStatusCode eStatusCode )
    {
        m_eStatusCode = eStatusCode;
    }

    // Set how many seconds until this response expires
    void SetExpirationSeconds( uint32 unExpirationSeconds )
    {
        m_unExpirationSeconds = unExpirationSeconds;
    }

    // Set when this response was last modified
    void SetLastModified( RTime32 rtLastModified )
    {
        m_rtLastModified = rtLastModified;
    }

    // Get the status code for the response
    EHTTPStatusCode GetStatusCode() const
    {
        return m_eStatusCode;
    }

    // Get how many seconds until this response expires
    uint32 GetExpirationSeconds() const
    {
        return m_unExpirationSeconds;
    }

    // Get when the response was last modified
    RTime32 GetLastModified() const
    {
        return m_rtLastModified;
    }

    // extended arrays include their element name as an object in JSON and VDF output formats
    void SetExtendedArrays( bool bExtendedArrays )
    {
        m_bExtendedArrays = bExtendedArrays;
    }
    bool HasExtendedArrays() const
    {
        return m_bExtendedArrays;
    }
    ...
    const CWebAPIValues *GetRootValue() const
    {
        return m_pValues;
    }
    CWebAPIValues *GetRootValue()
    {
        return m_pValues;
    }
    ...
    void SetJSONAnonymousRootNode( bool bAnonymousRootNode )
    {
        m_bJSONAnonymousRootNode = bAnonymousRootNode;
    }
    bool HasJSONAnonymousRootNode() const
    {
...
class CWebAPIValues
{
   ...
   public:
    ...
    const CWebAPIValues *FindChild( const char *pchName ) const
    {
        return const_cast< CWebAPIValues * >( this )->FindChild( pchName );
    }

    // Get the first child of this node
    CWebAPIValues *GetFirstChild();
    const CWebAPIValues *GetFirstChild() const
    {
    ...
    const CWebAPIValues *GetNextChild() const
    {
        return m_pNextPeer;
    }
    ...
    const CWebAPIValues *GetParent() const
    {
        return m_pParent;
    }
    ...
    {
        return m_nNamePos >= 0 ? ( ( const char * )m_pStringBuffer->Base() + m_nNamePos ) : NULL;
    }

    // get the name of the elements of this numeric array (if this is an array)
    const char *GetElementName() const
    {
        return m_nArrayChildElementNamePos >= 0 ? ( ( const char * )m_pStringBuffer->Base() + m_nArrayChildElementNamePos ) : NULL;
    }
    ...
    bool IsObject() const
    {
        return GetType() == k_EWebAPIValueType_Object;
    }

    // returns true if this is an object
    bool IsArray() const
    {
        return GetType() == k_EWebAPIValueType_NumericArray;
    }
    ...
    {
        return GetType() == k_EWebAPIValueType_Null;
    }
...
class CWebAPIValuesAD
{
   public:
    ...
    CWebAPIValues *operator->()
    {
        if ( !m_pwav ) m_pwav = new CWebAPIValues( "root" );
        return m_pwav;
    }
    operator CWebAPIValues *()
    {
        if ( !m_pwav ) m_pwav = new CWebAPIValues( "root" );
        return m_pwav;
    }
    operator const CWebAPIValues *() const
    {
        return m_pwav;
    }
