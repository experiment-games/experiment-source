...
#define EG_ASSERTERROR( EmitGroup, ... )                                                                                                                                     \
    do                                                                                                                                                                       \
    {                                                                                                                                                                        \
        if ( EmitGroup.GetConsoleLevel() >= CGCEmitGroup::kMsg_Error || EmitGroup.GetLogLevel() >= CGCEmitGroup::kMsg_Error ) EmitGroup.Internal_AssertError( __VA_ARGS__ ); \
    } while ( 0 )
#define EG_ERROR( EmitGroup, ... )                                                                                                                                     \
    do                                                                                                                                                                 \
    {                                                                                                                                                                  \
        if ( EmitGroup.GetConsoleLevel() >= CGCEmitGroup::kMsg_Error || EmitGroup.GetLogLevel() >= CGCEmitGroup::kMsg_Error ) EmitGroup.Internal_Error( __VA_ARGS__ ); \
    } while ( 0 )
#define EG_WARNING( EmitGroup, ... )                                                                                                                                         \
    do                                                                                                                                                                       \
    {                                                                                                                                                                        \
        if ( EmitGroup.GetConsoleLevel() >= CGCEmitGroup::kMsg_Warning || EmitGroup.GetLogLevel() >= CGCEmitGroup::kMsg_Warning ) EmitGroup.Internal_Warning( __VA_ARGS__ ); \
    } while ( 0 )
#define EG_MSG( EmitGroup, ... )                                                                                                                                 \
    do                                                                                                                                                           \
    {                                                                                                                                                            \
        if ( EmitGroup.GetConsoleLevel() >= CGCEmitGroup::kMsg_Msg || EmitGroup.GetLogLevel() >= CGCEmitGroup::kMsg_Msg ) EmitGroup.Internal_Msg( __VA_ARGS__ ); \
    } while ( 0 )
#define EG_VERBOSE( EmitGroup, ... )                                                                                                                                         \
    do                                                                                                                                                                       \
    {                                                                                                                                                                        \
        if ( EmitGroup.GetConsoleLevel() >= CGCEmitGroup::kMsg_Verbose || EmitGroup.GetLogLevel() >= CGCEmitGroup::kMsg_Verbose ) EmitGroup.Internal_Verbose( __VA_ARGS__ ); \
    } while ( 0 )
#define EG_EMIT( EmitGroup, eLvl, ... )                                                                                             \
    do                                                                                                                              \
    {                                                                                                                               \
        if ( EmitGroup.GetConsoleLevel() >= eLvl || EmitGroup.GetLogLevel() >= eLvl ) EmitGroup.Internal_Emit( eLvl, __VA_ARGS__ ); \
    } while ( 0 )
#define EG_BOLDMSG( EmitGroup, ... )                                                                                                                                     \
    do                                                                                                                                                                   \
    {                                                                                                                                                                    \
        if ( EmitGroup.GetConsoleLevel() >= CGCEmitGroup::kMsg_Error || EmitGroup.GetLogLevel() >= CGCEmitGroup::kMsg_Error ) EmitGroup.Internal_BoldMsg( __VA_ARGS__ ); \
    } while ( 0 )
...
#define EmitInfo( EmitGroup, ConsoleLevel, LogLevel, ... )                                                                                                                       \
    do                                                                                                                                                                           \
    {                                                                                                                                                                            \
        if ( EmitGroup.GetConsoleLevel() >= ( ConsoleLevel ) || EmitGroup.GetLogLevel() >= ( LogLevel ) ) EGInternal_EmitInfo( EmitGroup, ConsoleLevel, LogLevel, __VA_ARGS__ ); \
    } while ( 0 )
