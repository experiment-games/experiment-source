...
class CIMsgNetPacketAutoRelease
{
   public:
    ...
    ~CIMsgNetPacketAutoRelease()
    {
        SAFE_RELEASE( m_pMsgNetPacket );
    }

    void Replace( CNetPacket *pNetPacket, bool bIsClientMsg )
    {
        SAFE_RELEASE( m_pMsgNetPacket );
        m_pMsgNetPacket = IMsgNetPacketFromCNetPacket( pNetPacket );
    }
   ...
   protected:
    ...
    CIMsgNetPacketAutoRelease( const CIMsgNetPacketAutoRelease &that )
    {
        AssertMsg( false, "Not safe to copy since releases references on destruction" );
    }
...
class CMsgBase_t
{
   public:
    ...
    uint8 *PubVarData()
    {
        return ( m_pubPkt + m_cubMsgHdr + m_cubStruct );
    }
    const uint8 *PubVarData() const
    {
        return ( m_pubPkt + m_cubMsgHdr + m_cubStruct );
    }
    ...
    MSG_HEADER_TYPE &Hdr()
    {
        return *( MSG_HEADER_TYPE * )( m_pubPkt );
    }
    const MSG_HEADER_TYPE &Hdr() const
    {
        return *( MSG_HEADER_TYPE * )( m_pubPkt );
    }
    uint32 CubHdr() const
    ...
    const uint8 *PubBody() const
    {
        return m_pubBody;
    }
    uint32 CubBody() const
    {
        return CubPkt() - CubHdr();
    }
    ...
    {
        return m_cubPkt - m_cubMsgHdr - m_cubStruct;
    }  // Our current offset within the var data block
    ...
    const uint8 *PubReadCur() const
    {
        return m_pubVarRead;
    }
    uint32 CubReadRemaining() const
    {
        return ( uint32 )( m_pubPkt + m_cubPkt - m_pubVarRead );
    }
    void AdvanceReadPtr( int cubSkip )
    {
        m_pubVarRead += cubSkip;
    }
    ...
    template < typename T >
    bool BReadStructure( T &structure );
    ...
    uint32 GetWriteOffset() const
    {
        return m_cubPkt;
    }
    void SetWriteOffset( uint32 nWriteOffset )
    {
        m_cubPkt = nWriteOffset;
    }
