...
class CJob
{
    ...
    EJobPauseReason GetPauseReason() const
    {
        return m_ePauseReason;
    }
    ...
    const CJobTime &GetTimeSwitched() const
    {
        return m_STimeSwitched;
    }
    // return microseconds run since we were last continued
    uint64 GetMicrosecondsRun() const
    {
        return m_FastTimerDelta.GetDurationInProgress().GetUlMicroseconds();
    }
    bool BJobNeedsToHeartbeat() const
    {
        return ( m_STimeNextHeartbeat.CServerMicroSecsPassed() >= 0 );
    }
    ...
    bool BHoldsAnyLocks() const
    {
        return m_vecLocks.Count() > 0;
    }
    int GetLockCount() const
    {
        return m_vecLocks.Count();
    }
    void ReleaseLocks();
    ...
    {
        m_flags.m_bits.m_bWaitTimeout = true;
    }
    ...
    void SetStartParam( void *pvStartParam )
    {
        Assert( NULL == m_pvStartParam );
        m_pvStartParam = pvStartParam;
    }
    void SetFromFromMsg( bool bRunFromMsg )
    {
        m_bRunFromMsg = true;
    }
    ...
    virtual bool BYieldingRunJob( void *pvStartParam )
    {
        Assert( false );
        return true;
    }  // implement this if your job can be started directly
    virtual bool BYieldingRunJobFromMsg( IMsgNetPacket *pNetPacket )
    {
        Assert( false );
        return true;
    }  // implement this if your job can be started by a network message
...
inline CJob &GJobCur()
{
    Assert( g_pJobCur != NULL );
    return *g_pJobCur;
}

#define AssertRunningJob()           \
    {                                \
        Assert( NULL != g_pJobCur ); \
    }
#define AssertRunningThisJob()       \
    {                                \
        Assert( this == g_pJobCur ); \
    }
#define AssertNotRunningThisJob()    \
    {                                \
        Assert( this != g_pJobCur ); \
    }
#define AssertNotRunningJob()        \
    {                                \
        Assert( NULL == g_pJobCur ); \
    }
...
class CLock
{
    ...
    CJob *GetJobWaitingQueueHead()
    {
        return m_pJobToNotifyOnLockRelease;
    }
    CJob *GetJobWaitingQueueTail()
    {
    ...
    int16 GetLockType() const
    {
        return m_nsLockType;
    }
    void SetLockType( int16 nsLockType )
    {
        m_nsLockType = nsLockType;
    }
    uint64 GetLockSubType() const
    {
        return m_unLockSubType;
    }
    void SetLockSubType( uint64 unLockSubType )
    {
        m_unLockSubType = unLockSubType;
    }
    int32 GetWaitingCount() const
    {
        return m_nWaitingCount;
    }
    int64 GetMicroSecondsSinceLock() const
    {
        return m_sTimeAcquired.CServerMicroSecsPassed();
    }
