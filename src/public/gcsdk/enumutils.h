...
#define ENUMSTRINGS_END( etype )                                                \
    }                                                                           \
    ;                                                                           \
    const char* PchNameFrom##etype( etype nValue )                              \
    {                                                                           \
        for ( uint i = 0; i < Q_ARRAYSIZE( s_##etype ); i++ )                   \
        {                                                                       \
            if ( s_##etype[i].nValue == nValue ) return s_##etype[i].pszString; \
        }                                                                       \
        AssertMsg2( false, "Missing String for %s (%d)", #etype, nValue );      \
        return "Unknown";                                                       \
    }                                                                           \
...
#define ENUMSTRINGS_REVERSE( etype, default )                                                             \
    }                                                                                                     \
    ;                                                                                                     \
    const char* PchNameFrom##etype( etype nValue )                                                        \
    {                                                                                                     \
    ...
    {                                                                                                     \
        for ( uint i = 0; i < Q_ARRAYSIZE( s_##etype ); i++ )                                             \
        {                                                                                                 \
            if ( !Q_stricmp( s_##etype[i].pszString, pchName ) ) return ( etype )( s_##etype[i].nValue ); \
        }                                                                                                 \
        return default;                                                                                   \
    }
