...
class ISteamInventory
{
   public:
    ...
    virtual bool GetResultItemProperty( SteamInventoryResult_t resultHandle,
                                        ...
                                        STEAM_OUT_STRING_COUNT( punValueBufferSizeOut ) char *pchValueBuffer,
                                        uint32 *punValueBufferSizeOut ) = 0;
    ...
    virtual bool ExchangeItems( SteamInventoryResult_t *pResultHandle,
                                STEAM_ARRAY_COUNT( unArrayGenerateLength ) const SteamItemDef_t *pArrayGenerate,
                                STEAM_ARRAY_COUNT( unArrayGenerateLength ) const uint32 *punArrayGenerateQuantity,
    ...
    // to pchValueBuffer. If the results do not fit in the given buffer, partial
    // results may be copied.
    virtual bool GetItemDefinitionProperty( SteamItemDef_t iDefinition, const char *pchPropertyName, STEAM_OUT_STRING_COUNT( punValueBufferSizeOut ) char *pchValueBuffer, uint32 *punValueBufferSizeOut ) = 0;
...
struct SteamInventoryResultReady_t
{
    enum
    {
        k_iCallback = k_iSteamInventoryCallbacks + 0
    };
...
struct SteamInventoryFullUpdate_t
{
    enum
    {
        k_iCallback = k_iSteamInventoryCallbacks + 1
    };
    SteamInventoryResult_t m_handle;
};
...
struct SteamInventoryDefinitionUpdate_t
{
    enum
    {
        k_iCallback = k_iSteamInventoryCallbacks + 2
    };
};
...
struct SteamInventoryEligiblePromoItemDefIDs_t
{
    enum
    {
        k_iCallback = k_iSteamInventoryCallbacks + 3
    };
    EResult m_result;
    CSteamID m_steamID;
...
struct SteamInventoryStartPurchaseResult_t
{
    enum
    {
        k_iCallback = k_iSteamInventoryCallbacks + 4
    };
    EResult m_result;
    uint64 m_ulOrderID;
    uint64 m_ulTransID;
...
struct SteamInventoryRequestPricesResult_t
{
    enum
    {
        k_iCallback = k_iSteamInventoryCallbacks + 5
    };
    EResult m_result;
