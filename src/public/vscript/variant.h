...
class CVariantBase
{
    ...
    CVariantBase( int val )
        : m_flags( 0 ), m_type( FIELD_INTEGER )
    {
        m_int = val;
    }
    CVariantBase( uint32 val )
        : m_flags( 0 ), m_type( FIELD_UINT )
    {
        m_uint = val;
    }
    //	CVariantBase( int64 val ) :			m_flags( 0 ), m_type( FIELD_INTEGER64 )	{ m_int64 = val; }
    CVariantBase( uint64 val )
        : m_flags( 0 ), m_type( FIELD_UINT64 )
    {
        m_uint64 = val;
    }
    CVariantBase( float val )
        : m_flags( 0 ), m_type( FIELD_FLOAT )
    {
        m_float = val;
    }
    CVariantBase( float64 val )
        : m_flags( 0 ), m_type( FIELD_FLOAT64 )
    {
        m_float64 = val;
    }
    CVariantBase( char val )
        : m_flags( 0 ), m_type( FIELD_CHARACTER )
    {
        m_char = val;
    }
    CVariantBase( bool val )
        : m_flags( 0 ), m_type( FIELD_BOOLEAN )
    {
        m_bool = val;
    }
    CVariantBase( HSCRIPT val )
        : m_flags( 0 ), m_type( FIELD_HSCRIPT )
    {
        m_hScript = val;
    }
    CVariantBase( CBaseHandle val )
        : m_flags( 0 ), m_type( FIELD_EHANDLE )
    {
        m_hEntity = val.ToInt();
    }
    ...
    CVariantBase( const CVariantBase< CValueAllocator > &variant )
        : m_flags( 0 ), m_type( FIELD_VOID )
    {
        variant.AssignTo( this );
    }
    void operator=( const CVariantBase< CValueAllocator > &variant )
    {
        variant.AssignTo( this );
    }
    ...
    bool IsNull() const
    {
        return ( m_type == FIELD_VOID );
    }

    operator int() const
    {
        Assert( m_type == FIELD_INTEGER || m_type == FIELD_FLOAT );
        return ( m_type == FIELD_INTEGER ) ? m_int : ( int )m_float;
    }
    operator uint() const
    {
        Assert( m_type == FIELD_UINT );
        return m_uint;
    }
    //	operator int64() const					{ Assert( m_type == FIELD_INTEGER64 );	return m_int64; }
    operator uint64() const
    {
        Assert( m_type == FIELD_UINT64 );
        return m_uint64;
    }
    operator float() const
    {
        ( m_type == FIELD_INTEGER || m_type == FIELD_FLOAT );
        return ( m_type == FIELD_FLOAT ) ? m_float : ( float )m_int;
    }
    operator float64() const
    {
        Assert( m_type == FIELD_FLOAT64 );
        return m_float64;
    }
    ...
    {
        Assert( m_type == FIELD_VECTOR2D );
        return m_pData ? *( Vector2D * )m_pData : vec2_origin;
    }
    operator const Vector &() const
    {
        Assert( m_type == FIELD_VECTOR );
        return ( m_pVector ) ? *m_pVector : vec3_origin;
    }
    ...
    {
        Assert( m_type == FIELD_QANGLE );
        return m_pData ? *( QAngle * )m_pData : vec3_angle;
    }
    operator char() const
    {
    ...
    operator CBaseHandle() const
    {
        Assert( m_type == FIELD_EHANDLE );
        return CBaseHandle( m_hEntity );
    }
    operator CBaseEntity *() const
    {
        Assert( m_type == FIELD_EHANDLE );
        return CHandle< CBaseEntity >( CBaseHandle( m_hEntity ) );
    }
    operator const Quaternion &() const
    {
        Assert( m_type == FIELD_QUATERNION );
        return m_pData ? *( Quaternion * )m_pData : quat_identity;
    }
    ...
    void operator=( const QAngle &vec )
    {
        CopyData( vec );
    }
    ...
    void operator=( QAngle *a )
    {
        CopyData( *a );
    }
    void operator=( const QAngle *a )
    {
        CopyData( *a );
    }
    ...
    bool AssignTo( QuaternionAligned *pDest ) const
    {
        return AssignTo( ( Quaternion * )pDest );
    }
    bool AssignTo( VectorAligned *pDest ) const
    {
        return AssignTo( ( Vector * )pDest );
    }
...
template < typename T >
struct VariantDeducer_t
{
    enum
    {
        FIELD_TYPE = FIELD_TYPEUNKNOWN
    };
};
#define DECLARE_DEDUCE_FIELDTYPE( fieldType, type ) \
    template <>                                     \
    struct VariantDeducer_t< type >                 \
    {                                               \
        enum                                        \
        {                                           \
...
#define DECLARE_NAMED_FIELDTYPE( fieldType, strName )      \
    template <>                                            \
    inline const char *VariantFieldTypeName< fieldType >() \
    {                                                      \
        return strName;                                    \
    }
...
//-----------------------------------------------------------------------------
template < class CValueAllocator >
inline CVariantBase< CValueAllocator >::CVariantBase( const Vector &val, bool bCopy )
    : m_flags( 0 ), m_type( FIELD_VOID )
{
    CopyData( val, true );
}
...
template < class CValueAllocator >
inline CVariantBase< CValueAllocator >::CVariantBase( const Vector2D &val, bool bCopy )
    : m_flags( 0 ), m_type( FIELD_VOID )
{
    CopyData( val, bCopy );
}
...
template < class CValueAllocator >
inline CVariantBase< CValueAllocator >::CVariantBase( const char *val, bool bCopy )
    : m_flags( 0 ), m_type( FIELD_VOID )
{
    CopyData( val, bCopy );
}
...
template < class CValueAllocator >
inline CVariantBase< CValueAllocator >::CVariantBase( const Quaternion &val, bool bCopy )
    : m_flags( 0 ), m_type( FIELD_VOID )
{
    CopyData( val, bCopy );
}
...
void CVariantBase< CValueAllocator >::ConvertToCopiedData( bool silent )
{
    if ( ( m_flags & SV_FREE ) == 0 )
    {
        ...
        {
            case FIELD_CSTRING:
                CopyData( m_pszString, true );
                break;
            case FIELD_VECTOR2D:
                CopyData( *( Vector2D * )m_pData, true );
                break;
            case FIELD_VECTOR:
                CopyData( *m_pVector, true );
                break;
                //		case FIELD_VECTOR4D: CopyData( *(Vector4D*)m_pData, true ); break;
            case FIELD_QANGLE:
                CopyData( *( QAngle * )m_pData, true );
                break;
            case FIELD_QUATERNION:
                CopyData( *( Quaternion * )m_pData, true );
                break;
            default:
                if ( !silent )
...
inline bool CVariantBase< CValueAllocator >::AssignTo( CBaseHandle *pDest ) const
{
    ...
    {
        case FIELD_EHANDLE:
            *pDest = CHandle< CBaseEntity >( m_hEntity );
            return true;
        default:
    ...
    switch ( m_type )
    {
        case FIELD_VOID:
            *pDest = vec3_origin;
            return false;
        case FIELD_QANGLE:
            *pDest = *( Vector * )m_pData;
            return true;  // Should we allow this free assignment?
        case FIELD_VECTOR:
            *pDest = *m_pVector;
            return true;
        case FIELD_CSTRING:
    ...
    switch ( m_type )
    {
        case FIELD_VOID:
            *pDest = vec3_angle;
            return false;
        case FIELD_VECTOR:
            *pDest = *( QAngle * )m_pVector;
            return true;  // Should we allow this free assignment?
        case FIELD_QANGLE:
            *pDest = *( QAngle * )m_pData;
            return true;
        case FIELD_CSTRING:
        {
            int nParsed = sscanf( m_pszString, "%f %f %f", &pDest->x, &pDest->y, &pDest->z );
...
inline bool CVariantBase< CValueAllocator >::AssignTo( uint *pDest ) const
{
    switch ( m_type )
    {
        ...
        case FIELD_UINT64:
            *pDest = ( uint )clamp( m_uint64, 0 /*UINT_MIN*/, UINT_MAX );
            return true;
        case FIELD_BOOLEAN:
            *pDest = m_bool;
            return true;
        case FIELD_INTEGER:
            if ( m_int < 0 ) return false;
            *pDest = ( uint )clamp( m_int, 0, UINT_MAX );
            return true;
        case FIELD_UINT:
            *pDest = m_uint;
            return true;
    ...
    switch ( m_type )
    {
        case FIELD_VOID:
            *pDest = 0;
            return false;
        case FIELD_INTEGER:
            V_snprintf( pDest, nBufLen, "%d", m_int );
            return true;
        case FIELD_UINT:
            V_snprintf( pDest, nBufLen, "%u", m_uint );
            return true;
        case FIELD_FLOAT:
            V_snprintf( pDest, nBufLen, "%f", m_float );
            return true;
        case FIELD_FLOAT64:
            V_snprintf( pDest, nBufLen, "%f", m_float64 );
            return true;
        case FIELD_BOOLEAN:
            V_snprintf( pDest, nBufLen, "%s", m_bool ? "true" : "false" );
            return true;
        case FIELD_CHARACTER:
            V_snprintf( pDest, nBufLen, "%c", m_char );
            return true;
        case FIELD_CSTRING:
            V_strncpy( pDest, m_pszString, nBufLen );
            return true;
        case FIELD_VECTOR2D:
        {
        ...
        case FIELD_VECTOR:
            V_snprintf( pDest, nBufLen, "%f %f %f", m_pVector->x, m_pVector->y, m_pVector->z );
            return true;
        case FIELD_QANGLE:
        {
...
inline bool operator==( const CVariantBase< CValueAllocator1 > &v1, const CVariantBase< CValueAllocator2 > &v2 )
{
    ...
    switch ( v1.m_type )
    {
        case FIELD_INTEGER:
        {
            return v1.m_int == v2.m_int;
        }
        case FIELD_UINT:
        {
            return v1.m_uint == v2.m_uint;
        }
        case FIELD_FLOAT:
        {
            return v1.m_float == v2.m_float;
        }
        case FIELD_CHARACTER:
        {
            return v1.m_char == v2.m_char;
        }
        case FIELD_BOOLEAN:
        {
            return v1.m_bool == v2.m_bool;
        }
        case FIELD_HSCRIPT:
        {
            return v1.m_hScript == v2.m_hScript;
        }
        case FIELD_UTLSTRINGTOKEN:
        {
            return v1.m_utlStringToken == v2.m_utlStringToken;
        }
    }
