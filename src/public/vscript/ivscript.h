...
{
    ScriptClassDesc_t( void ( *pfnInitializer )() )
        : m_pszScriptName( 0 ), m_pszClassname( 0 ), m_pszDescription( 0 ), m_pBaseDesc( 0 ), m_pfnConstruct( 0 ), m_pfnDestruct( 0 ), pHelper( NULL )
    {
        ( *pfnInitializer )();
...
#define DEFINE_SCRIPT_INSTANCE_HELPER( className, p )                                              \
    template <>                                                                                    \
    IScriptInstanceHelper *GetScriptInstanceHelperOverride< className >( IScriptInstanceHelper * ) \
    {                                                                                              \
        return p;                                                                                  \
    }
...
template <>
inline ScriptClassDesc_t *GetScriptDesc< ScriptNoBase_t >( ScriptNoBase_t * )
{
    return NULL;
}
...
class IScriptVM
{
    ...
    template < typename T >
    HSCRIPT RegisterInstance( T *pInstance )
    {
    ...
    bool ValueExists( const char *pszKey )
    {
        return ValueExists( NULL, pszKey );
    }
    ...
    {
        return SetValue( NULL, pszKey, value );
    }
...
class CScriptScopeT : public CDefScriptScopeBase
{
    ...
    bool ValueExists( const char *pszKey )
    {
        return GetVM()->ValueExists( m_hScope, pszKey );
    }
    bool SetValue( const char *pszKey, const ScriptVariant_t &value )
    {
        return GetVM()->SetValue( m_hScope, pszKey, value );
    }
    bool GetValue( const char *pszKey, ScriptVariant_t *pValue )
    {
        return GetVM()->GetValue( m_hScope, pszKey, pValue );
    }
    void ReleaseValue( ScriptVariant_t &value )
    {
        GetVM()->ReleaseValue( value );
    }
    bool ClearValue( const char *pszKey )
    {
        return GetVM()->ClearValue( m_hScope, pszKey );
    }
...
class CScriptFuncHolder
{
    ...
    bool IsValid()
    {
        return ( hFunction != INVALID_HSCRIPT );
    }
    bool IsNull()
    {
        return ( !hFunction );
