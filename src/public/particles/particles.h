...
class CParticleOperatorInstance
{
   public:
    ...
    virtual uint32 Emit( CParticleCollection *pParticles, float flOpCurStrength, void *pContext ) const
    {
        return 0;
    }
...
class CParticleCollection
{
   public:
    ...
    int GetControlPointIndex( void ) const
    {
        return m_nTargetCP;
    }
...
inline void CParticleCollection::SetControlPointOrientation( int nWhichPoint, const Vector &forward, const Vector &right, const Vector &up )
{
    Assert( ( nWhichPoint >= 0 ) && ( nWhichPoint < MAX_PARTICLE_CONTROL_POINTS ) );
...
inline Vector CParticleCollection::TransformAxis( const Vector &SrcAxis, bool bLocalSpace, int nControlPointNumber )
{
    if ( bLocalSpace )
    {
        return  // mxmul
...
class CParticleSystemDefinition
{
   ...
   private:
    ...
    void ParseOperators( const char *pszName, ParticleFunctionType_t nFunctionType, CDmxElement *pElement, CUtlVector< CParticleOperatorInstance * > &out_list );
    void WriteChildren( CDmxElement *pElement );
    void WriteOperators( CDmxElement *pElement, const char *pOpKeyName, const CUtlVector< CParticleOperatorInstance * > &inList );
...
FORCEINLINE bool ParticlesCheckFloatMasked( const fltx4 &x4, int nBlockIndex, CParticleCollection *pParticles, bool bSilent = false )
{
    ...
    Assert( nRemaining > 0 );
    if ( ParticlesCheckFloatTest( SubFloat( x4, 0 ) ) && ( nRemaining < 2 || ParticlesCheckFloatTest( SubFloat( x4, 1 ) ) ) && ( nRemaining < 3 || ParticlesCheckFloatTest( SubFloat( x4, 2 ) ) ) && ( nRemaining < 4 || ParticlesCheckFloatTest( SubFloat( x4, 3 ) ) ) )
    {
        return true;
    }
    if ( !bSilent )
...
{
    if ( ParticlesCheckFloatMasked( fv.x, nBlockIndex, pParticles, true ) && ParticlesCheckFloatMasked( fv.y, nBlockIndex, pParticles, true ) && ParticlesCheckFloatMasked( fv.z, nBlockIndex, pParticles, true ) )
    {
        return true;
    }
    ParticlesCheckFloatDoFail();
...
FORCEINLINE bool ParticlesCheckFloat( const Vector &v )
{
    if ( ParticlesCheckFloatTest( v.x ) && ParticlesCheckFloatTest( v.y ) && ParticlesCheckFloatTest( v.z ) )
    {
        return true;
    }
    ParticlesCheckFloatDoFail();
...
FORCEINLINE bool ParticlesCheckFloat( const Quaternion &v )
{
    if ( ParticlesCheckFloatTest( v.x ) && ParticlesCheckFloatTest( v.y ) && ParticlesCheckFloatTest( v.z ) && ParticlesCheckFloatTest( v.w ) )
    {
        return true;
    }
    ParticlesCheckFloatDoFail();
...
FORCEINLINE bool ParticlesCheckColorMasked( const fltx4 &x4, int nBlockIndex, CParticleCollection *pParticles, bool bSilent = false )
{
    ...
    int nRemaining = pParticles->m_nActiveParticles - nBlockIndex;
    Assert( nRemaining > 0 );
    if ( ParticlesCheckColorTest( SubFloat( x4, 0 ) ) && ( nRemaining < 2 || ParticlesCheckColorTest( SubFloat( x4, 1 ) ) ) && ( nRemaining < 3 || ParticlesCheckColorTest( SubFloat( x4, 2 ) ) ) && ( nRemaining < 4 || ParticlesCheckColorTest( SubFloat( x4, 3 ) ) ) )
    {
        return true;
    }
    if ( !bSilent )
...
{
    if ( ParticlesCheckColorMasked( fv.x, nBlockIndex, pParticles, true ) && ParticlesCheckColorMasked( fv.y, nBlockIndex, pParticles, true ) && ParticlesCheckColorMasked( fv.z, nBlockIndex, pParticles, true ) )
    {
        return true;
    }
    ParticlesCheckFloatDoFail();
