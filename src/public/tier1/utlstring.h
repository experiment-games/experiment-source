...
class CUtlStringBuilder
{
    ...
    void Append( const CUtlStringBuilder &str )
    {
        Append( str.String(), str.Length() );
    }
    ...
    }  // how much room is there to scribble
    ...
    union Data
    {
        ...
        struct _Stack
        {
            ...
            void SetBytesLeft( char n )
            {
                m_szString[MAX_STACK_STRLEN] = n;
            }
        ...
        bool IsHeap() const
        {
            return ( ( Heap.sentinel & STRING_TYPE_SENTINEL ) != 0 );
        }

        char *Access()
        {
            return IsHeap() ? Heap.m_pchString : Stack.m_szString;
        }
        const char *String() const
        {
            return IsHeap() ? Heap.m_pchString : Stack.m_szString;
        ...
        size_t Length() const
        {
            return IsHeap() ? Heap.m_nLength : ( MAX_STACK_STRLEN - Stack.BytesLeft() );
        }
        bool IsEmpty() const
        {
            if ( IsHeap() )
        ...
        size_t Capacity() const
        {
            return IsHeap() ? Heap.m_nCapacity : MAX_STACK_STRLEN;
        }
        ...
        bool HasError() const
        {
            return IsHeap() && ( ( Heap.sentinel & STRING_TYPE_ERROR ) != 0 );
        }
