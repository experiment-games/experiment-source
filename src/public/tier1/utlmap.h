...
template < typename T >
FORCEINLINE_TEMPLATE T &AnalyzeMapRef( T &ref )
{
    return ref;
}
...
#define FOR_EACH_MAP_FAST( mapName, iteratorName )                                                                                          \
    for ( int iteratorName = 0; AnalyzeMapRef( mapName ).IsUtlMap && iteratorName < AnalyzeMapRef( mapName ).MaxElement(); ++iteratorName ) \
        if ( !AnalyzeMapRef( mapName ).IsValidIndex( iteratorName ) )                                                                       \
            continue;                                                                                                                       \
        else
