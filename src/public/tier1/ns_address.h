...
class CPeerToPeerAddress
{
   public:
    ...
    CPeerToPeerAddress( const CSteamID &steamID, int nSteamChannel )
        : m_AddrType( P2P_STEAMID ), m_steamID( steamID ), m_steamChannel( nSteamChannel )
    {
    }
...
struct ns_address
{
    ...
    ns_address( const netadr_t &other )
        : m_AddrType( NSAT_NETADR ), m_adr( other )
    {
        m_steamID.Clear();
    }
    ...
    ns_address( const CPeerToPeerAddress &steamID )
        : m_AddrType( NSAT_P2P ), m_steamID( steamID )
    {
        m_adr.Clear();
    }
