...
class CPackedStore
{
    ...
    {
        m_pFileTracker = pFileTracker;
        m_PackedStoreReadCache.m_pFileTracker = pFileTracker;
    }
    ...
    MD5Value_t &GetDirFileMD5Value()
    {
        return m_TotalFileMD5;
    }
    bool BTestDirectoryHash();
    bool BTestMasterChunkHash();
    CUtlSortVector< ChunkHashFraction_t, ChunkHashFractionLess_t > &AccessPackFileHashes()
    {
        return m_vecChunkHashFraction;
    }
    ...
    int GetWriteChunkSize() const
    {
        return m_nWriteChunkSize;
    }

    int GetHighestChunkFileIndex()
    {
        return m_nHighestChunkFileIndex;
    }

    void DiscardChunkHashes( int iChunkFileIndex );

    const CUtlVector< uint8 > &GetSignaturePublicKey() const
    {
        return m_SignaturePublicKey;
    }
    const CUtlVector< uint8 > &GetSignature() const
    {
        return m_Signature;
    }
    ...
    bool BFileContainedHashes()
    {
        return m_vecChunkHashFraction.Count() > 0;
    }
    ...
    struct CFileHeaderFixedData *FindFileEntry(
        ...
        char const *pExtension,
        uint8 **pExtBaseOut = NULL,
        uint8 **pNameBaseOut = NULL );
