...
struct audio_device_description_t
{
    ...
    explicit audio_device_description_t( eSubSystems_t nSubsystem )
        : m_nSubsystemId( ( uint8 )nSubsystem )
    {
        Assert( nSubsystem >= 0 && nSubsystem <= UINT8_MAX );
    }
...
struct audio_device_init_params_t
{
    ...
    audio_device_init_params_t()
        : m_bOverrideSpeakerConfig( false ), m_bOverrideDevice( false ) {}
...
class IAudioDevice2
{
    ...
    inline int MixChannelCount() const
    {
        return m_nChannels > 6 ? 6 : m_nChannels;
    }  // 7.1 mixes as 5.1
    inline int BitsPerSample() const
    {
        return m_nSampleBits;
    }
    ...
    {
        return m_nChannels > 2 ? true : false;
    }
    inline bool IsSurroundCenter() const
    {
        return m_nChannels > 4 ? true : false;
    }
    inline bool IsActive() const
    {
        return m_bIsActive;
    }
    inline bool IsHeadphone() const
    {
        return m_bIsHeadphone;
    }  // mixing makes some choices differently for stereo vs headphones, expose that here.
    inline bool CanDetectBufferStarvation()
    {
        return m_bSupportsBufferStarvationDetection;
    }
    inline bool IsCaptureDevice()
    {
        return m_bIsCaptureDevice;
    }

    inline int DeviceSampleBytes( void ) const
    {
