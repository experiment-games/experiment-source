...
int spng_decode_image(spng_ctx *ctx, void *out, size_t len, int fmt, int flags)
{
    ...
    ctx->zstream.next_in = (void *)ctx->data;
...
spng_ctx *spng_ctx_new2(struct spng_alloc *alloc, int flags)
{
    ...
    ctx->optimize_option = (uint32_t)(~0u);
