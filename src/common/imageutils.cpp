...
ConversionErrorType ImgUtl_ConvertJPEGToTGA( const char *jpegpath, const char *tgaPath, bool bRequirePowerOfTwo )
{
    ...
    if ( ( bRequirePowerOfTwo && ( !IsPowerOfTwo( jpegInfo.image_height ) || !IsPowerOfTwo( jpegInfo.image_width ) ) ) || jpegInfo.output_components != 3 )
    {
        jpeg_destroy_decompress( &jpegInfo );
        fclose( infile );
...
//-----------------------------------------------------------------------------
GLOBAL( void )
jpeg_UtlBuffer_dest( j_compress_ptr cinfo, CUtlBuffer *pBuffer )
{
...
ConversionErrorType ImgUtl_ResizeBitmap( Bitmap_t &destBitmap, int nWidth, int nHeight, const Bitmap_t *pImgSource )
{
    ...
    if (
        pImgSource->Format() != IMAGE_FORMAT_RGBA8888 || pImgSource->Stride() != pImgSource->Width() * 4 )
    {
    ...
    return ImgUtl_StretchRGBAImage(
        pImgSource->GetBits(), pImgSource->Width(), pImgSource->Height(), destBitmap.GetBits(), destBitmap.Width(), destBitmap.Height() );
}
