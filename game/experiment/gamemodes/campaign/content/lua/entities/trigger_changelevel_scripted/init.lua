...
function ENT:ChangeLevelNow(pActivator)
	...
	if (cvars.FindVar("g_debug_transitions"):GetInt()) then
		Msg("CHANGE LEVEL: " .. tostring(self.m_szMapName) .. " " .. tostring(self.m_szLandmarkName) .. "\n")
	end

	-- If we're debugging, don't actually change level
	if (cvars.FindVar("g_debug_transitions"):GetInt() == 0) then
		engine.ChangeLevel(self.m_szMapName)
	end
end

function ENT:TouchChangeLevel(pOther)
	local client = ToBasePlayer(pOther)
	if (not pOther:IsPlayer()) then
		return
	end

	if (client:IsSinglePlayerGameEnding()) then
		-- Some semblance of deceleration, but allow player to fall normally.
		-- Also, disable controls.
		local vecVelocity = client:GetAbsVelocity()
		vecVelocity.x = vecVelocity.x * 0.5
		vecVelocity.y = vecVelocity.y * 0.5
		client:SetAbsVelocity(vecVelocity)
		client:AddFlag(32)
		return
	end

	if (not client:IsInAVehicle() and client:GetMoveType() == 8) then
		DevMsg("In scripted level transition: " ..
		tostring(self.m_szMapName) .. " " .. tostring(self.m_szLandmarkName) .. "\n")
		return
	end
